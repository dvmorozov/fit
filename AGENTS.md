...
fitting arbitrary data to arbitrary classes of models**, built as much for
**research** and **teaching** as for use:

- **For research:** results reproduce, every override the user can notice
  explains itself (non-negotiable 7), and nothing degrades silently.
- **For teaching:** everything a user meets explains what it is, why it is
  allowed or refused, whether that rests on the field's canonical sources, on
  convention or on this software's own choice, what it does not cover, and where
  to read more (non-negotiable 10).

Every field is a **module** (see below), and no field is privileged. The one this
application grew out of, diffraction, is still in the framework tree; it is being
moved out as a public module (`docs/internal/diffraction-extraction.md`), and
until that is done, treat its presence as a debt, not a precedent.

Judge every change by one question: *what does this cost the person who adds the
next module, for a field nobody here has thought about - and does what it adds
explain itself?* An implementation that is slightly more work now and removes an
edit from every future extension is the right trade — that is what the
capability model buys. The contract every module is written against is
`docs/contributing/module-architecture.md`.
...
9. **No Pascal `label` and no `goto`, anywhere.** Free Pascal emits a
   user-declared label as a *global* symbol, and clang refuses a global symbol
   between `.cfi_startproc` and `.cfi_endproc` - so on x86_64-darwin one `goto`
   makes the unit unassemblable, and the error names a line one past the end of
   the file. Two of them cost three release cycles and the Intel macOS download
   before anyone looked at the source; see `docs/contributing/building.md`.
   `repeat ... until True` with `Break` replaces the usual "skip to the end of
   this block" jump.

10. **Everything a user meets explains itself.** A curve type, a rule, a greyed
    menu entry, a refusal: each is reachable through the explanation registry
    (`Common/explanation_registry.pas`) with a summary, a body, its standing
    (canonical, convention, this software's choice, or unsettled), its
    limitations and its sources. A registry-walking completeness test enforces
    it, so the next thing registered without an explanation fails by name. A
    rule that calls itself canonical quotes and cites its source; one that
    cannot is not canonical.

    **The registry is also the user guide.** Help > Explain Everything and the
    published site's guide are both drawn from it, so a feature is documented by
    explaining it there - in the chapter that owns it (`Desktop/guide_*.pas`,
    registered through `client_explanations`; a module registers its own) - and
    nowhere else. Every menu command is named in the guide by the path a user
    follows (`Model > Background > Subtract > Automatically`);
    `testcase_client_explanations` fails for a designed menu entry the guide does
    not name, so a command added to a menu without a word about it fails by name.

11. **The internet is reached only through `TCurlClient`
    (`Common/curl_client.pas`), which runs the system `curl`; traffic between
    this program's own processes uses the built-in `fphttpclient`. Nothing uses
    `opensslsockets`.** A data source, a module's included, fetches only through
    the `IWebClient` it is given. Free Pascal 3.2.2 cannot load OpenSSL 3; on
    macOS it loaded Apple's legacy 0.9.8, which public services refuse.
    `tools/build-tests/network_boundary.tests.ps1` enforces the rule; see "The
    network boundary" in `docs/contributing/architecture.md`.
...
- **`Desktop/Fit.lpi` has three build modes** - "Any platform", "Windows
  ...
  test passes.
- **On macOS a bare executable has no menu bar and no Dock tile, however complete
  the `TMainMenu` is.** `lsappinfo` reports `type="BackgroundOnly"` and a null
  bundle id for one, and `"Foreground"` for the same binary reached through an
  `.app` - and the LCL puts the menu in the *screen* menu bar, so a window with no
  menu in it is what a correct menu looks like there. The build wraps the binary
  (`New-MacRunShell`); do not go looking for the missing menu in the Pascal.
- **`Fit` and `fit` in one directory are the same file on a Mac**, whose default
  volume is case-insensitive - as are `$Client` and `$client` in PowerShell, whose
  variable names are case-insensitive too. Both shipped: the bundle's launcher
  overwrote the client binary it starts, and the fix's first draft copied a file
  onto itself. A test that reads a script's *text* sees neither; build the thing
  and ask the files.
...
- **"Connect to host:443 failed" from `fphttpclient` can be a refused TLS
  handshake**, not the network: `ssockets` words both the same. That is how the
  macOS download failure hid. `CurlExitMeaning` keeps the two apart.
...
average. Check with `-Task coverage-gaps` before you report. A module may also
declare units that must stay at **100 %** (`tests/coverage-full.txt` in its own
repository); the ratchet fails any of them below that, or missing from the counted
rows - see [testing.md](docs/contributing/testing.md).
...
| What is in a project file, and what is deliberately not? | the user guide's Projects and files chapter (`Desktop/guide_projects.pas`); the reasoning is in the headers of `Common/fit_project_document.pas` and `Desktop/fit_project_restore.pas` |
...
> tasks, and provides the model elements, the rules and the computations that
> class needs - and explains every one of them. The framework provides only the
> hosting: the fitting engine, transport, persistence, the window, and the
> registries modules contribute through.

So a feature that only one class of tasks needs belongs to that module, whatever
it is; a feature that seems to need the framework to know about one field means
a field-neutral seam is missing. The seams, the assumptions still standing in the
way and the criteria a new seam must meet are in
`docs/contributing/module-architecture.md`. Public and private modules use one
contract.
