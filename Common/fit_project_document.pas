...
type
    ...
    TProjectParam = record
        ...
        Error: double;
        { WHAT THE USER MADE OF IT in the user curve's dialog: its role, as the
          ordinal of TParameterType, and whether its value is held. A fitted
          curve's parameters have no such choice and leave HasKind False. }
        HasKind: boolean;
        Kind: longint;
        Held: boolean;
    end;
    ...
    { Which settings a document actually states.

      A MEMBER A FILE LEAVES OUT IS NOT A ZERO. Read as one, an absent
      maxRFactor was a ceiling no fit can meet, an absent backFactor a zero the
      peak search divides by, and an absent curveScaling switched scaling off.
      What a file does not state leaves the engine's own value. }
    TProjectSetting = (psWaveLength, psMaxRFactor, psBackFactor,
        psCurveThresh, psMinimizerKind, psLossKind, psWeighting,
        psBackgroundVariation, psCurveScaling);
    TProjectSettingSet = set of TProjectSetting;
    ...
    TProjectSettings = record
        ...
        { The members above the document states; the rest keep the engine's
          value when it is applied. A capture states them all. }
        Stated: TProjectSettingSet;
    end;
    ...
    TProjectProvenance = record
        ...
        LoaderName: string;
        AppVersion: string;
        { WHERE THE FILE ITSELF CAME FROM, when it was fetched rather than
          chosen on this computer. SourcePath then names a file in the
          downloaded-data folder, which says nothing about what the data IS or
          who published it - and that is exactly what has to be quoted when a
          result is published.

          Empty for a file the user opened themselves, which is why these are
          written and read as optional keys: a project saved before data
          sources existed is read unchanged. }
        OriginSourceId: string;
        OriginSourceTitle: string;
        { What was asked for, in the words the user typed. }
        OriginQuery: string;
        { The address the bytes came from. }
        OriginAddress: string;
        { When, as ISO 8601 - text rather than a TDateTime because it is
          written into a file that outlives this program's date handling. }
        OriginRetrievedAt: string;
    end;
    ...
    { The working context: what the user would otherwise have to set up again.
      ...
      where they live. }
    { How one coordinate was shown, and what the data said it was. }
    TProjectAxis = record
        { The axis mode's id (the desktop's axis_mode_registry), or '' - which
          a file written before ids means, or one where nothing was chosen. }
        Mode: string;
        { Whether the user chose Mode. Only a choice is put back: anything else
          is whatever the session happened to show. }
        ChosenByUser: boolean;
        { The user's own axis for this coordinate, kept whether or not it is
          the one in force. }
        CustomName: string;
        CustomUnit: string;
        CustomForward: string;
        CustomInverse: string;
        { What the loaded data said its coordinate is, as an axis mode id.
          SAVED because a project stores the profile's points and not the
          file: nothing reads the file again, so nothing else could say. }
        DataMode: string;
    end;

    TProjectDates = array of TDateTime;

    TProjectUi = record
        Argument: TProjectAxis;
        Value: TProjectAxis;
        { The date each profile point was recorded on, when the argument counts
          bars of a dated series; empty otherwise. SAVED for the reason DataMode
          is: the bar numbers cannot say what day each one was, and nothing
          reads the file again. }
        ArgumentDates: TProjectDates;
        { The display-mode integer a file written before axis ids holds for the
          argument. Read so the desktop can migrate it
          (axis_choice.LegacyArgumentModeId); never written. A flag of its own
          rather than a sentinel value, because 0 was a real mode and a record
          made with Default() would otherwise claim it. }
        HasLegacyViewMode: boolean;
        LegacyViewMode: longint;
...
{ AModified as this format writes a time, and ACreated carried from the document
  ...
  zone is not a time. }
const
    { Every setting: what a capture states. }
    AllProjectSettings = [Low(TProjectSetting)..High(TProjectSetting)];
