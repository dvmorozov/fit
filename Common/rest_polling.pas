...
@abstract(Which REST routes report progress, as opposed to act.)
...
{ True for a route that only REPORTS - /problems/<id>/state, /async and
  /rfactor - which the client reads every time it refreshes what the window
  offers, rather than one it calls because the user did something. Read that
  often, they are logged a tier below the calls that change anything. Accepts either a bare path or a full URL, so a caller holding
...
begin
    ...
    //  'progress' is two routes, and both are polled: a problem's, which the
    //  desktop reads ten times a second while a fit runs, and a stateless
    //  fit's, which the server reads from a backend fitting remotely.
    Result := (Last = 'state') or (Last = 'async') or (Last = 'rfactor') or
        (Last = 'progress');
end;
