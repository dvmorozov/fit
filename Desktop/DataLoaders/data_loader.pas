...
uses
    ...
    title_points_set, coordinate_axis;
...
type
    ...
    protected
        ...
        FSourceName: string;
        { What the format this loader was reached through says its coordinates
          are (data_loader_registry.TDataLoaderInfo). }
        FDeclaredModes: array[TAxisDimension] of string;
    ...
    public
        ...
        { Told by whoever created this loader what its FORMAT says the
          coordinates are - the registration's answer, since one reader can
          serve formats that mean different things. }
        procedure DeclareCoordinates(const AArgumentMode, AValueMode: string);
        { The format's answer by default. A reader that decides a coordinate
          as it reads - which column, which kind of abscissa - overrides it. }
        function CoordinateMode(ADimension: TAxisDimension): string; virtual;
        { The date each point was recorded on, by the point's index, when the
          data carries dates its argument is not - bars read from a dated
          price series. Empty otherwise. }
        function ArgumentDates: TAxisDates; virtual;
...
procedure TDataLoader.DeclareCoordinates(const AArgumentMode, AValueMode: string);
begin
    FDeclaredModes[adArgument] := AArgumentMode;
    FDeclaredModes[adValue] := AValueMode;
end;

function TDataLoader.ArgumentDates: TAxisDates;
begin
    Result := nil;
end;

function TDataLoader.CoordinateMode(ADimension: TAxisDimension): string;
begin
    Result := FDeclaredModes[ADimension];
end;
