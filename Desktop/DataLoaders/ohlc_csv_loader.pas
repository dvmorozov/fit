...
{ Sets the two defaults above from the stored choice - ordinals of
  TOhlcValueColumn and TOhlcXMode, as Data > Price Data and the settings file
  hold them. A value out of range means the loader's own default: a settings
  file edited by hand, or written by a version with more columns. }
procedure ApplyPriceDataChoice(AValueColumn, AArgument: longint);
...
procedure ApplyPriceDataChoice(AValueColumn, AArgument: longint);
begin
    if (AValueColumn >= Ord(Low(TOhlcValueColumn))) and
        (AValueColumn <= Ord(High(TOhlcValueColumn))) then
        DefaultOhlcValueColumn := TOhlcValueColumn(AValueColumn)
    else
        DefaultOhlcValueColumn := vcClose;
    if (AArgument >= Ord(Low(TOhlcXMode))) and
        (AArgument <= Ord(High(TOhlcXMode))) then
        DefaultOhlcXMode := TOhlcXMode(AArgument)
    else
        DefaultOhlcXMode := xmBarIndex;
end;
        ...
        begin
            ...
            if FDateLayout = dlAmbiguous then
                ...
                raise EInvalidFileFormat.Create(
                    ...
                    'choose Data > Price Data > Argument > Bar Number, which ' +
                    'does not use the dates at all.');
            ...
            if FDateLayout = dlContradictory then
                raise EInvalidFileFormat.Create(
                    ...
                    'file, or choose Data > Price Data > Argument > Bar Number, ' +
                    'which does not use the dates.');

            if FDateLayout = dlNone then
                raise EInvalidFileFormat.Create(
                    'Data > Price Data > Argument > Date is chosen, but this ' +
                    'file has no usable date column. Choose Data > Price Data > ' +
                    'Argument > Bar Number instead.');
        end;

        ValueIdx := IndexOfValueColumn;
        if ValueIdx < 0 then
            raise EInvalidFileFormat.Create(
                'The price column chosen under Data > Price Data > Value is ' +
                'not in this file. Choose another there.');
