...
uses {$IFDEF UNIX} {$IFDEF UseCThreads}
    ...
    //  FIRST, before Interfaces creates the QApplication: a client started
    //  from a snap's terminal gives the snap's environment back here, or KDE's
    //  file dialog freezes the window rebuilding a cache it can never build.
    snap_environment,
...
{$IFDEF FIT_RECORD}
    //  The switch that records a fit - only in a build made for recording.
    screen_recording,
{$ENDIF}
    ...
    ui_dpi,
    TurboPowerIPro,
    //  The Explain pane's HTML view. Named HERE and nowhere else: the component
    //  needs the printing package, which cannot build against the headless
    //  widget set the test suite links the main form with - so the form names
    //  only int_explain_view, and linking this unit is what registers the view.
    //  Leaving it out gives a window whose start-up check says the pane has no
    //  view, rather than a blank space nobody can explain.
    ipro_explain_view,
    user_points_set,
...
begin
    ...
    if Length(EnvironmentRepairedAtStartup) > 0 then
        WriteLog(DescribeEnvironmentChanges(EnvironmentRepairedAtStartup), Warning);
    ...
    begin
        ...
        //  And every toolbar against the white space its icons were given,
        //  measured from the buttons themselves: a bar the spacing never
        //  reached looks like an ordinary toolbar and nothing else reports it.
        FormMain.ReportToolBarSpacing;
        ...
        //  And whether this build's data sources registered at all, which only
        //  the running binary can say: the suites register them themselves, so
        //  an application whose start-up skipped the call would still have a
        //  green suite - that is exactly how a whole vertical once shipped
        //  missing from the compute server.
        FormMain.ReportDataSources;
        //  And what each HTML pane's zoom buttons do when clicked, which no
        //  headless suite can reach: a button wired to nothing looks exactly
        //  like one that works. It puts the sizes back where it found them.
        FormMain.ReportPaneZoom;
        ...
        //  And the pointer's readout beside the chart, whose captions are the
        //  axes' names and so change width with the data. Before the fit
        //  below, which is the one check that runs to convergence.
        FormMain.ReportReadoutColumns;
        //  And what the window draws while a fit is actually running, which no
        //  headless suite can see: a timer firing in a real event loop against
        //  a real server. Last, because it runs a fit to convergence and leaves
        //  the model fitted.
        FormMain.ReportLiveFitProgress;
    ...
    //  Syntax /RECORD_FIT=<dir>. The ordinary session, recorded: the project
    //  start-up opened above has its model cleared and fitted automatically,
    //  through the same actions the menus run, inside Application.Run below,
    //  and a frame of the window is written to <dir> each time the chart has
    //  been updated. The process ends when the fit is over. See
    //  screen_recording. The directory is made absolute against the working
    //  directory: it is somewhere to write to, and ResolvedPath's search is
    //  for files that must already exist.
{$IFDEF FIT_RECORD}
    if CmdLineParamFound(RecordSwitch, CmdLineParam) then
        FormMain.StartRecordingFit(ExpandFileName(Trim(CmdLineParam)));
{$ENDIF}
    Application.Run;
