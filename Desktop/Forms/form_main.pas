...
uses
    ...
    Laz_XMLCfg, neutron_points_set, gauss_points_set,
    ...
    //  How large the report tabs and the Explain pane are drawn.
    , report_zoom
    //  What the chart area shows while a fit runs, decided in fit_progress.
    , fit_progress, client_explanations
    //  The window performs the import of a downloaded file (IImportHost) and
    //  is handed where it came from (TDownloadOrigin); the wizard decides
    //  everything else about it.
    , data_source_import, data_source_wizard, data_source_registration
    //  What the self-check asks the running build about its data sources.
    , data_source, data_source_registry, data_source_findings
    //  What was downloaded this session and never used, thrown away on the
    //  way out.
    , download_session
    //  And the caption measurement, for the one window that is built in
    //  code and so is not among the forms the walk finds.
    , ui_dpi
    , data_loader_registration, minimizer_registry, minimizer_registration
    //  Where a module keeps its own choices between sessions.
    , module_preferences
    , int_ui_host
    ...
    //  The axes the chart is drawn on: the modes a build registers for either
    //  coordinate, and the window's own record of which is in force.
    , axis_mode_registry, chart_axes
    //  The Explain pane's view, by its seam only - see int_explain_view.
    , int_explain_view
    //  The window taking pictures of itself: /CAPTURE_SCREENSHOTS and
    //  /CAPTURE_SCREENCAST.
{$IFDEF FIT_RECORD}
    , window_recorder, screen_recording
{$ENDIF}
    , app_modules
    , app_version
    ...
    , model_outline, model_curve_rows
    , tool_pane_layout
    //  The list of curves the client holds.
    ...
    , ui_selfcheck
    //  How much white space a toolbar button gets around its icon. Decided
    //  there so it can be asserted without a display; applied here, which is
    //  the only place there are toolbars.
    , toolbar_metrics
...
type
  { One entry of an axis menu: the widget, and the coordinate and mode it
    stands for - the mode as an id, which a menu item has nowhere to keep. }
  TAxisMenuEntry = record
    Item: TMenuItem;
    Dimension: TAxisDimension;
    ModeId: string;
  end;

  { TFormMain }
  TFormMain = class(TForm, IUiHost, IProjectHost, IImportHost)
    ...
    ActionReportZoomIn: TAction;
    ActionReportZoomOut: TAction;
    ActionExplainZoomIn: TAction;
    ActionExplainZoomOut: TAction;
    ...
    ActionNewFromDataSource: TAction;
    ...
    MenuValueTransformation: TMenuItem;
    MenuValueUseRule: TMenuItem;
    ...
    procedure ActionNewFromDataSourceExecute(Sender: TObject);
    ...
    procedure ActionReportZoomInExecute(Sender: TObject);
    procedure ActionReportZoomOutExecute(Sender: TObject);
    procedure ActionExplainZoomInExecute(Sender: TObject);
    procedure ActionExplainZoomOutExecute(Sender: TObject);
    ...
    { Every entry of both axis menus, the automatic one included. }
    procedure AxisMenuClick(Sender: TObject);
    ...
    { Modal dialog to enter a user-defined axis (name, unit, forward + inverse
      formulas of x) under the title ACaption. Returns True when confirmed.
      Built in code (no resource). }
    function ShowCustomAxisDialog(const ACaption: string; var AName, AUnit,
        AForward, AInverse: string): boolean;
  ...
  protected
    ...
    { The axes the chart is drawn on: the mode chosen for each coordinate, the
      user's own definitions, and what the automatic rule answers. The one
      object the viewer, the grid, the titles, the readout and the menus all
      read (chart_axes), so they cannot disagree about which axis is in force. }
    FAxes: TChartAxes;
    { Both axis menus' entries - the menu bar's and the chart popup's - built
      from the registered modes, so a click and a tick are looked up here
      rather than by caption. }
    FAxisEntries: array of TAxisMenuEntry;
    { The projects File > Open Recent offers, by the tag of their entries. }
    FRecentPaths: array of string;
    ...
    { The sentence the panel shows when it has no rows, as last drawn - part of
      what decides whether the panel has to be drawn again. }
    FShownEmptyText: string;
    { How many of FShownRows, from the first, are FShownPanelId's own; the rest
      are the framework's rows for curves those do not stand for. Decides whose
      entries the row menu asks for (model_outline.RowMenuPanelId). }
    FShownOwnRowCount: longint;
    { The framework's latest rows and each module's latest push, from which
      what the panel shows is composed (model_outline.ComposePanel). Kept,
      because the two arrive separately - the framework's on every refresh, a
      module's when it redraws - and each must survive the other's arrival. }
    FPanelSources: TPanelSources;
    ...
    { How large each HTML pane is drawn, as one of report_zoom's steps.

      ONE SIZE FOR EVERY REPORT TAB, because the tabs are one surface - a
      reader who enlarged one report and found the next one small again would
      be looking at a setting that follows the tab rather than the window. The
      Explain pane keeps its own, because it has its own buttons and sits
      somewhere else entirely. }
    FReportZoom: longint;
    FExplainZoom: longint;
    { The strips those sizes are clicked from - one of the report tabs' (they
      all carry the same pair) and the Explain pane's. Held only so the window
      can check itself: a strip nobody can count the buttons of is one nothing
      reports as empty. }
    FReportZoomStrip: TToolBar;
    FExplainZoomStrip: TToolBar;
    ...
    { The toolbar spacing as it was last applied, and the smallest button the
      bars were sized to clear - both in the scale they were applied in. Read
      by the self-check, which must not work them out a second time. }
    FToolBarMetrics: TToolBarMetrics;
    FToolBarMinimumButton: longint;
...
{$IFDEF FIT_RECORD}
    { /RECORD_FIT: the frames being taken, where, and how far the fit is. Only
      in a build made for recording (tools/build-lib/capture.ps1): no release
      carries it. }
    FRecorder: TWindowRecorder;
    FRecordDir: string;
    FRecordStage: (rsSizing, rsFitting, rsTail, rsDone);
    FRecordSizingTries: longint;
    FRecordWasAnimating: boolean;
    FRecordFrameQueued: boolean;
{$ENDIF}
  ...
  public
    ...
    {  Puts the client's curve list into GridParameters. A wrapper rather than something
       the list inherits: the list is engine-side data and must not carry a
    ...
    { Starts the viewer and the menus on the automatic axes: the axes are the
      project's, and one opened at start-up puts its own back. }
    procedure RestoreAxes;
    ...
    { Central axis switch: hands FAxes what the model, the data and the
      wavelength say, and redraws from it - the chart, the grid, both titles,
      both readout captions, the ticks, and whether the wavelength can be set. }
    procedure ApplyAxes;
    { Asks the model and the data again what the automatic axes are, and
      applies them only if the answer changed: the model changes with a fit, a
      detection or a deletion, none of which goes through a menu, and a redraw
      on every state refresh would be a redraw on every click. }
    procedure RefreshAutomaticAxes;
    { The client's curve list as it is now, or nil. The only way the window
      reaches it: the client frees and replaces it on every refresh. }
    function CurrentCurveList: TMSCRCurveList;
    { ATitle as this widget set shows a window title: the Cocoa widget set
      takes '&' in one as a shortcut mark and drops it (LiteralCaption), and
      the others show it as it is. }
    function WindowCaption(const ATitle: string): string;
    { Gives both readout captions one width, so the two values form a column
      whatever the axes are called, and runs each value to the panel's edge. }
    procedure AlignReadoutColumns;
    { Builds both axis menus - Data > Argument Transformation > Use Rule, Data >
      Value Transformation > Use Rule, and the chart's popup - from the
      registered modes. After the modules have registered theirs. }
    procedure BuildAxisMenus;
    { Chooses AModeId for ADimension, asking first for what it cannot be drawn
      without. False when the user cancelled that, and nothing changed. }
    function ChooseAxisMode(ADimension: TAxisDimension;
        const AModeId: string): boolean;
    { The wavelength, asked for because an axis cannot be drawn without it.
      False when refused or cancelled. }
    function AskForWaveLength: boolean;
    { The user's own axis for ADimension, asked for in the custom-axis dialog.
      False when cancelled or refused. }
    function AskForAxisDefinition(ADimension: TAxisDimension): boolean;
    { The main chart's marks, written by the axis in force - a date as a date,
      a logarithmic value as the value. }
    procedure ChartXMarks(AMin, AMax: double; var AStart, AStep: double;
        var AHandled: boolean);
    function ChartXMarkText(AValue, AStep: double): string;
    procedure ChartYMarks(AMin, AMax: double; var AStart, AStep: double;
        var AHandled: boolean);
    function ChartYMarkText(AValue, AStep: double): string;
    ...
    function FrameworkRowsNow: TOutline;
    function ShownContents: TPanelContents;
    ...
    { Gives every toolbar in this window the same white space around its
      icons, from toolbar_metrics and scaled for this display.

      EVERY BAR THERE IS, found by walking this form's components rather than
      named one by one: the bars are declared in two places - the designed ones
      in form_main.lfm and the panes' zoom strips built at run time - and a
      list of names here would be a list that the next bar is not on. A bar
      added anywhere is spaced by this without an edit. }
    procedure SpaceToolBars;
    { What each toolbar ended up measuring, taken from the widgets. }
    function ToolBarReadingsNow: TToolBarReadings;
    ...
    { What data sources this RUNNING build has, and whether the command that
      reaches them exists. Registration is a property of the start-up sequence
      and of what linked, so no headless suite can answer it. }
    procedure ReportDataSources;
    function ReadModelTreeUnderPicksType: TModelTreeReading;
    function FirstModulePanelId: string;
        { Whether every legend row still names the series it carries. }
    procedure ReportLegendPairing;
    { Whether the pointer's readout beside the chart lines up and shows its
      values whole, measured with the widest value the loaded data has. }
    procedure ReportReadoutColumns;
    procedure ReportLiveFitProgress;
    { And what each HTML pane's own zoom buttons do when they are clicked -
      which is the half of the zoom no headless suite can reach: the ladder is
      arithmetic and is tested as arithmetic, the wiring is a widget. }
    procedure ReportPaneZoom;
    { And whether every toolbar really did get that white space, measured in
      the running window - the only place a bar the spacing never reached can
      be told from one it did. }
    procedure ReportToolBarSpacing;
{$IFDEF FIT_RECORD}
        { Records the fit of the project opened at start-up into numbered frames
          in ADir: /RECORD_FIT. See screen_recording. }
    procedure StartRecordingFit(const ADir: string);
    procedure RecordBegin(Data: PtrInt);
    procedure RecordStartFit(Data: PtrInt);
    procedure RecordFitEnded;
    procedure RecordFinish(Data: PtrInt);
    procedure RecordFrame(Data: PtrInt);
{$ENDIF}
        { True while a fit is being recorded - never, in a build without
          FIT_RECORD. }
    ...
    { The two-button strip down the left of a pane that is drawn as HTML, built
      to look like the designed toolbars on the other bottom tabs. }
    function BuildZoomToolBar(AParent: TWinControl; const AName: string;
        AZoomIn, AZoomOut: TAction): TToolBar;
    { One pane's two buttons clicked, and what both panes did. }
    function ZoomReadingFor(const APane: string; AStrip: TToolBar;
        AZoomIn, AZoomOut: TAction; var AMine: longint;
        const AOther: longint): TZoomReading;
    { Draws every report tab at FReportZoom / the Explain pane at FExplainZoom,
      and tells the commands what is left of the ladder. }
    procedure ApplyReportZoom;
    procedure ApplyExplainZoom;
    ...
    { Restores how large the report tabs and the Explain pane are drawn. A
      stored size this build does not offer - or the zero an older settings
      file holds - is not a size somebody chose; report_zoom says so. }
    ...
    procedure ShowModuleReport(const AReport: TModuleReport;
        ...
        virtual;
    { Throws away what was downloaded this session and never used. }
    procedure DiscardUnusedDownloads;
    { IImportHost - the window's part of turning a downloaded file into a
      project. The ORDER of these four is data_source_import's, where it can
      be tested; this window only performs them. }
...
uses
    ...
    //  reach them.
    model_clearing,
    //  File > New Project from Data Source: the wizard's window. Every
    //  decision it makes is in data_source_wizard, which this window has no
    //  part in.
    data_source_window,
    //  The axes: the framework's own modes, the rule that chooses among them,
    //  and the one diffraction mode the wavelength command still switches to.
    axis_mode_registration, axis_choice, diffraction_axis_modes, guide_data,
...
begin
    ...
    //  NEW DATA STARTS ON AUTOMATIC: the axes are the document's, and a
    //  choice made for other data would caption this one. The same file read
    //  again (Reload Profile) keeps what was chosen for it. Asked before the
    //  source below is remembered, which is what makes it "the same" next time.
    NewData := FProjectFlow.IsNewSource(FileName);
    ...
    Caption := WindowCaption(GetWindowTitle(ApplicationProperties.Title, ExtractFileName(FileName)));
...
procedure TFormMain.ActionNewFromDataSourceExecute(Sender: TObject);
begin
    //  The window over the wizard, and the wizard decides everything in it.
    //  What comes back is a downloaded file the user has already seen; turning
    //  it into a project is the four steps below, in data_source_import's
    //  order, performed through this window as IImportHost.
    NewProjectFromDataSource(Self, Self as IImportHost);
end;

{ ------------------------------ IImportHost -------------------------------- }

function TFormMain.MayReplaceDocument: boolean;
begin
    //  The same question File > New Project asks, and it is asked BEFORE the
    //  document is touched: a user who says no must find their work as it was.
    Result := FProjectFlow.MayReplaceDocument;
end;
...
procedure TFormMain.ImportProfileFile(const APath: string);
begin
    //  THE ORDINARY IMPORT. A downloaded file is a file, and there is
    //  deliberately no second way in: the loader registry chooses the reader,
    //  the client copies the profile and pushes it to the server, and the
    //  window retitles itself - exactly as File > Import Profile does.
    LoadDataFile(APath);
end;

procedure TFormMain.DiscardUnusedDownloads;
var
    Gone: longint;
begin
    Gone := CurrentDownloadSession.Discard(@DefaultDeleteFile);
    if Gone > 0 then
        //  Said in the log rather than in a box: it is housekeeping the user
        //  did not ask for and does not have to answer.
        WriteLog(Format('threw away %d downloaded file(s) no project uses',
            [Gone]), log.Notification);
end;

function TFormMain.DownloadFolder: string;
begin
    //  Empty until the user has chosen one, and then the wizard uses the
    //  per-user default - see app_settings.DownloadFolder.
    Result := FSettings.DownloadFolder;
end;

procedure TFormMain.RememberDownloadFolder(const APath: string);
begin
    FSettings.DownloadFolder := APath;
end;

procedure TFormMain.RememberOrigin(const AOrigin: TDownloadOrigin);
begin
    //  AFTER the import, which filled provenance from the file it read - for a
    //  download that is a name in a cache directory, and it says nothing about
    //  the service, the query or the day.
    FProjectFlow.RememberOrigin(AOrigin.SourceId, AOrigin.SourceTitle,
        AOrigin.Query, AOrigin.Address,
        FormatDateTime('yyyy-mm-dd"T"hh:nn:ss', AOrigin.RetrievedAt));
end;
...
{ Deletes the curves the parameter table has selected.
  ...
  the window's curve list - which is a COPY: GetCurveAttributes answers
...
var
    ...
    Removed: longint;
begin
    if not Assigned(CurrentCurveList) then
        Exit;
    ...
    try
        ...
        begin
            Id := CurrentCurveList.CurveHandleOfRow(Row - GridParameters.FixedRows);
            if Id <> '' then
                Handles.Add(Id);
        end;
...
begin
    ...
    //  Both axes, in the main menu and in the chart's popup: one entry per
    //  coordinate is ticked at a time, and which one changes with the choice.
    for i := 0 to High(FAxisEntries) do
        Checkable([FAxisEntries[i].Item]);
end;
...
begin
    //  RECORDING A FIT, the one question the session is asked - Model > Clear
    //  Model's - is answered yes, as the user who started it would.
    if Recording then
    begin
        WriteLog('record: answered yes to "' + ATitle + '"', log.Notification);
        Exit(True);
    end;
...
//  ---- the two panes drawn as HTML. A step, then redraw at it: the size is
//  held here and applied to the views, which is what makes one number answer
//  for however many report tabs this build has.

procedure TFormMain.ActionReportZoomInExecute(Sender: TObject);
begin
    FReportZoom := ZoomedIn(FReportZoom);
    ApplyReportZoom;
end;

procedure TFormMain.ActionReportZoomOutExecute(Sender: TObject);
begin
    FReportZoom := ZoomedOut(FReportZoom);
    ApplyReportZoom;
end;

procedure TFormMain.ActionExplainZoomInExecute(Sender: TObject);
begin
    FExplainZoom := ZoomedIn(FExplainZoom);
    ApplyExplainZoom;
end;

procedure TFormMain.ActionExplainZoomOutExecute(Sender: TObject);
begin
    FExplainZoom := ZoomedOut(FExplainZoom);
    ApplyExplainZoom;
end;
...
{$ENDIF}
    ...
    WriteSettings;
    //  AND THE DOWNLOADS NOBODY WANTED. Every look at a data source writes a
    //  file - the preview has to read something - and most of those looks end
    //  in "no". The ones that became a project are the user's and stay; the
    //  rest are this program's litter, and this is the moment to sweep it.
    //  Only the application's own downloads directory is touched: a file the
    //  user moved somewhere of their own is theirs by that act.
    DiscardUnusedDownloads;
    ...
    //  The viewer draws through these; it is told first, so nothing it does on
    //  the way out reads freed axes.
...
begin
    ...
    //  The parameter table's presenter. Owns no data - the client's curve
    //  list is the model, and the service owns that - so it lives as long as
    //  the form.
    ...
    Caption := WindowCaption(GetWindowTitle(ApplicationProperties.Title, ''));
    //  Let the background calculation thread report fatal errors to the user.
    main_calc_thread.OnCalcError := ShowCalcError;

    //  The axes the chart is drawn on, before anything can draw. Their menus
    //  are built further down, once every module has registered its modes.
    FAxes := TChartAxes.Create;
    //  The chart's marks are the axis's to write when it has a reason to - a
    //  date written as a date, a log value read in the value.
    Chart.OnXMarks := ChartXMarks;
    Chart.OnXMarkText := ChartXMarkText;
    Chart.OnYMarks := ChartYMarks;
    Chart.OnYMarkText := ChartYMarkText;
    ...
    //  The framework's own axis modes BEFORE the modules': registration order is
    //  menu order, and the field-neutral entries head every axis menu.
    RegisterAllAxisModes;
    //  WHERE A MODULE KEEPS ITS OWN CHOICES, named before any module registers
    //  or declares its menu: a module ticks what it kept, and reads it on first
    //  use. Named here, not by default, so no test binary writes the user's
    //  own file (module_preferences).
    UseModulePreferencesFile(DefaultModulePreferencesFile);
    ...
    //  Both axis menus, from every mode registered by now - the framework's and
    //  every module's front door's.
    BuildAxisMenus;
    //  And where data can be FETCHED from, after the loaders: a source says
    //  which kinds of file it produces, and what can read one is the loader
    //  registry's answer. A module's own sources registered from its front
    //  door above are already in by now.
    ...
    BuildRightPanelTabs;
    BuildLeftPanelTabs;
    //  BEFORE THE PANES THEY SIZE, and before the settings have been read:
    //  what the user last chose is applied by RestoreZoom once they have. A
    //  field left at zero would still draw correctly - report_zoom repairs it
    //  - but a window whose fields say one thing and whose panes show another
    //  is a state nobody chose.
    ...
    //  AND THEN THE SPACING, over every toolbar at once - after the zoom
    //  strips above, which are the bars a list of names would have missed.
    SpaceToolBars;
    ...
    //  How large the user last read the two HTML panes at.
    RestoreZoom;
...
begin
    ...
    //  RECORDING A FIT (/RECORD_FIT): the chart has just been updated, so this
    //  is the moment to take a frame. Taken once the paint is over rather than
    //  inside it, and never for the repaint the render itself causes.
{$IFDEF FIT_RECORD}
    if Recording and not (Assigned(FRecorder) and FRecorder.Rendering) and
        not FRecordFrameQueued then
    begin
        FRecordFrameQueued := True;
        Application.QueueAsyncCall(RecordFrame, 0);
    end;
{$ENDIF}
end;
...
begin
    ...
    FValueIndex := Index;
    FDrawReticule := True;
    //  In the words of the axes in force: a date as a date, a logarithmic value
    //  as the value rather than its logarithm.
    LabelPositionValue.Caption := FAxes.Axis(adArgument).ReadoutText(Xg);
    LabelIntensityValue.Caption := FAxes.Axis(adValue).ReadoutText(Yg);
end;
...
begin
    ...
    if (ColNum < GridParameters.FixedCols) or
       (RowNum < GridParameters.FixedRows) then
        Exit;
    if not CurrentCurveList.ColumnParameterType(RowNum - GridParameters.FixedRows,
...
procedure TFormMain.BuildAxisMenus;
const
    //  The prefix a generated entry's name starts with, per coordinate.
    NamePrefix: array[TAxisDimension] of string = ('MenuAxis', 'MenuValueAxis');
    //  The popup's two submenus, one per coordinate.
    PopupCaption: array[TAxisDimension] of string = ('Argument', 'Value');
var
    Hosts: array[TAxisDimension] of TMenuItem;
    Popups: array[TAxisDimension] of TMenuItem;
    AutomaticTopic: array[TAxisDimension] of string;
    Modes: TAxisModeClasses;
    D: TAxisDimension;
    i: longint;

    procedure AddEntry(AHost: TMenuItem; ADimension: TAxisDimension;
        const AModeId, ACaption, ATopic: string);
    var
        Item: TExplainedMenuItem;
    begin
        Item := TExplainedMenuItem.Create(Self);
        ...
        Item.Name := FWidgetNames.NameFor(NamePrefix[ADimension], ACaption);
        Item.Caption := ACaption;
        //  EVERY ENTRY EXPLAINS ITSELF, a module's included: the topic is the
        //  mode's own, declared where it was registered.
        Item.Topic := ATopic;
        Item.OnTopicHovered := RowMenuTopicHovered;
        Item.OnClick := AxisMenuClick;
        AHost.Add(Item);
        SetLength(FAxisEntries, Length(FAxisEntries) + 1);
        FAxisEntries[High(FAxisEntries)].Item := Item;
        FAxisEntries[High(FAxisEntries)].Dimension := ADimension;
        FAxisEntries[High(FAxisEntries)].ModeId := AModeId;
    end;

begin
    Hosts[adArgument] := MenuUseRule;
    Hosts[adValue] := MenuValueUseRule;
    AutomaticTopic[adArgument] := ArgumentAxesTopic;
    AutomaticTopic[adValue] := ValueAxesTopic;
    for D := Low(TAxisDimension) to High(TAxisDimension) do
    begin
        //  A SUBMENU PER COORDINATE in the chart's popup, which used to list
        //  the argument's entries alone: two lists of the same captions -
        //  Logarithmic, Custom - side by side would not say which axis each
        //  one is for.
        Popups[D] := TMenuItem.Create(Self);
        Popups[D].Name := FWidgetNames.NameFor('PopupAxis', PopupCaption[D]);
        Popups[D].Caption := PopupCaption[D];
        PopupViewMode.Items.Add(Popups[D]);

        //  AUTOMATIC FIRST: it is the default, and a visible entry rather than
        //  hidden behaviour, so the user can see WHY a caption changed with the
        //  model or the data - and can hand the axis back to them.
        AddEntry(Hosts[D], D, AutomaticAxisModeId, 'Automatic', AutomaticTopic[D]);
        AddEntry(Popups[D], D, AutomaticAxisModeId, 'Automatic', AutomaticTopic[D]);

        //  THEN WHATEVER IS REGISTERED, in registration order: the framework's
        //  own, then the fields still in this tree, then every module's.
        Modes := AxisModesFor(D);
        for i := 0 to High(Modes) do
        begin
            AddEntry(Hosts[D], D, Modes[i].Info.Id, Modes[i].CaptionFor(D),
                Modes[i].Info.Topic);
            AddEntry(Popups[D], D, Modes[i].Info.Id, Modes[i].CaptionFor(D),
                Modes[i].Info.Topic);
        end;
    end;
end;

procedure TFormMain.AxisMenuClick(Sender: TObject);
var
    i: longint;
begin
    for i := 0 to High(FAxisEntries) do
        if FAxisEntries[i].Item = Sender then
        begin
            if ChooseAxisMode(FAxisEntries[i].Dimension, FAxisEntries[i].ModeId) then
                ApplyAxes;
            Exit;
        end;
end;

function TFormMain.ChooseAxisMode(ADimension: TAxisDimension;
    const AModeId: string): boolean;
begin
    //  WHAT THE MODE CANNOT BE DRAWN WITHOUT is asked for first, and a refusal
    //  or a cancel leaves the axis as it was. Which modes need what is the
    //  mode's own declaration, asked of FAxes - not a list of menu entries.
    FAxes.SetWaveLength(CurrentWaveLength);
    case FAxes.NeedsParameter(AModeId) of
        apWaveLength:
            if not AskForWaveLength then
                Exit(False);
        apDefinition:
            if not AskForAxisDefinition(ADimension) then
                Exit(False);
    end;
...
{$warnings off}
function TFormMain.AskForWaveLength: boolean;
var
    WaveLength: double;
    Refusal: string;
begin
    ...
    //  separator around a StrToFloat that raises on a typo - so the separator
    //  was never put back, and the exception reached the top-level handler,
    //  which logs at Fatal and STOPS THE SERVER POLL. A typo in a text box
    //  disconnected the user from the compute server. See findings.md.
    //  BOTH REFUSALS are in coordinate_axis, beside the axis that divides by
    //  this number: not a number, and not greater than zero - zero being what
    //  "not set" already means to the client, so accepting it would silently do
    //  nothing at all.
    if not WavelengthFromText(InputWavelengthDlg.EditWavelength.Text,
        WaveLength, Refusal) then
    begin
    ...
    FAxes.SetWaveLength(WaveLength);
    Result := True;
end;
{$warnings on}

procedure TFormMain.ApplyAxes;
var
    D: TAxisDimension;
    i: longint;
    List_: TMSCRCurveList;
begin
    CheckAssigned(FFitViewer, 'the viewer that draws into this window');
    CheckAssigned(FAxes, 'the axes the chart is drawn on');

    //  WHAT THE AUTOMATIC RULE ASKS, fresh: the model's curves, the data and
    //  the selection may all have changed since the last time.
    FAxes.SetWaveLength(CurrentWaveLength);
    if Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient) then
    begin
        for D := Low(TAxisDimension) to High(TAxisDimension) do
            FAxes.SetPreferences(D, FitClientApp_.FitClient.AxisPreferences(D));
        //  So a date axis names each bar by its day (TBarDateAxis).
        FAxes.SetArgumentDates(FitClientApp_.FitClient.GetArgumentDates);
    end;

    //  The viewer redraws on being handed the axes.
    FFitViewer.Axes := FAxes;
    //  The parameters grid shows positions through the argument axis, so it is
    //  told the RESOLVED mode - it knows nothing of the model or the data.
    //
    //  THE CLIENT'S LIST AS IT IS NOW (CurrentCurveList): the client frees and
    //  replaces it on every refresh (findings.md).
    List_ := CurrentCurveList;
    if Assigned(List_) then
    begin
        List_.FArgumentMode := FAxes.ResolvedModeId(adArgument);
        List_.FArgumentDefinition := FAxes.Choice(adArgument).Definition;
        FCurveGrid.Assign(GridParameters, List_);
    end;

    for i := 0 to High(FAxisEntries) do
        SetMenuEntryChecked(FAxisEntries[i].Item,
            FAxes.IsTicked(FAxisEntries[i].Dimension, FAxisEntries[i].ModeId));

    {  The wavelength governs only the axes that read it; everywhere else it is
       meaningless, so it is gated off there. Asked of the mode in force rather
       than of a list of modes, so it stays right for the automatic entry,
       whose mode depends on the model and the data. }
    if Assigned(MenuSetRuleParameters) then
        MenuSetRuleParameters.Enabled := FAxes.ReadsWaveLength;

    //  THE WORDS FOLLOW THE AXES: both titles, and both captions of the
    //  pointer's readout on the right - which said "Position:" and
    //  "Intensity:" whatever was drawn.
    Chart.XAxisLabel := FAxes.Axis(adArgument).Title;
    Chart.YAxisLabel := FAxes.Axis(adValue).Title;
    Chart.ShowAxisLabel := True;
    LabelPositionCaption.Caption := FAxes.Axis(adArgument).ReadoutCaption;
    LabelIntensityCaption.Caption := FAxes.Axis(adValue).ReadoutCaption;
    AlignReadoutColumns;
    Chart.Invalidate;
end;

procedure TFormMain.AlignReadoutColumns;
var
    Width_: integer;
begin
    //  ONE COLUMN FOR BOTH VALUES. The captions are the axes' names now, so
    //  their widths differ with the data - "Position:" over "Price:" - and each
    //  value started wherever its own caption ended. Both captions take the
    //  wider one's width, measured in their own font.
    Width_ := LabelPositionCaption.Canvas.TextWidth(LabelPositionCaption.Caption);
    if LabelIntensityCaption.Canvas.TextWidth(LabelIntensityCaption.Caption) > Width_ then
        Width_ := LabelIntensityCaption.Canvas.TextWidth(LabelIntensityCaption.Caption);
    LabelPositionCaption.Constraints.MinWidth := Width_;
    LabelIntensityCaption.Constraints.MinWidth := Width_;
    //  EACH VALUE RUNS TO THE PANEL'S EDGE by its anchor in the form file.
    //  Set here in code, the same anchor was never re-resolved and both values
    //  came out 0 px wide - "Price: (" - which -Task check-ui now measures.
    PanelRightContent.ReAlign;
end;

function TFormMain.WindowCaption(const ATitle: string): string;
begin
    Result := ATitle;
    if WidgetSet.LCLPlatform = lpCocoa then
        Result := LiteralCaption(ATitle);
end;

function TFormMain.CurrentCurveList: TMSCRCurveList;
begin
    //  ASKED, NEVER KEPT. The window held its own pointer to this list, and the
    //  client frees and replaces the list on every refresh while the pointer
    //  was re-aimed only when the new one was tabulated - which an emptied
    //  model never is. Every reader asks here instead (findings.md).
    Result := nil;
    if Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient) then
        Result := FitClientApp_.FitClient.CurveAttributesForDisplay;
end;

procedure TFormMain.RefreshAutomaticAxes;
var
    D: TAxisDimension;
    Changed: boolean;
begin
    if not Assigned(FAxes) or not Assigned(FFitViewer) then
        Exit;
    if not Assigned(FitClientApp_) or not Assigned(FitClientApp_.FitClient) then
        Exit;
    Changed := False;
    for D := Low(TAxisDimension) to High(TAxisDimension) do
        if FAxes.UpdatePreferences(D, FitClientApp_.FitClient.AxisPreferences(D)) then
            Changed := True;
    if Changed then
        ApplyAxes;
end;
...
procedure TFormMain.ChartXMarks(AMin, AMax: double; var AStart, AStep: double;
    var AHandled: boolean);
begin
    if Assigned(FAxes) then
        AHandled := FAxes.Axis(adArgument).ChooseMarks(AMin, AMax, AStart, AStep);
end;

function TFormMain.ChartXMarkText(AValue, AStep: double): string;
begin
    //  The chart's own format unless the axis writes its own marks.
    if Assigned(FAxes) and FAxes.Axis(adArgument).FormatsMarks then
        Result := FAxes.Axis(adArgument).MarkText(AValue, AStep)
    else
        Result := Trim(Format('%6.4g', [AValue]));
end;

procedure TFormMain.ChartYMarks(AMin, AMax: double; var AStart, AStep: double;
    var AHandled: boolean);
begin
    if Assigned(FAxes) then
        AHandled := FAxes.Axis(adValue).ChooseMarks(AMin, AMax, AStart, AStep);
end;

function TFormMain.ChartYMarkText(AValue, AStep: double): string;
begin
    if Assigned(FAxes) and FAxes.Axis(adValue).FormatsMarks then
        Result := FAxes.Axis(adValue).MarkText(AValue, AStep)
    else
        Result := Trim(Format('%6.4g', [AValue]));
end;
...
function TFormMain.ShowCustomAxisDialog(const ACaption: string; var AName,
    AUnit, AForward, AInverse: string): boolean;
const
...
function TFormMain.AskForAxisDefinition(ADimension: TAxisDimension): boolean;
const
    DialogCaption: array[TAxisDimension] of string =
        ('Custom Argument Axis', 'Custom Value Axis');
var
...
begin
    Result := False;
    Definition := FAxes.Choice(ADimension).Definition;
    Axis.Name := Definition.Name;
    Axis.Units := Definition.UnitName;
    Axis.Forward_ := Definition.Forward;
    Axis.Inverse := Definition.Inverse;
    ...
    if not ShowCustomAxisDialog(DialogCaption[ADimension], Axis.Name,
        Axis.Units, Axis.Forward_, Axis.Inverse) then
        Exit;
    ...
    Definition.UnitName := Axis.Units;
    Definition.Forward := Axis.Forward_;
    Definition.Inverse := Axis.Inverse;
    FAxes.SetDefinition(ADimension, Definition);
    Result := True;
end;

procedure TFormMain.RestoreAxes;
begin
    CheckAssigned(FFitViewer, 'the viewer that draws into this window');

    //  AUTOMATIC, NOT A REMEMBERED CHOICE: the axes are the project's, and a
    //  project opened at start-up puts its own back (ApplyWorkingContext). A
    //  choice kept in the settings followed the user into the next project.
    FAxes.SetWaveLength(FitClientWaveLength);
    FAxes.Reset;
    ApplyAxes;
end;
...
begin
    //  Only the automatic axes depend on the selected type, and only over an
    //  empty model; asking again unconditionally keeps this a single call at
    //  each selection site instead of a rule duplicated across them.
    RefreshAutomaticAxes;
...
begin
    ...
    Result := BuildProjectContext(
        FAxes,
        FitClientApp_.FitClient.GetDataCoordinateMode(adArgument),
        FitClientApp_.FitClient.GetDataCoordinateMode(adValue),
        FitClientApp_.FitClient.GetArgumentDates,
...
var
    D: TAxisDimension;
begin
    //  THE PROJECT'S AXES, AND NOTHING ELSE'S: what the last document chose is
    //  dropped first, so an axis this project never chose is Automatic - and a
    //  new project, which applies an empty plan, starts there too.
    FAxes.Reset;
    //  THE DAY EACH BAR WAS, for the same reason as what the data is below.
    FitClientApp_.FitClient.SetArgumentDates(APlan.ArgumentDates);
    for D := Low(TAxisDimension) to High(TAxisDimension) do
    begin
        //  WHAT THE DATA IS, always: the project holds the points, not the
        //  file, so nothing else will ever say it again.
        FitClientApp_.FitClient.SetDataCoordinateMode(D, APlan.Axes[D].DataMode);
        //  WHAT THE USER CHOSE, only if they did. After the wavelength, which
        //  the restore has already put back, so a remembered sin(theta)/lambda
        //  is not refused for want of one.
        if APlan.Axes[D].Apply then
        begin
            FAxes.SetWaveLength(CurrentWaveLength);
            FAxes.Restore(D, APlan.Axes[D].Mode, True, APlan.Axes[D].Definition);
        end;
    end;
    ApplyAxes;
...
begin
    Caption := WindowCaption(GetWindowTitle(ApplicationProperties.Title, ProjectTitle(APath)));
    ...
    begin
        ...
        //
        //  AN EMPTY PATH LEAVES IT ALONE: a new project - and every import
        //  from a data source, which starts one - has no path, and writing
        //  that over what was remembered made the application stop reopening
        //  the last project the user actually had. recent_project owns the
        //  rule, as it owns the list below.
        FSettings.LastProjectFile :=
            RememberedAfterShowing(FSettings.LastProjectFile, APath);
...
begin
    ...
    begin
        ...
        begin
            ...
            //  THE PATH BY POSITION, not the caption: a caption has its '&'s
            //  doubled so the menu shows them, and is no longer the path.
            Item.Tag := Length(FRecentPaths);
            SetLength(FRecentPaths, Length(FRecentPaths) + 1);
            FRecentPaths[High(FRecentPaths)] := Lines[i].Path;
        end;
...
begin
    //  Only lines that open a project were given this handler, and each one's
    //  tag is where its path was kept.
    if not (Sender is TMenuItem) then
        Exit;
    if (TMenuItem(Sender).Tag < 0) or
        (TMenuItem(Sender).Tag > High(FRecentPaths)) then
        Exit;
    FProjectFlow.OpenRecentProject(FRecentPaths[TMenuItem(Sender).Tag]);
end;
...
begin
    ...
    RequestStateRefresh;
    //  A recorded fit has ended: the result is held for a moment, then the
    //  recording closes the session.
{$IFDEF FIT_RECORD}
    if Recording and (FRecordStage = rsFitting) then
        RecordFitEnded;
{$ENDIF}
end;
...
begin
    ...
    begin
        ...
        //  The wavelength is what the sin(theta)/lambda axis divides by, so
        //  giving one switches to it, as it always has. Naming a diffraction
        //  mode here is the debt the wavelength itself is (A2 in
        //  module-architecture): both leave the framework together.
...
begin
    ...
    //  How far each HTML pane still has to go. The sizes are this window's;
    //  which sizes exist is report_zoom's.
    Result.ReportCanZoomIn := CanZoomIn(FReportZoom);
    Result.ReportCanZoomOut := CanZoomOut(FReportZoom);
    Result.ExplainCanZoomIn := CanZoomIn(FExplainZoom);
    Result.ExplainCanZoomOut := CanZoomOut(FExplainZoom);
end;
...
begin
    ...
    begin
        RefreshModelStructure;
        //  The model or the data may have changed under the automatic axes -
        //  a fit, a detection, a deletion, a profile loaded. Not during an
        //  operation, for the reason above: the model does not change then.
        RefreshAutomaticAxes;
    end;
...
begin
    ...
    LayoutToolPane;
    //  AND THE TOOLBARS AGAIN, for the same reason the pane is laid out again:
    //  their spacing is arithmetic over the font, and a rescale changes the
    //  font. A TToolBar scales its own ButtonWidth and BorderSpacing here and
    //  leaves Indent - the white space before its first button - exactly as it
    //  was, so a bar left to the LCL alone comes out with an outer margin at
    //  the display's size and an inner one still at the design's. Asking for
    //  all of it again is one line and cannot disagree with itself.
    SpaceToolBars;
end;

{ HOW MUCH WHITE SPACE, AND WHY IT IS NOT LEFT TO THE WIDGET SET. A toolbar
  left at its own metrics draws a button barely bigger than the icon in it and
  starts the first one on its edge: the icons run together in one unbroken
  block, nothing marks where one command ends and the next begins, and a
  pointer aimed between two of them still hits one. The sizes are
  toolbar_metrics'; scaling them for this display is ui_scaling's; applying
  them is this. }
procedure TFormMain.SpaceToolBars;
var
    ...
    Spaced: longint;
begin
    //  Scale96ToFont rather than Scale96ToForm: these sizes sit beside text
    //  the user reads, and it is the font the rest of this window is measured
    //  against - the same call the Tools pane and the zoom strips make.
    Metrics := ToolBarMetricsFor(
        Scale96ToFont(ToolBarIconSize96),
        Scale96ToFont(ToolBarButtonPadding96),
        Scale96ToFont(ToolBarMargin96));
    //  KEPT, because the self-check below must judge the bars against what was
    //  applied to them rather than scale the constants a second time. The two
    //  answers are not the same number: this window is built before its font is
    //  final, so the same Scale96ToFont call returned 4 here and 5 by the time
    //  the check ran, and every bar in the window was reported as unspaced over
    //  a pixel nobody could see. A check that re-derives what it is checking
    //  can fail a window that is exactly as it was asked to be.
    FToolBarMetrics := Metrics;
    FToolBarMinimumButton := Scale96ToFont(ToolBarMinimumTarget96);
    Spaced := 0;
    for i := 0 to ComponentCount - 1 do
    begin
        if not (Components[i] is TToolBar) then
            Continue;
        Bar := TToolBar(Components[i]);
        Bar.ButtonWidth := Metrics.ButtonWidth;
        Bar.ButtonHeight := Metrics.ButtonHeight;
        //  THE SPACE AROUND THE BAR IS THE WHOLE BOUNDARY, on all four sides
        //  at once, and it is what separates the buttons from the grid or the
        //  chart the bar is docked against.
        Bar.BorderSpacing.Around := Metrics.Margin;
        //  AND NO INDENT ON TOP OF IT. A toolbar's indent is added to the
        //  LEADING EDGE of the rectangle its buttons are laid out in and to no
        //  other side, so asking for both put twice the white space on one
        //  edge: a vertical strip sat further from its panel's side than from
        //  its top, which is plain to see once anyone looks at one. Nothing is
        //  lost by dropping it - the margin above already holds the first
        //  button off that edge, by the same amount as the other three.
        Bar.Indent := 0;
        //  AND NO BORDER OF ITS OWN. A TToolBar draws one on all four sides,
        //  and while the buttons sat against it that border was the frame
        //  around them. With the margin between the two it is a rule floating
        //  above the first button with nothing under it - a separator pointing
        //  at nothing, which is what the zoom strips showed the moment they
        //  were spaced. The margin is the boundary now; two boundaries read as
        //  a mistake, and this one is drawn by the widget set rather than
        //  asked for.
        Bar.EdgeBorders := [];
        Inc(Spaced);
    end;
    //  SAID, because a walk that matches nothing looks exactly like a walk
    //  that spaced everything, and this one is over components rather than
    //  over a list anybody maintains.
    WriteLog(Format(
        'ui: spaced %d toolbar(s) at %dx%d buttons with a %d px margin',
        [Spaced, Metrics.ButtonWidth, Metrics.ButtonHeight, Metrics.Margin]),
        log.Notification);
end;

function TFormMain.ToolBarReadingsNow: TToolBarReadings;
var
    i, j: longint;
    Bar: TToolBar;
    Button: TToolButton;
    Reading: TToolBarReading;
begin
    Result := nil;
    for i := 0 to ComponentCount - 1 do
    begin
        if not (Components[i] is TToolBar) then
            Continue;
        Bar := TToolBar(Components[i]);
        Reading := Default(TToolBarReading);
        Reading.Name := Bar.Name;
        Reading.Attached := Assigned(Bar.Parent);
        Reading.ButtonCount := Bar.ButtonCount;
        //  FROM THE BUTTONS THEMSELVES, not from what was asked of the bar: a
        //  widget set that declines the size, or a button left autosizing, is
        //  exactly the case this reading exists to catch.
        for j := 0 to Bar.ButtonCount - 1 do
        begin
            Button := Bar.Buttons[j];
            //  A SEPARATOR IS NOT A TARGET. It is deliberately a few pixels
            //  wide and nobody aims at it, so measuring it would report every
            //  bar that has one.
            if Button.Style in [tbsSeparator, tbsDivider] then
                Continue;
            if (Reading.SmallestButtonWidth = 0) or
                (Button.Width < Reading.SmallestButtonWidth) then
                Reading.SmallestButtonWidth := Button.Width;
            if (Reading.SmallestButtonHeight = 0) or
                (Button.Height < Reading.SmallestButtonHeight) then
                Reading.SmallestButtonHeight := Button.Height;
        end;
        Reading.DrawsEdgeBorder := Bar.EdgeBorders <> [];
        //  THE SPACE AROUND THE BAR, which is its boundary on every side, and
        //  separately whatever it adds to its leading edge alone - the two are
        //  not interchangeable, and a bar carrying both is uneven rather than
        //  generous.
        Reading.Margin := Bar.BorderSpacing.Around;
        Reading.LeadingIndent := Bar.Indent;
        SetLength(Result, Length(Result) + 1);
        Result[High(Result)] := Reading;
    end;
end;

procedure TFormMain.ReportToolBarSpacing;
var
    Readings: TToolBarReadings;
    Findings: TUiFindings;
    i: longint;
begin
    Readings := ToolBarReadingsNow;
    //  AGAINST WHAT THE SPACING ACTUALLY APPLIED, in the scale it applied it
    //  in - see SpaceToolBars. What this asks is whether every bar in the
    //  window went through that walk, which is the one thing no headless suite
    //  can answer; whether the numbers themselves are big enough is asserted
    //  against the constants in testcase_toolbar_metrics, where it needs no
    //  display at all.
    Findings := ToolBarFindings(Readings, FToolBarMinimumButton,
        FToolBarMetrics.Margin);
    for i := 0 to High(Findings) do
        WriteLog('ui: ' + Findings[i], Warning);
    //  EACH BAR'S OWN NUMBERS, whether or not anything is wrong with it. The
    //  rules below answer "is this bar spaced at all"; what a reader comparing
    //  two bars needs is what each one actually measures, and pixels in a
    //  screenshot cannot tell a bar's margin from the bevel of the panel it
    //  sits on.
    for i := 0 to High(Readings) do
        WriteLog(Format('ui: toolbar %s: %d button(s), smallest %dx%d, ' +
            'margin %d, indent %d, border %s',
            [Readings[i].Name, Readings[i].ButtonCount,
             Readings[i].SmallestButtonWidth, Readings[i].SmallestButtonHeight,
             Readings[i].Margin, Readings[i].LeadingIndent,
             BoolToStr(Readings[i].DrawsEdgeBorder, True)]), log.Notification);
    WriteLog(ToolBarSpacingSummary(Length(Readings), Length(Findings)),
        log.Notification);
end;
...
begin
    ...
    //  A HOST PANEL RATHER THAN THE VIEW ITSELF, so the pane can carry its own
    //  zoom buttons: the strip is alLeft inside this panel and the view fills
    //  what is left, exactly as the other tabs of this window are built. The
    //  panel is what the splitter now resizes.
    ExplainHost := TPanel.Create(Self);
    ExplainHost.Name := 'PanelExplain';
    ExplainHost.Parent := FTabModel;
    ExplainHost.Align := alBottom;
    ExplainHost.Height := Scale96ToFont(240);
    ExplainHost.BevelOuter := bvNone;
    ExplainHost.Caption := '';
    //  ITS OWN BUTTONS AND ITS OWN SIZE. Zooming the report from a button on
    //  the report must not resize a pane the user is not looking at, so each
    //  pane carries the pair that moves it.
    FExplainZoomStrip := BuildZoomToolBar(ExplainHost, 'ToolBarExplainZoom',
        ActionExplainZoomIn, ActionExplainZoomOut);

    FExplainPane := CreateExplainView(Self, ExplainHost);
    ...
    FExplainPane.Control.Align := alClient;
    FExplainPane.SetOnLink(ExplainPaneLinkFollowed);
    FExplainPane.SetZoom(FExplainZoom);
    ...
    ExplainSplit.Top := ExplainHost.Top - 1;
    ...
    CheckThat((FExplainPane.Control.Parent = ExplainHost) and
        (ExplainHost.Parent = FTabModel),
        'the Explain pane is attached under the Model panel, inside the ' +
        'panel that carries its zoom buttons');
...
procedure TFormMain.ReportReadoutColumns;
var
    Rows: TReadoutRows;
    Findings: TUiFindings;
    Profile: TNeutronPointsSet;
    Widest: array[TAxisDimension] of string;
    Caption_, Value_: array[TAxisDimension] of TLabel;
    D: TAxisDimension;
    Text_: string;
    i: longint;
begin
    Caption_[adArgument] := LabelPositionCaption;
    Caption_[adValue] := LabelIntensityCaption;
    Value_[adArgument] := LabelPositionValue;
    Value_[adValue] := LabelIntensityValue;
    //  ONLY WHAT IS ON SCREEN HAS A LAYOUT. The widget set resolves anchors
    //  for shown controls alone, and under /CHECK_UI the window may never be
    //  shown - every label then reads as 0 px wide wherever it would really
    //  be. Said, and not measured, rather than reported as a fault.
    if not LabelIntensityValue.IsVisible then
    begin
        WriteLog('ui: the readout beside the chart is not on screen, so ' +
            'whether its values line up and fit was not measured', log.Notification);
        Exit;
    end;
    //  THE WIDEST THE DATA CAN MAKE THEM: every point's reading, in the words
    //  of the axes in force, and the longest kept. A readout that fits "0.00"
    //  says nothing about "1004.21".
    Widest[adArgument] := '';
    Widest[adValue] := '';
    Profile := nil;
    if Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient) and
        (FitClientApp_.FitClient.OpenState = OpenSuccess) then
        Profile := FitClientApp_.FitClient.GetProfilePoints;
    if Assigned(Profile) then
        for i := 0 to Profile.PointsCount - 1 do
        begin
            Text_ := FAxes.Axis(adArgument).ReadoutText(
                FAxes.Axis(adArgument).ToDisplay(Profile.PointXCoord[i]));
            if Length(Text_) > Length(Widest[adArgument]) then
                Widest[adArgument] := Text_;
            Text_ := FAxes.Axis(adValue).ReadoutText(
                FAxes.Axis(adValue).ToDisplay(Profile.PointYCoord[i]));
            if Length(Text_) > Length(Widest[adValue]) then
                Widest[adValue] := Text_;
        end;
    for D := Low(TAxisDimension) to High(TAxisDimension) do
        Value_[D].Caption := Widest[D];
    //  The anchors are resolved by the layout, so it is let run first.
    Application.ProcessMessages;
    ...
    begin
        i := Ord(D);
        Rows[i].Caption := Caption_[D].Caption;
        Rows[i].CaptionRight := Caption_[D].Left + Caption_[D].Width;
        Rows[i].ValueLeft := Value_[D].Left;
        Rows[i].ValueWidth := Value_[D].Width;
        Rows[i].Value := Value_[D].Caption;
        Rows[i].ValueTextWidth := Value_[D].Canvas.TextWidth(Value_[D].Caption);
    end;
    //  THE GEOMETRY BEHIND ANY FINDING, so a failure says where things were.
    for D := Low(TAxisDimension) to High(TAxisDimension) do
        WriteLog(Format('ui: readout %s caption %d..%d, value %d..%d in %s ' +
            '(width %d, parent %s width %d, visible %s, parent visible %s)',
            [Caption_[D].Caption,
            Caption_[D].Left, Caption_[D].Left + Caption_[D].Width,
            Value_[D].Left, Value_[D].Left + Value_[D].Width,
            Value_[D].Name, Value_[D].Width, Value_[D].Parent.Name,
            Value_[D].Parent.ClientWidth, BoolToStr(Value_[D].IsVisible, True),
            BoolToStr(Value_[D].Parent.IsVisible, True)]), log.Debug);
    Findings := ReadoutFindings(Rows);
    for i := 0 to High(Findings) do
        WriteLog('ui: ' + Findings[i], Warning);
    WriteLog(ReadoutSummary(Length(Rows), Length(Findings)), log.Notification);
end;
...
begin
    ...
    Started := Now;
    //  THE OUTER ARRAYS COPIED, for the reason ReadModelTreeUnderPicksType
    //  gives: a record copy shares them.
    Saved := FPanelSources;
    Saved.Held.Ids := Copy(FPanelSources.Held.Ids);
    Saved.Held.Rows := Copy(FPanelSources.Held.Rows);
    PanelId := FirstModulePanelId;
    try
        FitClientApp_.FitClient.MinimizeDifference;

        //  A MODULE'S ROWS WHILE THE FIT RUNS, as a module's redraw pushes
        //  them. Not drawn now: drawing puts the selection back, which calls
        //  into the module, and a module that asks the engine then waits for
        //  the fit's lock with the window behind it. Read at once, because the
        //  module's own redraw may push over them before the fit ends.
        if (PanelId <> '') and
            (FitClientApp_.FitClient.AsyncState = AsyncWorks) then
        begin
            DuringFit := nil;
            SetLength(DuringFit, 1);
            DuringFit[0].Id := 'check-ui.during-fit';
            DuringFit[0].CurveId :=
                FitClientApp_.FitClient.FitService.GetCurveInstanceId(0);
            DuringFit[0].Caption := 'A pattern pushed while a fit ran';
            Reading.PanelPushChecked := True;
            ShowModulePanel(PanelId, DuringFit);
            Reading.PanelDrawnDuringFit := False;
            for i := 0 to High(FShownRows) do
                if FShownRows[i].Id = 'check-ui.during-fit' then
                    Reading.PanelDrawnDuringFit := True;
        end;
        ...
        //  AND ONCE IT HAS ENDED, the panel shows what it has been told - by
        //  whatever the window itself does when a fit ends. Compared with a
        //  fresh composition rather than the row pushed above, because the
        //  module's own redraw may have pushed over it since.
        if Reading.PanelPushChecked then
            Reading.PanelDrawnAfterFit := (FShownPanelId =
                ComposePanel(FPanelSources, SelectedPlacedByPointSet).PanelId) and
                SameOutline(FShownRows,
                    ComposePanel(FPanelSources, SelectedPlacedByPointSet).Rows);
    finally
...
{ RECORDING A FIT: /RECORD_FIT=<dir>.

  THE ORDINARY SESSION, WITH FRAMES TAKEN. The project opened at start-up is the
  one start-up opens anyway. Then, inside Application.Run, the window runs
  Model > Clear Model, View > Animation Mode (when it is off) and Fit >
  Automatically through those actions' own Execute - the code a click on the
  menu runs - and nothing else is driven. A frame is taken where the chart has
  just been updated (ChartPaintTiming), so the frames are what a user watching
  the fit would see. The one thing this mode answers for the user is the Clear
  Model confirmation (Confirm).

  IT CHANGES THE MODEL of the project it opened, and ends the process without
  saving: Application.Terminate, not Close, which would ask about the unsaved
  change. Animation Mode is put back as it was, through its action. }
{$IFDEF FIT_RECORD}
const
    { The window's size in every frame, so that one recording replaces the
      last on the page without the layout around it moving. }
    RecordWidth = 1440;
    RecordHeight = 900;
    { The cleared project before the fit, and the result after it, in frames
      of the video (screen_recording.RecordPlayFps a second). }
    RecordLeadFrames = 2 * RecordPlayFps;
    RecordTailFrames = 3 * RecordPlayFps;

procedure TFormMain.StartRecordingFit(const ADir: string);
begin
    if not (Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient) and
        (FitClientApp_.FitClient.OpenState = OpenSuccess)) then
    begin
        WriteLog('record: no project was opened at start-up, so there is no fit ' +
            'to record', Warning);
        Application.Terminate;
        Exit;
    end;
    FRecordDir := ADir;
    FRecordStage := rsSizing;
    FRecordSizingTries := 0;
    //  In the application loop, once it runs - not here, before it has.
    Application.QueueAsyncCall(RecordBegin, 0);
end;

{$ENDIF}

function TFormMain.Recording: boolean;
begin
{$IFDEF FIT_RECORD}
    Result := FRecordDir <> '';
{$ELSE}
...
procedure TFormMain.RecordBegin(Data: PtrInt);
begin
    //  A DESKTOP SMALLER THAN THE WINDOW shrinks it, and every picture comes out
    //  a different size and layout from the last - said, and nothing recorded.
    if (Screen.Width < RecordWidth) or (Screen.Height < RecordHeight) then
    begin
        WriteLog(Format('record: the desktop is %dx%d, smaller than the %dx%d ' +
            'window the recording is made at - enlarge the VM''s screen',
            [Screen.Width, Screen.Height, RecordWidth, RecordHeight]), Warning);
        FRecordStage := rsDone;
        Application.Terminate;
        Exit;
    end;
    WindowState := wsNormal;
    SetBounds(Left, Top, RecordWidth, RecordHeight);
    //  Model > Clear Model.
    FActionClearModel.Execute;
    //  View > Animation Mode, so the curves move while the engine works. Asked
    //  of the client: the action's tick is only brought up to date at idle.
    FRecordWasAnimating := FitClientApp_.FitClient.AnimationMode;
    if not FRecordWasAnimating then
        ActionAnimationMode.Execute;
    //  THE FIT STARTS ONCE THE WINDOW HAS TAKEN ITS SIZE (RecordFrame): the
    //  compositor applies a new size when it gets to it, and a window that was
    //  maximised may come back at whatever size it had before - every frame
    //  would then be of another layout than the last recording's. No timer of
    //  its own: the window's one timer asks for fit progress and nothing else,
    //  and the resize and the cleared model both repaint the chart.
    Chart.Invalidate;
end;

procedure TFormMain.RecordStartFit(Data: PtrInt);
begin
    //  The recording's clock starts here, with the cleared project held as a
    //  still before the fit.
    FRecorder := TWindowRecorder.Create(Self, FRecordDir);
    FRecorder.Capture;
    FRecorder.Hold(RecordLeadFrames);
    WriteLog(Format('record: the fit starts at frame %d',
        [FRecorder.FramesWritten + 1]), log.Notification);
    FRecordStage := rsFitting;
    //  Fit > Automatically.
    ActionDoAllAutomatically.Execute;
end;

procedure TFormMain.RecordFitEnded;
begin
    FRecordStage := rsTail;
    //  Once the window has finished handling the end of the fit.
    Application.QueueAsyncCall(RecordFinish, 0);
end;

procedure TFormMain.RecordFinish(Data: PtrInt);
begin
    //  The result as it stands, held as a still.
    FRecorder.Capture;
    FRecorder.Hold(RecordTailFrames);
    if not FRecordWasAnimating then
        ActionAnimationMode.Execute;
    //  Every frame handed to the writer is on disk before the count is said.
    FRecorder.Finish;
    WriteLog(Format('record: %d frame(s) painted in %d ms and encoded in %d ms, ' +
        '%d not taken', [FRecorder.Renders, FRecorder.PaintMs, FRecorder.SaveMs,
        FRecorder.Dropped]), log.Debug);
    WriteLog(Format('record: %d frame(s) written to %s',
        [FRecorder.FramesWritten, FRecordDir]), log.Notification);
    FRecordStage := rsDone;
    Application.Terminate;
end;

procedure TFormMain.RecordFrame(Data: PtrInt);
const
    { How many repaints the window is given to take its size. }
    SizingTries = 30;
begin
    FRecordFrameQueued := False;
    if FRecordStage = rsSizing then
    begin
        if (Width = RecordWidth) and (Height = RecordHeight) then
        begin
            FRecordStage := rsFitting;
            Application.QueueAsyncCall(RecordStartFit, 0);
            Exit;
        end;
        Inc(FRecordSizingTries);
        if FRecordSizingTries > SizingTries then
        begin
            WriteLog(Format('record: the window stayed %dx%d instead of the %dx%d ' +
                'the recording is made at', [Width, Height, RecordWidth,
                RecordHeight]), Warning);
            FRecordStage := rsDone;
            Application.Terminate;
            Exit;
        end;
    ...
    //  EVERY UPDATE OF THE CHART DURING THE FIT is a frame: the video is the
    //  sequence the fit went through, however fast the window drew it.
    if Assigned(FRecorder) and (FRecordStage = rsFitting) then
        FRecorder.Capture;
end;
{$ENDIF}
...
procedure TFormMain.ReportDataSources;
var
    Reading: TDataSourceReading;
...
begin
    ...
    Reading := Default(TDataSourceReading);
    Reading.SourceCount := Length(Sources);
    for i := 0 to High(Sources) do
        if not Sources[i].Info.NeedsNetwork then
            Inc(Reading.OfflineCount);
    //  THE SAME WALK THE SUITE RUNS, here over what this binary really has -
    //  including any source a module contributed, which the framework's own
    //  suite never sees.
    Reading.Findings := JoinedFindings(
        data_source_findings.DataSourceFindings(@RegisteredTopicResolves));
    Reading.CommandFound := Assigned(ActionNewFromDataSource);
    Reading.CommandEnabled := Reading.CommandFound and
        ActionNewFromDataSource.Enabled;

    //  AND THE WINDOW THE SOURCES ARE CHOSEN IN, which is built in code when
    //  the menu entry is chosen - so it is the one window in this application
    //  whose captions the walk over existing forms never measures. Built here,
    //  measured, and freed: it is a measurement, not a session.
    Window := TDataSourceWindow.CreateWizard(Self, Self as IImportHost);
    try
        ReportClippedCaptionsOf(Window);
        //  WHAT THE WIDGET SET QUEUED FOR THIS WINDOW RUNS WHILE IT EXISTS.
        //  The Cocoa widget set resizes a scroll view LATER, through one
        //  global object holding the last control it was asked about
        //  (TASyncLCLControlAdjustSizer, cocoawsscrollers.pas) - so a window
        //  built and freed without the queue running in between left that
        //  call aimed at freed memory, and whichever check next let the queue
        //  run died of it, with classic scroll bars on (findings.md).
        Application.ProcessMessages;
    finally
        Window.Free;
    end;

    Findings := ui_selfcheck.DataSourceFindings(Reading);
    for i := 0 to High(Findings) do
        WriteLog('ui: ' + Findings[i], Warning);
    //  A verdict either way: a check that speaks only when it finds something
    //  cannot be told from one that never ran.
    WriteLog(DataSourceSummary(Reading), log.Notification);
end;
...
begin
    ...
    TableHandle := '';
    TableRow := -1;
    if Assigned(CurrentCurveList) then
    begin
        TableRow := CurrentCurveList.RowNamingAnotherCurve(FSelectedCurveId);
        TableHandle := CurrentCurveList.CurveHandleOfRow(TableRow);
    end;
    if TableHandle = '' then
    ...
    //  ---- AND A MODULE'S NESTED ROWS, with an ordinary type selected - over
    //  the model built above, whose two curves the rows stand for.
    TreeReading := ReadModelTreeUnderPicksType;
    Findings := ModelTreeFindings(TreeReading);
    for i := 0 to High(Findings) do
        WriteLog('ui: ' + Findings[i], Warning);
    WriteLog(ModelTreeSummary(TreeReading), log.Notification);

    FSelectedRowId := Was;
end;

{ WHAT THE MODEL PANEL DRAWS for a module's nested rows while a curve type placed
  by picks is selected - the case in which it once drew a nested model flat.

  THE APPLICATION'S OWN ORDER: the rows arrive through ShowModulePanel, as a
  module's redraw sends them, and then the framework refreshes the panel, as it
  does on every command refresh - both under the ordinary type. Under the
  module's own type the old rule showed the tree too, so the type is selected
  here rather than whatever the settings restored.

  Named by the first module that declares a panel, because rows under an id no
  module claims are shown once and never composed, which is not the path a
  module takes. Everything is put back afterwards: the rows each source last
  sent, the selected type, the panel. }
{ The panel id of the first module that declares one, or '' in a build with
  none. The self-check pushes rows under it, because rows under an id no module
  claims are shown once and never composed - not the path a module takes. }
function TFormMain.FirstModulePanelId: string;
var
    Mods: TUiModuleArray;
    i: longint;
begin
    ...
    for i := 0 to High(Mods) do
        if Trim(Mods[i].PanelId) <> '' then
            Exit(Mods[i].PanelId);
end;

function TFormMain.ReadModelTreeUnderPicksType: TModelTreeReading;
const
    ParentRowId = 'check-ui.nested-parent';
    ChildRowId = 'check-ui.nested-child';
    StaleRowId = 'check-ui.stale';
    { A handle no curve of the check's model carries - as none of the open
      model's carries the handle of a curve from a model since closed. }
    StaleHandle = '{0C4EC4E0-0000-4000-8000-000000000001}';
var
    PanelId, Handle0, Handle1, NodeId: string;
    Selector: ICurveTypeSelector;
    Iterator: ICurveTypeIterator;
    Cls: TCurveClass;
    WasType, PicksType: TCurveTypeId;
    HasPicksType: boolean;
    Saved: TPanelSources;
    Rows: TOutline;
    Node: TTreeNode;
    Curves: TSelfCopiedCompList;

    function IdOfNode(ANode: TTreeNode): string;
    begin
        Result := '';
        if Assigned(ANode) and Assigned(FOutlineGuids) and
            (PtrInt(ANode.Data) > 0) then
            Result := FOutlineGuids[PtrInt(ANode.Data) - 1];
    end;

    function NodeOfRow(const ARowId: string): TTreeNode;
    begin
        Result := FModuleTree.Items.GetFirstNode;
        while Assigned(Result) and (IdOfNode(Result) <> ARowId) do
...
begin
    ...
    PanelId := FirstModulePanelId;
    //  No module describes the model in this build: the verdict says so.
    if PanelId = '' then
        Exit;
    Result.Ran := True;

    //  A TYPE PLACED BY PICKS, and one that asks nothing: a type with
    //  parameters to configure opens a dialog, and the user-defined factory
    //  builds a curve of its own.
    Selector := TCurveTypesSingleton.CreateCurveTypeSelector;
    Iterator := TCurveTypesSingleton.CreateCurveTypeIterator;
    if not (Assigned(Selector) and Assigned(Iterator)) then
        Exit;
    WasType := Selector.GetSelectedCurveType;
    HasPicksType := False;
    Iterator.FirstCurveType;
    while True do
    begin
        Cls := Iterator.GetCurrentCurveClass;
        if Assigned(Cls) and (Trim(Cls.PlacedByPointSet) = '') and
            (not IsEqualGUID(Iterator.GetCurveTypeId,
                TUserPointsSet.GetCurveTypeId)) and
            (not Cls.GetConfigurablePointsSet.HasConfigurableParameters) then
        begin
            PicksType := Iterator.GetCurveTypeId;
            HasPicksType := True;
            Break;
        end;
        //  "Is this the last item", not "stop iterating".
        if Iterator.EndCurveType then
            Break;
        Iterator.NextCurveType;
    end;
    if not HasPicksType then
        Exit;
    FitClientApp_.FitClient.SelectCurveType(PicksType);
    FitClientApp_.FitClient.UpdateComputedData(True);
    Result.PicksTypeSelected := FrameworkFillsStructure(SelectedPlacedByPointSet);

    Curves := FitClientApp_.FitClient.CurvesForDisplay;
    if Assigned(Curves) then
        Result.CurveCount := Curves.Count;
    Handle0 := FitClientApp_.FitClient.FitService.GetCurveInstanceId(0);
    Handle1 := FitClientApp_.FitClient.FitService.GetCurveInstanceId(1);

    //  A PARENT AND ITS CHILD, as a module describing a hierarchy sends them:
    //  its own row ids, each carrying the handle of the curve it stands for.
    Rows := nil;
    SetLength(Rows, 2);
    Rows[0].Id := ParentRowId;
    Rows[0].CurveId := Handle0;
    Rows[0].Caption := 'A pattern a module placed';
    Rows[0].Indent := 0;
    Rows[1].Id := ChildRowId;
    Rows[1].CurveId := Handle1;
    Rows[1].Caption := 'A pattern nested in it';
    Rows[1].Indent := 1;

    //  THE OUTER ARRAYS COPIED: a record copy shares its dynamic arrays, and
    //  HoldPanelRows replaces a panel's rows in place - so a plain copy would
    //  "restore" the very rows this check pushed.
    Saved := FPanelSources;
    Saved.Held.Ids := Copy(FPanelSources.Held.Ids);
    Saved.Held.Rows := Copy(FPanelSources.Held.Rows);
    try
        ShowModulePanel(PanelId, Rows);
        RefreshModelStructure;

        Node := FModuleTree.Items.GetFirstNode;
        while Assigned(Node) do
        begin
            //  A node with no row behind it is the empty-panel sentence.
            if PtrInt(Node.Data) > 0 then
                Inc(Result.NodesDrawn);
            NodeId := IdOfNode(Node);
            if NodeId = ChildRowId then
                Result.ChildUnderParent := IdOfNode(Node.Parent) = ParentRowId;
            Node := Node.GetNext;
        end;

        //  COLLAPSED AS A USER COLLAPSES IT, then the refresh every command
        //  change runs. Nothing changed, so nothing may be drawn again - a
        //  rebuild opens every subtree. Found again by its row afterwards,
        //  because a rebuild would have made a new node.
        Node := NodeOfRow(ParentRowId);
        if Assigned(Node) then
        begin
            Node.Expanded := False;
            CheckState;
            Node := NodeOfRow(ParentRowId);
            Result.CollapseSurvivedRefresh := Assigned(Node) and
                (not Node.Expanded);
        end;

        //  THE SELECTED CURVE FOLLOWS THE SELECTED ROW. A row selected as a
        //  click selects it, then the curve cleared as a refused Delete curve
        //  clears it, then the refresh every command change runs.
        Node := NodeOfRow(ChildRowId);
        if Assigned(Node) then
        begin
            FModuleTree.Selected := Node;
            SetSelectedCurveId('');
            CheckState;
            Result.CurveFollowsRowAfterRefresh := FSelectedCurveId = Handle1;
        end;

        //  ROWS OF A MODEL THAT IS NOT OPEN: kept, because nothing clears a
        //  module's rows when a model is replaced, and a module whose redraw
        //  exits early never replaces them itself.
        Rows := nil;
        SetLength(Rows, 1);
        Rows[0].Id := StaleRowId;
        Rows[0].CurveId := StaleHandle;
        Rows[0].Caption := 'A pattern of a model since closed';
        Rows[0].Indent := 0;
        ShowModulePanel(PanelId, Rows);
        RefreshModelStructure;
        Result.StaleRowsShown := Assigned(NodeOfRow(StaleRowId));
    finally
        FPanelSources := Saved;
        FitClientApp_.FitClient.SelectCurveType(WasType);
        FitClientApp_.FitClient.UpdateComputedData(True);
        RefreshAxisForSelectedCurveType;
    end;
end;
...
begin
    ...
    if Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient) and
        ...
        Exit;
    Mods := RegisteredUiModules;
    //  PER ROW, not per panel: the rows listed after a module's tree are the
    //  framework's, and asking only that module about one offers nothing.
    Asked := RowMenuModulesToAsk(Mods, RowMenuPanelId(ShownContents,
        FRowMenuRowId));
    Decls := nil;
...
{ A strip of zoom buttons down the left of a pane, built to match the toolbars
  the other bottom tabs carry in the designed form: icons only, no captions,
  the same image list. How much room each icon gets is not decided here or
  there - SpaceToolBars gives every bar in the window the same white space.

  BY ANALOGY AND NOT BY COPY. The designed toolbars are in form_main.lfm
  because their tabs are; the report tabs are made at run time, so theirs is
  made here - and everything a reader would check between them (width, flatness,
  image list, the 25 px pitch down the strip) is stated in one place. }
function TFormMain.BuildZoomToolBar(AParent: TWinControl; const AName: string;
    AZoomIn, AZoomOut: TAction): TToolBar;
var
    Bar: TToolBar;

    procedure ButtonFor(AAction: TAction; ATop: longint; const ASuffix: string);
    var
        Button: TToolButton;
    begin
        Button := TToolButton.Create(Self);
        Button.Name := AName + ASuffix;
        Button.Parent := Bar;
        Button.Left := 1;
        Button.Top := ATop;
...
begin
    ...
    Bar.Parent := AParent;
    Bar.Align := alLeft;
    Bar.Width := Scale96ToFont(27);
    Bar.AutoSize := True;
    Bar.Flat := False;
    Bar.ShowCaptions := False;
    Bar.Images := ImageListToolbars;
    //  A STARTING POSITION ONLY. The bar lays its own buttons out from its
    //  ButtonWidth and ButtonHeight, and those - with the margins - are given
    //  to every bar in the window at once by SpaceToolBars, which runs after
    //  this. What is here is what the designed bars carry, so a strip looks
    //  right in the moment between being built and being spaced.
    ButtonFor(AZoomIn, Scale96ToFont(2), 'In');
    ButtonFor(AZoomOut, Scale96ToFont(27), 'Out');
    //  ATTACHED, AND CHECKED TO BE: a control built and never parented is
    //  invisible, and nothing reports it.
    CheckThat((Bar.Parent = AParent) and (Bar.ButtonCount = 2),
        'a zoom strip is attached to its pane, with both of its buttons');
    Result := Bar;
end;
...
begin
    ...
    begin
        ...
        //  BEFORE THE VIEW, because the view is alClient and fills whatever
        //  the strip has not taken. The other bottom tabs carry the same
        //  strip from the designed form.
        FReportZoomStrip := BuildZoomToolBar(Sheet,
            FWidgetNames.NameFor('ReportZoomBar', FReportTabs[i].Module),
            ActionReportZoomIn, ActionReportZoomOut);
...
{ Redraws every report tab at the size the window now holds, and lets the
  commands grey the button that has run out of ladder.

  EVERY TAB, not the one in front: they are one surface, and a tab that kept
  the size it had when it was last looked at would disagree with its neighbour
  for no reason the user could see. }
{ Clicks a pane's two zoom buttons in the real window and reads what happened
  to both panes, through the same actions the buttons on the strips run.

  THROUGH THE ACTION, NOT THE FIELD. Setting FReportZoom here would check this
  method against itself; executing the action is the path a click takes, and a
  button wired to nothing is exactly the defect worth finding.

  IT PUTS THE SIZES BACK. This runs in the window the user is about to be given
  (or, under /CHECK_UI, in one whose settings are about to be written), so a
  check that left a pane at the top of its ladder would have changed a setting
  by measuring it. }
function TFormMain.ZoomReadingFor(const APane: string; AStrip: TToolBar;
    AZoomIn, AZoomOut: TAction; var AMine: longint;
    const AOther: longint): TZoomReading;
var
    Started, StartedOther: longint;
begin
    Result := Default(TZoomReading);
    Result.Pane := APane;
    Result.Buttons := 0;
    if Assigned(AStrip) then
        Result.Buttons := AStrip.ButtonCount;

    Started := AMine;
    StartedOther := AOther;
    Result.Before := Started;
    Result.OtherBefore := StartedOther;

    AZoomIn.Execute;
    Result.AfterIn := AMine;
    AZoomOut.Execute;
    Result.AfterOut := AMine;
    Result.OtherAfter := AOther;

    //  To the top, and then asked whether it still offers more. CheckState is
    //  what the window does after every click, so this is the state a user
    //  would find the button in.
    while CanZoomIn(AMine) do
        AZoomIn.Execute;
    CheckState;
    Result.OfferedInAtTop := AZoomIn.Enabled;
    while CanZoomOut(AMine) do
        AZoomOut.Execute;
    CheckState;
    Result.OfferedOutAtBottom := AZoomOut.Enabled;

    //  Back where it started, by the same road.
    while AMine < Started do
        AZoomIn.Execute;
    CheckState;
end;

procedure TFormMain.ReportPaneZoom;
var
    Readings: TZoomReadings;
    Findings: TUiFindings;
    i: longint;
begin
    //  THE REPORT HALF ONLY WHERE THERE IS A REPORT. A build that links no
    //  module that reports has no analysis tab, so there is no strip on one -
    //  which is not a defect, and must not read as two missing buttons.
    Readings := nil;
    if Assigned(FReportZoomStrip) then
    begin
        SetLength(Readings, 1);
        Readings[0] := ZoomReadingFor('analysis report', FReportZoomStrip,
            ActionReportZoomIn, ActionReportZoomOut, FReportZoom, FExplainZoom);
    end
    else
        WriteLog(NoReportTabToZoom, log.Notification);
    SetLength(Readings, Length(Readings) + 1);
    Readings[High(Readings)] := ZoomReadingFor('Explain pane',
        FExplainZoomStrip, ActionExplainZoomIn, ActionExplainZoomOut,
        FExplainZoom, FReportZoom);

    Findings := ZoomFindings(Readings);
    for i := 0 to High(Findings) do
        WriteLog('ui: ' + Findings[i], Warning);
    WriteLog(ZoomSummary(Length(Readings), Length(Findings)), log.Notification);
end;

procedure TFormMain.RestoreZoom;
var
    i: longint;
begin
    FReportZoom := UsableZoom(FSettings.ReportZoom);
    FExplainZoom := UsableZoom(FSettings.ExplainZoom);
    //  THE VIEWS DIRECTLY, not through Apply*Zoom: those ask the window to
    //  decide every command again, and this runs while it is still being
    //  built. The first poll does that anyway.
    for i := 0 to High(FReportViews) do
        if Assigned(FReportViews[i]) then
            FReportViews[i].SetZoom(FReportZoom);
    if Assigned(FExplainPane) then
        FExplainPane.SetZoom(FExplainZoom);
end;

procedure TFormMain.ApplyReportZoom;
var
    i: longint;
begin
    for i := 0 to High(FReportViews) do
...
procedure TFormMain.ApplyExplainZoom;
begin
    if Assigned(FExplainPane) then
        FExplainPane.SetZoom(FExplainZoom);
    CheckState;
...
begin
    ...
    begin
        if not SelectModelRowById(RowId) then
            ShowHint(RowLinkMissHint(FPanelSources, RowId));
    end
    else if AHref <> '' then
        OpenURL(AHref);
end;
...
begin
    ...
    if not Assigned(FitClientApp_.FitClient) then
        Exit;
    //  BUILT WHATEVER TYPE IS SELECTED. Whether a module's description is
    //  shown instead is decided from what each has described, where the rows
    //  are composed (ShowModulePanel); these rows are also what fills in the
    //  curves no module describes.
    //
    ...
    ShowModulePanel(FrameworkStructureId, FrameworkRowsNow);
end;

{ What the panel is showing, as ComposePanel answered it. }
function TFormMain.ShownContents: TPanelContents;
begin
    Result.PanelId := FShownPanelId;
    Result.Rows := FShownRows;
    Result.OwnRowCount := FShownOwnRowCount;
end;

{ The framework's rows for the model the client holds now. model_curve_rows
  reads them, where a test can. }
function TFormMain.FrameworkRowsNow: TOutline;
begin
    Result := nil;
    if not (Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient)) then
        Exit;
    Result := ModelOutlineOf(ModelCurveRowsOf(
        FitClientApp_.FitClient.CurvesForDisplay,
        FitClientApp_.FitClient.CurveAttributesForDisplay));
end;
...
begin
    ...
    //  KEPT, THEN COMPOSED: the panel shows what the MODEL holds - a module's
    //  tree whenever it describes something, whatever type the Tools list has
    //  selected, with the framework's rows for the curves it does not.
    //  model_outline.ComposePanel decides; rows under an id no module claims
    ...
    PanelId := APanelId;
    Rows := ARows;
    OwnRowCount := Length(ARows);
    //  A MODULE'S ROWS ARE JUDGED AGAINST THE MODEL AS IT IS NOW. They are
    //  shown only while they name a curve the model holds, and a pattern just
    //  placed is pushed before the next command refresh rebuilds the
    //  framework's rows - so those are rebuilt here first, or the new
    //  pattern's rows would be judged against a model without it.
    if APanelId <> FrameworkStructureId then
        FPanelSources.FrameworkRows := FrameworkRowsNow;
    if KeepPanelRows(FPanelSources, APanelId, PanelIds, ARows) then
    begin
        Shown := ComposePanel(FPanelSources, SelectedPlacedByPointSet);
        PanelId := Shown.PanelId;
        Rows := Shown.Rows;
        OwnRowCount := Shown.OwnRowCount;
    end;

    //  KEPT, NOT DRAWN, WHILE AN OPERATION RUNS - the rule the framework's own
    //  refresh follows (action_state.ModelStructureMayBeRebuilt), now for a
    //  module's push too. Drawing puts the selection back, which hands the row
    //  to the module owning it; a module that answers by asking the engine
    //  waits for the fit's lock, and the whole window waits with it. What was
    //  kept is drawn by the refresh that follows the operation.
    if Assigned(FitClientApp_) and Assigned(FitClientApp_.FitClient) and
        not ModelStructureMayBeRebuilt(FitClientApp_.FitClient.AsyncState) then
        Exit;
    ...
    Text := PanelTextFor(RegisteredUiModules, PanelId);
    EmptyText := Text.EmptyText;
    if PanelId = FrameworkStructureId then
    ...
    //  NOT DRAWN AGAIN WHEN NOTHING CHANGED. This is reached on every command
    //  refresh, and a rebuild clears the tree, opens every subtree the user
    //  closed and puts the selection back - which hands the row to the module
    //  owning it, a call into that module on every refresh. What decides
    //  sameness is model_outline.SameOutline.
    if (PanelId = FShownPanelId) and SameOutline(Rows, FShownRows) and
        (EmptyText = FShownEmptyText) then
    begin
        //  BUT THE SELECTED CURVE STILL FOLLOWS THE SELECTED ROW. A refused
        //  Delete curve clears the curve and leaves the row; drawing the panel
        //  again used to select the row, and so its curve, again. Not drawn,
        //  the curve is put back here - without touching the tree, so no
        //  module is called.
        if CurveToReselect(FShownRows, FSelectedRowId, FSelectedCurveId,
            Handle) then
            SetSelectedCurveId(Handle);
        Exit;
    end;
    FShownEmptyText := EmptyText;
    FShownOwnRowCount := OwnRowCount;
    ...
    try
        ...
        Nodes := BuildOutlineNodes(Rows, DetachedSuffix);
...
begin
    ...
    Choice := CurveChoiceFromTable(FShownRows,
        CurrentCurveList.CurveHandleOfRow(ARow), FSelectedCurveId);
...
begin
    ...
    FileName := GetConfigFileName;
    //  NO AXIS: the axes are the project's and are saved with it.
    ...
    //  How large the user was reading each HTML pane. A size chosen once and
    //  lost on exit would have to be chosen again every session, which is the
    //  same as not having the buttons.
    FSettings.ReportZoom := FReportZoom;
    FSettings.ExplainZoom := FExplainZoom;
