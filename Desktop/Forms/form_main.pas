...
begin
    MessageDlg('Compute Backends',
        ...
        'setup is described in the user guide, under Compute backends ' +
        '(docs/user-guide/compute-backends.md).' + LineEnding +
        LineEnding +
...
begin
    ...
    ActionGlossary.Hint := 'Lists everything this build can explain - every ' +
        'curve type and every rule a module enforces - and shows each one.';
...
begin
    ...
    if Assigned(FCurveList) then
        for i := 0 to FCurveList.Count - 1 do
            if (FCurveList.CurveHandleOfRow(i) <> '') and
                (FCurveList.CurveHandleOfRow(i) <> FSelectedCurveId) then
            begin
                TableRow := i;
                TableHandle := FCurveList.CurveHandleOfRow(i);
                Break;
            end;
    if TableHandle = '' then
...
{ THE SAME SELECTION, WHEREVER THE CURVE IS CHOSEN. Choosing it in the table
  selects the panel's row for it rather than setting the handle beside the
  panel: the panel's Delete curve acts on the held handle, so a panel showing
  one curve while another is held would aim that command at a curve the user
  is not looking at. Selecting the row fires ModulePanelSelectionChanged, which
  sets the handle, the highlight and the Explain pane exactly as a click would.

  Matched by the row's own id, never by position: the node's Data indexes
  FOutlineGuids, and the rows are asked which of them stands for the curve.
  Where none does - a pack's panel shows only its own rows - the panel is left
  with nothing selected and the handle is held anyway, so the chart still shows
  the curve chosen and the panel points at nothing rather than something else. }
...
begin
    if not Assigned(FCurveList) then
        Exit;
    Handle := FCurveList.CurveHandleOfRow(ARow);
    if (Handle = '') or (Handle = FSelectedCurveId) then
        Exit;

    Node := nil;
    Index := RowIndexForCurveHandle(FShownRows, Handle);
    if (Index >= 0) and Assigned(FModuleTree) and Assigned(FOutlineGuids) then
    begin
        RowId := FShownRows[Index].Id;
        Node := FModuleTree.Items.GetFirstNode;
        while Assigned(Node) do
        begin
            if (PtrInt(Node.Data) > 0) and
                (FOutlineGuids[PtrInt(Node.Data) - 1] = RowId) then
                Break;
            Node := Node.GetNext;
        end;
    end;
