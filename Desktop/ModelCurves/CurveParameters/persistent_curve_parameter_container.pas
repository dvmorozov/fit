...
type
    ...
    private
        ...
        function GetHeld: boolean;
        procedure SetHeld(AHeld: boolean);
    ...
    published
        ...
        { Whether the fit is kept from varying the parameter - held at its value
          in Set Curve Type Properties. Absent in files written before it
          existed, which then load varied, as they always did. }
        property Held: boolean read GetHeld write SetHeld default False;
    end;
...
function TPersistentCurveParameterContainer.GetHeld: boolean;
begin
    Result := FSpecialCurveParameter.VariationDisabled;
end;

procedure TPersistentCurveParameterContainer.SetHeld(AHeld: boolean);
begin
    FSpecialCurveParameter.VariationDisabled := AHeld;
end;
