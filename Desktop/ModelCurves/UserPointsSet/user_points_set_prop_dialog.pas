...
type
    ...
    TUserPointsSetPropDlg = class(TForm)
        ...
        CheckListHeld: TCheckListBox;
        LabelHeldParameters: TLabel;
        ...
        procedure CheckListHeldClickCheck(Sender: TObject);
...
{ EVERY ROW, NOT THE SELECTED ONE. The fixed list above needs a deferred tick
  because it acts on ItemIndex, which can arrive after the tick does; this list
  carries each row's state in its own box, so reading them all has no order. }
procedure TUserPointsSetPropDlg.CheckListHeldClickCheck(Sender: TObject);
var
    i: longint;
begin
    for i := 0 to CheckListHeld.Items.Count - 1 do
        HoldParameter(TSpecialCurveParameter(CheckListHeld.Items.Objects[i]),
            CheckListHeld.Checked[i]);
end;
...
procedure TUserPointsSetPropDlg.FillCheckListHeld;
var
    i, Index: longint;
    Choices: TParameterChoices;
begin
    Choices := HeldChoices(FCurveType.Parameters);
    CheckListHeld.Items.Clear;
    for i := 0 to High(Choices) do
    begin
        Index := CheckListHeld.Items.AddObject(Choices[i].Name,
            Choices[i].Parameter);
        CheckListHeld.Checked[Index] := Choices[i].Marked;
    end;
end;
