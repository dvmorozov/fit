...
uses
    explanation, Classes, curve_points_set, curve_types_singleton, delta_sigma_curve_parameter,
...
class function TAsymPseudoVoigtPointsSet.Explanation: TExplanation;
begin
    Result := NewExplanation('', '',
        'A pseudo-Voigt peak made asymmetric by giving its two sides ' +
        'different widths, sigma + deltasigma on the right and sigma - ' +
        'deltasigma on the left.',
        esModelChoice);
    AddParagraph(Result,
        'Each side is (1 - eta) Gaussian + eta Lorentzian with the width ' +
        'of that side, both scaled to height A at x0. Unlike ' +
        'Pseudo-Voigt, A here is the peak height, not its area.');
    AddParagraph(Result,
        'Splitting the width is an empirical way to follow an asymmetric ' +
        'peak; this software chose that form, it is not taken from a ' +
        'published profile function.');
    AddLimitation(Result,
        'The asymmetry is descriptive only: it does not model a physical ' +
        'cause of asymmetry such as axial divergence.');
    AddLimitation(Result,
        'deltasigma must stay smaller than sigma, or the left side has no ' +
        'width.');
    AddLimitation(Result,
        'A is a height, so its value is not comparable with the ' +
        'area-normalised types.');
end;
