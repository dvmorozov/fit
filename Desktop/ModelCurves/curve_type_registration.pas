...
begin
    ...
    //  EVERY TYPE THAT IS PRESENT EXPLAINS ITSELF, and this is the call every
    //  binary that creates curves already makes - so the provider is registered
    //  on the path the application takes, not on one only a test takes. After
    //  the check: a binary missing a type stops above rather than explaining
    //  the ones it happens to have.
    RegisterCurveTypeExplanations;
end;
