...
uses
    explanation, curve_types_singleton, gauss_points_set, named_points_set, points_set,
    SimpMath, SysUtils;
...
class function TLorentzPointsSet.Explanation: TExplanation;
begin
    Result := NewExplanation('', '',
        'A symmetric peak with heavy tails, whose area is A, centred at ' +
        'x0, with full width at half maximum sigma.',
        esCanonical);
    AddParagraph(Result,
        'The Cauchy (Lorentzian) density scaled by A: A (2 / (pi sigma)) ' +
        '/ (1 + (2 (x - x0) / sigma)^2). A is the area and sigma here is ' +
        'the full width at half maximum, not a standard deviation.');
    AddParagraph(Result,
        'It is the natural line shape of a damped oscillator and of ' +
        'lifetime broadening, and in diffraction the shape small ' +
        'crystallite size tends to give.');
    Result.Quote := 'f(x) = 1 / (pi s (1 + ((x - t) / s)^2))';
    AddLimitation(Result,
        'Its tails decay so slowly that its variance is undefined; the ' +
        'fitted area depends noticeably on how much of the tail the fit ' +
        'interval includes.');
    AddLimitation(Result,
        'Real peaks rarely have pure Lorentzian wings and a Gaussian core ' +
        'together - use Voigt or Pseudo-Voigt for a mixture.');
    AddReference(Result,
        'NIST/SEMATECH e-Handbook of Statistical Methods',
        'section 1.3.6.6.3, Cauchy Distribution',
