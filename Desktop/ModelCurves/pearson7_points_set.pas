...
class function TPearson7PointsSet.Explanation: TExplanation;
begin
    Result := NewExplanation('', '',
        'A symmetric peak of height A and full width at half maximum ' +
        'sigma, whose tail weight m runs from Lorentzian to Gaussian.',
        esCanonical);
    AddParagraph(Result,
        'A / (1 + (2^(1/m) - 1) (2 (x - x0) / sigma)^2)^m. With this ' +
        'scaling sigma is exactly the full width at half maximum for ' +
        'every m.');
    AddParagraph(Result,
        'm = 1 gives a Lorentzian; as m grows the wings shrink and the ' +
        'shape tends to a Gaussian. It is widely used for X-ray ' +
        'diffraction peaks.');
    Result.Quote := 'y = A / (1 + (2^(1/m) - 1) (2 (x - x0) / w)^2)^m';
    AddLimitation(Result,
        'A is the height, not the area.');
    AddLimitation(Result,
        'Very large m is numerically indistinguishable from a Gaussian, ' +
        'so m is poorly determined for nearly Gaussian peaks.');
    AddReference(Result,
        'M. M. Hall, V. G. Veeraraghavan, H. Rubin and P. G. Winchell, ' +
        'The approximation of symmetric X-ray peaks by Pearson type VII ' +
        'distributions, J. Appl. Cryst. 10 (1977)',
        'pp. 66-68',
        '');
end;
