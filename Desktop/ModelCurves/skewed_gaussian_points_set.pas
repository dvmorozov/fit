...
class function TSkewedGaussianPointsSet.Explanation: TExplanation;
begin
    Result := NewExplanation('', '',
        'A Gaussian made asymmetric by the shape parameter beta, ' +
        'following the skew-normal distribution.',
        esCanonical);
    AddParagraph(Result,
        'A Gaussian density of scale sigma at x0, multiplied by (1 + ' +
        'erf(beta (x - x0) / (sigma sqrt 2))) and scaled by A. beta = 0 ' +
        'gives the ordinary Gaussian; positive beta skews the peak to the ' +
        'right.');
    AddParagraph(Result,
        'It adds asymmetry with one parameter while keeping Gaussian ' +
        'tails.');
    Result.Quote := 'f(x) = 2 phi(x) Phi(alpha x)';
    AddLimitation(Result,
        'x0 is the location parameter, not the position of the maximum, ' +
        'once beta is not zero.');
    AddLimitation(Result,
        'The tails stay Gaussian, so it cannot represent a long ' +
        'exponential tail - use EMG for that.');
    AddReference(Result,
        'A. Azzalini, A class of distributions which includes the normal ' +
        'ones, Scand. J. Statist. 12 (1985)',
        'pp. 171-178',
        '');
end;
