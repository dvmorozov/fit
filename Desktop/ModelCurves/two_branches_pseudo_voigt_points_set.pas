...
uses
    explanation, amplitude_curve_parameter, argument_axis, Classes, curve_points_set,
...
class function T2BranchesPseudoVoigtPointsSet.Explanation: TExplanation;
begin
    Result := NewExplanation('', '',
        'A pseudo-Voigt peak with an independent width and ' +
        'Gaussian-Lorentzian mix on each side of its maximum.',
        esModelChoice);
    AddParagraph(Result,
        'Left of x0 it uses sigma and eta, right of x0 it uses sigmaright ' +
        'and etaright; both sides reach the height A at x0, so the curve ' +
        'is continuous there. A is the peak height, not its area.');
    AddParagraph(Result,
        'Giving each branch its own shape follows strongly asymmetric ' +
        'peaks more freely than Asym. Pseudo-Voigt, at the price of two ' +
        'more parameters.');
    AddLimitation(Result,
        'Four shape parameters for one peak correlate strongly; on noisy ' +
        'data the branches can trade width for mix without improving the ' +
        'fit.');
    AddLimitation(Result,
        'The slope is not continuous at x0 when the two branches differ, ' +
        'which a physical profile would not show.');
end;
