...
type
    ...
    TUiCommand = (
        ...
        ucDeleteCurve, ucClearModel,
        //  The results grid
        ucCopy, ucDelete, ucSelectAll,
        //  The chart
        ucZoomIn, ucZoomOut, ucViewMarkers, ucUseRule, ucValueUseRule,
        //  The two panes drawn as HTML - the module report tabs and the
        //  Explain pane - each with its own pair, because each is zoomed by
        //  its own buttons and neither may answer for the other.
        ucReportZoomIn, ucReportZoomOut, ucExplainZoomIn, ucExplainZoomOut
        );
    ...
    TUiInputs = record
        ...
        { Whether each HTML pane has a size left to step to. FOUR FLAGS, NOT
          TWO SIZES: which sizes exist is report_zoom's, and gathering the
          answer rather than the number keeps this unit free of the ladder.

          NOT GATED ON ANYTHING BEING OPEN. The Explain pane says what a
          command means before any data is loaded, and a report tab is only
          shown once its module has reported - so in both cases the pane is on
          screen exactly when its buttons are, and nothing else decides. }
        ReportCanZoomIn: boolean;
        ReportCanZoomOut: boolean;
        ExplainCanZoomIn: boolean;
        ExplainCanZoomOut: boolean;
        { Whether the model holds any curve at all - what clearing it follows.
          Not the panel's selection: clearing is about the whole model, and a
          command greyed until some row is clicked would hide that. }
        ModelHasCurves: boolean;
    end;
...
{ Whether the window may call into a module now.

  NOT WHILE AN OPERATION RUNS. What a module does when the window calls it is
  the module's business and cannot be assumed cheap: a module that answers by
  asking the engine about the model waits for the running fit's session lock,
  and it waits on the WINDOW's thread. The window then stops entirely - no
  repaint, no progress poll, no answer to the user - for the length of the fit.
  It was reported twice as "the application hangs and there is no animation",
  with the live progress view working correctly the whole time and nobody able
  to see it.

  The client's own rule - only polled routes while a fit runs - cannot cover
  this, because the call is the module's rather than the client's. So the window
  does not make it at all until the operation is over.

  THE THREE DOORS. The panel rebuild on idle re-establishes the selected row and
  hands it back (ModelStructureMayBeRebuilt below); the user selecting a row
  hands it back directly; the user right-clicking one asks the module what it
  offers. All three are shut by this, and all three reopen at Done - where the
  panel is rebuilt from the result, which notifies the module with the model it
  actually has. }
function ModuleMayBeCalledBack(AAsync: TAsyncState): boolean;

{ Whether the window may rebuild the Model panel now.

  A rebuild puts the selection back, and that hands the row to whichever module
  owns the panel - so this is the rule above, under the name of the thing that
  would trip it. Nothing is lost by waiting: an operation does not change the
  model, and the frames an animated fit draws are pictures of it rather than the
  thing itself. }
function ModelStructureMayBeRebuilt(AAsync: TAsyncState): boolean;

implementation

function ModuleMayBeCalledBack(AAsync: TAsyncState): boolean;
begin
    Result := AAsync <> AsyncWorks;
end;

function ModelStructureMayBeRebuilt(AAsync: TAsyncState): boolean;
begin
    //  Said through the rule it is an instance of, so the two cannot drift.
    Result := ModuleMayBeCalledBack(AAsync);
end;
...
begin
    ...
    Result.ModelHasCurves := False;
    //  Neither pane can be stepped until the window says how large it is
    //  drawing them - which it does on every refresh.
    Result.ReportCanZoomIn := False;
    Result.ReportCanZoomOut := False;
    Result.ExplainCanZoomIn := False;
    Result.ExplainCanZoomOut := False;
end;
...
begin
    ...
    //  The value's rules on the same terms as the argument's: an axis of an
    //  empty chart is nothing to transform.
    Result[ucValueUseRule].Enabled := AInputs.GraphHasSeries;

    //  ---- the HTML panes. Independent of the document as well: how large
    //  something is drawn is not a thing that can be unavailable, only a thing
    //  that has reached the end of its ladder.
    Result[ucReportZoomIn].Enabled := AInputs.ReportCanZoomIn;
    Result[ucReportZoomOut].Enabled := AInputs.ReportCanZoomOut;
    Result[ucExplainZoomIn].Enabled := AInputs.ExplainCanZoomIn;
    Result[ucExplainZoomOut].Enabled := AInputs.ExplainCanZoomOut;
    ...
    //  Clearing the model needs only a model with something in it. Greyed, not
    //  hidden, for an empty one - for the same reason as above.
    Result[ucClearModel].Enabled := AInputs.ModelHasCurves;
    ...
    begin
        ...
        Result[ucClearModel].Enabled := False;
