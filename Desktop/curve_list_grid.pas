...
A CELL MAY BE TYPED INTO, in a column the fit varies (SetColOptions). It was a
display only - StaticOptions carries no goEditing and the form said AutoEdit =
False - and the model's inverse, TCurveListBase.ApplyRowCellText, had nothing
calling it. Nothing here reads the grid back either: the window sends a typed
value to the engine through TFitClient.EditCurveParameter, and the engine's
rebuilt model refills the table.
...
begin
    ...
    begin
        ...
        //  With goEditing: a value the fit varies can be typed. Which columns
        //  take typing is SetColOptions's, below.
        Options := StaticOptions + [goEditing];
    end;
