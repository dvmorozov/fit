...
type
    ...
    TCurveTypeInfo = record
        ...
        { One sentence saying what the type is - its explanation's summary. }
        Hint: string;
    end;
    ...
    TCurveMenuEntry = record
        ...
        Checked: boolean;
        Tag: longint;
        { What the entry says on hover, carried from the type. }
        Hint: string;
    end;
...
type
    ...
    TCurveListRow = record
        ...
        { The registry's handle, which comes back on a click. Meaningless on a
          header. }
        Tag: longint;
        { The type's hint; empty on a header, which names no curve. }
        Hint: string;
    end;
...
{ The hint the Tools-pane list shows with the pointer over row AIndex: that curve
  type's own summary, so choosing a type starts from knowing what it is. A header
  names no curve and gets no hint; off the rows, and for a type with nothing to
  say, the list's own AFallback. }
function CurveListHintAt(const ARows: TCurveListRows; AIndex: longint;
...
begin
    Result := AFallback;
    if (AIndex < 0) or (AIndex > High(ARows)) then
        Exit;
    if ARows[AIndex].IsHeader or (ARows[AIndex].Hint = '') then
        Exit;
    Result := ARows[AIndex].Hint;
end;
...
var
    ...
    procedure AddRow(const ACaption: string; AIsHeader, ASelected: boolean;
        ATag: longint; const AHint: string);
    begin
            ...
            for j := 0 to High(AEntries) do
                if AEntries[j].Group = Order[i] then
                    AddRow(AEntries[j].Caption, False, AEntries[j].Checked,
                        AEntries[j].Tag, AEntries[j].Hint);
        end;
    finally
        Order.Free;
    end;
...
begin
    ...
    begin
        ...
        //  The factory's too: creating a user curve is as worth explaining as
        //  choosing a built-in one.
        Result[i].Hint := ATypes[i].Hint;
