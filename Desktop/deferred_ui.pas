...
async call, the idle handler, the dialog, and asking whether a menu is open.

THE RULES, each with a failure behind it:

  * AT MOST ONE DIALOG IS OUTSTANDING, and a second request while one is pending
    is DROPPED, not queued. A fault on a path that events drive repeatedly
    would otherwise stack dialogs until nothing else on screen can be reached.
  ...
  * NOTHING RUNS WHILE A MENU IS OPEN. A dialog opened over a dropped-down menu
    ...
    outstanding and is taken up when the application next goes idle - which
    is after the menu's own loop has ended.
...
type
    ...
    TDeferredWork = (dwNothing, dwMenuRebuild, dwStateRefresh, dwDialog);
    ...
    public
        ...
        { Records that the window must re-derive what it shows from the state
          behind it - the commands, the panes, the menus.

          ASKED FOR BY EVENTS, NOT BY A CLOCK. The window used to re-derive
          everything on a timer twice a second, whether anything had changed or
          not; now each thing that CAN change the state - an action, a selection,
          a server operation finishing, a model update - asks.

          False when the caller has nothing to queue: a refresh is already
          outstanding, and one covers every request made before it runs; OR the
          state is being applied right now, and a request raised by applying it
          is not a change - honouring it would refresh forever. }
        function RequestStateRefresh: boolean;
        { Brackets applying the state. Taking the refresh clears the request, so
          anything that changes the state after this is a new request. }
        procedure BeginStateRefresh;
        procedure EndStateRefresh;
...
begin
    ...
    //  Then the state, applied to the menus the rebuild produced, and before a
    //  modal dialog that would hold it back behind it.
    if FRebuildPending then
        Result := dwMenuRebuild
    else if FRefreshPending then
        Result := dwStateRefresh
...
begin
    Result := FRebuildPending or FRefreshPending or FDialogPending;
end;
...
function TDeferredUi.RequestStateRefresh: boolean;
begin
    //  Raised by applying the state itself: not a change.
    if FApplyingState then
        Exit(False);
    Result := not FRefreshPending;
    FRefreshPending := True;
end;

procedure TDeferredUi.BeginStateRefresh;
begin
    FRefreshPending := False;
    FApplyingState := True;
end;

procedure TDeferredUi.EndStateRefresh;
begin
    FApplyingState := False;
end;
