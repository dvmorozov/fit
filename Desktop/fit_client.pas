...
uses
    ...
    , client_log
    //  What a running fit shows, and the report it is drawn from.
    , fit_progress, fit_progress_json, fit_points_json
    //  The rows an exported table is written from.
    , table_text
    //  Which axis the model and the data say each coordinate is on.
    , coordinate_axis, axis_choice
    ;
...
type
    { The two pick sets a table row can move a pick of. }
    TPickKind = (pkCurvePositions, pkFitBounds);

    { Modes of selectiion of active point set. }
    TSelMode    =
        (ModeSelectNothing, ModeSelectIntervalBounds, ModeSelectBackground,
        ModeSelectCurvePositions, ModeSelectRFactorBounds,
        { Picks going into a MODULE's own point set - which one is
          ModulePickSet. Reuses the collect-then-act shape of
          ModeSelectIntervalBounds, so the gesture is one users already know
          from selecting a stretch of the profile.

          THERE WERE TWO MORE, for "characteristic points of a peak" and "curve
          bounds". Both collected picks that nothing ever read - the command
          their prompt sent the user to, Move Peak to Results, never existed -
          so both were removed with their menu entries.
    ...
    protected
        ...
        { What the profile's coordinates are, as the loader said or as the
          project remembered it: axis mode ids, '' where nothing said. Asked by
          the automatic axis rule (axis_choice), second after the model. }
        FDataCoordinateModes: array[TAxisDimension] of string;
        ...
        { Which change of the profile, reported by the running operation, is
          the one drawn. }
        FProfileRevisionShown: longint;
    ...
    protected
        ...
        { HOW THE RUNNING FIT IS GOING, as the last poll left it. Made on first
          use rather than in the constructor: a client built through the
          inherited one has none, and every test fixture builds it that way. }
        FProgress: TFitProgressModel;
        { A poll is drawing its answer. The window's readouts process messages,
          so its timer can fire inside one. }
        FPollingProgress: boolean;
        { How many frames OF THE MODEL have been drawn since this operation
          started - the curves moving, which is what Animation Mode means.

          Counted because the window reports on itself under /CHECK_UI, and this
          is the one number no other object holds: the viewer is handed curves
          by several paths and cannot tell a frame from a final refresh. }
        FModelFramesDrawn: longint;
        { How many progress views have been handed to the window since this
          operation started. }
        FProgressViewsDrawn: longint;
        { How often the window's timer reached this client during the operation,
          how many of those asked the engine, and how many of those failed. The
          three separate a timer that never ran from a poll that refused itself
          from a connection that would not answer - which is the whole question
          when a fit goes by without the user seeing any of it. }
        FProgressTicks, FProgressPolls, FProgressFailures: longint;
        function Progress: TFitProgressModel;
        { Starts a fit: the chart shows at once that it runs, then the work is
          handed to the thread. See PollProgress for what happens meanwhile. }
        procedure StartFit(AOp: TServerOp);
        { Starts a computation that is not a fit - bounds, background, positions:
          timed like a fit, with no progress view over the data it works on. }
        procedure StartComputation(AOp: TServerOp; ADone: TThreadMethod);
        { Tells the engine, BEFORE the fit starts, that its model is being
          watched. See the body for why waiting for the first poll is too late. }
        procedure AskForTheFirstFrame;
        { The operation is over: its clock stops, and its view, if it had one,
          goes. Every completion handler starts with this. }
        procedure EndProgress;
        procedure ShowFitProgress;
        { Draws the model as a snapshot has it, for Animation Mode. }
        procedure ShowProgressSnapshot(const ASnapshot: TFitProgressSnapshot);
        { The computed series and the curves, each replaced by a set already in
          hand. Both a refresh from the server and an animated frame draw
          through these, so a frame is drawn by the rules a result is. }
        procedure ReplaceComputedProfile(AProfile: TTitlePointsSet);
        procedure ReplaceDeltaProfile(AProfile: TTitlePointsSet);
        procedure ReplaceCurves(ACurves: TSelfCopiedCompList);
...
{$ENDIF}
    ...
    public
        ...
        { What the chart area shows while a fit runs. A contract of its own
          rather than part of IFitViewer - see int_fit_viewer - and nil is an
          ordinary answer: a client with no progress view still fits. }
        FProgressView: IFitProgressView;
        ...
        { Removes every curve from the model, and answers how many went.

          THE ONE-CURVE VERB, SPENT ON EVERY HANDLE, rather than a second
          removal mechanism: what has to go with a curve - its pick, its
          identity, a module's markup - is decided by TFitService.DeleteCurve,
          and a bulk path would have to agree with it forever.

          HANDLES FIRST, THEN SPENT. Each removal renumbers what is left, and a
          module may take other instances with the one asked for, so a handle
          that has already gone is skipped rather than refused. The model is
          re-read ONCE at the end.

          GRACEFUL, NOT ALL-OR-NOTHING. A curve the server declines to remove
          is stepped over and the rest still go; the first refusal's words come
          back in AReason for the caller to report once. Raising on it stopped
          the clearing half-way, leaving curves that could have gone. Answers
          how many removals the server ACCEPTED - whether the model is empty
          afterwards is the engine's to say, and the caller asks it. }
        function ClearModel(out AReason: string): longint;
        ...
        { The summary table as File > Export > Summary Table writes it, built
          from this client's model with every digit; empty with no table. }
        function SummaryExportRows: TTextRows;
        ...
        { The three Remove All commands. The Remove methods above only drop
          this client's copy, which is what a refresh needs before it reads the
          engine's; these empty the set ON THE ENGINE, where model input lives,
          and then read back. Dropping only the copy is what Remove All used to
          do, and the next refresh drew every pick again. }
        procedure RemoveAllCurvePositions;
        procedure RemoveAllRFactorBounds;
        procedure RemoveAllBackgroundPoints;
        ...
        procedure ReplacePointInRFactorBounds(
            PrevXValue, PrevYValue, NewXValue, NewYValue: double);
        { Moves pick AIndex of AKind to the position typed as ATyped - the
          Curve Positions and Fit Intervals tables call this when a cell is
          typed over. The rule - nearest sample, what is refused and why - is
          points_tables.PlanPickMove. True when it moved; False otherwise, with
          AWhy saying why when the text was refused and empty when there was
          nothing to do. }
        function MovePick(AKind: TPickKind; AIndex: longint;
            const ATyped: string; out AWhy: string): boolean;
        { A value typed into row ARow, column ACol of the Curve Attributes
          table: sent to the engine for that parameter of that curve, and the
          model re-read. What is sent, and what is refused, is
          TCurveListBase.PlanParameterEdit. True when it was sent. }
        function EditCurveParameter(ARow, ACol: longint; const ATyped: string;
        ...
        { What the profile's ADimension is, as an axis mode id, or ''. Set by
          loading a file, and by restoring a project, which reads no file. }
        function GetDataCoordinateMode(ADimension: TAxisDimension): string;
        procedure SetDataCoordinateMode(ADimension: TAxisDimension;
            const AModeId: string);
        { The date of each profile point, when the argument counts bars of a
          dated series; empty otherwise. Set by loading a file and by restoring
          a project, like the coordinate modes beside it. }
        function GetArgumentDates: TAxisDates;
        procedure SetArgumentDates(const ADates: TAxisDates);
        { What the automatic axis rule asks about ADimension: what the model's
          curves prefer, what the data says, and what the curve type selected
          in the Tools list prefers. }
        function AxisPreferences(ADimension: TAxisDimension): TAxisPreferences;
        ...
        { Re-reads everything from the engine into this client's own copies.
          ...
          WHAT RE-READING A DATA FILE CANNOT DO. LoadDataSet reads a data file,
          and restoring a project reads none - its profile is in the project.
        ...
        { Draws AProfile as the profile - the one a running operation changed,
          as its progress report carries it. }
        procedure ShowProfileOf(const AProfile: TPointsData);
        procedure Done;
        { Asks the server how the running fit is going and shows it. The window
          calls it on a timer; outside a fit it asks nothing.

          ONLY A POLLED ROUTE IS ASKED WHILE A FIT RUNS. The fit holds its
          problem's lock, and any other read would park the window's thread
          until the fit was over - which is the freeze this exists to end. So a
          frame is drawn from the progress reply alone, never from a second
          request, and a module overlay waits for the finished model. }
        ...
        property ModelFramesDrawn: longint read FModelFramesDrawn;
...
function TFitClient.SummaryExportRows: TTextRows;
begin
    Result := summary_table.SummaryExportRows(FExperimentalProfile, FCurves,
...
function TFitClient.ClearModel(out AReason: string): longint;
var
    Handles: array of string;
    i, Index: longint;
begin
    Result := 0;
    AReason := '';
    ...
    SetLength(Handles, FFitService.GetCurveCount);
    for i := 0 to High(Handles) do
        Handles[i] := FFitService.GetCurveInstanceId(i);
    ...
    try
        for i := 0 to High(Handles) do
        begin
            Index := FFitService.IndexOfCurveInstance(Handles[i]);
            if Index < 0 then
                Continue;
            try
                FFitService.DeleteCurve(Index);
            ...
            //  As DeleteCurve does, and for its reason.
            if Assigned(FFitViewer) then
                FFitViewer.HideSeriesOwnedBy(Handles[i]);
            Inc(Result);
        end;
    finally
        UpdateComputedData(True);
...
procedure TFitClient.RemoveAllCurvePositions;
begin
    CheckThat(Assigned(FitService), 'the fitting engine this client talks to is missing');
    //  The service takes the set it is handed.
    FitService.SetCurvePositions(TTitlePointsSet.Create(nil));
    UpdateComputedData(True);
end;

procedure TFitClient.RemoveAllRFactorBounds;
begin
    CheckThat(Assigned(FitService), 'the fitting engine this client talks to is missing');
    FitService.SetRFactorBounds(TTitlePointsSet.Create(nil));
    UpdateComputedData(True);
end;

procedure TFitClient.RemoveAllBackgroundPoints;
var
    Empty: TTitlePointsSet;
begin
    CheckThat(Assigned(FitService), 'the fitting engine this client talks to is missing');
    //  NOT handed over: over HTTP the background setter copies and leaves the
    //  set with its caller, as SubtractBackground relies on.
    Empty := TTitlePointsSet.Create(nil);
    try
        FitService.SetBackgroundPointsSet(Empty);
    finally
        Empty.Free;
    end;
...
procedure TFitClient.ShowProfileOf(const AProfile: TPointsData);
var
    Points: TTitlePointsSet;
begin
    //  THE SAME AS ShowProfile BELOW, with the points from the report rather
    //  than from a request: a running fit holds the problem, and a request for
    //  the profile would wait for it to finish.
    Points := TitlePointsSetOf(AProfile);
    if FSelectedAreaMode then
    begin
        RemoveSelectedArea;
        FSelectedArea := Points;
        FSelectedArea.WaveLength := FWaveLength;
        FSelectedArea.FTitle := SelectedIntervalName;
        PlotSelectedProfileInterval;
    end
...
{$ENDIF}
    ...
    ReplaceComputedProfile(FitService.GetCalcProfilePointsSet);
    ReplaceDeltaProfile(FitService.GetDeltaProfilePointsSet);
    ...
    ReplaceCurves(FitService.GetCurves);
...
begin
    ...
    //  FIRST, so the result is drawn on the model's own chart rather than under
    //  the progress view - and however the fit ended, since the thread calls
    //  this after a failure too.
    EndProgress;
    ShowProfile;
    UpdateComputedData(True);
...
function TFitClient.Progress: TFitProgressModel;
begin
    if not Assigned(FProgress) then
        FProgress := TFitProgressModel.Create;
    Result := FProgress;
end;

procedure TFitClient.StartFit(AOp: TServerOp);
begin
    //  The previous model is out of date the moment another fit starts...
    Clear;
    //  ...but the data is not an answer, and it is what is being fitted. Drawn
    //  again from the copy already held, so nothing is asked of the server and
    //  the chart area is never blank.
    if FSelectedAreaMode then
        PlotSelectedProfileInterval
    else
        PlotExpProfile;
    Progress.Start;
    //  The operation's profile changes are counted from its start.
    FProfileRevisionShown := 0;
    FModelFramesDrawn := 0;
    FProgressViewsDrawn := 0;
    FProgressTicks := 0;
    FProgressPolls := 0;
    FProgressFailures := 0;
    //  AT ONCE, before any round trip: the user pressed Fit and sees it begin.
    //  FIRST of the three, and that order is the whole promise of this feature:
    //  asking for a frame is itself a request, and a server that is slow to
    //  answer it would delay the screen changing at all.
    ShowFitProgress;
    //  BEFORE THE FIT, not on the first poll: see AskForTheFirstFrame.
    if FAnimationMode then
        AskForTheFirstFrame;
    RunAsync(AOp, Done);
end;

{ ASKED BEFORE THE FIT BEGINS, and this is what makes Animation Mode work at all
  for the fit an ordinary user runs.

  The engine builds a frame only while somebody is asking for one, and it builds
  it when it next improves. Asked on the first POLL instead, the request arrives
  a moment after the fit started - and one curve on one peak has done all its
  improving by then, so the engine had nothing to build a frame from and the
  user watched an unchanged chart until the result appeared. A fit big enough to
  watch hid this completely, which is exactly why it was found by using the
  program rather than by a test.

  One unlocked request, made before the fit holds anything, so it costs the
  window nothing. }
procedure TFitClient.AskForTheFirstFrame;
begin
    try
        //  The reply is of no interest: what matters is that the engine now
        //  knows a watcher is there. Nothing is applied from it - the fit has
        //  not started, so it describes the previous operation.
        FitService.GetFitProgress(0, True);
    except
        //  A dropped ask is a frame or two missed, not a failed fit.
        on E: Exception do
            LogClientWarning('asking for the first animation frame failed: ' +
                E.Message);
    end;
end;

procedure TFitClient.StartComputation(AOp: TServerOp; ADone: TThreadMethod);
begin
...
procedure TFitClient.EndProgress;
var
    Said: string;
begin
    if not (Assigned(FProgress) and FProgress.Active) then
        Exit;
    //  WHAT THE USER JUST WATCHED, said by the program itself. A fit that drew
    //  nothing looks exactly like a fit that was quick, and the only record of
    //  the difference was the user's word for it - twice.
    //  THE WINDOW'S OWN CLOCK, not the engine's: the engine's elapsed arrives
    //  in the reports, so a fit whose polls stopped reports the moment they
    //  stopped - which would call a fit nobody watched for 22 seconds a fit of
    //  a fifth of one, and say nothing.
    Said := FitEpilogueText(FProgress.ShowsView, FAnimationMode,
        FProgress.WatchedSecondsAt(RunTime), FProgressViewsDrawn, FProgress.SampleCount,
        FModelFramesDrawn, FProgressTicks, FProgressPolls, FProgressFailures);
    if Said <> '' then
    begin
        if FitWasInvisible(FProgress.ShowsView, FAnimationMode,
            FProgress.WatchedSecondsAt(RunTime), FProgressViewsDrawn, FProgress.SampleCount,
            FModelFramesDrawn) then
            LogClientWarning(Said)
        else
            LogUiAction(Said);
    end;
    FProgress.Finish;
    if FProgress.ShowsView and Assigned(FProgressView) then
        FProgressView.HideFitProgress;
end;

procedure TFitClient.ShowFitProgress;
begin
    if Progress.ShowsView and Assigned(FProgressView) then
    begin
        FProgressView.ShowFitProgress(Progress.View(FAnimationMode));
        Inc(FProgressViewsDrawn);
    end;
    //  LAST, because the window's readouts process messages - the fit may end
    //  inside them, and Done must then be the last thing drawn.
    if Assigned(FFitViewer) then
    begin
        FFitViewer.ShowTime;
        //  Only a fit has an R-factor to show while it runs.
        if Progress.ShowsView then
            FFitViewer.ShowRFactor;
    end;
end;

procedure TFitClient.PollProgress;
var
    Report: TFitProgressReport;
begin
    Inc(FProgressTicks);
    if FPollingProgress or (FAsyncState <> AsyncWorks) or
        (not Assigned(FProgress)) or (not FProgress.Active) or
        (not Assigned(FitService)) then
        Exit;
    FPollingProgress := True;
    try
        try
            Inc(FProgressPolls);
            Report := FitService.GetFitProgress(FProgress.NextSeq, FAnimationMode);
        except
            //  A DROPPED POLL IS A FRAME NOT DRAWN, not a failed fit: the fit is
            //  still running, and the next tick asks again.
            on E: Exception do
            begin
                Inc(FProgressFailures);
                LogClientWarning('reading the progress of the running fit ' +
                    'failed: ' + E.Message);
                Exit;
            end;
        end;
        FProgress.Apply(Report);
        //  THE PROFILE FIRST, so the frame below is drawn against the profile
        //  it was fitted to: Fit > Automatically takes the background off it
        //  before it fits. Only a new revision is drawn.
        if FAnimationMode and Report.HasProfile and
            (Report.ProfileRevision <> FProfileRevisionShown) then
        begin
            ShowProfileOf(Report.Profile);
            FProfileRevisionShown := Report.ProfileRevision;
        end;
        if FAnimationMode and Report.HasSnapshot then
            ShowProgressSnapshot(Report.Snapshot);
        ShowFitProgress;
    finally
        FPollingProgress := False;
    end;
end;

procedure TFitClient.ShowProgressSnapshot(const ASnapshot: TFitProgressSnapshot);
var
    Curves: TSelfCopiedCompList;
    i: longint;
begin
    //  A FRAME IS NOT A RESULT: the tables and the legend keep the last finished
    //  one until the fit is over, as animation always did.
    if Assigned(FFitViewer) then
    begin
{$IFDEF USE_GRIDS}
        FFitViewer.SetUpdateGrids(False);
{$ENDIF}
{$IFDEF USE_LEGEND}
        FFitViewer.SetUpdateLegends(False);
{$ENDIF}
    end;
    ReplaceComputedProfile(TitlePointsSetOf(ASnapshot.ComputedProfile));
    ReplaceDeltaProfile(TitlePointsSetOf(ASnapshot.DeltaProfile));
    Curves := TSelfCopiedCompList.Create;
    for i := 0 to High(ASnapshot.Curves) do
        Curves.Add(NamedCurveOf(ASnapshot.Curves[i].Id, ASnapshot.Curves[i].Points));
    ReplaceCurves(Curves);
    //  TO THE VIEWER DIRECTLY, not through PlotCurves, which waits for the
    //  parameter list. A frame carries no parameters - reading them is a
    //  request the running fit would hold up - and with the grids frozen the
    //  viewer does not read them.
    if Assigned(FFitViewer) then
        FFitViewer.PlotCurves(Self, FCurves, FCurveAttributes);
    Inc(FModelFramesDrawn);
end;

procedure TFitClient.ReplaceComputedProfile(AProfile: TTitlePointsSet);
begin
    //  WHEN EACH OF THESE IS NAMED AND WHEN IT IS DRAWN is in computed_series,
    //  where the rule can be stated once and tested.
    RemoveComputedProfile;
    FComputedProfile := AProfile;
    if Assigned(FComputedProfile) then
    begin
...
procedure TFitClient.ReplaceDeltaProfile(AProfile: TTitlePointsSet);
begin
    RemoveDeltaProfile;
    FDeltaProfile := AProfile;
    if Assigned(FDeltaProfile) then
    begin
...
procedure TFitClient.ReplaceCurves(ACurves: TSelfCopiedCompList);
begin
    HideCurves;
    FCurves.Free;
    FCurves := ACurves;
...
function TFitClient.GetDataCoordinateMode(ADimension: TAxisDimension): string;
begin
    Result := FDataCoordinateModes[ADimension];
end;

procedure TFitClient.SetDataCoordinateMode(ADimension: TAxisDimension;
    const AModeId: string);
begin
    FDataCoordinateModes[ADimension] := Trim(AModeId);
end;

function TFitClient.GetArgumentDates: TAxisDates;
begin
    Result := FArgumentDates;
end;

procedure TFitClient.SetArgumentDates(const ADates: TAxisDates);
begin
    FArgumentDates := Copy(ADates);
end;

function TFitClient.AxisPreferences(ADimension: TAxisDimension): TAxisPreferences;
var
    Titles: TStringArray;
    i: longint;
begin
    //  THE CURVES THE MODEL HOLDS, by their titles - the one statement of a
    //  curve's type the client receives (model_outline.CurveTypeNameOfTitle).
    Titles := nil;
    if Assigned(FCurves) then
    begin
        SetLength(Titles, FCurves.Count);
        for i := 0 to FCurves.Count - 1 do
            if FCurves.Items[i] is TTitlePointsSet then
                Titles[i] := TTitlePointsSet(FCurves.Items[i]).FTitle
            else
                Titles[i] := '';
    end;
    Result := ModelAxisPreferences(ADimension, Titles,
        FDataCoordinateModes[ADimension],
...
function TFitClient.EditCurveParameter(ARow, ACol: longint;
    const ATyped: string; out AWhy: string): boolean;
var
    ParamIndex: longint;
    Value: double;
begin
    AWhy := '';
    Result := Assigned(FCurveAttributes) and
        FCurveAttributes.PlanParameterEdit(ARow, ACol, ATyped, ParamIndex,
            Value, AWhy);
    if not Result then
        Exit;
    //  ROW IS CURVE: the table lists the engine's curves in its order.
    FitService.SetCurveParameter(ARow, ParamIndex, Value);
    UpdateComputedData(True);
end;

function TFitClient.MovePick(AKind: TPickKind; AIndex: longint;
    const ATyped: string; out AWhy: string): boolean;
var
...
begin
    AWhy := '';
    if AKind = pkCurvePositions then
        Picks := FCurvePositions
    else
        Picks := FRFactorBounds;
    //  THE PROFILE THE PICKS ARE SAMPLES OF: the selected interval when one is
    //  in force, which is what the chart shows and a pick is made on.
    if FSelectedAreaMode and Assigned(FSelectedArea) then
        Result := PlanPickMove(FSelectedArea, Picks, AIndex, ATyped, Move, AWhy)
    else
        Result := PlanPickMove(FExperimentalProfile, Picks, AIndex, ATyped,
            Move, AWhy);
    if not Result then
        Exit;
    if (AKind = pkFitBounds) and
        not BoundMoveKeepsOrder(Picks, AIndex, Move.NewX) then
    begin
        AWhy := 'A bound can move only between the bounds beside it, or the ' +
            'intervals would pair up differently. Remove the interval and ' +
            'pick it again to make a different one.';
        Exit(False);
    end;
    if AKind = pkCurvePositions then
        ReplacePointInCurvePositions(Move.PrevX, Move.PrevY, Move.NewX, Move.NewY)
    else
        ReplacePointInRFactorBounds(Move.PrevX, Move.PrevY, Move.NewX, Move.NewY);
end;
...
begin
    ...
    //  WHAT THE NEW DATA IS, replacing what the old data was: the loader has
    //  just read it, so this is the one moment the answer is known.
    FDataCoordinateModes[adArgument] := FDataLoader.CoordinateMode(adArgument);
    FDataCoordinateModes[adValue] := FDataLoader.CoordinateMode(adValue);
    FArgumentDates := Copy(FDataLoader.ArgumentDates);
end;
...
begin
    ...
    FSelectedAreaMode := False;
    FSelectionMode := ModeSelectNothing;
    //  No data, so nothing says what its coordinates are.
    FDataCoordinateModes[adArgument] := '';
    FDataCoordinateModes[adValue] := '';
    FArgumentDates := nil;
...
begin
    ...
    StartFit(FitService.DoAllAutomatically);
end;
...
begin
    ...
    StartFit(FitService.MinimizeDifference);
end;
...
begin
    ...
    { Curve positions and R-factor bounds are set by AddPointToCurvePositions,
      AddPointToRFactorBounds. }
    StartFit(FitService.MinimizeNumberOfCurves);
end;
...
begin
    ...
    StartComputation(FitService.ComputeCurveBounds, ComputeCurveBoundsDone);
end;
...
begin
    ...
    StartComputation(FitService.ComputeBackgroundPoints, ComputeBackgroundPointsDone);
end;
...
begin
    ...
    StartComputation(FitService.ComputeCurvePositions, ComputeCurvePositionsDone);
end;
...
begin
    ...
    StartComputation(FitService.SelectAllPointsAsCurvePositions,
        ComputeCurvePositionsDone);
end;
...
begin
    //  WHILE A FIT RUNS, FROM ITS PROGRESS. The statistics route waits for the
    //  fit's lock, so asking it from the window's readouts froze the window for
    //  the whole fit - and the answer is in the last poll anyway.
    if Assigned(FProgress) and FProgress.Active then
        Exit(FProgress.ElapsedText);
    CheckThat(Assigned(FitService), 'the fitting engine this client talks to is missing');
...
begin
    //  The same reason, and the same number the server reports afterwards. A
    //  computation that is not a fit has no R-factor of its own to report.
    if Assigned(FProgress) and FProgress.Active and FProgress.ShowsView then
        Exit(FProgress.RFactorText);
    CheckThat(Assigned(FitService), 'the fitting engine this client talks to is missing');
