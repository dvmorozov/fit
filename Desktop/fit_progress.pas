...
uses
    SysUtils, Math, fit_progress_json;
...
type
    ...
    public
        ...
        procedure Start(AShowsView: boolean = True; ANow: TDateTime = 0);
        { How long the window has been watching, by its own clock. }
        function WatchedSecondsAt(ANow: TDateTime): double;
...
procedure TFitProgressModel.Start(AShowsView: boolean; ANow: TDateTime);
begin
    FActive := True;
    ...
    FLossName := '';
    FEngineName := '';
    //  Zero means "take the clock off the wall": the production caller has no
    //  reason to name a moment, and a test has every reason to.
    if ANow = 0 then
        FStartedAt := Now
    else
        FStartedAt := ANow;
...
function TFitProgressModel.WatchedSecondsAt(ANow: TDateTime): double;
begin
    Result := (ANow - FStartedAt) * SecsPerDay;
end;
...
begin
    ...
    Result := FloatToStrF(LatestValue, ffFixed, 10, 8);
end;
...
begin
    ...
    Result.XAxisLabel := 'Elapsed time, s';
