...
begin
    ...
    try
        ...
        //  COMPOSED IN MEMORY, THEN PUT IN PLACE WHOLE. This comment used to say
        //  the file was not written over in place while the code opened it with
        //  fmCreate - truncating it - and wrote into it, so a save that failed
        //  half way left neither the old project nor the new one. safe_replace
        //  writes beside the file and swaps a complete one in.
        S := TMemoryStream.Create;
        try
            if not WriteProjectToStream(Doc, S) then
            begin
                AFault := 'The project could not be written to "' + APath + '".';
                Exit;
            end;
            S.Position := 0;
            Result := ReplaceFileWith(APath, S, Fault);
            if not Result then
                AFault := 'The project could not be saved to "' + APath + '": ' +
                    Fault + '. The file there is as it was.';
        finally
            S.Free;
        end;
    except
