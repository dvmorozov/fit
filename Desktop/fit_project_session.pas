...
{ What the project would hold if it were saved now, reduced to one string that
  changes when - and only when - that content does: the working context of the
  window (axis, tab, selection) and the time stamps are left out. What the
...
uses
    md5, fit_project_archive, fit_project_json, special_curve_parameter;
...
begin
    try
        ...
        for i := 0 to High(Parts) do
            Text := Text + Parts[i].Name + #0 + Parts[i].Content + #0;
