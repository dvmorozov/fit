...
{$IFNDEF SERVER}
    fit_client, int_fit_viewer, fit_progress,
{$ENDIF}
    Forms, mscr_specimen_list, coordinate_axis, chart_axes, TAGraph, module_view_types,
    module_report_types;
...
//  Both coordinates are drawn through the window's TChartAxes (chart_axes),
//  which is the one place the modes in force are resolved.
...
{$ENDIF}
        { The axes both coordinates are drawn on. The window's, not owned here:
          the chart's titles, the readout and the menus are read off the same
          object, so what is drawn and what it is called cannot disagree. }
        FAxes: TChartAxes;
        ...
        { Tells the window that what it shows may have changed. }
        procedure StateChanged;

        procedure SetAxes(AAxes: TChartAxes);
        { A stored argument (Point2T) as the argument axis in force shows it. }
        function DisplayX(ARawValue: double): double;
        { A stored value as the value axis in force shows it - NaN where it has
          no place there, which the chart leaves as a gap. ABand says the series
          is a band spanning the chart (a fit interval), whose y is no value of
          the data at all and is never transformed. }
        function DisplayY(ARawValue: double; ABand: boolean): double;
    ...
    private
        ...
        { The curve whose series stand out, or empty. Kept here rather than
          on the series, because the series are freed and re-created on every
          recompute - see HighlightSeriesOwnedBy. }
        FHighlightedCurveId: string;
        { Draws one series as SeriesEmphasis says it stands out. The one place
          that is applied, for the series already drawn and each one added. }
        procedure ApplyEmphasis(Serie: TTASerie; const AOwnerCurveId: string);
...
{$ENDIF}
    public
        ...
        { Draws every series drawn for one curve so that it stands out - the
          curve selected in the Model panel, framework's rows and a
          contributor's alike, since both name the curve by its handle. Empty
          highlights nothing.

          NOT ON IFitViewer: the window is its only caller and holds this
          class, and the presenter never selects a row - an interface member
          would have been two test doubles' worth of code nothing calls.

          A STATE, NOT A ONE-OFF: the chart replots its curves on every
          recompute and contributors replot their own series, so the view
          keeps the handle and applies it to each series as it is added. How
          far a series stands out is series_style's SeriesEmphasis. }
        procedure HighlightSeriesOwnedBy(const ACurveId: string);
...
{$IFNDEF SERVER}
        ...
        { Methods of IFitProgressView interface: the window draws them. }
...
{$ENDIF}
        ...
        { The axes to draw on; setting them redraws. Nil draws the stored
          coordinates as they are. }
        property Axes: TChartAxes read FAxes write SetAxes;
        property Form: TForm read FForm write FForm;
        { RAISED BY EVERY CALLBACK THAT CHANGES WHAT IS SHOWN. The client reports
          every model change through this viewer, so this is the event the window
          re-derives its commands and panes from - instead of asking on a timer
          whether anything changed. }
        property OnStateChanged: TNotifyEvent read FOnStateChanged write FOnStateChanged;
        { What each series on the chart was drawn for. Read by the window's
          self-check, which measures the series it names (ui_selfcheck). }
        property Series: TSeriesRegister read FSeries;
    end;
...
begin
    ...
    with PointsSet do
        for i := 0 to PointsCount - 1 do
            Serie.AddXY(DisplayX(Point2T[i]),
                DisplayY(PointIntensity[i], Serie.IsBackgroundBand),
                Serie.SeriesColor);
end;
...
{$ENDIF}
    ...
    //  AND DRAWN AS THE SELECTION SAYS, here for the same reason: every
    //  series a recompute or a contributor re-creates passes this way, so a
    //  highlight cannot be lost by a replot.
    ApplyEmphasis(Serie, AOwnerCurveId);
...
begin
    ...
    with PointsSet do
        for j := 0 to PointsCount - 1 do
            Serie.SetYValue(j, DisplayY(PointIntensity[j], Serie.IsBackgroundBand));
end;
...
begin
    ...
    //  A caption belongs to a point BY POSITION, so sorting under captions
    //  would put each one on a different point.
    CheckThat(not (AStyle.Sorted and Assigned(ALabels) and (ALabels.Count > 0)),
        'a series with a caption per point is not sorted');
    if AStyle.Sorted then
        APoints.Sort;
    PlotPointsSet(APoints);
    //  Every time, not only when the series is new: a replot may bring
    //  different captions, and nil takes them away.
    TTASerie(TFormMain(Form).Chart.GetSerie(FCurves.IndexOf(APoints)))
        .SetCaptions(ALabels);
end;
...
procedure TFitViewer.ApplyEmphasis(Serie: TTASerie;
    const AOwnerCurveId: string);
var
    Emphasis: TSeriesEmphasis;
begin
    Emphasis := SeriesEmphasis(AOwnerCurveId, FHighlightedCurveId);
    Serie.LineWidth := Emphasis.LineWidth;
    Serie.DrawOnTop := Emphasis.OnTop;
end;

{ THE SELECTED CURVE, TOLD APART ON A CROWDED CHART. By the register, so every
  series the curve owns stands out - its line and whatever a contributor drew
  for it - and nothing that belongs to the model as a whole does. }
procedure TFitViewer.HighlightSeriesOwnedBy(const ACurveId: string);
var
    i: longint;
begin
    CheckAssigned(Form, 'the main window this viewer draws into');
    FHighlightedCurveId := ACurveId;
    for i := 0 to FSeries.Count - 1 do
        ApplyEmphasis(TTASerie(FSeries.Item(i).Serie),
            FSeries.Item(i).OwnerCurveId);
    TFormMain(Form).Chart.Invalidate;
{$IFDEF USE_LEGEND}
    //  Its rows read bold for the highlighted series, so it is repainted too.
    TFormMain(Form).CheckListBoxLegend.Invalidate;
{$ENDIF}
end;
...
procedure TFitViewer.ShowModuleReport(const AReport: TModuleReport);
begin
    //  NEVER FORWARD. This is the refresh a module runs after every change; the
    //  user asking to see the report goes through IUiHost instead.
    TFormMain(Form).ShowModuleReport(AReport, False);
end;
...
procedure TFitViewer.ShowFitProgress(const AView: TFitProgressView);
begin
    TFormMain(Form).ShowFitProgress(AView);
end;

procedure TFitViewer.HideFitProgress;
begin
    TFormMain(Form).HideFitProgress;
end;
{$ENDIF}

procedure TFitViewer.StateChanged;
begin
...
procedure TFitViewer.SetAxes(AAxes: TChartAxes);
begin
    FAxes := AAxes;
...
function TFitViewer.DisplayX(ARawValue: double): double;
begin
    //  Single-sourced in TChartAxes, so the plotted transform, the axis
    //  caption and the parameters grid can never drift apart.
    if Assigned(FAxes) then
        Result := FAxes.Axis(adArgument).ToDisplay(ARawValue)
    else
        Result := ARawValue;
end;

function TFitViewer.DisplayY(ARawValue: double; ABand: boolean): double;
begin
    //  A BAND'S y IS NOT A VALUE. A fit interval is drawn as a stripe the
    //  height of the chart, whatever y its points hold; on a logarithmic axis
    //  those would have no logarithm, and the stripe would vanish.
    if ABand or not Assigned(FAxes) then
        Result := ARawValue
    else
        Result := FAxes.Axis(adValue).ToDisplay(ARawValue);
end;
...
begin
    ...
    begin
        ColCount  := 2;
        //  No spare row: a bound is moved by typing over its cell, not added.
        RowCount  := 1;
        FixedCols := 0;
        FixedRows := 1;
...
begin
    ...
    //  A NEW LIST ON EVERY REFRESH, from the client, which knows nothing of the
    //  axes - so it is told the argument axis in force here, or its positions
    //  would be shown as stored until the next change of axis.
    if Assigned(FAxes) then
    begin
        CurveList.FArgumentMode := FAxes.ResolvedModeId(adArgument);
        CurveList.FArgumentDefinition := FAxes.Choice(adArgument).Definition;
    end;
