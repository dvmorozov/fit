...
uses
    ...
    int_fit_service, fit_statistics, fit_progress_json, mscr_specimen_list,
    named_points_set,
...
const
    ...
    { A running fit's progress is asked from the window's thread ten times a
      second; one that does not come back quickly is a frame skipped. }
...
type
    ...
    private
        FBaseUrl:   string;
        FProblemId: longint;
        { THE CONNECTION THE READS SHARE. A client per request meant a connect
          per poll of a running fit, and a connect to a server busy with that
          fit blocks: the window's own thread sat in connect() while the engine
          worked, and the fit it was drawing went undrawn. Kept open and reused;
          a failed request drops it and the next one opens another. }
        FKept: TFPHTTPClient;
        FKeptLock: TCriticalSection;
        FConnectionsOpened: longint;
    ...
    public
        ...
        { How many connections this service has opened to the server. The reads
          of one fit share one: see Fetch. }
        property ConnectionsOpened: longint read FConnectionsOpened;
...
uses
    ...
    //  How a point set and a curve are rebuilt from the wire - shared with the
    //  client's animated frames, so both name a curve alike.
    wire_point_sets;

constructor THttpFitService.Create(const ABaseUrl: string);
begin
    inherited Create;
    FKeptLock := TCriticalSection.Create;
    FBaseUrl := ABaseUrl;
...
function THttpFitService.Fetch(const AUrl: string; ATimeoutMs: integer): string;
    ...
    begin
        ...
        begin
            FKept := NewClient(ATimeoutMs);
            //  HTTP/1.1 with the connection kept: one connect for every read
            //  that follows, which is what keeps a poll off the accept queue
            //  of a server busy with the fit.
            FKept.KeepConnection := True;
        end;
        FKept.IOTimeout := ATimeoutMs;
        Result := FKept.Get(AUrl);
    end;

begin
    Result := '';
    //  The reads come from the window's thread, the writes from the thread a
    //  long operation runs on; only the kept connection is shared.
    FKeptLock.Acquire;
    try
        try
            Result := Attempt;
            ...
            on E: Exception do
            begin
                //  A DROPPED CONNECTION IS NOT A FAILURE YET: the server closes
                //  an idle one, and the next read opens another. Only a second
                //  failure is reported.
                FreeAndNil(FKept);
                try
                    Result := Attempt;
                except
                    on E2: EUserException do
                        raise;
                    on E2: Exception do
                    begin
                        FreeAndNil(FKept);
...
{ A module's document, or the server's refusal raised as the user's to read.

  The body of a module resource IS the resource - the framework does not own that
  vocabulary and must not parse it - which is why this channel never went through
  CheckedResponse. But a REFUSAL is not the module's document: it is the same
  ok:false envelope every other route answers with, carrying the error the
  server wrote for the user - and dropping it left the caller to explain a reply
  it could not read: a detection request answered "returned a reply this version
  cannot read", for a server that had just explained, in a sentence naming the
  fix, that its Python component was not started.

  ONLY ok:false is claimed. A document that says nothing about "ok" is the
  module's and is passed through untouched. }
function ModuleReply(const ABody: string): string;
var
    D: TJSONData;
    Obj: TJSONObject;
begin
    ...
    try
        ...
        //  Not readable as JSON at all: the module's business, not this
        //  function's. It answers for its own document.
        D := nil;
    end;
    if not (D is TJSONObject) then
    begin
    ...
    Obj := TJSONObject(D);
    try
        if (Obj.IndexOfName('ok') >= 0) and not Obj.Get('ok', True) then
        begin
            LogClientWarning('the compute server rejected a module request: ' +
                Obj.Get('error', '(no message)'));
            raise EUserException.Create(Obj.Get('error',
                'The compute server rejected the request.'));
        end;
    finally
        Obj.Free;
    end;
...
begin
    ...
    //  CheckedResponse; only a refusal is read out of it.
    Result := ModuleReply(Fetch(Url('/modules/' + AResource), Timeout));
end;
...
begin
    ...
    if FindModuleResource(AResource, Info) and Info.LongRunning then
        ...
        //  is most useful for.
        Timeout := LONG_OPERATION_TIMEOUT_MS;
    Result := ModuleReply(Send('POST', Url('/modules/' + AResource),
        APayload, Timeout));
end;
...
{ How the running fit is going. Asked ten times a second while it runs, on the
  window's own thread, of a route the server answers without waiting for the fit.

  A REPLY THAT CANNOT BE READ IS REFUSED, not read as an empty report: an empty
  report says "not busy", and a client that believed it would take the progress
  down while the fit went on. A short timeout, because this is asked from the
  window's thread and a wedged server must cost a skipped frame, not a frozen
  window. }
function THttpFitService.GetFitProgress(ASince: longint;
    AWithSnapshot: boolean): TFitProgressReport;
begin
    if not FitProgressFromJsonString(Fetch(Url(Format(
        '/progress?since=%d&snapshot=%d', [ASince, Ord(AWithSnapshot)])),
        PROGRESS_TIMEOUT_MS), Result) then
        raise Exception.Create('The compute server sent a progress report ' +
...
begin
    ...
    try
        ...
        begin
            ...
            Container.Parameter.Type_ := TParameterType(O.Get('type', 0));
            //  Held at its value; absent from an older server, meaning varied.
            Container.Parameter.VariationDisabled := O.Get('held', False);
        end;
    finally
        R.Free;
    end;
