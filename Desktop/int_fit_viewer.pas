...
uses
    ...
    title_points_set, module_view_types, module_report_types, fit_progress;
...
type
    ...
    IFitViewer = interface
        ...
        { Shows a module's report in its report tab, never bringing the tab
          forward: this is the refresh after every change, and see
          IUiHost.ShowModuleReport for why a refresh must not take focus. }
...
{$ENDIF}
    ...
    { What the chart area shows while a fit runs.

      ITS OWN INTERFACE, NOT TWO MORE MEMBERS OF IFitViewer, and that is the
      point of it. IFitViewer is implemented outside this repository - a module's
      presenter tests carry their own double of it - so every member added there
      is an edit to every module's tests, for a feature none of them draws. A
      separate contract costs a module nothing: the framework's viewer implements
      both, and the client holds this one in a field of its own. }
    IFitProgressView = interface
        { Shows how a running fit is going, instead of the empty chart a fit used
          to leave behind it. Called at once when a fit starts and again on every
          poll; AView says everything to draw, and fit_progress decides all of
          it, so the view only puts it on screen. }
        procedure ShowFitProgress(const AView: TFitProgressView);
        { The fit is over: the chart area goes back to the model. }
