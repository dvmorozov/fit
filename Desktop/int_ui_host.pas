...
type
    ...
    IUiModule = interface
        ...
        { WHICH SHARED PANEL this module's structure rows go under - its own
          ...
          THE WINDOW COMPARES IT with the selected type's PlacedByPointSet: rows
          pushed under it are shown while one of this module's types is
          selected, and held until one is otherwise
          (model_outline.ModulePanelRowsShown). An id of the module's own
          choosing would never match, and its rows would never be shown. }
