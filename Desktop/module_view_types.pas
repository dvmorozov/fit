...
type
    ...
    TOutlineRow = record
        ...
        { The handle the MODEL addresses this row's curve by, or empty when the
          ...
          ONE HANDLE, AND THAT IS A KNOWN LIMIT. Selecting the row highlights
          this curve's series on the chart, and commands on one curve act on it.
          A parent row whose children are curves of their own does not bring the
          children with it. How to lift that - derived from the rows nested
          under this one, or stated by the contributor - is recorded in
          docs/internal/roadmap.md, section 3a. }
        CurveId:    string;
        ...
        IsDetached: boolean;
        { The explanation this row stands for, or '' when there is none. The
          Explain pane shows it when the row is selected - on every platform,
          which is what makes a row the place to explain everything a context
          menu over it offers, where a pointer resting on a menu entry is not
          reported (Cocoa). }
        Topic:      string;
    end;
