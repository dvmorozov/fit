...
{ Which node the panel selects after it is filled: the one carrying
  AWasSelected when it is still there, otherwise the first node that stands for
  something, or -1 when none does.

  WHY NOT NOTHING. A panel with nothing selected leaves the Explain pane showing
  whatever curve type happens to be chosen in the Tools list - which need not be
  in the model at all - and leaves every command on one curve disabled. The
  first element of the model is what a user opening the panel is looking at. }
function InitialOutlineIndex(const ANodes: TOutlineNodes;
    const AWasSelected: string): longint;
...
var
    i: longint;
begin
    //  The user's choice first, while it still names a row.
    Result := IndexOfOutlineId(ANodes, AWasSelected);
    if Result <> -1 then
        Exit;
    //  Otherwise the first row that stands for something - never a heading.
