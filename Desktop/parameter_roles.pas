...
{ The "held" check-list: the ordinary parameters - shared or varied, not the
  position, whose hold the list above already gives - marked where the fit is
  kept from varying the parameter at all. }
function HeldChoices(AParams: Curve_parameters): TParameterChoices;

{ Holds AParameter at its value, or lets the fit vary it again: the engine's own
  flag, which both engines honour. Its kind is left as it is. }
procedure HoldParameter(AParameter: TSpecialCurveParameter; AHeld: boolean);
...
begin
    ...
    begin
        P := AParams[i];
        if Assigned(P) and ((P.Type_ = Shared) or (P.Type_ = Variable)) then
            AddChoice(Result, P.Name, P, P.VariationDisabled);
    end;
end;

procedure HoldParameter(AParameter: TSpecialCurveParameter; AHeld: boolean);
begin
    AParameter.VariationDisabled := AHeld;
end;
