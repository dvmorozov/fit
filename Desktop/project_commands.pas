...
{ Why Reload Profile has nothing to read: the document names no data file. }
function NoSourceToReloadNotice: string;
{ Why Reload Profile did not read APath: it is not there any more. }
function SourceGoneNotice(const APath: string): string;
...
function NoSourceToReloadNotice: string;
begin
    Result := 'This project does not say which data file its profile came ' +
        'from, so there is nothing to reload. Use File > Import Profile to ' +
        'read a file.';
end;

function SourceGoneNotice(const APath: string): string;
begin
    //  THE WHOLE PATH, unlike the questions above: the user has to find the
    //  file, and the folder is the half of the answer they are missing.
    Result := 'The data file this project came from is no longer at' +
        LineEnding + APath + LineEnding + 'Nothing was reloaded. Use File > ' +
        'Import Profile to read it from where it is now.';
end;
