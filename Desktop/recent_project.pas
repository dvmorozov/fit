...
type
    ...
    { One line of File > Open Recent, as the menu should show it. }
    TRecentMenuLine = record
        { What the line reads. THE WHOLE PATH for a project, because two of them
          in different folders are very often called the same thing - written
          as a caption shows it (app_version.LiteralCaption). }
        Caption: string;
        { The project the line opens, as the file system names it. Not the
          caption: that has its '&'s doubled so a menu shows them. }
        Path: string;
        { Whether clicking it does anything. False only for the placeholder
          below, which exists to be read rather than used. }
        Enabled: boolean;
        { Whether the caption is a path to open. False for the placeholder,
          which the window must not hand to the project flow. }
        OpensAProject: boolean;
    end;

    TRecentMenuLines = array of TRecentMenuLine;
...
{ What the application should remember as the project to reopen, given what it
  remembered before and the document it is showing now.

  AN UNSAVED DOCUMENT IS NOT A PROJECT TO REOPEN. Every new project - and every
  import from a data source, which starts one - shows a document with no path,
  and writing that empty path over what was remembered loses a convenience for
  nothing: the next start then opens nothing at all. That is what it did, and
  fetching a series once was enough to stop the last project reopening.

  A path that IS there replaces what came before, which is the ordinary case:
  the project you just opened or saved is the one you want back. }
function RememberedAfterShowing(const ARemembered, APath: string): string;

{ The projects a stored list names, most recent first. }
function RecentProjects(const AStored: string): TStringDynArray;

{ What File > Open Recent should show for a stored list. NEVER EMPTY.

  A MENU THAT DISAPPEARS READS AS A BUILD WITHOUT THE FEATURE, so an empty list
  is one disabled line saying so rather than no lines at all - the user learns
  that the list is empty instead of wondering where the command went.

  The line that says so is marked as opening nothing, because the window
  otherwise has only the caption to go on: the captions ARE the paths, which is
  what makes clicking one need no lookup, and a placeholder handed to the project
  flow would be opened as a file called "(none)". }
function RecentMenu(const AStored: string): TRecentMenuLines;
...
begin
    ...
    begin
        SetLength(Result, 1);
        Result[0].Caption := '(none)';
        Result[0].Enabled := False;
        Result[0].OpensAProject := False;
        Exit;
    end;
    SetLength(Result, Length(Names));
    for i := 0 to High(Names) do
    begin
        Result[i].Caption := LiteralCaption(Names[i]);
        Result[i].Path := Names[i];
        Result[i].Enabled := True;
        Result[i].OpensAProject := True;
    end;
end;
