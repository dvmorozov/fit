...
blockquote, a - plus font, and drawn by the same view, so a report and the rule
it links to look like one application and a later change of HTML component has
one thing to support.

COLOUR IS ADDED TO THE WORD, NEVER INSTEAD OF IT. A page of thirty bullets that
all open with a bold word is a page whose two broken rules are hidden in it, so a
failure and a missed guideline are drawn in a colour of their own. The word is
still there, still bold, still first: the font tag wraps it. That is what keeps
the note below this one true - the report reads the same to someone who sees no
colour at all - and it is why a pass and an info are left uncoloured, since a
page where everything is coloured picks nothing out.
...
const
    ...
    { A broken rule, and a guideline missed. Dark enough to read on the panel's
      own background; the word beside them is what carries the meaning. }
    FailColour = '#B00020';
    WarningColour = '#8A5A00';
...
{ The colour AStatus is drawn in, or '' to leave it in the body text's colour. }
function StatusColour(AStatus: TReportStatus): string;
...
function StatusColour(AStatus: TReportStatus): string;
begin
    //  An 'else' rather than a branch per status: a status added later is drawn
    //  in the body colour, which is wrong in no way a reader can be misled by.
    case AStatus of
        rpFail:    Result := FailColour;
        rpWarning: Result := WarningColour;
        else       Result := '';
    end;
end;

{ AHtml in AColour, or AHtml itself when AColour is ''. AColour is one of the
  constants above and never a module's text, so it is written out as it stands. }
function Coloured(const AHtml, AColour: string): string;
begin
    if AColour = '' then
        Result := AHtml
    else
        Result := '<font color="' + AColour + '">' + AHtml + '</font>';
end;
...
var
    E: TExplanation;
begin
    Result := '<li>' + Coloured('<b>' + ReportStatusCaption(F.Status) + '</b>',
        StatusColour(F.Status)) + ' ' + EscapeHtml(F.Title);
    if F.Detail <> '' then
...
begin
    ...
    S := S + '<p>' + Coloured('<b>Verdict: ' +
        ReportStatusCaption(AReport.Verdict) + '</b>',
        StatusColour(AReport.Verdict)) + ' - ' +
