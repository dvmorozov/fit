...
type
    ...
    { How far one series stands out from the rest. Separate from TSeriesStyle
      because it is not fixed when the series is created: it follows the Model
      panel's selection, and the series already on the chart change with it. }
    TSeriesEmphasis = record
        { In pixels at 96 dpi, as MarkerSize is. It is the width of the pen the
          whole series is drawn with, so a contributor's marker-only series
          owned by the selected curve stands out too, by its outlines. }
        LineWidth: integer;
        { Drawn after every series that is not, so no curve added later can
          cover it. Asked of each series by the chart, never done by reordering
          the chart's list, which the viewer's lookups assume is in add order. }
        OnTop: boolean;
    end;

const
    NormalLineWidth: integer = 1;
    { WIDER, NOT DASHED. The chart draws a curve one segment per pair of
      neighbouring points, and every segment restarts the dash pattern - so on a
      profile sampled densely enough to fit, a dashed curve is drawn solid. Width
      survives any sampling, and the colour, which is how the legend names the
      curve, is kept. }
    HighlightedLineWidth: integer = 3;

{ How far a series stands out, given the curve it was drawn for and the curve
  selected in the Model panel.

  AN EMPTY HANDLE MATCHES NOTHING, on either side. A series of the model as a
  whole - the profile, the residual, the bounds - has no owner, and a row that
  names no curve selects none; equal as strings, they must still not highlight
  the profile. The same rule as TSeriesRegister.OwnedBy. }
function SeriesEmphasis(const AOwnerCurveId,
    AHighlightedCurveId: string): TSeriesEmphasis;

{ Whether a series, as the chart currently draws it, reads as the highlighted
  one: wide AND on top. Both, because a wide curve under a later one is still
  hidden, and one drawn last at the normal width does not stand out.

  THE ONE ANSWER to that question, asked by the legend, which marks the row of
  a highlighted series, and by the window's self-check, which reads it off
  the chart - so the two can never mean different things by "highlighted". }
function SeriesDrawnHighlighted(ALineWidth: integer; AOnTop: boolean): boolean;
...
function SeriesEmphasis(const AOwnerCurveId,
    AHighlightedCurveId: string): TSeriesEmphasis;
begin
    Result.OnTop := (AHighlightedCurveId <> '') and
        (AOwnerCurveId = AHighlightedCurveId);
    if Result.OnTop then
        Result.LineWidth := HighlightedLineWidth
    else
        Result.LineWidth := NormalLineWidth;
end;

function SeriesDrawnHighlighted(ALineWidth: integer; AOnTop: boolean): boolean;
begin
    Result := AOnTop and (ALineWidth > NormalLineWidth);
end;
