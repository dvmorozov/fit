...
{ Where the window looks for its compute server when nothing else is configured,
  given the value of FIT_PORT ('' when it is unset).

  THE LAUNCHER'S RULE, NOT A COPY OF IT. The launchers start the server on the
  port FIT_PORT names; a window with its own fixed default then looked for it at
  8787 and did not find it. Both now read the port through
  launcher_rules.LauncherPort. }
function DefaultServerUrlFor(const APortVariable: string): string;

{ The same, reading FIT_PORT from the environment - what the window calls. }
...
uses
    SysUtils, launcher_rules;

function DefaultServerUrlFor(const APortVariable: string): string;
begin
    Result := ServerBaseUrl(LauncherPort(APortVariable));
end;

function DefaultServerUrl: string;
begin
    Result := DefaultServerUrlFor(GetEnvironmentVariable(LauncherPortVariable));
end;
