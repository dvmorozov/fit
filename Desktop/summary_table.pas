...
type
    ...
    private
        ...
        { The same cells as a file writes them: a number with every digit
          (table_text.ExportedNumber), a heading as it is. '' where FCells has
          the text to use. }
        FExport: array of array of string;
        FColCount: longint;
        FRowCount: longint;
        FIntervals: longint;
        procedure SetSize(ACols, ARows: longint);
        { Writes AValue at [ACol, ARow]: rounded for the screen, exact for a
          file. }
        procedure PutNumber(ACol, ARow: longint; AValue: double);
    public
        constructor Create;
        ...
        { The cell as File > Export > Summary Table writes it: every digit of a
          number, where CellAt rounds to what the screen shows. }
        function ExportCellAt(ACol, ARow: longint): string;
...
{ The whole table as File > Export > Summary Table writes it, built from the
  model: every row, every column, the numbers exact (ExportCellAt). Empty when
  the model gives no table. }
function SummaryExportRows(AProfile: TTitlePointsSet;
    ACurves: TSelfCopiedCompList; AComputed, ADelta,
    ABounds: TTitlePointsSet): TTextRows;
...
begin
    ...
    SetLength(FExport, ACols);
    for i := 0 to ACols - 1 do
    begin
        SetLength(FCells[i], ARows);
        SetLength(FExport[i], ARows);
    end;
end;

procedure TSummaryTable.PutNumber(ACol, ARow: longint; AValue: double);
begin
    FCells[ACol][ARow] := CurveValueText(AValue);
    FExport[ACol][ARow] := ExportedNumber(AValue);
end;

function TSummaryTable.ExportCellAt(ACol, ARow: longint): string;
begin
    Result := CellAt(ACol, ARow);
    if (Result <> '') and (FExport[ACol][ARow] <> '') then
        Result := FExport[ACol][ARow];
end;
...
begin
    ...
    begin
        ...
        begin
            PutNumber(0, RowIndex + j - LeftIndex, AProfile.PointXCoord[j]);
            PutNumber(1, RowIndex + j - LeftIndex, AProfile.PointYCoord[j]);
            PutNumber(2, RowIndex + j - LeftIndex, AComputed.PointYCoord[j]);
            PutNumber(3, RowIndex + j - LeftIndex, ADelta.PointYCoord[j]);
        end;
        ...
        begin
            ...
            begin
                ...
                if (CurveRow >= LeftIndex) and (CurveRow <= RightIndex) then
                    PutNumber(ColIndex, RowIndex + CurveRow - LeftIndex,
                        Curve.PointYCoord[k]);
            end;
            Inc(ColIndex);
        end;
...
begin
    ...
    try
        if Table.Build(AProfile, ACurves, AComputed, ADelta, ABounds) <> soBuilt then
            Exit;
        SetLength(Result, Table.RowCount);
        for Row := 0 to Table.RowCount - 1 do
        begin
            SetLength(Result[Row], Table.ColCount);
            for Col := 0 to Table.ColCount - 1 do
                Result[Row][Col] := Table.ExportCellAt(Col, Row);
        end;
    finally
        Table.Free;
    end;
