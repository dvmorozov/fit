...
const
    ...
    { Removing every curve at once, from the Model menu. }
    CmdClearModel = 'ClearModel';
...
begin
    ...
    //  CLEARING THE WHOLE MODEL is the opposite case: it needs no row, so it is
    //  global, and it sits in the Model menu beside the "Remove All" entries
    //  rather than on the pane. A destructive command one click away among the
    //  everyday Pick/Auto buttons is a mis-click waiting to happen; a menu entry
    //  followed by a confirmation is not.
    Add(Cmd(CmdClearModel, ucClearModel,
        'ActionClearModel', tkAction, False,
        csMenu, '', ''));
    ...
    //  The two panes drawn as HTML. THEIR BUTTONS ARE ON THE PANES THEMSELVES,
    //  not in the Tools pane - a size applies to the thing being read, and a
    //  button for it anywhere else would have to say which pane it meant. They
    //  are declared here all the same, so their availability is decided by the
    //  same table as everything else rather than by the form.
    Add(Cmd('ReportZoomIn', ucReportZoomIn, 'ActionReportZoomIn', tkAction,
        False, csMenu, '', ''));
    Add(Cmd('ReportZoomOut', ucReportZoomOut, 'ActionReportZoomOut', tkAction,
        False, csMenu, '', ''));
    Add(Cmd('ExplainZoomIn', ucExplainZoomIn, 'ActionExplainZoomIn', tkAction,
        False, csMenu, '', ''));
    Add(Cmd('ExplainZoomOut', ucExplainZoomOut, 'ActionExplainZoomOut',
        tkAction, False, csMenu, '', ''));
    Add(Cmd('UseRule', ucUseRule, 'MenuUseRule', tkMenuItem, False,
        csMenu, '', ''));
    Add(Cmd('ValueUseRule', ucValueUseRule, 'MenuValueUseRule', tkMenuItem,
        False, csMenu, '', ''));
end;
