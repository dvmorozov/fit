...
{ The same measurement over ONE form, and how many of its captions do not fit.

  WHY IT IS SEPARATE. The walk above measures the forms that EXIST, and a window
  built in code when a menu entry is chosen does not - so its captions were the
  only ones in the application nothing could measure. A caller that creates such
  a window during the self-check hands it here. }
function ReportClippedCaptionsOf(AForm: TCustomForm): longint;
...
{ How many of AForm's captions do not fit, logging each one. AMeasure carries
  the canvas the text is measured on: one bitmap for a whole walk. }
function ClippedCaptionsOn(AForm: TCustomForm; AMeasure: TBitmap): longint;
var
    j, Needed: integer;
    C: TControl;
begin
    ...
    begin
        ...
        begin
            ...
            if (C is TCustomListBox) or (C is TSplitter) or
                ...
                (C is TShape) then
                Continue;
            AMeasure.Canvas.Font.Assign(C.Font);
            Needed := AMeasure.Canvas.TextWidth(C.Caption);
...
begin
    ...
    try
        Measure.SetSize(1, 1);
        //  See the walk below: a form that has never been shown has not
        //  resolved its anchors, and its children carry whatever the streaming
        //  or the constructor left them.
        AForm.HandleNeeded;
        Result := ClippedCaptionsOn(AForm, Measure);
    finally
        Measure.Free;
    end;
    WriteLog(Format('layout: checked %s, %d caption(s) do not fit',
        [AForm.ClassName, Result]), Notification);
end;
...
begin
    ...
    try
        ...
        begin
            ...
            Inc(Reported, ClippedCaptionsOn(Screen.CustomForms[i], Measure));
        end;
    finally
        Measure.Free;
    end;
