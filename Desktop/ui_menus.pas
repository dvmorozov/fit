...
{ True while a menu is dropped down - the menu bar, a submenu of it, or a popup.
  ...
  Neither is a thing the user asked for at that moment: both arrive from an
  event, a queued call, or the calculation thread, at whatever instant the main
  loop happened to reach them. So the rule is not "do it carefully" but DO
  ...
  the window refreshes is worth taking it away from them.
...
{ Ticks or unticks a menu entry, and never at the cost of the menu the user is
  ...
  than performed. The window does not apply its state under an open menu at
  all (deferred_ui), so this is the last guard rather than the usual path.
