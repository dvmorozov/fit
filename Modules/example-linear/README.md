...
A complete, working module in eight files. Nothing in the framework names it, and
...
| File | What it is |
|---|---|
| `linear_points_set.pas` | the curve type — the module's actual content, and what it explains about itself |
| `example_explanations.pas` | the module's own explanation provider: its one rule, stated with its standing |
| `example_ui.pas` | its menu, declared as data: one entry, explained before it is chosen |
...
The suite runs the public tests **and** this module's — 10 more than the public
