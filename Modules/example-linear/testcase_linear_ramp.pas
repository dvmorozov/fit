...
uses Classes, SysUtils, fpcunit, testregistry,
  ...
  special_curve_parameter, explanation, explanation_registry,
  curve_type_registration, curve_type_explanations, int_ui_host,
  mock_ui_host, example_explanations, example_ui, report_tabs;
...
type
  ...
  published
    ...
    procedure ItsOneRuleSaysItIsThisSoftwaresChoice;
    procedure ItsMenuEntryIsExplainedBeforeItIsChosen;
    procedure ChoosingItPutsTheRampsExplanationInFrontOfTheUser;
    procedure AModuleWithoutAReportAddsNoTab;
  end;
...
{ WHAT A MODULE OWES THE USER, checked the way a module of your own should check
  it: everything it registers resolves to a complete explanation, walked over the
  registry after the front door has run - so a rule added without one fails here
  by name. }
procedure TLinearRampTest.EverythingTheModuleRegistersExplainsItself;
var
...
begin
  ...
  RegisterExampleModule;
  Findings := ExplanationFindings(RegisteredExplanationProviders);
  Text := '';
  for i := 0 to High(Findings) do
    Text := Text + Findings[i] + LineEnding;
  AssertEquals('', Text);
  AssertTrue('the curve type is explained',
    RegisteredTopicResolves(CurveTypeTopic(TLinearPointsSet)));
  AssertTrue('and so is the module''s own rule',
    RegisteredTopicResolves(PositiveWidthTopic));
end;

procedure TLinearRampTest.ItsOneRuleSaysItIsThisSoftwaresChoice;
var
  E: TExplanation;
begin
  RegisterExampleModule;
  AssertTrue(FindExplanation(PositiveWidthTopic, E));
  //  No literature states a rule about a teaching example; calling it canonical
  //  would claim a source that does not exist.
  AssertTrue(E.Standing = esModelChoice);
end;

procedure TLinearRampTest.ItsMenuEntryIsExplainedBeforeItIsChosen;
var
  Items: TUiMenuDeclArray;
begin
  Items := ExampleUiModule.MenuItems;
  AssertEquals(1, Length(Items));
  AssertEquals(CurveTypeTopic(TLinearPointsSet), Items[0].Topic);
  AssertTrue('and says what it does', Length(Items[0].Hint) > 20);
end;

procedure TLinearRampTest.ChoosingItPutsTheRampsExplanationInFrontOfTheUser;
var
  Host: TMockUiHost;
  HostRef: IUiHost;
begin
  Host := TMockUiHost.Create;
  HostRef := Host;
  try
    ExampleUiModule.Command(ExplainRampId, '', HostRef);
    AssertEquals(CurveTypeTopic(TLinearPointsSet), Host.LastExplanation);
  finally
    HostRef := nil;
    Host.Free;
  end;
...
procedure TLinearRampTest.AModuleWithoutAReportAddsNoTab;
var
  Mods: TUiModuleArray;
begin
  //  A ramp judges nothing, so it declines the report tab - and the window's
  //  decision, asked over this module, makes none.
  AssertEquals('', ExampleUiModule.ReportCaption);
  SetLength(Mods, 1);
  Mods[0] := ExampleUiModule;
  AssertEquals(0, Length(ReportTabsFor(Mods)));
end;
