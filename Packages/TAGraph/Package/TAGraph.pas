...
uses
  ...
  Windows,
  {$ENDIF}
   SysUtils, Classes, Controls, Graphics, Dialogs, StdCtrls, Math;
...
type
  ...
  { Where the X marks go, for an axis whose values have units of their own. Set
    AHandled to place them; left False, the chart places its own. }
  TChartMarksEvent=procedure(AMin,AMax:Double;var AStart,AStep:Double;
    var AHandled:Boolean) of object;
  { The text of the X mark at AValue. Left unassigned, marks print as numbers. }
  TChartMarkTextEvent=function(AValue,AStep:Double):string of object;
  { The Y axis has the same two, for the same reason: a value axis can have units
    of its own too - a logarithmic one is read in the quantity, not in its log. }
...
// Graph = coordinates in the graph
    ...
    FLineWidth:Integer;                        // pen width at 96 dpi
    FDrawOnTop:Boolean;
    FCaptions:TStringList;                     // One per point, in point order
    ...
    procedure SetDrawOnTop(Value:Boolean);
    ...
    procedure DrawHatch(ACanvas:TCanvas;AX1,AY1,AX2,AY2:Integer;
      ADescending:Boolean);
    { Each point's caption, beside it and above, where the point is inside
      the plot area. }
    procedure DrawCaptions(XMin,XMax,YMin,YMax,Larg:Integer);
  protected
  ...
  public
    ...
    { False for a point with no place on the chart - a coordinate that is NaN,
      which is how an axis says "this value cannot be shown here" (the
      logarithm of zero, say). Such a point is kept, so indices still match the
      data, and left undrawn: no marker, no caption, no line to or from it, no
      part in the extents, and the pointer never snaps to it. }
    function  PointIsDrawn(Index:Integer):Boolean;
    ...
    { The width of the pen the series is drawn with, lines and marker
      outlines alike, in pixels at 96 dpi - scaled like everything else
      (TTAChart.Sc). One unless something stands the series out. }
    property LineWidth:Integer read FLineWidth write SetLineWidth;
    { Drawn after every series that is not, so nothing added later covers
      it. Asked by DisplaySeries - the series is never moved in SeriesList,
      whose order the viewer's lookups by index rely on. }
    ...
    { A caption for each point, in the order the points were added: a wave's
      pivots are numbered or lettered, and the number is what the user reads.
      Copied, so the caller keeps its list; nil takes every caption away. They
      follow the points by position, so a caller that reorders its points must
      hand them again. }
    procedure SetCaptions(ACaptions:TStrings);
    { The caption of point AIndex, or '' for a point that has none. }
    function  PointCaption(AIndex:Integer):string;

    procedure BeginUpdate;
    procedure EndUpdate;
    { The extents taken again from every drawn point. }
    procedure RecalculateExtents;
  published
  ...
  private
    ...
    FOnXMarks:TChartMarksEvent;
    FOnXMarkText:TChartMarkTextEvent;
    FOnYMarks:TChartMarksEvent;
    FOnYMarkText:TChartMarkTextEvent;
  ...
  public
    ...
    { The X axis's marks, when its values want marks of their own - see
      TChartMarksEvent and TChartMarkTextEvent. }
    property OnXMarks:TChartMarksEvent read FOnXMarks write FOnXMarks;
    property OnXMarkText:TChartMarkTextEvent read FOnXMarkText write FOnXMarkText;
    { The Y axis's marks, on the same terms as the X axis's. }
    property OnYMarks:TChartMarksEvent read FOnYMarks write FOnYMarks;
    property OnYMarkText:TChartMarkTextEvent read FOnYMarkText write FOnYMarkText;
...
function TTASerie.PointIsDrawn(Index:Integer):Boolean;
begin
Result:=not IsNan(XGraph^[Index]) and not IsNan(YGraph^[Index]);
end;

const
  { Where an undrawn point is put on the canvas: far outside it, so every test
    that already keeps a marker or caption inside the plot area leaves it out
    without having to be taught about NaN, and small enough that arithmetic on
    it (the pointer's distance to it) cannot overflow. }
...
// Calculate again all points. A NaN cannot be rounded to a pixel - it raises -
// so an undrawn point is parked off the canvas instead (see UndrawnImage).
for i:=0 to NbPoints-1 do
   if PointIsDrawn(i) then
      (Chart as TTAChart).GraphToImage(XGraph^[i],YGraph^[i],XImage^[i],YImage^[i])
   else
      begin
      XImage^[i]:=UndrawnImage;
      YImage^[i]:=UndrawnImage;
      end;
...
with (Chart as TTAChart) do
   ...
   Canvas.Pen.Width:=Sc(Self.FLineWidth);
   end;
...
for i:=0 to NbPoints-2 do
   ...
   with (Chart as TTAChart) do
      ...
      //  A GAP, not a line through it: a segment with an undrawn end joins
      //  nothing to something, and drawing it would invent the data between.
      if FShowLines and PointIsDrawn(i) and PointIsDrawn(i+1) then
         //  ONE PASS - the four Breaks below are exits, not iterations. Each
         //  means the same thing: this segment's line is settled, go on to its
         //  point marker. They were `goto Points`, and a user-declared label is
         //  the one thing the assembler on x86_64-darwin will not take - Free
         //  Pascal emits it as a global symbol, and a global symbol is illegal
         //  between .cfi_startproc and .cfi_endproc. Break compiles to the same
         //  local jump the goto did.
         repeat
...
if FCaptions.Count>0 then DrawCaptions(XMin,XMax,YMin,YMax,Larg);
...
procedure TTASerie.DrawCaptions(XMin,XMax,YMin,YMax,Larg:Integer);
var
   i:Integer;
   S:string;
begin
with Chart as TTAChart do
   ...
   Canvas.Font.Color:=FSeriesColor;
   Canvas.Brush.Style:=bsClear;
   for i:=0 to NbPoints-1 do
      begin
      S:=PointCaption(i);
      //  Inside the plot area only, the same test the markers use: a caption
      //  for a point scrolled out of view would be written over the axes.
      if (S<>'') and (XImage^[i]>XMin) and (XImage^[i]<XMax) and
         (YImage^[i]>YMin) and (YImage^[i]<YMax) then
         Canvas.TextOut(XImage^[i]+Larg+1,
            YImage^[i]-Larg-Canvas.TextHeight(S),S);
      end;
   end;
end;

procedure TTASerie.SetCaptions(ACaptions:TStrings);
begin
if ACaptions=nil then FCaptions.Clear
else FCaptions.Assign(ACaptions);
if Chart<>nil then Chart.Invalidate;
end;

function TTASerie.PointCaption(AIndex:Integer):string;
begin
if (AIndex>=0) and (AIndex<FCaptions.Count) then Result:=FCaptions[AIndex]
else Result:='';
end;
...
// Update max. An undrawn point takes no part: the chart's extents are where
// the drawn points are, and comparing with NaN is itself an invalid operation.
...
var
   ...
   Val:Double;
begin
//  The incremental bookkeeping below compares the old and new values with the
//  extents; with a NaN on either side there is nothing to compare, so the
//  extents are taken again from every drawn point.
if not(UpdateInProgress) and (IsNan(Value) or not PointIsDrawn(Index)) then
   begin
   XGraph^[Index]:=Value;
   RecalculateExtents;
   Chart.Invalidate;
...
procedure TTASerie.SetLineWidth(Value:Integer);
begin
...
procedure TTASerie.SetDrawOnTop(Value:Boolean);
begin
if Value=FDrawOnTop then Exit;
FDrawOnTop:=Value;
if Chart<>nil then Chart.Invalidate;
end;
...
procedure TTASerie.RecalculateExtents;
var
   i:Integer;
   Val:Double;
begin
...
SetLength(YWidths,0);
Handled:=False;
if Assigned(FOnYMarks) then
   FOnYMarks(FYGraphMin,FYGraphMax,Debut,Pas,Handled);
if not Handled then
...
if FYGraphMin<>FYGraphMax then
   ...
   while Marque<=FYGraphMax+Pas*10e-10 do
      ...
      if (Marque>=FYGraphMin) then
         ...
         if Assigned(FOnYMarkText) then
            MyText:=FOnYMarkText(Marque,Pas)
         else
...
Pas:=1;
Handled:=False;
if Assigned(FOnXMarks) then
   FOnXMarks(FXGraphMin,FXGraphMax,Debut,Pas,Handled);
if not Handled then
...
if FXGraphMin<>FXGraphMax then
   ...
   while Marque<=FXGraphMax+Pas*10e-10 do
      ...
      if (Marque>=FXGraphMin) then
         ...
         Canvas.Brush.Color:=Color;
         if Assigned(FOnXMarkText) then
            MyText:=FOnXMarkText(Marque,Pas)
         else
            MyText:=Trim(Format('%6.4g',[Marque]));
...
// Y graduations - the same marks the measuring pass above walked
MaxLargTexte:=0;
Debut:=FYGraphMax;
Pas:=1;
YIndex:=0;
...
var
   Pass,i,SeriePass:Integer;
   Serie:TComponent;
   Started,Elapsed:Int64;
begin
// Update all series: pass 0 draws the background bands, pass 1 everything else,
// pass 2 the series standing out (DrawOnTop), so no curve drawn later covers them.
for Pass:=0 to 2 do
   for i:=0 to SeriesList.Count-1 do
      begin
      Serie:=SeriesList[i];
      if Serie is TTASerie then
         begin
         if (Serie as TTASerie).IsBackgroundBand then SeriePass:=0
         else if (Serie as TTASerie).DrawOnTop then SeriePass:=2
         else SeriePass:=1;
         if SeriePass=Pass then
            begin
            Started:=Int64(GetTickCount64);
      ...
      if TASerie.ShowPoints or TASerie.ShowLines then   //  DM 27/11/07
         begin
            for PointNumber:=0 to TASerie.Count-1 do
               if TASerie.PointIsDrawn(PointNumber) then
               begin
