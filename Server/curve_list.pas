...
uses
    Classes, SysUtils, checks, curve_instance_id, persistent_curve_parameters,
    self_copied_component, special_curve_parameter, table_text;
...
type
    ...
    public
        ...
        { THE TABLE AS FILE > EXPORT > CURVE PARAMETERS WRITES IT: a heading
          row of parameter names, then one row per curve with every digit of
          each value (table_text.ExportedNumber). An uncertainty gets a column
          of its own beside its value, headed "<name> error", where RowCellText
          puts it in the same cell - which a spreadsheet cannot read as a
          number. A parameter a curve lacks is blank, as on screen. }
        function ExportRows: TTextRows;

        { WHICH CURVE A ROW OF THE TABLE STANDS FOR, as the handle the model
          addresses it by, or '' for a row outside the data and for a curve
          never issued a handle - whose all-zero identifier would otherwise
          read as a real one and match nothing. ARow is zero-based within the
          data, as for RowCellText. The one answer both the table's Delete and
          choosing a curve in the table follow. }
        function CurveHandleOfRow(const ARow: longint): string;

        { The first row naming a curve OTHER than ACurveId, or -1. A row naming
          no curve is never the answer, so with nothing selected this is the
          first row naming any. The window's self-check chooses it, so that a
          selection which did not move cannot pass for one that did. }
        function RowNamingAnotherCurve(const ACurveId: string): longint;
        ...
        { WHAT A VALUE TYPED INTO THE TABLE ASKS OF THE ENGINE: the index of
          the parameter within its curve - found by the column's NAME, since
          curves of different types order their parameters differently - and
          the value in the engine's terms, the inverse of the display
          transform. The table itself is not changed: the engine rebuilds the
          model with the value and the table is refilled from it.

          False with AWhy in words when the text is refused (not a number) or
          the parameter cannot be typed (computed from the others, or the
          argument); False with AWhy empty when the curve has no parameter of
          that column's name - its cell is blank, and there is nothing to do. }
        function PlanParameterEdit(const ARow, ACol: longint;
            const AText: string; out AParamIndex: longint; out AValue: double;
...
function TCurveListBase.CurveHandleOfRow(const ARow: longint): string;
begin
    Result := '';
    if (ARow < 0) or (ARow >= Count) then
        Exit;
    if not (Items[ARow] is Curve_parameters) then
        Exit;
    if not IsCurveInstanceId(Curve_parameters(Items[ARow]).FInstanceId) then
        Exit;
    Result := CurveInstanceIdToWire(Curve_parameters(Items[ARow]).FInstanceId);
end;

function TCurveListBase.RowNamingAnotherCurve(const ACurveId: string): longint;
var
    i: longint;
    Handle: string;
begin
    ...
    for i := 0 to Count - 1 do
    begin
        Handle := CurveHandleOfRow(i);
        if (Handle <> '') and (Handle <> ACurveId) then
            Exit(i);
    end;
end;
...
function TCurveListBase.ExportRows: TTextRows;
var
    Names: TStringList;
...
begin
    ...
    try
        CollectColumnNames(Names);
        //  A column for an uncertainty only where some curve has one: the
        //  native engine estimates none, and a column of blanks is noise.
        SetLength(HasError, Names.Count);
        for j := 0 to Names.Count - 1 do
        begin
            HasError[j] := False;
            for i := 0 to Count - 1 do
            begin
                Parameter := Curve_parameters(Items[i]).FindByName(Names[j]);
                if Assigned(Parameter) and (Parameter.Error >= 0) then
                    HasError[j] := True;
            end;
        end;
        ...
        begin
            ...
            Row[High(Row)] := Names[j];
            if HasError[j] then
            begin
                SetLength(Row, Length(Row) + 1);
                Row[High(Row)] := Names[j] + ' error';
            end;
        end;
        Result[0] := Row;
        ...
        begin
            ...
            begin
                ...
                if Assigned(Parameter) then
                    Row[n] := ExportedNumber(RecalcParamValue(Parameter));
                if HasError[j] then
                begin
                    SetLength(Row, n + 2);
                    Row[n + 1] := '';
                    if Assigned(Parameter) and (Parameter.Error >= 0) then
                        Row[n + 1] := ExportedNumber(Parameter.Error);
                end;
            end;
            Result[i + 1] := Row;
        end;
...
function TCurveListBase.PlanParameterEdit(const ARow, ACol: longint;
    const AText: string; out AParamIndex: longint; out AValue: double;
    out AWhy: string): boolean;
var
    CurveParameters: Curve_parameters;
    Parameter, Copy_: TSpecialCurveParameter;
    Names: TStringList;
    Typed: double;
    i: longint;
begin
    ...
    try
        ...
        for i := 0 to CurveParameters.Count - 1 do
            if SameText(CurveParameters.Parameters[i].Name, Names[ACol]) then
            begin
                AParamIndex := i;
                Break;
            end;
        if AParamIndex < 0 then
            Exit;
        Parameter := CurveParameters.Parameters[AParamIndex];

        if Parameter.Type_ in [Argument, Calculated] then
        begin
            AWhy := Parameter.Name + ' is computed from the other parameters, ' +
                'not fitted, so it cannot be typed.';
            Exit;
        end;
        if not TryStrToFloat(Trim(AText), Typed) then
        begin
            AWhy := '"' + Trim(AText) + '" is not a number, so ' +
                Parameter.Name + ' was not changed.';
            Exit;
        end;
        //  THE ENGINE'S TERMS, through a copy: the display may show a
        //  transformed value (an angle for a position), and planning must not
        //  change what the table holds.
        Copy_ := Parameter.CreateCopy;
        try
            ReverseCalcParamValue(Copy_, Typed);
            AValue := Copy_.Value;
        finally
            Copy_.Free;
        end;
