...
type
    ...
    private
        ...
        { The profile as this operation last changed it, and how many times. }
        FProfileRevision: longint;
        FProfile: TPointsData;
    ...
    public
        ...
        { The operation changed the profile to AProfile - the background taken
          off it. Reported with the frames, so an animated fit is drawn against
          the profile it fits. }
        procedure StoreProfile(const AProfile: TPointsData);
        { The samples from ASince on, and the snapshot - and the profile the
          operation changed - when AWithSnapshot. }
...
begin
    ...
    try
        ...
        FSnapshotTakenAt := 0;
        //  A profile an earlier operation changed may be older than the one
        //  the client has loaded since; drawn over it, it would undo the load.
        FProfileRevision := 0;
        FProfile := Default(TPointsData);
    finally
...
procedure TFitProgressLog.StoreProfile(const AProfile: TPointsData);
begin
    EnterCriticalSection(FLock);
    try
        //  Replaced whole, like the snapshot, so a report holding the previous
        //  one keeps a consistent picture.
        FProfile := AProfile;
        Inc(FProfileRevision);
    finally
        LeaveCriticalSection(FLock);
    end;
end;
...
begin
    ...
    try
        ...
        if AWithSnapshot and (FProfileRevision > 0) then
        begin
            Result.HasProfile := True;
            Result.ProfileRevision := FProfileRevision;
            Result.Profile := FProfile;
        end;
    finally
        LeaveCriticalSection(FLock);
    end;
