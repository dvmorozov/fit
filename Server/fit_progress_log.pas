...
uses
    SysUtils, Math, fit_points_json, fit_progress_json;
...
type
    ...
    public
        ...
        procedure Start(ANow: TDateTime; const ALossName: string = '';
        ...
        procedure Finish(ANow: TDateTime);
        ...
        procedure Add(ANow: TDateTime; AValue: double);
        { A client wants to see the model as the fit goes. }
        procedure AskForSnapshot(ANow: TDateTime);
        { Whether the engine should take a snapshot now. }
        function SnapshotDue(ANow: TDateTime): boolean;
        procedure StoreSnapshot(ANow: TDateTime;
            const AComputedProfile, ADeltaProfile: TPointsData;
            const ACurves: TFitProgressCurves);
        { The samples from ASince on, and the snapshot when AWithSnapshot. }
        function Report(ANow: TDateTime; ASince: longint;
...
function TFitProgressLog.SecondsBetween(AFrom, ATo: TDateTime): double;
begin
    Result := (ATo - AFrom) * SecsPerDay;
end;

procedure TFitProgressLog.Start(ANow: TDateTime; const ALossName: string;
    const AEngineName: string);
begin
...
procedure TFitProgressLog.Finish(ANow: TDateTime);
begin
    EnterCriticalSection(FLock);
    try
        if FBusy then
...
procedure TFitProgressLog.Add(ANow: TDateTime; AValue: double);
begin
...
procedure TFitProgressLog.AskForSnapshot(ANow: TDateTime);
begin
    EnterCriticalSection(FLock);
    try
...
function TFitProgressLog.SnapshotDue(ANow: TDateTime): boolean;
begin
    EnterCriticalSection(FLock);
    try
...
procedure TFitProgressLog.StoreSnapshot(ANow: TDateTime;
    const AComputedProfile, ADeltaProfile: TPointsData;
    const ACurves: TFitProgressCurves);
begin
    EnterCriticalSection(FLock);
...
function TFitProgressLog.Report(ANow: TDateTime; ASince: longint;
    AWithSnapshot: boolean): TFitProgressReport;
var
    i, First: longint;
begin
