...
uses
    ...
    //  THE CALLBACK IS AN INTERFACE (int_client_callback below), and that is
    //  all the engine needs: FitProxy is an IClientCallback and nothing here
    //  names a class that implements one. Naming fit_server_proxy as well
    //  linked the desktop CLIENT into the compute server - the proxy forwards
    //  to fit_client_stub, which casts to TFitClient - and through it the
    //  client's tables, series and progress view. Nothing constructed any of
    //  it: one unused entry in a uses clause did it.
    , fit_task, int_client_callback, int_fit_service, MyExceptions, run_clock
    , fit_statistics, fit_service_statistics, fit_advice, rfactor_text
    ...
    , fpjson
    , checks
    , fit_points_json, fit_progress_json, fit_progress_log;
...
type
    ...
    protected
        ...
        { Guards the task list against the one request that may now arrive while
          an operation is running: Stop. Every other route waits for the
          problem's lock, so the list was touched by one thread only - and the
          stop that could not reach a running fit was the price of that. }
        FTaskLock:   TRTLCriticalSection;
        ...
        { Whether the background has been subtracted from the profile held now.
          A FACT OF ITS OWN, not read off the state: marking an interval or a
          position also leaves BackNotRemoved, and the automatic run that took
          that to mean "subtracted" fitted a raw profile's baseline with a
          curve on every sample. }
        FBackgroundSubtracted: boolean;
        ...
        { When the running operation began, on the run clock. }
        FStartTime: TRunTime;
        { WHAT A RUNNING FIT HAS REACHED, for a reader on another thread. The
          fit holds the problem's lock throughout, so this is the one thing a
          poll can read while it runs; see fit_progress_log. }
        FProgressLog: TFitProgressLog;
    ...
    protected
        ...
        { Rebuilds the computed profile and the curves as they stand and hands
          them to the progress log, for a client animating the fit. }
        procedure TakeProgressSnapshot(ANow: TRunTime; AStarting: boolean = False);
        { The model as the fit found it, as the first frame. }
        procedure ShowStartingModel;
    ...
    public
        ...
        function ProgressLossName: string;
        function ProgressEngineName: string;
...
begin
    ...
    FProgressLog := TFitProgressLog.Create;
    InitCriticalSection(FTaskLock);
...
begin
    if State = AsyncOperation then
        ...
        CheckThat(not FBackgroundSubtracted, 'the background must still be in place before it can be removed again')
    else
...
begin
    ...
    FStartTime := RunTime;
    //  Timed like a fit, so the client's clock reads this run and not the last.
    FProgressLog.Start(RunTime, ProgressLossName, ProgressEngineName);
...
begin
    ...
    RefuseIf(ProfileRefusal(State));
    //  A SECOND INTERVAL REPLACES THE FIRST. It used to be refused with
    //  'Range of data already selected' - which the client never showed - so the chart drew the new
    //  interval while every fit went on running over the old one. The indices
    //  are into the whole profile either way, so taking the new one is exact.
    CheckAssigned(FExpProfile, 'the experimental profile');
...
function PointsDataOf(APoints: TPointsSet; const ATitle: string): TPointsData; forward;

procedure TFitService.ShowProfile;
begin
    if Assigned(FitProxy) then
        FitProxy.ShowProfile;
    //  AND FOR A CLIENT ACROSS HTTP, which the callback above never reaches:
    //  the profile as it now is goes into the progress log, and travels with
    //  the frames of the running operation (TFitProgressReport.Profile). The
    //  one the client draws - the selected interval when there is one.
    if FSelectedAreaMode then
        FProgressLog.StoreProfile(PointsDataOf(FSelectedArea, ''))
    else
        FProgressLog.StoreProfile(PointsDataOf(FExpProfile, ''));
end;

{$HINTS off}
{ EVERY BACKEND REACHES THIS, which is why the progress of a fit is recorded here
  and nowhere else. The native minimizer arrives through TFitTask.ShowCurMin on
  each improvement; a backend fitting in another process relays each improvement
  it hears about through the same call. So the loss curve a client draws is the
  same whichever engine ran and whatever the model is - nothing here asks.

  THE MODEL IS REBUILT ONLY WHEN SOMEBODY IS WATCHING. This used to recollect the
  curves and the computed profile on every improvement "for animation mode", for
  a client that had long since stopped receiving it. The log now says when a
  snapshot is wanted: while a client asks for one, and no more often than it can
  use. DoneProc rebuilds both when the fit ends, as it always did.
...
begin
    Moment := RunTime;
    FProgressLog.Add(Moment, FCurrentMinimum);
    if FProgressLog.SnapshotDue(Moment) then
        TakeProgressSnapshot(Moment);
    if Assigned(FitProxy) then
...
{ A point set as the wire carries it. }
...
procedure TFitService.TakeProgressSnapshot(ANow: TRunTime; AStarting: boolean);
var
...
begin
    try
        ...
        CreateDeltaProfile;
        CollectCurves;
        SetLength(Curves, FCurves.Count);
        for i := 0 to FCurves.Count - 1 do
        begin
            Curve := TNamedPointsSet(FCurves.Items[i]);
            //  Under the handle and the type name GET /curves gives it, so an
            //  animated frame and the final redraw name each curve alike.
            Curves[i].Id := GetCurveInstanceId(i);
            Curves[i].Points := PointsDataOf(Curve, Curve.GetCurveTypeName);
        end;
        FProgressLog.StoreSnapshot(ANow, PointsDataOf(FCalcProfile, ''),
            PointsDataOf(FDeltaProfile, ''), Curves, AStarting);
    except
        //  A PICTURE MUST NOT STOP THE FIT IT IS A PICTURE OF. Anything that
        //  goes wrong here is logged and the fit carries on; the client simply
        //  keeps the previous frame.
        on E: Exception do
            WriteLog('could not take a progress snapshot: ' + E.Message, Warning);
    end;
end;
...
{ THE MODEL AS THE FIT FOUND IT, as the first frame, when somebody is watching.
  A fit of a few hundred milliseconds took its only frame at its first
  improvement, and whether a poll landed between that and the end was a race -
  lost often enough that an ordinary fit was sometimes not animated at all.
  Stored now, any poll that finds the fit running has something to draw. On the
  fit's own thread, after the tasks exist and before any of them runs. }
procedure TFitService.ShowStartingModel;
var
    Moment: TRunTime;
begin
    Moment := RunTime;
    if FProgressLog.SnapshotDue(Moment) then
        TakeProgressSnapshot(Moment, True);
end;
...
begin
    ...
    if not FBackgroundSubtracted then
    begin
...
begin
    ...
    //  HELD FOR THE WHOLE BUILD, because a Stop arriving in the middle of it
    //  would otherwise read a list that has been freed and not yet replaced.
    //  The wait it costs that request is the length of building the tasks,
    //  which is object creation and no fitting at all.
    EnterCriticalSection(FTaskLock);
    try
    FTaskList.Free;
    FTaskList := nil;
    ...
    // end;
        ...
        LeaveCriticalSection(FTaskLock);
    end;
end;
...
{ Stops the operation that is running, keeping what it has reached.

  THIS USED TO BE UNABLE TO FIND ANYTHING TO STOP, and said so in a comment: a
  task ran to completion inside RecreateMainCalcThread, so by the time any other
  request was served the operation was over. That was true of a server where
  every route waited for the problem's lock. It stopped being true when the
  progress routes began to be answered without it - and the stop route is now
  answered without it too (fit_rest_api.IsUnlockedRoute), which is what lets
  this reach a fit that is still running.

  WHAT THE USER SAW UNTIL THEN, and reported: Stop pressed in the middle of a
  fit hung the window for the rest of that fit - the request was waiting for the
  lock the fit held - and then said "the calculation not started", because by
  the time it was let in the operation had finished. A button that does nothing
  slowly and then denies there was anything to do.

  THE FLAG IS THE ENGINE'S OWN. Every task checks it between iterations
  (TFitTask.StopAsyncOper), so the fit ends where it stands and DoneProc runs
  the way it does after any fit: the curves it had reached are kept, which is
  what the command has always promised. A backend fitting in another process
  finishes its current call first and stops at the next check.

  Nothing running is still refused, which is the answer to pressing Stop when
...
begin
    ...
    WriteLog(Format('stop requested; %d task(s) told to end', [Stopped]),
        Notification);

    //  NO TASK TO TELL, and the problem says an operation is running: the busy
    //  flag is stale. Recovered rather than left, which is what this method did
    //  before it could reach a task at all.
    if Stopped = 0 then
...
{ What this problem is minimising, and what is minimising it, in the words the
  user chose them by. Read on the thread that STARTS an operation and recorded
  in the progress log, because the progress route is answered on another thread
  without the problem's lock and must read nothing off the engine. }
function TFitService.ProgressLossName: string;
begin
    Result := LossName(FLossKind);
end;

function TFitService.ProgressEngineName: string;
var
    Info: TMinimizerInfo;
begin
    if FindMinimizer(FMinimizerKind, Info) then
        Result := Info.Name
    else
        //  A kind this build does not have is not a fault here: the header
        //  simply does not name the engine.
        Result := '';
end;

function TFitService.GetFitProgress(ASince: longint;
    AWithSnapshot: boolean): TFitProgressReport;
var
    Moment: TRunTime;
begin
    //  READS THE LOG AND NOTHING ELSE. This is answered on another thread while
    //  a fit runs, without the problem's lock; touching the engine from here
    //  would be a data race with the fit it reports on.
    Moment := RunTime;
    if AWithSnapshot then
        FProgressLog.AskForSnapshot(Moment);
    Result := FProgressLog.Report(Moment, ASince, AWithSnapshot);
end;
...
begin
    ...
    //  A FIT INTERVAL IS REQUIRED to build curves from picks: with none there
    //  is no model here, and a fit started without intervals computes them
    //  first. A module whose markup defines whole patterns needs none - its
    //  patterns say where they lie - and CreateTasks gives such a markup the
    //  whole profile, so a placed pattern is drawn at once.
    ...
    begin
        //  NOT READY MEANS NO MODEL. Without picks or without a fit interval
        //  nothing can be built, and the curves collected from the last build
        //  describe input that is gone. They used to stay while the setters
        //  cleared their attributes, so the two lists disagreed and the next
        //  read of the model - the refresh after Model > Curve Positions >
        //  Remove All - asked for the attributes of a curve that had none.
        FCurves.Clear;
        FResultedCurvePositions.Clear;
        FCurveAttributes.Clear;
    end;
...
begin
    ...
    Started := RunTime;
    //  THE PROGRESS LOG IS CLOSED HOWEVER THE OPERATION ENDS. A poll that still
    //  read busy after a failed fit would keep the client's live chart up over
    //  a problem that is no longer doing anything.
    try
        try
            ACurrentTask;
        except
            //  A TASK THAT RAISES NEVER REACHES ADoneProc - and ADoneProc is what
            //  takes the problem back out of AsyncOperation. Without this the
            //  problem stayed marked busy for the rest of its life: the client's
            //  status bar read "Minimising R-factor" forever, and the next
            //  request that tries to interrupt that phantom operation - opening
            //  another data file, say - went to AbortAsyncOper and died there.
            //
            //  Restored the way DoneProc restores it: the state that preceded
            //  the operation, assigned directly rather than through SetState,
            //  because nothing was computed and there is nothing to reinitialise.
            FState := FSavedState;
            WriteLog(Format('operation failed after %d ms; state restored to %s',
                [Round((RunTime - Started) * 1000), FitServerStateName(FState)]),
                Warning);
            raise;
        end;
        ADoneProc;
    finally
        FProgressLog.Finish(RunTime);
    end;
