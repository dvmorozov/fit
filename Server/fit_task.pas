...
type
    ...
    protected
        ...
        { Set while the automatic decomposition removes curves, so each re-fit
          uses the looser convergence that algorithm needs - see
          TDownhillSimplexMinimizer.UseDecompositionConvergence. }
        FDecomposing: boolean;
    ...
    public
        ...
        { Notifies service about achievement of new minimum value.
          ...
          PUBLIC, and widened for a production caller: a backend that fits in
          another process relays each improvement it hears about through this
          same call (remote_fit_progress), so a fit's progress reaches the service
          by one funnel whichever engine ran it. }
...
uses
    SimpMath, int_fit_backend, run_clock,
...
begin
    ...
    if FDecomposing then
        TDownhillSimplexMinimizer(FMinimizer).UseDecompositionConvergence(
            FMaxAcceptableRFactor);
...
begin
    ...
    try
        ...
        WriteLog(Format('fit: DONE in %.2f s, R-factor %.6g',
            [RunTime - Started, GetRFactor]), log.Notification);
    end;
end;
