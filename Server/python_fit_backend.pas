...
const
    ...
    { A progress poll is asked several times a second; one that does not come back
      quickly is a frame skipped, not a reason to hold up the fit's thread. }
    PROGRESS_POLL_TIMEOUT_MS = 2000;
...
type
    ...
    protected
        ...
        { The progress route, asked while the POST is out. The same seam. }
...
uses
    ...
    //  What the sidecar reaches while it fits, relayed into the task.
    remote_fit_progress;
...
function TPythonFitBackend.Get(const AUrl: string): string;
var
    C: TFPHTTPClient;
begin
    C := TFPHTTPClient.Create(nil);
    try
        C.ConnectTimeout := CONNECT_TIMEOUT_MS;
        C.IOTimeout := PROGRESS_POLL_TIMEOUT_MS;
        Result := C.Get(AUrl);
    finally
        C.Free;
    end;
...
begin
    ...
    Problem.ProgressId := NewProgressId;
    WriteLog(Format('Python fit: %d curve(s), weighting=%s, POST %s',
        [Length(Problem.Curves), Problem.Weighting, FUrl]), Notification);

    //  The sidecar publishes what it reaches under the id, and the relay brings
    //  it into this task while the POST is out - see remote_fit_progress.
    Body := PostRelayingProgress(ATask, Post, Get, FUrl + '/fit',
        FitProblemToJson(Problem), ProgressUrlFor(FUrl, Problem.ProgressId));
