...
{ Starts the compute server, leaves it running, and returns once it has said it
  is listening - or has died, or the budget has passed.
  ...
  NO PROBE REPEATED. The server is told a port to say "ready" on, and this
  blocks on that one event (readiness_channel). When this returns and the
  listener is freed, the server does not care: only the sidecar treats its
  starter's connection as a lifeline. }
procedure StartServerAndWait(const AExe: string; APort: word);
var
    P: TProcess;
...
begin
    ...
    try
        ...
        try
            ...
            //  No wait option among them: the server runs until the session ends,
            //  so waiting on it would mean no window ever appeared.
            P.Options := [];
            P.ShowWindow := swoHIDE;
            try
                P.Execute;
            except
                //  Nothing to say and nowhere to say it: this is a GUI binary with
                //  no console. The client is started regardless and reports the
                //  server it could not reach, naming the address.
                Exit;
            end;
        finally
            P.Free;
        end;
        //  Ready, dead or late: the client starts either way, and says what it
        //  finds - which is the message the user can act on.
        Listener.WaitForReady(LauncherReadyBudgetMs);
    finally
        Listener.Free;
    end;
end;
...
begin
    ...
    begin
        ...
        if ServerExe <> '' then
        begin
            StartServerAndWait(ServerExe, Port);
        end;
    end;
