...
type
    ...
    TFitProblem = record
        ...
        { Names this fit's progress on the far side, so GET /fit/progress?id= can
          be asked while the POST is still out. Empty = the sender does not want
          progress, which is what every older peer sends. }
        ProgressId:          string;
    end;
...
begin
    ...
    try
        ...
        //  ABSENT WHEN EMPTY, so a problem sent to an older peer is exactly the
        //  message it was before the field existed.
        if P.ProgressId <> '' then
            O.Add('progressId', P.ProgressId);
        Result := O.AsJSON;
    finally
        O.Free;
    end;
