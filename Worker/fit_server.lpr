...
machine. The client never starts it: the installed launchers do, before the
client, and a server started by hand works the same way.
...
client keeps fit_client.log, so the two processes never share a file). --log-level
...
uses
    ...
    cthreads,
    {$ENDIF}
    //  Next, as in the client: started from a snap's terminal, the server
    //  would look for the Python sidecar under the snap's XDG_DATA_HOME and
    //  hand the snap's environment on to it.
    snap_environment,
    ...
    //  POST /fit and the progress it publishes while it runs.
    stateless_fit,
    ...
    //  The server that keeps a connection open across requests - see the unit
    //  for what the stock one does instead, and what it costs the window.
    keep_alive_http_server;
...
var
    ...
    { What each running stateless fit has reached, for GET /fit/progress. }
    GFitProgress: TFitProgressStore;
...
begin
    ...
    try
        ...
        begin
            AResponse.Content := RunStatelessFit(ARequest.Content, GFitProgress);
            AResponse.Code := 200;
            Exit;
        end;
        //  ...and what it has reached so far, asked by the backend relaying it
        //  while that POST is still out. Served on another connection's thread.
        if IsStatelessFitProgressRoute(ARequest.Method, ARequest.URI) then
        begin
            AResponse.Content := StatelessFitProgressReply(GFitProgress,
                ARequest.URI);
            AResponse.Code := 200;
            Exit;
        end;
...
begin
    ...
    //  Its own file: the desktop client keeps fit_client.log, and two processes must
    ...
    GFitProgress := TFitProgressStore.Create;
    ...
    Server := TKeepAliveHttpServer.Create(nil);
    try
