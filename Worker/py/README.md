...
Everything goes through the pinned `requirements.txt`, installed into its own
virtual environment - never into a system Python. The steps, for every platform,
are in the user guide, [Setting up the Python engine](https://dvmorozov.github.io/fit/guide-fitting.html#fitting-python-setup)
(also in the application, under Help → Explain Everything).
With the environment in place, on macOS and Linux:

```
PY=~/.local/share/fit/py/sidecar/bin/python
$PY Worker/py/fit_backend.py --port 8788     # serve on http://127.0.0.1:8788
$PY -m pip install --only-binary=:all: pytest coverage   # test-only, once
$PY -m pytest -q Worker/py                   # its own tests
```

The environment is **not** in this directory and not in the checkout at all: it
goes to `%LOCALAPPDATA%\Fit\py\sidecar` or `~/.local/share/fit/py/sidecar`, so
every clone on the machine shares one copy and none of it is ever synced between
machines. `FIT_PY_HOME` overrides that location, for both the build script and
the server that launches the sidecar.
