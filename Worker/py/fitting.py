...
#  The least time between two progress publications. The solver evaluates the
#  model thousands of times a second and the desktop reads four times a second;
#  building an outcome for every improvement would cost the fit for nothing.
...
def _curves_out(seeds: list[list[dict]], params, with_errors: bool) -> list[dict]:
    """The curves in the outcome's shape, parameters in the order the seeds gave
    them. A shared parameter reports its single tied value on every curve.

    Shared by the final outcome and by progress, so the Pascal relay reads a
    progress reply exactly as it reads the result."""
    curves = []
    for i, specs in enumerate(seeds):
        pout = []
        for spec in specs:
            p = params[_lmfit_key(i, spec)]
            error = -1.0
            #  -1 when lmfit could not estimate it (fixed param, a non-finite
            #  covariance) - and always during the fit, when there is none yet.
            if with_errors and p.stderr is not None:
                error = _json_float(p.stderr, -1.0)
            pout.append({"name": spec["name"], "value": _json_float(p.value),
                         "error": error})
        curves.append({"params": pout})
    return curves


def _progress_reporter(seeds, publish, clock=time.monotonic,
                       interval=PROGRESS_INTERVAL_S):
    """An lmfit ``iter_cb`` that publishes the best parameters reached so far.

    Only an IMPROVEMENT is published - the native engine reports the same way,
    so the loss curve means the same thing whichever engine drew it - and no more
    often than *interval*. It returns None: a truthy return would stop the fit.
    """
    state = {"best": np.inf, "last": -np.inf}

    def iter_cb(params, iteration, resid, *args, **kws):
        score = float(np.sum(np.square(resid)))
        if not np.isfinite(score) or score >= state["best"]:
            return None
        state["best"] = score
        now = clock()
        if now - state["last"] < interval:
            return None
        state["last"] = now
        publish({"errorCode": 0, "rFactor": _json_float(score),
                 "curves": _curves_out(seeds, params, with_errors=False)})
        return None

    return iter_cb


def fit_problem(problem: dict, progress=None) -> dict:
    ...
    progress: called with an outcome-shaped dict as the fit improves, or None.
    """
    ...
    reporter = _progress_reporter(seeds, progress) if progress else None
    ...
    return {
        ...
        "curves": _curves_out(seeds, result.params, with_errors=True),
