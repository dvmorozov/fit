...
def test_main_announces_readiness_and_watches_the_lifeline(
        tmp_path, monkeypatch, restore_root_logging):
    """fit_server passes --lifeline-port. The sidecar connects, says it is ready
    only once its HTTP server exists, and blocks on the connection - so it never
    outlives the server, without asking every few seconds whether it is there."""
    events = []
    ...
    def fake_connect(port, host="127.0.0.1"):
        events.append(("connect", port))
        return sock
    ...
    class FakeThread:
        ...
        def start(self):
            events.append(("watch", self.target, self.args[0], self.daemon))
    ...
    monkeypatch.setattr(fit_backend.lifeline, "connect", fake_connect)
    monkeypatch.setattr(fit_backend.lifeline, "announce_ready", fake_announce)
    ...
    assert names[0] == "connect" and events[0][1] == 4242
    #  Ready only once the server exists - never before anything listens.
    assert names.index("server") < names.index("ready")
    watch = next(e for e in events if e[0] == "watch")
    assert watch[1] is fit_backend.lifeline.wait_for_parent_to_end
    assert watch[2] is sock
    assert watch[3] is True                  #  a daemon: it must not keep us alive
    assert names[-1] == "closed"             #  finally: server_close ran


def test_main_without_a_lifeline_connects_to_nothing(
        tmp_path, monkeypatch, restore_root_logging):
    """Started by hand, for development: no parent to announce to or outlive."""
    def refuse(*_a, **_k):
        raise AssertionError("connected without a lifeline port")
    ...
    monkeypatch.setattr(fit_backend.lifeline, "connect", refuse)
    monkeypatch.setattr(fit_backend, "ThreadingHTTPServer", FakeServer)
    assert fit_backend.main(["--port", "0",
                             "--log-file", str(tmp_path / "log.txt")]) == 0


def test_main_uses_the_connection_made_before_the_imports(
        tmp_path, monkeypatch, restore_root_logging):
    """The connection was made at import time, before numpy: main announces on
    it and watches it, and does not connect a second time."""
    events = []
    ...
    def refuse(*_a, **_k):
        raise AssertionError("connected a second time")
    ...
    class FakeThread:
        ...
        def start(self):
            events.append(("watch", self.args[0]))
    ...
    monkeypatch.setattr(fit_backend, "_EARLY_PARENT", early)
    monkeypatch.setattr(fit_backend.lifeline, "connect", refuse)
    monkeypatch.setattr(fit_backend.lifeline, "announce_ready",
                        lambda s: events.append(("ready", s)))
    monkeypatch.setattr(fit_backend.threading, "Thread", FakeThread)
    monkeypatch.setattr(fit_backend, "ThreadingHTTPServer", FakeServer)

    assert fit_backend.main(["--port", "0", "--lifeline-port", "4242",
                             "--log-file", str(tmp_path / "log.txt")]) == 0
    assert ("ready", early) in events
    assert ("watch", early) in events


def test_the_parent_pid_watcher_is_gone():
    """Polling a process id every two seconds is what the lifeline replaced."""
    assert not hasattr(fit_backend, "_parent_alive")
    assert not hasattr(fit_backend, "_exit_when_orphaned")
