...
function FitProgressSchema: TJSONObject;
var
    Sample, Curve, Snapshot: TJSONObject;
begin
    Sample := ObjectSchema('One improvement the engine reported.');
    AddProp(Sample, 'seq', Prim('integer',
        'Issued in order and never reused, across operations too.'));
    AddProp(Sample, 't', Prim('number', 'Seconds since the operation started.'));
    AddProp(Sample, 'value', Prim('number',
        'The total R-factor reached - the number the stats route reports ' +
        'once the fit is over.'));
    Require(Sample, ['seq', 't', 'value']);

    Curve := ObjectSchema('One curve of the model, under its handle.');
    AddProp(Curve, 'id', Prim('string',
        'The handle GET /problems/{id}/curves gives the same curve.'));
    AddProp(Curve, 'points', SchemaRef('PointsSet'));
    Require(Curve, ['id', 'points']);

    Snapshot := ObjectSchema('The model as it stood at one sample. Present ' +
        'only when snapshot=1 was asked for and one has been taken.');
    AddProp(Snapshot, 'seq', Prim('integer',
        'The last sample recorded when it was taken.'));
    AddProp(Snapshot, 'profile', SchemaRef('PointsSet'));
    AddProp(Snapshot, 'delta', SchemaRef('PointsSet'));
    AddProp(Snapshot, 'curves', ArrayOf(Curve, ''));
    Require(Snapshot, ['seq', 'profile', 'curves']);

    Result := ObjectSchema('How a running fit is going - or how the last one ' +
        'went, once it is over.');
    AddProp(Result, 'ok', Prim('boolean', ''));
    AddProp(Result, 'busy', Prim('boolean', 'A fit is running.'));
    AddProp(Result, 'elapsed', Prim('number',
        'Seconds it has run, or how long it took.'));
    AddProp(Result, 'nextSeq', Prim('integer',
        'One past the last seq issued: what to ask from next time.'));
    AddProp(Result, 'samples', ArrayOf(Sample, 'In the order they arrived. ' +
        'A long fit is thinned, keeping its first and latest sample.'));
    //  ABSENT UNTIL AN OPERATION HAS STARTED, which is why these are described
    //  rather than required: they are recorded when one starts, and an idle
    //  problem has nothing to name.
    AddProp(Result, 'lossName', Prim('string',
        'What the operation is minimising, in the words the user chose it ' +
        'by. Absent when none has run.'));
    AddProp(Result, 'engineName', Prim('string',
        'The minimiser running it, under its registered name. Absent when ' +
        'none has run, or when this build does not have that minimiser.'));
    AddProp(Result, 'snapshot', Snapshot);
    Require(Result, ['ok', 'busy', 'elapsed', 'nextSeq', 'samples']);
end;
...
begin
    ...
    Result.Add('FitProgress', FitProgressSchema);
