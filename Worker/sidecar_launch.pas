...
begin
    ...
    Result := '';
{$IFDEF WINDOWS}
    //  LOCALAPPDATA rather than APPDATA: the roaming one would carry a tree of
    //  compiled extensions between machines, where they mean nothing - they are
    //  built for one Python minor version on one platform.
    Base := ALocalAppData;
    if Base <> '' then
        Result := IncludeTrailingPathDelimiter(Base) + 'Fit\py';
{$ELSE}
    Base := AXdgData;
    if Base = '' then
    begin
