...
## Which HTTP client, where

Calls between the processes use the built-in `fphttpclient`, plain HTTP. The
client's only internet traffic - fetching data for a data source - goes through
`TWebClient` and `TCurlClient` (`Common/curl_client.pas`), which run the system
`curl`. See [The network boundary](architecture.md#the-network-boundary-two-transports-each-where-it-belongs).
...
The server documents itself. `GET /openapi.json` is an OpenAPI 3.0.3 document that the
running binary assembles from its own routing and its own registries, and `GET /docs` is a
Swagger UI page that renders it, with **Try it out** working against the same process.
Nothing is stored: a build's document is what that build answers, so it cannot be stale and
there is no file to regenerate.

```
./scripts/build-app.ps1 -Task api-docs    # build, start, open the page
curl http://127.0.0.1:8787/openapi.json   # or ask a server that is already running
```

Registering an action or a module resource makes it appear there with no edit to anything
that describes the API, because the enumerations are read from the registries at the moment
the document is built (`Worker/rest_spec.pas`, and the schemas in `rest_spec_schemas.pas`).

The table below is orientation - what each route is *for*, and why it has the shape it has.
...
| `GET /openapi.json`, `GET /docs` | this build's own OpenAPI document, and the Swagger UI page that reads it. The only reply the server sends that is not JSON |
...
| `POST /problems/{id}/actions/stop` | ends the running operation, keeping what it reached. The one **mutating** route answered without the problem's lock, because a stop that waits for the fit is a stop that never happens. See *Stopping a fit* below |
...
| `POST /fit`, `GET /fit/progress?id={progressId}` | the stateless fit a remote backend is asked for, and what it has reached while that POST is out (`found: false` before the first improvement). Answered in `fit_server` before the REST surface |
| `GET /problems/{id}/progress?since={seq}&snapshot=0\|1` | how a running fit is going: the R-factor samples recorded since `seq` (pass back `nextSeq`), the elapsed time, what is being minimised and by what (`lossName`, `engineName`), and with `snapshot=1` the model as it stood at a recent sample. Answered without the problem's lock. See *Live fit progress* below |
...
**Every route is tested at the first two levels: 23 of 23 classify, and 23 of 23
run through `HandleRoute`. The gap is the third.**

A fourth claim arrived with the self-description routes and is checked separately:
that what the document SAYS is what the server does. `testcase_rest_spec.pas` feeds
every documented path back through `RouteOf`, compares the enumerations against the
registries and the schemas against the keys live replies carry;
`testcase_rest_spec_e2e.pas` asks the started binary the same questions over a
socket, where the content type and the contents of the deployed build are visible
and nothing in-process can see them.

| route | routing | in-process | e2e |
|---|---|---|---|
| `rtHealth` | yes | yes | yes |
| `rtOpenApi` | yes | yes | yes |
| `rtDocs` | yes | yes | yes |
...
Once it is up, `http://127.0.0.1:8787/docs` is its own API documentation - see
[Ask the server, not this page](#ask-the-server-not-this-page) above.
...
Two things live there. The **polled routes** — `/problems/{id}/state`, `/async`, `/rfactor` and `/progress` — which
...
- the window **does not poll**. It re-derives what it offers - the commands, the Tools pane, the
  Model panel, the context menu - when something that can change them happens: an action, a
  selection or focus change, a model update the viewer reports, a server operation finishing, a new
  server URL. Each of those asks for one refresh, requests coalesce, and a request raised while the
  refresh runs is ignored (`deferred_ui`). A refresh that fails reports the failure **once**; the
  next event asks again.

A failed **download** is worded from curl's exit code (`CurlExitMeaning`,
`TransportFailureMessage` in `Desktop/DataSources/web_client.pas`): host not
found, nothing answering, a stall, or a secure connection that could not be
established - never "check your connection" for a failed handshake.
...
failure with `MessageDlg` directly — not `Application.OnException`, not an event handler, not the
...
One rule goes with it: `deferred_ui` admits **one** dialog at a time, so a fault on a path that
events drive repeatedly cannot stack them. Work that has to wait for an open menu - a dialog, a menu
rebuild, the window-state refresh - is taken up when the application next goes **idle**, which it
does once the menu's own loop has ended; nothing asks a few times a second whether it has.

The server is **threaded** (`Server.Threaded := True`), and each problem has a lock: the operations
that touch the engine are serialized, while the progress routes (`/state`, `/async`, `/rfactor`,
`/progress`) are deliberately **not** locked. The window reads `/state` on its UI thread every time it refreshes
what it offers, including while an operation runs - if that queued behind the operation, the client's
UI thread would block for the whole fit, which is exactly what froze the app.

### Stopping a fit

Stop is the only mutating route served without the problem's lock, and the only one whose purpose is
to reach a problem that is **busy**. Served the ordinary way it waited for the fit it was meant to
interrupt: the window froze for the rest of that fit - the request is made on the window's own
thread - and the reply, when it finally came, was "the calculation not started", the operation
having ended while the request queued. A button that did nothing slowly and then denied there had
been anything to do.

- The route sets the engine's own termination flag on the running tasks (`TFitTask.StopAsyncOper`)
  under `TFitService`'s task lock, and touches nothing else of the model - which is why it needs
  none of the protection the session lock gives. The task lock exists because this is now the one
  request that can arrive while the task list is being built.
- Every task checks the flag between iterations, so the fit ends where it stands and `DoneProc`
  runs as it does after any fit: **the curves it had reached are kept**. A backend fitting in
  another process finishes its current call and stops at the next check.
- With nothing running the action is still refused, which is the right answer to pressing Stop when
  there is nothing to stop.
- `tests/testcase_live_stop.pas` is the proof, and it has to be an integration test: it needs a fit
  really running in another process while the request goes in.

### Live fit progress

A fit holds its problem's lock from the first iteration to the last, so every route that reads the
model waits until it is over. That is what used to leave the chart empty for the length of a fit.
What a running fit shows instead comes from one place that is written by the fit and read by somebody
else at the same time:

- **The engine records it.** `TFitService.ShowCurMin` is reached on every improvement, by the native
  minimizer directly and by any other backend through the same `TFitTask.ShowCurMin` funnel, so it
  adds a sample (the total R-factor, the number `/stats` reports afterwards) to a
  `TFitProgressLog` (`Server/fit_progress_log.pas`). The log has its own lock, held for a copy and
  nothing more; samples carry a `seq` that keeps counting across operations, and a long fit is
  thinned, keeping its first and latest sample.
- **Snapshots are rationed.** A snapshot rebuilds the computed profile and copies every curve. It is
  taken only while a client has asked with `snapshot=1` in the last two seconds, and no more than
  twice a second. Nobody animating means no snapshots at all - the engine used to rebuild the model on
  every improvement for an animation nobody received.
- **The client polls on its window thread**, ten times a second, from a form timer - the same
  thread `/state` is read on, which is safe precisely because the route is unlocked. The interval
  matters: an ordinary fit is over in a few hundred milliseconds, so at a quarter of a second it
  drew no frames at all. The client also asks for the first frame BEFORE starting the fit, because
  the engine builds one only when it next improves and a short fit has improved already by the time
  the first poll lands. While a fit
  runs, `TFitClient` asks only polled routes: the status bar's time and R-factor come from the last
  poll, not from `/stats`, and an animated frame is drawn from the snapshot alone.
- **Nothing else on the window's thread may take the lock either, and one thing did.** The client's
  own rule - only polled routes while a fit runs - does not cover calls the client never makes. The
  window rebuilds the Model panel on idle; a rebuild re-establishes the selected row; that hands the
  row back to whichever module owns the panel; and a module that answers by asking the engine about
  the model waits for the fit's lock on the window's thread. The whole window then stops for the
  length of the fit - no repaint, no poll, no answer to the user - while the progress feature below
  works perfectly and nobody can see it. It was reported twice as "the application hangs and there
  is no animation", and it reproduced only in a build with a module registered, which is why every
  framework-only check passed. `action_state.ModelStructureMayBeRebuilt` is the rule: the panel is
  not rebuilt while an operation runs, and `Fit.lpr`'s `/CHECK_UI` measures the consequence by
  running a real fit and counting the frames the window drew.
- **Every engine reports through the same funnel.** A backend fitting in another process - the
  Python sidecar, or a compute server used as a remote backend - is sent a `progressId` with the
  problem and publishes the best outcome it has reached under it; `GET /fit/progress?id=` answers
  with it while the `POST /fit` is still out (`found: false` before the first improvement, never a
  404 for an unseen id). `Server/remote_fit_progress.pas` runs the POST on a helper thread and, on
  the thread that owns the task, applies each reply with the same `ApplyOutcomeToTask` the final
  reply goes through and calls `TFitTask.ShowCurMin` - so the service records it exactly as it
  records the native engine's. A peer without the route answers 404, which stops the asking and
  nothing else; the view then says the engine reports no intermediate progress. On the Pascal side
  the stateless fit and its progress route are `Worker/stateless_fit.pas`; on the sidecar they are
  `fit_backend.py` and `progress_store.py`.
- **The reply names the objective and the engine.** `lossName` and `engineName` are recorded in the
  progress log when an operation STARTS - on the thread that starts it - because the route is
  answered on another thread without the problem's lock and must read nothing off the engine as it
  replies. They are absent from a server that does not carry them, and the header then reads as it
  did before either existed. The header names both because the same model converges differently
  under another objective or another minimiser, and a convergence chart that does not say which is
  being watched is a number without a unit.
- **What is shown** is decided in `Desktop/fit_progress.pas`: the data stays drawn under a header the
  moment Fit is pressed; once samples arrive, a chart of log10 R-factor against elapsed time replaces
  the model's chart; with View > Animation Mode ticked the model is redrawn instead; an engine that
  reports nothing is said to report nothing, with the clock still running.
...
**Where the script is looked for** (`Worker/sidecar_launch.pas`, probed by
`python_sidecar.LocatePython`): the development layouts around the running binary
first, then `py/` beside it for an installed tree - and finally, `Worker/py` of each
repository checked out **beside** the binary's own. That last rule is what a module
build needs: a pack is its own repository and keeps no file here, so its `fit_server`
runs from `<pack>/Worker/o` with the framework's script nowhere above it. It mirrors
the search `fit_backend.load_module_routes` already does in the other direction, for a
module's `<name>_routes.py`.

The sidecar is bound to fit_server's lifetime by a **lifeline**, not by a process id
someone keeps asking about (`Common/readiness_channel.pas`, `Worker/py/lifeline.py`):

- fit_server listens on a loopback port of its own and starts the sidecar with
  `--lifeline-port N`;
- the sidecar connects **at once**, before its slow imports - so a sidecar that dies
  importing closes the connection and fit_server stops waiting immediately - and writes
  `ready` once its HTTP port is bound;
- fit_server keeps the connection open for as long as it owns the sidecar. When fit_server
  ends, cleanly or with a `kill -9`, the operating system closes it; the sidecar's blocked
  read returns end of file and it exits.

The same channel tells a launcher that `fit_server` is up: `fit_server --ready-port N`
says `ready` once its accept loop is running, and `fit_server --start-detached` starts a
detached server and returns only when it has - which is what the shell launchers and
`fit_launcher` run, instead of probing `/health` until something answers. The sidecar
listens only on loopback; the client never addresses it.
