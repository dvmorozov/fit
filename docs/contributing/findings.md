...
## The selected curve type is not the model

A wave count over a price series was captioned **2\*Theta [deg]** along the
bottom and **Intensity** up the side. The first fix for the argument axis had
moved its default from a global setting to "the axis the selected curve type
defines" - and the selection is what the Tools list has highlighted, which is
what the user will place *next*. The program starts with a diffraction peak
highlighted, so a model holding nothing but wave patterns was still drawn in a
scattering angle. Every test passed: they selected a type and asserted its
caption, which is exactly the path that was right. The value axis had no default
at all - "Intensity" was a word written into the form.

The same shape had already been found once, in the Model panel ("The Model panel
shows what the model holds, not what the Tools list selects"): **the selection
is an intention, not a fact about the model.**

**The rule** (`axis_choice`, per coordinate): the model's curves when they
agree, then what the data says it is - its format's registration or its reader
- then what the curves assume when the data says nothing (a wave count assumes
bars), and the selection only while the model is empty. `.dat` declares nothing:
any two-column series is saved as one, price samples included, and declaring
2 Theta for it brought the defect straight back. A curve type answers only
for a coordinate it really knows: a wave pattern's value is a price, but whether
its argument is a bar or a date is the data's to say. The red test is in the
pack's suite, over the real client and server, because the defect was in the
join: the framework's rule was right for what it was told, and the pack's
patterns told it nothing.

**And what the data said is saved in the project.** A project stores the
profile's points, not the file - nothing reads the file again - so without it a
reopened price series would lose its dates.

## The window keeps a pointer to a list the client frees

`TFormMain.FCurveList` points at the client's curve attributes, and the client
frees and replaces those on every refresh. The pointer is re-aimed only when the
new list is tabulated (`TFitViewer.FillCurveTable`), which an emptied model never
is - so between a refresh and the next tabulation it names freed memory.

For years nothing read it in that window except a menu click on an axis. When the
axes began to follow the model, the state refresh started re-applying them after
model changes, which would have read the freed list; `ApplyAxes` now asks the
client for its list as it is. The pointer is gone since: every reader - the cell
colouring, the deletion, the selection - asks the client through
`CurrentCurveList`, so there is no copy to go stale.

## `-Task check-ui` dies in its live-fit step on macOS with classic scroll bars

An access violation in the LCL's Cocoa widget set,
`TASyncLCLControlAdjustSizer.DoAdjustSize` (cocoawsscrollers.pas): one global
object remembers the LAST control asked to resize its scroll view and queues the
resize for later, so a control freed before the queue runs is resized after its
death. It happens only when macOS draws non-overlay scroll bars (System Settings
> Appearance > Show scroll bars: Always, or a mouse attached). Found 2026-09-24,
and reproduced on the unchanged tree - it predates the axis-mode work. The fault
address alone symbolises wrongly; use the image base in the macOS crash report
(`~/Library/Logs/DiagnosticReports/Fit-*.ips`) with `atos -l`.

The control it named was the data-source wizard's: `ReportDataSources` builds that
window to measure its captions and frees it without the message queue ever
running, so the queued resize outlives it and the next check that lets the queue
run - the live fit, or the readout check before it - dies. The window now lets the
queue run before it is freed. **Any window built and freed in one breath has the
same exposure on macOS.**
...
## Every download failed on macOS, and the message blamed the network

On a Mac every data source failed with *"Connect to fred.stlouisfed.org:443
failed.. Check the address and this computer's connection"*, while `curl`
fetched the same address.

The client used `TFPHTTPClient` with `opensslsockets`, which loads OpenSSL by
name at the first https request. Free Pascal 3.2.2 knows no OpenSSL 3 name, and
on Darwin the first name that resolved was Apple's legacy `libssl.0.9.8.dylib`
(TLS 1.0 at most). It loaded, so no "could not initialise OpenSSL" error came;
the TLS 1.2 handshake failed, and `ssockets` reports a failed handshake with the
same text as a refused connection. Linux and Windows lack OpenSSL 3 names in
3.2.2 as well; whether they failed the same way was not verified.

**Why nothing caught it.** Every web-client test replaced `Transfer`, the one
method that opened a socket. No test made a real https request.

**The fix.** Downloads run the system `curl` through `TCurlClient`; OpenSSL is
gone from every build and package, and curl's exit code is mapped to a kind
(`CurlExitMeaning`) so a failed handshake is worded as one.
`testcase_web_transport.HttpsReachesAServerThatRequiresTls12` failed with the
user's message before the fix; `network_boundary.tests.ps1` keeps the rule.

**The rule.** A transport seam is where unit tests stand; at least one test must
still drive the real transport, on the platform that ships.
...
- **An axis is display-only, for either coordinate.** It converts a stored
  coordinate for the user and supplies the caption. It never alters stored data
  or the fit.
- **A point with no place on an axis is NaN, and the chart leaves a gap.** The
  logarithm of zero is the case that forced it. A clamp would draw a point that
  is not in the data; raising would stop the whole plot over one sample. TAGraph
  keeps the point, so indices still match the data (picking is by index), and
  `PointIsDrawn` leaves it out of lines, markers, captions, extents and the
  pointer's snapping. A band series (a fit interval) is never given a y
  transform: its y is not a value, and on a log axis the stripe would vanish.
- **An amplitude is not carried through the value axis.** It is a height above
  whatever lies beneath the curve, not a point on the vertical axis, so a
  logarithm of it would not be where the curve is drawn. Positions are carried
  through the argument axis because a position IS an argument.
...
## The Python component was "not started" on a machine where it works

Detect answered *"The wave detector returned a reply this version cannot read"*.
The server log said something else entirely:

```
--> POST /problems/1/modules/<module>/decompose
Python sidecar unavailable (not configured or failed to start)
<-- 503 ... { "ok": false, "error": "This feature needs the Python component,
   which could not be started. Set it up in Worker/py: ..." }
```

Two independent defects, and each one alone was enough to make the feature
unusable.

**The sidecar could not be located in a module build.** `LocatePython` looks for
`fit_backend.py` around the running binary. A pack is its own repository and
keeps no file in this one, so its `fit_server` runs from `<pack>/Worker/o`, where
the script is not above it, not beside it, and not under it - it is in the
framework repository *next door*. Every private build therefore had no sidecar at
all, on machines with a perfectly good venv. The Python half already solved the
same problem in the other direction: `load_module_routes` searches each
repository beside its own for a module's `<name>_routes.py`. The Pascal half now
does the reverse - `SidecarUmbrellaCandidates` and `SidecarSiblingCandidates` say
which paths, `python_sidecar` does the probing - and one of the depths is the one
a *test* binary runs from, which is why the pack's real decompose test now runs
instead of ignoring itself for a missing venv.

**The refusal was thrown away in transit.** The module channel deliberately does
not go through `CheckedResponse`: a module's document is its own vocabulary and
the framework must not parse it. But a refusal is not that document - it is the
same `ok:false` envelope every other route answers with, carrying the sentence
the server wrote for the user. Dropping it left the caller holding a body it
could not parse as its own, so the only thing it could say was that the reply was
unreadable. `ModuleReply` now claims exactly the refusal and nothing else: a
document that says nothing about `ok` is passed through untouched.

**The shape:** the good message existed, was written for exactly this situation,
named the remedy, and reached a log file nobody was looking at while the user
read a sentence that named nothing. A generic "cannot read this" over a channel
that carries other people's documents should be suspected of hiding a message,
not of describing one.

## A pack's redraw emptied the selection on a model it had no part in

`check-ui-pro` said *"after the chart was replotted, the window held no selected
curve"*. The public build passed the same check. The difference was a pack that
pushes its Model-panel rows on every redraw, and nothing in the window asked
whether those rows described the model on screen. With a type placed by picks
selected, a replot redrew the pack too. Its push, empty because it had nothing
placed, replaced the framework's rows and the selection on them.

The contract already said the answer. `IUiModule.PanelId` is the module's own
`PlacedByPointSet`, so the window can tell whether one of its types is selected.
But the pack had named its panel with a word of its own, and the window never
made the comparison. Now it does (`ModulePanelRowsShown`). Rows from a module
whose type is not selected are held (`HoldPanelRows`) and shown the moment it is
(`HeldRowsToShow`). The pack names its panel by its point set, with a test that
pins the two together.

Two follow-on defects came out of the same change, and both are fixed with tests.
Rows were first kept only while hidden, so switching away and back showed an older
push. `PushPanelRows` now keeps every push. A report's "show" link to a row that
was kept, not deleted, said the part was "no longer in the Model panel".
`RowLinkMissHint` now tells the user to select one of its types.

**The shape:** a contract stated in a comment, and nothing that relied on it. The
only check a comment gets is the code that depends on it, and this one had none
until the window started comparing.

## Open Project froze the window, and not one line of it was Fit's

File > Open Project left the window "not responding" for good. Neither log had a
line after the dialog was asked for, and reading a project was fine: the same
files opened with `/PROJECT=`. A backtrace, taken under `gdb` because
`ptrace_scope=1` refuses an attach, was all KDE:

```
TFormMain.AskProjectToOpen -> TOpenDialog.Execute -> QFileDialog_selectNameFilter
  -> KDirOperator::checkPreviewSupport -> KSycoca::ensureCacheValid
  -> KBuildSycoca::recreate
```

The client had been started from the terminal of VS Code installed as a snap, and
a snap's children inherit the snap's environment: `XDG_DATA_HOME` and
`XDG_DATA_DIRS` under `~/snap/code/<rev>` and `/snap/code/<rev>`, its GTK and
locale paths, and `SNAP`. KDE keys its service cache on the data directories, and
for that set it could never be built - `kbuildsycoca5` ran two and a half minutes
and died on `std::bad_alloc` - so every file dialog rebuilt it on the GUI thread.
With the desktop's directories the rebuild takes 0.2 s. The server was exposed
the quiet way: it looks for the Python sidecar under `XDG_DATA_HOME`.

**The first fix was in the wrong place.** It repaired the environment in
`-Task run`, from VS Code's own record of what it replaced (`*_VSCODE_SNAP_ORIG`)
- one editor, one launch path. `Common/snap_environment` now does it inside both
programs, named first in their `uses` so it runs before `Interfaces` creates the
`QApplication`.

**The second fix waited for a variable the user's terminal did not have.** It
acted only when `SNAP` was set, and was verified from a shell that carried it.
The user's VS Code terminal had had `SNAP` and every `SNAP_*` removed while the
snap's directories stayed in everything else, so the repair did nothing and the
window hung as before. A snap's directories are now recognised by where snapd
puts them - `/snap/<name>`, `/var/lib/snapd/snap/<name>`, `/var/snap/<name>`,
`~/snap/<name>` - with the host's `/snap/bin` and `/var/lib/snapd/desktop` kept;
`SNAP*` goes, a path list keeps the rest in order, and a Fit that is itself a
snap keeps its environment. The test that pins it is built from the hung
client's own environment, read from `/proc`. Verify from the environment the
user runs, not from your own shell.

**Two copies of one environment.** Qt reads libc's `environ`; FPC's
`GetEnvironmentVariable` and `TProcess` read `System.envp`. A repair only one of
them sees fixes the window and not the sidecar, or the reverse. The tests read
both.

**The shape:** a hang with nothing in the logs is a hang outside the code that
writes them. The console's `kfilefiltercombo: Could not find filter` warning
looked like the lead and was noise - KDE rewriting a correct Qt filter into its
own `mask|description` form.

**And the build hid a stale binary.** Between the two fixes a rebuild left the
previous executable in place - its `Fit.compiled` said `Complete="False"` - and
`Build-Client` reported nothing, because it took lazbuild's exit code as the
whole answer. It now checks the file: the server must be rewritten by its
build, and the client, which lazbuild may rightly skip, must be newer than
every unit source (`Confirm-ClientBuilt`, `Get-NewestSource`). Project files are
left out of that comparison because lazbuild saves the `.lpi` after linking.

## A nested model showed as a flat list while an ordinary curve type was selected

A pack placed a pattern count - an impulse with a diagonal nested as its third
wave - and the Model panel listed the five patterns flat, captioned the way the
framework captions a curve ("Impulse (5)  at 76"). Selecting a pack type in the
Tools list brought the tree back. The fix for the entry above had made the
panel follow the **selected type**: a module's rows were shown only while one
of its own types was selected. But the selected type says what the user will
place *next*; it says nothing about what the model *holds*. Choosing a curve to
add was enough to hide the model's hierarchy.

The panel now follows the model (`model_outline.ComposePanel`). A module's
latest push is shown whenever it has rows, whatever is selected. After it come
the framework's rows for every curve no row of it stands for, matched by
`CurveId`, so a model holding curves of several kinds hides none of them. What
the earlier fix protected still holds, because an empty push describes nothing:
a module with nothing placed never displaces the framework's rows. The selection
now only breaks a tie between two modules and picks whose "nothing yet" an
empty panel shows. Rows under an id no module declares, such as the self-check's,
are shown once and not kept (`KeepPanelRows`).

**The shape:** a fix keyed on the nearest available signal rather than on the
fact in question. "Which type is selected" was easy to compare. "What does the
model hold" was the question.

Three follow-on defects came out of the review of that change, each fixed with a
test that failed first in the running window (`-Task check-ui-pro`):

- **Every refresh drew the panel again.** The old rule had skipped a rebuild
  for a module's rows already on show; composing dropped that, and the panel is
  refreshed on every command refresh. A pattern the user collapsed opened again
  at once, and the selection was handed back to the module each time. The window
  now draws the panel only when what it would draw differs
  (`model_outline.SameOutline`).
- **Rows could outlive their model.** Nothing cleared a module's kept rows when
  a project was closed. Rather than a hook every module must remember to call,
  a push counts only while one of its rows names a curve the open model holds
  (`DescribesTheModel`). When a module pushes, the framework's rows are rebuilt
  first, so a pattern placed a moment ago is judged against the model it is in.
- **The framework's rows were built where no test could reach them.** They now
  come from `model_curve_rows`, which the pack's end-to-end tests call to
  compose the panel the way the window does, with the pack's own redraw
  supplying its half.
- **A module's push was drawn while a fit ran.** The framework's refresh waited
  for an operation to end (`ModelStructureMayBeRebuilt`); a module's push did
  not, and drawing hands the selected row back to the module, which may wait on
  the fit's lock with the window behind it. The push is now kept and drawn by the
  refresh that follows the operation. The self-check pushes rows during its real
  fit and asserts both halves.
- **Not redrawing lost a curve selection.** A refused Delete curve clears the
  selected curve and leaves the row; the old per-refresh redraw had re-selected
  the row and so its curve. When the panel is not redrawn, the curve the selected
  row stands for is now selected again (`CurveToReselect`).
- **Rows carrying no handle at all are shown, not judged.** Such a push cannot
  be checked against the model, so it is shown as every push was before
  (`DescribesTheModel`).
- **The row menu is chosen per row.** Rows listed after a module's tree are the
  framework's, so they ask every module, as the framework's own panel does
  (`RowMenuPanelId`). Covered by unit tests only: with one module, no window
  reading can tell the two apart.

## A model holds curves from one placement path at a time

**Open - left as it is for now, recorded for review.** Found while testing the
Model panel against a reopened project.

**What the user sees.** A module places its curves from its own markup - an
analysis pack's patterns, say. With one of that module's types selected they are
built and shown. Select an ordinary curve type (one placed by picks), and they
stay on screen - but only until the next edit that rebuilds the model. The first
such edit, adding a fit interval bound for instance, removes every one of them
without a word. Measured through the real server: two curves placed; two after
the ordinary type was selected; none after one fit-interval edit, and none left
in the module's own drawing either. A project saved while the ordinary type was
selected reopens the same way, with the markup's curves missing.

**Why it happens.** `TFitTask.RecreateCurves` builds the model along one of two
paths, chosen by the selected curve type's `PlacedByPointSet`: a type placed
from a point set has that set's registered builder make every instance, and
any other type builds curves of itself from the picks. The markup is consulted
only on the first path, so under an ordinary type nothing rebuilds what the
markup placed. Reopening restores the settings, the selected type among them,
before the modules' state (`rsSettings` before `rsModules` in `TRestoreStep`),
so it takes the second path too.

**Why it is built this way, as far as the record goes.** The code started with
one path - curves of the selected type from picks - and the point-set path was
added beside it as an either/or. The only reason recorded for that shape is the
comment in `RecreateCurves`: deciding from the class rather than from an attempt,
because falling through to the picks path with nothing marked built one curve
per data point, which the user saw as a hang. Nothing found says that curves
from markup and curves from picks were meant never to share a model; the rest of
the framework already assumes they may (the Model panel, the parameters grid,
`AGENTS.md`'s "a model may hold curves of different types").

**Options, for the review.**

- **Both paths on every rebuild.** Build what every module's markup places,
  whatever type is selected, then the selected type's curves from the picks.
  This makes "a model may hold curves of different types" true of the engine,
  but it changes how every model is built - the one-curve-per-point guard, the
  pruning keyed on one x per curve, the identity of instances across rebuilds -
  and needs its own design before code.
- **One path, said out loud.** Keep the either/or, and have an edit that would
  drop markup-placed curves say so first and allow cancelling (non-negotiable
  7); have reopening select the type its markup needs. Small, and it ends the
  silent loss without deciding the larger question.

The Model panel's end-to-end test of a reopened project saves while the
module's own type is selected for this reason, and says so where it does.


## A coverage figure with nothing saying which tests produced it

The report `tools/coverage/coverage-project.md` ends in `## Total for this
project's logic: 26252/27704 = 94.76%` and said nowhere that it was measured over
the **unit half alone**. The committed baseline carried the sentence - "taken over
the UNIT suite only" - because `Invoke-CoverageBaseline` writes that header itself;
the report it is copied from did not. So the gated number read as the project's
coverage to anyone who opened the file, and the full figure (the union of both
halves, 96.97 % on the same denominator) sat unquoted in `coverage-union.md`.

Two things around it made that worse rather than better.

**The menu offered four sibling entries whose labels all began "Coverage"** -
Coverage, Coverage union, Coverage gaps, Coverage baseline - with no indication of
which produced a file, which read one and which rewrote a **committed** one. They
sat in that order, so the promotion was one keypress from the two read-only
entries. A session that meant to measure ran the promotion: `baseline-project.md`
moved from 94.13 %/22843 lines to 94.76 %/27704 and was left uncommitted in the
tree, raising the floor for every later run. Nothing in the run said the file was
tracked, and nothing asked.

**The first entry asked "Scope [1]" after it was chosen**, so which scope had been
measured was a prompt answered seconds earlier and never echoed again - and the
default was the narrow scope that is almost never what someone wants.

**What was done.** Every report now carries a `Measured:` block under its title,
naming the suite and, for the gated ones, saying in as many words that this is not
the project's full coverage and where the full figure comes from
(`Get-CoverageProvenance`, a pure function over scope and "did the integration half
run", so the wording is tested without measuring anything). The union announces its
first stage as stage 1 of 2, because that stage's output is otherwise
indistinguishable from a finished gated run - and a union that dies in the hour
that follows leaves exactly that on screen. The menu has one `Coverage` entry whose
steps are ordered measure, read, promote, each saying what it writes; the scopes are
entries rather than a prompt. Promotion asks for the word `promote` to be typed
(`Confirm-TypedName`, the guard already used before deleting a remote) **before** it
measures - afterwards the answer is "well, it is measured now".

**The general shape**, which is the reason this is written down: a number that does
not carry its own provenance will be read as the most flattering thing it could
mean, and a destructive step beside read-only ones will eventually be chosen by
someone reaching for its neighbour. Neither was a bug in the measurement, and no
test could have failed - the arithmetic was right the whole time.

## The toolbars were never spaced, and three checks could not have said so

The window's toolbars ran at the widget set's own metrics: a button barely larger
than the 16 px icon in it, the first one hard against the bar's edge. The icons
therefore read as one unbroken block - nothing marks where one command ends and
the next begins - and a pointer aimed between two of them still hits one, because
there is no gap to miss into. Every headless suite was green, `check-ui` measured
every caption against its control and found nothing, and the screenshot that
prompted the change had been in the repository for months.

**Why nothing could have caught it.** It is not a wrong value anywhere; it is the
absence of a decision. There was no number to be wrong. The spacing now comes from
`Desktop/toolbar_metrics.pas` - icon, padding, margin, and the smallest target a
pointer can be expected to hit - which `testcase_toolbar_metrics` asserts without a
display, including that the default padding really does clear that target, written
against the constants so editing either of them into a button the user would miss
fails by name.

**Applied by walking the window's components, never by naming bars.** There are
two places bars are declared - `form_main.lfm` and `BuildZoomToolBar` - so a list
of names here would be a list the next bar is not on. What can still go wrong is a
bar built somewhere that walk does not reach, which looks exactly like an ordinary
toolbar; that is what `ReportToolBarSpacing` asks in the running window, and
`-Task check-ui` now requires its verdict by name like the others.

**Then three more, each invisible to the rule before it.** The bars kept their
own borders - a `TToolBar` draws one on all four sides - which read as the frame
around the buttons while they sat against it, and as a rule floating above the
first button once they did not. They were also given an `Indent` on top of the
margin, and a toolbar's indent applies to the leading edge and to nothing else,
so that one edge was twice the other three: a vertical strip sat further from its
panel's side than from its top. And with every bar then passing every rule, the
main one still looked wrong - its panel, alone among them, kept a bevel, so its
buttons sat two pixels further in than the rest.

**The last one is the general lesson.** What a reader compares is one bar with the
next, and the odd one out is the only one anyone notices - so the check now judges
the bars against each other as well as against the numbers, and prints what each
one measured whether or not anything is wrong. Pixels in a screenshot cannot tell
a bar's margin from the bevel of the panel it sits on; the widgets can, and they
were never asked until they were.

**And the trap inside the check.** Written the obvious way - scale the constants
again at check time and compare - it reported every one of the seven bars as
unspaced. This window is built before its font is final, so the same
`Scale96ToFont(4)` returned 4 in `FormCreate` and 5 by the time the check ran. A
check that re-derives what it is checking fails a window that is exactly as it was
asked to be, so the metrics that were applied are kept and the check is made
against those. The general shape: a self-check must read what the program did, not
recompute what it should have done.

## `prerequisites` installed a Chrome the diagram gate cannot use

The diagram gate launches a browser through puppeteer, and puppeteer launches one
exact build out of its own cache, by version - never the machine's Chrome, however
many there are on it. `Confirm-PuppeteerBrowser` asked only whether *a* chrome was
listed, so a cache holding the build an older puppeteer had pinned satisfied it and
nothing was fetched. Two script tests then failed with "Could not find Chrome (ver.
153.0.8010.36)" on a machine that had just been through the prerequisites task,
with a working Chrome in `/usr/bin` and 151.0.7922.47 in the cache.

**What was done.** The installer is asked for unconditionally: it is the only thing
that knows which build this puppeteer wants, and it costs a path being printed when
that build is already there. The version is deliberately not parsed here - a second
place that knows the pinned version is a second place to drift.

## A coverage report answered about yesterday's sources and said so nowhere

`-Task coverage-gaps` reads the measurement already on disk, which is what makes it
cost seconds and is deliberate. But a unit written since that measurement was taken
is not in it, and the report does not mention the file at all - so "nothing
uncovered in what you just wrote" and "what you just wrote was never measured"
print the same thing. That happened on the change above: `toolbar_metrics.pas` was
simply absent from the report, and the absence read as a pass.

**What was done.** `Get-SourcesNewerThan` compares the counted roots against the
measured binary's timestamp, and the task refuses - naming the files and the
command to run - rather than warning. This is the one task run to decide whether
new code is covered, which is the single question a stale measurement answers
wrongly.

## A registry a test could not take a source back out of

The completeness walk over the data sources walks *the registry*, so the
framework's own tests registered three deliberately broken sources to see what
each is reported as. The registry has no way to unregister one - nothing in the
application ever needs it - so those probes stayed in it for every later test in
the process, and the next walk reported faults nobody had put in the build. It
surfaced as a **module's** suite failing over three sources that module had never
heard of.

**What was done.** `FindingsFor` takes the sources to walk; `DataSourceFindings`
is the one-line wrapper that passes the registry. A test that wants to see what
an incomplete source is reported as now hands one over instead of installing it.
The registry still has no unregister, and still needs none.

## A test that registered one guide chapter reordered the user guide

The same walk needs an explanation topic to resolve, so a test registered the
guide's Data chapter to get one. Chapters reach the index **in registration
order**, and registering one early put Data before the chapter that precedes it -
failing a test in a different file with "Data comes after the chapter before it",
which reads as a defect in the guide rather than in the test that caused it.

**What was done.** The walk takes a resolver, so the chapter is looked up
directly instead of being registered; where a test really does need the registry,
it registers the chapters **the way the application does**, all of them, in the
application's order.

## RRUFF's data endpoints are gone, and a source was not written against them

A mineral spectra source was planned for `Modules/open-data-spectra`. Every
documented `rruff.info` path now answers 404 - the host serves a different
database entirely - so there is nothing to write a reader against (checked
2026-09-23).

**What was done.** Nothing was written. A scraper aimed at a service that no
longer answers would have shipped a source that can only fail, and its failure
would have read as a defect in this program. The module ships the NIST WebBook
source instead, and roadmap §7 records why RRUFF is absent.

## A price feed that now requires a browser

Stooq was to be the pack's OHLC source. Its CSV endpoint now answers with a
JavaScript proof-of-work page that a browser solves before the data is served, so
no HTTP client can fetch it (checked 2026-09-23).

**What was done.** Not implemented, and said so rather than shipped broken. FRED
is the pack's source: keyless, CSV, one stable address per series, and the shape
the price loader already reads - a date column and a value column.

## A window built in code was the one window nothing measured

`ReportClippedCaptions` walks the forms that EXIST, which is every form the
streaming created - and the data source wizard is built in code when its menu
entry is chosen, so it existed only while a user was looking at it. Its captions
were therefore the only ones in the application no check could reach.

**What was done.** The measurement was split: `ReportClippedCaptionsOf` takes one
form, and the self-check builds the wizard, measures it and frees it. It reported
two clipped buttons on the first run - "Create Project" and "Back to results" in a
`TButton`'s default width - which is exactly the kind of thing that otherwise
ships and is noticed by a user. Every button in that window now sizes to its own
caption, which is also what keeps it right in another language.

## Three layout defects the caption check could not see

The window built in code was measured by the self-check (above) and reported
nothing wrong once its buttons sized themselves. Then somebody opened it. It had:

* **two rails.** Choosing a source called `BuildRail` again, which CREATED a
  second list box; the first stayed on the form, taking a fifth of the window
  and pushing the query fields off the right-hand edge;
* **fields at absolute positions.** Left 150, width 420 - correct in a window
  nothing else takes width from, and off the edge in the real one;
* **buttons backwards.** With `alRight` the FIRST control created is the
  LEFTMOST, so creating Cancel first left the row reading Cancel, Create
  Project, Next, Back.

**Why the check missed all three.** It measures each caption against the control
holding it, and every one of these controls was wide enough for its own text. A
caption check cannot see a control in the wrong PLACE, a duplicate control, or a
row in the wrong ORDER.

**What was done.** The rail is built once and only its text changes; each field
is a band that fills the page, with the caption on the left and the editor taking
what is left; the buttons are created in reading order. And the window is now
rendered headlessly and LOOKED AT - `xvfb-run` plus `TCustomForm.GetFormImage`
writes a PNG of the real form, which is how the remaining two were found (a hint
drawn above the box it describes, since controls aligned to the top are ordered
by their `Top` and not by creation; and a hint cut off mid-sentence in a row too
short for two lines). That costs a minute and answers the one question no
assertion in this repository was asking: what does it look like?

## A service answered nothing because of the User-Agent, and the reader took the blame

The first real use of a data source failed: FRED, series SP500, a six-year
range - "The CSV file is empty." The data was not missing. The same address
answered 29,345 bytes to `curl` a second later.

**What was happening.** The client sent `User-Agent: Fit data source client`.
FRED's front door answers that with NOTHING AT ALL - no status, no body, a
connection left open until the timeout - and the empty body was saved, read, and
reported by the CSV reader as an empty file. So a working service and a correct
request reached the user as "the series you asked for does not exist".

Measured, not guessed:

| User-Agent | Answer |
|---|---|
| `Fit data source client` | hangs until the timeout |
| *(none)* | hangs until the timeout |
| `Mozilla/5.0`, `Mozilla/5.0 (X11; Linux x86_64) Fit/1.0` | hangs until the timeout |
| `curl/8.5.0` | 200, 29 KB |
| `Fit/1.0 (+https://dvmorozov.github.io/fit)` | 200, 29 KB |

**What was done.** The agent takes the conventional robot form - a product, a
version and a contact URL - and a test pins that SHAPE rather than the text, so
a future edit cannot quietly drop the part that matters. And the fetcher now
refuses an empty answer itself, in words naming the address, instead of saving
it for the reader to fail on: a service that answers with nothing is a fact
about the service, not about the file.

**The lesson worth keeping.** Every layer below reported exactly what it saw and
the sentence the user got was still false, because the only layer that knew the
request had succeeded *and* that the answer was empty was the one that had been
given no opinion. A failure has to be named where it is understood.

## The window froze for as long as the service took

The download ran in the window's own thread. Nothing repainted while it ran, the
Cancel the transport offered could not be pressed - there was no moment to press
it in - and a service that answers slowly (or not at all, as above) left the
application looking hung for the whole timeout.

**What was done.** `download_job.pas` runs the fetch in a thread and keeps what
happened rather than raising out of `Execute`, which would take the process with
it. The window waits on it, draws what has arrived, and turns Cancel into Stop
for the duration. The job's branches - a failure kept, a cancellation kept apart
from one, finished not true until everything is written - are tested by calling
`Execute` directly on a job that was never started, which is how a thread's own
code is reachable without timing.

## A download that lands where the user cannot name it

The wizard saved into a per-user folder and said so only on a step the user
walks past: the window afterwards showed the file's NAME and nothing else. What
they were left saying about their own data was "it stored it somewhere and I
don't know where" - which is also what makes a second, stale copy
('SP500 (2).csv') appear without any explanation.

**What was done.** The preview step carries a **Save in** row: the folder in
full, and a Change button that MOVES what was just fetched rather than asking
the service for it again - it is the file they have just looked at, and a second
request is both slower and a different answer once the series has moved on. The
choice is remembered between sessions, so it is a row to read rather than a
question to answer every time.

## An empty status line during a long fetch is a hung application

Fetching ran in the window's own thread. A service that takes tens of seconds -
FRED building a CSV, or anything behind a slow link - left the window drawing
nothing and answering nothing, which is indistinguishable from a crash to the
person looking at it.

**What was done.** `download_job` runs the fetch in a thread; the window waits on
it, drawing a spinner, how many kilobytes have arrived, and - after three
seconds - how long it has been and that Stop gives up. The spinner matters more
than the numbers: most services send no content length, so there is no
percentage to show, and a line that never changes reads as a frozen one.

## The compute server contained the entire user interface

The engine linked the main window. Not a helper from it - `form_main` itself,
297 code symbols, every dialog behind it, the desktop client class, its tables,
its series, and in time everything the window came to use. In a program that
draws nothing.

Two lines did it, and each had a guard that was not one:

* `Server/fit_task.pas` uses the user-defined curve, which reaches
  `configurable_user_points_set`, whose adapters include
  `curve_type_storage_adapter` - and that unit's implementation says
  `uses form_main`, because saving a user curve adds a menu entry. A guard was
  there: `{$IF NOT DEFINED(SERVER)}`. **No project defines SERVER.** Every one
  of them sets FIT, FITCLIENT and FITSERVER, so the guard never fired and
  nothing said so.
* `Server/fit_service.pas` had `uses fit_server_proxy` and never named the type.
  That proxy forwards to `fit_client_stub`, which casts to `TFitClient` - so one
  unused entry in a uses clause linked the desktop client into the engine. The
  only code that constructs either is a test.

**What it cost:** 25 MB of binary where 10 MB was the program. The LCL, the
grid package and the charting package were required by the server's project
file to satisfy it, and `fit_server.lpr` named `Interfaces` to initialise a
widget set it had no use for.

**What was done.** The guard names `FIT_NO_UI`, which the server projects
actually define; the unused `uses` is gone; `Desktop/app.pas` stopped naming
`Forms` it never used (that was how a module's front door pulled the LCL into
the private server); and the three user-interface packages and `Interfaces` are
out of both server projects. The public server is **10.3 MB** and the private
one **14.1 MB**, with zero widget-set code symbols in either.

**The rule this leaves.** A define nothing sets is a guard that is not there,
and nothing in a build will tell you. `tools/build-tests/server_without_ui.tests.ps1`
now asks the questions a build cannot: the server projects require no
user-interface package, the program names no widget-set unit, the guard names a
define the projects really set, and - when a server has been built here - the
binary carries no window.

## The panel that did not move was the one that changed

The wizard was reported twice as "looking strange", and the second report said
exactly what was wrong: *the source is selected in the right panel but the
related subelement appears in the left panel*. The window had the STEPS on the
left, in a rail, and the SOURCE LIST on the right as step 1 - so picking a
source on the right rewrote the list on the left, which is the half of the
window a user reads as fixed. Every source has a different sequence, so the
left-hand side changed under a selection made on the right.

**What the first attempt got wrong, and why it is worth recording.** The
complaint was read as a wording problem: the rail was numbered, and each
completed step was made to show the choice taken in it. That is a better rail,
and it left the structure - the thing complained about - exactly as it was. The
user's words were about WHERE the two things were, not about what they said.

**What was done.** Master and detail, which is what the content was all along:

* the sources are a permanent list down the left, with the selected one's
  summary under them. They never change under anything;
* the right-hand side is the chosen source's own sequence, with a breadcrumb
  over it - `1. Find > 2. Choose > 3. Preview` - showing where you are, what is
  behind you (with the answer given there) and what is still to come. It is
  built from labels, since the LCL has no such control;
* **choosing the source is no longer a step.** `TDataSourceWizard.StageOf` and
  `StageCount` count only the steps after it, because counting the source made
  the sequence depend on the thing that chose the sequence;
* choosing a source enters its first step at once - the list is the choice, and
  it stays on screen, so there is nothing left to confirm with a Next.

**Two defects the rework uncovered, both invisible until the structure changed:**

* **a source that asks nothing was never asked.** The search ran when the Find
  step was LEFT, so the bundled samples - which declare no question, and
  therefore have no Find step - reached their file list having never been
  searched. The list was empty, and nothing reported anything wrong. It now
  happens on entering the step, in the presenter, and before the step changes,
  so a source with nothing to offer leaves the user on the list they chose
  from rather than on an empty page;
* **switching source kept the old source's step.** Choosing was only reachable
  from step 1 before, so nothing reset it. With the list always on screen,
  picking another source at the preview would have shown the new source's name
  over the old one's page.

**The rule this leaves.** When a user says a window looks wrong, the sentence
usually names two things and where they are. Changing what one of them SAYS is
not an answer to where it IS.

## Regenerating the guide rewrote ten pages that had not changed

`-Task guide-pro` on Linux reported ten modified pages under
`docs/user-guide/generated/` after a change that touched two topics. Every line
of every page differed. The pages are tracked with **CRLF** - they were first
generated on Windows - and the generator writes the platform's own line ending,
so running it anywhere else rewrites the whole file and buries the real change
in it.

**What was done here.** The eight pages whose content had not changed were put
back to CRLF, leaving the two real changes visible. Nothing was changed in the
generator: which ending these files should carry is a decision with a
cross-platform answer (a `.gitattributes` entry, or one deliberate conversion
to LF), and it should be taken once rather than by whoever last ran the task on
which operating system.

**The tell.** A generated file that reports as wholly modified after a small
change has almost certainly changed its line endings, not its content:
`file docs/.../page.md` against `git show HEAD:docs/.../page.md | file -` says
so in one line.

## An ampersand in a name was eaten by the window

A project saved as `S&P500.fitproj` was shown as "SP500" in the window's title on
macOS and in File > Recent Projects everywhere. A caption treats `&` as the mark
of the next letter's keyboard accelerator, so the character never reached the
screen; the file name was right all along. `app_version.LiteralCaption` doubles
it, for a caption that shows a name rather than a command. The title is
escaped only under Cocoa, which is where the LCL reads a form's caption that
way. A recent entry no longer carries its path in its caption, which is now
only for showing: the path is kept beside it and the click reads that, so
escaping the text shown cannot break opening the file.

## One project's axis followed the user into every later one

The chosen axis rules were kept in the settings file as well as in the project,
so a log axis chosen for one data set was the axis of the next new project and of
every file imported after it. The axes are the project's alone now: the settings
file publishes no axis property (`testcase_settings_model.AnAxisIsNotASetting`),
a new project and a different data file start on Automatic (`TChartAxes.Reset`,
`TProjectWorkflow.IsNewSource`), and changing an axis marks the project as
changed (`ProjectFingerprintParts`), because a change of axis that no save would
keep is a change lost in silence. An older settings file that still names the
axes is read without them (`DropUnpublishedProperties`).

## Every bar of the S&P 500 was in January 1900

The series was read by bar - its argument 0, 1, 2 ... - and shown on the Date
axis, which reads the argument as a day count: bar 0 was 1899-12-30. The reader
had thrown the dates away as soon as it chose to read by bar, so no axis could
have named the bars correctly. The OHLC reader now keeps one date per bar when it
can read them without guessing (never partially: one hole would name every later
bar by its neighbour's day), the client and the project carry them
(`TFitClient.GetArgumentDates`, `ui.json` `argumentDates`), and the price Date
mode builds a `TBarDateAxis` from them - still one bar per step, each named by its
day. `TPriceAxesTest.TheSP500ReadByBarIsShownByTheDayOfEachBar`, in the price-series
module's suite since price data moved there, goes from the real reader to the
readout; a mutation that drops the dates fails it with
the reported "1900-01-01".

## The public build offered Price to people with no price series

Data > Value Transformation > Use Rule listed Price, and Data > Price Data sat in
the Data menu, in the public build - which has nothing to analyse a price series
with. The price reader, its Bar, Date and Price modes and the menu choosing how a
file is read had stayed in the framework after the module that needs them
moved out, and each was registered "through the call a module makes",
which made the debt look paid.

**What moved.** The reader, the modes, the menu (now under that module's own menu,
declared through `IUiModule`), the choice (kept through `module_preferences`, the
seam added for it - it had been two properties of the framework's settings) and
the sample file (shipped from the module's `Data` folder by the package's
`Copy-PackageData`). The Web address, DOI and Sample data sources declared `.CSV`
among what they produce in a fixed list; they now derive it from the loaders
registered (`RegisteredExtensions`), so a module's format counts exactly where
the module is.

**What guards it.** `testcase_framework_boundary` scans the framework's sources for
the feature's names - a registry query cannot see the difference, because a build
that links the module holds the same registrations either way. The framework's
axis tests had used the price modes as generic stand-ins; they use the diffraction
modes now, and the price-specific ones live in the module's suite. A choice made
under the old menu is not carried over: it was a framework setting the framework
no longer reads.
