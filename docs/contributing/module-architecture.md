...
(`IUiModule.PanelId`). The window compares the two: rows the module pushes are
shown while one of its types is selected and held otherwise, then shown the
moment one is (`model_outline.ModulePanelRowsShown`, `HeldRowsToShow`). A module
redraws after every fit, and before this rule each redraw replaced another
model's rows and the selection on them.
