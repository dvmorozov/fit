...
**Anything that opens a socket or starts a process is integration**, loopback
included. What a transport *decides* stays a unit test through its seam:
`TCurlClient.Run` is the one method that starts curl, and `TWebClient.CreateCurl`
the one that picks it, so `testcase_curl_client` and `testcase_download_failures`
drive both over a recorded answer (`tests/mocks/mock_curl_client.pas`).

The real transport is tested in `tests/testcase_web_transport.pas`, against the
real `fit_server` started by `TWorkerProcessTest` - no purpose-built server. It
also stops a shell script standing in for curl (Unix only), and fetches one
public https page, `Ignore()`d when the host does not resolve.
...
### Units a module declares must stay fully covered

A ratchet stops a unit getting worse; it has nothing to say about one that has
always sat at 99 %. For code whose every branch is a verdict a user acts on — a
rule that accepts or rejects what they built — the uncovered line is a verdict
nothing checks. So a module repository may list such units in
`tests/coverage-full.txt`, one report path per line, and the ratchet passes that
list to `compare.py --must-be-full`.

- A listed unit below 100 % fails the ratchet, naming its hit/total.
- A listed unit **missing** from "this project's logic" fails too. Exclusion is
  therefore never a way to reach 100 %: a wrapper or vendor unit is not in those
  rows, and neither is a unit that stopped being linked into the unit suite.
- The list is the module's, found the way the module itself is found; the build
  scripts name no module. No list means no gate, and the ratchet says so.
- `tools/coverage/test_compare.py` tests the gate, and the ratchet runs it before
  comparing.
