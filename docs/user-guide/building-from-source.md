...
| curl | any recent | **at run time**, for fetching data from a source; macOS and Windows 10 (1803) or later ship it, and on Linux the packages depend on it |

Fit fits perfectly well without Python. The sidecar is an extra engine, not a
requirement.

No OpenSSL is needed to build or to run.
...
Set it up once as the user guide's
[Setting up the Python engine](https://dvmorozov.github.io/fit/guide-fitting.html#fitting-python-setup)
describes (also in the application, under Help → Explain Everything): a virtual environment built from the pinned requirements. It is built
once **per machine** - in
...
`FIT_PY_HOME` overrides the location. The same page says what goes in it and
why it is not installed system-wide.
