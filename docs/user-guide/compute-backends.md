...
Levenberg-Marquardt (Python/lmfit)**. Then fit exactly as usual (place curves, Minimize)
...
switch back to **Minimizer → Downhill Simplex (native)** and the native engine keeps
working.
...
- Publishing numbers that need **parameter uncertainties** → **Python**
  (Fit → Minimizer → Levenberg-Marquardt (Python/lmfit)).
