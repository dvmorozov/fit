...
identically under **both** minimizers (**Fit → Minimizer →** Downhill Simplex (native) or
Levenberg-Marquardt (Python/lmfit)), so you can switch engines freely.
...
**Every type explains itself in the application.** Select one in the curve-type
list, or a placed curve's row in the Model panel, and the **Explain pane** under the
Model tree shows what its formula means, what each parameter controls, where the
shape is used, what it does not cover, and where to read about it - with a
standing that says whether the shape is a published line profile or a choice this
software made. **Help ▸ Explain Everything…** lists every explanation the build
contains. The table below is a quick guide; the pane is the full account.
...
dependency-free fit; use the **Levenberg-Marquardt (Python/lmfit)** engine when you want **parameter
