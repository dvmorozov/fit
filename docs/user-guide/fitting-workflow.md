...
one edit that is refused — and about the alternative to it: letting the program
decompose the profile on its own.

## The loop

1. **Load the data.** *File → Import Profile…*, or open a saved project.
...
4. **Fit.** While it runs, the chart area shows how far it has got (see *While a
   fit runs*, below). When it finishes, the status bar shows the elapsed time and
   the achieved R-factor; *Fit Intervals* and *Summary* show the numbers per
   interval.
...
The chart never goes blank while a fit runs, whichever engine runs it and whatever
the model is.

- **At once**, a line above the chart says the fit has started, and your data
  stays on the chart.
- **By default**, as soon as the engine reports, the chart area shows the R-factor
  against elapsed time, on a logarithmic scale: a fit improves by factors of ten.
  The time axis reads in seconds, minutes or hours as the fit grows longer. Time
  the computer spends asleep or hibernated is not counted, here or in the status
  bar.
  The line above says what is being minimised and by which engine, how long the
  fit has run, the R-factor it has reached and how far that is below where it
  started. The last point is the result you are then shown.
- **With *View → Animation Mode* ticked**, the model's curves are redrawn over your
  data as the fit improves instead. It makes a fit slower, and the setting is
  remembered between sessions.
- **An engine that reports nothing until it finishes** — an older remote server,
  say — is said to be one, and the clock keeps running.
- **Stop ends the fit where it stands**, and keeps what it has reached: the curves
  as they were at that moment become the result, and you can fit again from there.
  It takes effect immediately, however long the fit would still have run.

Proposing curve bounds, background points or positions does not replace the chart,
but the status bar's clock runs for them too.

## Two ways round the loop

Every step above is on the menu bar — *Model → Curve Positions*, *Fit → Minimize
...
## Letting the program decompose the profile

**Fit → Automatically** (also on the toolbar) does the whole loop for you,
and decides how many curves the profile needs:

1. **Subtracts the background**, unless it has already been subtracted from the
   profile you are working on.
2. **Places a curve on every point of every peak** — every point of the profile
   that stands above the background threshold
   (*Model → Background → Set Background Fraction…*). Positions you picked
   yourself are used instead, if there are any.
3. **Marks a fit interval around each peak.** Intervals you marked are replaced.
4. **Removes curves one at a time**, re-fitting after each removal, for as long
   as the fit stays within the **maximum acceptable R-factor**. It first removes
   curves standing where the profile is steepest, then the weakest ones. A
   removal that cannot be fitted back within the limit is undone and ends that
   pass. Curves whose amplitude has collapsed to nothing go along the way.
5. **Fits what is left** to full precision.

The maximum acceptable R-factor is the one setting that decides the answer:
*Fit → Set Max Acceptable Difference…*, entered in percent, 0.01 % by default.
Ask for less accuracy and you get fewer, broader curves; ask for more and the
program keeps the curves it needs to deliver it.
[How many curves does the profile need?](https://dvmorozov.github.io/fit/how-many-curves.html)
explains the rule and how it compares with the published methods.

It uses the curve type selected in the Tools tab. On the two-peak sample
`Data/2.dat` with the two-branch Pseudo-Voigt it starts from 32 curves and ends
with 3, in well under a minute. Each re-fit on the way stops as soon as the
R-factor is back within the limit, because all it has to show is that it can
be.

**Fit → Minimize Number of Curves** runs steps 4 and 5 alone, on the positions
and intervals you already have. It proposes them itself only when there are
none, and it does not touch the background.

Curves removed by either command are gone for good, together with their
positions, so the next edit does not bring them back.
...
**Select a row to find its curve on the chart.** Everything drawn for that curve
is drawn wider, in its own colour, and on top of the other series, so it stays
visible however many curves the model holds. That includes any markers a curve
type draws for the curve. The profile, the difference and the other series that
belong to the model as a whole are never highlighted. Select a row that names no
curve and nothing is highlighted. The curve's row in the legend is shown in bold.

Choosing a curve's row in *Curve Attributes* selects the same curve: its row in
the Model tab is selected and the chart highlights it. If no row in the Model tab
stands for that curve, the Model tab shows nothing selected.
...
**Model → Clear Model… removes every curve at once**, after asking you and
saying how many will go. Each curve is removed exactly as a single deletion
removes it, picks included, so you start again from an empty model on the same
data. There is no undo. The entry is greyed while the model holds no curve and
while a fit is running. A curve that cannot go is left in place rather than
forced out - for example one a curve type builds per fit interval instead of from
a pick - and a single message says how many stayed and why.
