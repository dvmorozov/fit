...
A pack's part may also keep **the verdict it gave when the file was saved** — for
example a report on whether the model follows its field's rules, with the time
and the version that wrote it. That record is never read back as an input: on
opening, the model is checked again, and the saved verdict is shown only as what
the file said.
