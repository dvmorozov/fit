...
2. In **Create New Curve Type**, give the curve a **Name** and an **Expression** in terms
   ...
   Use the on-screen buttons or type directly. **Every function below is understood
   by both minimizers**: `exp`, `ln` (natural log), `log` (base 10), `sqrt`, `abs`;
   the circular `sin`, `cos`, `tg`, `ctg` and their inverses `arcsin`, `arccos`,
   `arctg`, `arcctg`; the hyperbolic `sh`, `ch`, `th`, `cth`, `sch`, `csch` and their
   inverses `arsh`, `arch`, `arth`, `arcth`; the operators `+ - * / ^`, and brackets.
   `sqr` and the constant `pi` have no button and are typed. Names are not case-sensitive, so
   `Tg`, `tg` and `TG` are the same function.

   `arctan` is accepted as another spelling of `arctg`.

   The **Levenberg-Marquardt (Python/lmfit)** minimizer supports the same set. If a formula uses a
   ...
   **Fit → Minimizer → Downhill Simplex (native)** for it.
...
1. Load `Data/1.dat` (**File → Import Profile…**, F2).
