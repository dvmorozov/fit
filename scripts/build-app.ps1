...
function New-MacRunShell([string] $Exe, [string] $IconSource) {
    if (-not $IsMacOS) { return $Exe }
    $dir = Split-Path -Parent $Exe
    $app = Join-Path $dir 'Fit.app'
    ...
    Get-ChildItem -Path $dir -Filter '*.app' -Directory -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -ne 'Fit.app' } |
...
<plist version="1.0">
<dict>
    <key>CFBundleName</key><string>Fit</string>
    <key>CFBundleDisplayName</key><string>Fit</string>
    <key>CFBundleExecutable</key><string>Fit</string>
    <key>CFBundleIdentifier</key><string>io.github.dvmorozov.fit.dev</string>
...
if ! curl -sf -m 2 "$URL" >/dev/null 2>&1; then
    ...
    "$LIB/fit_server" --port "$PORT" --start-detached \
        >>"$HOME/.local/share/fit/server.log" 2>&1 || true
fi
...
if ! curl -sf -m 2 "$URL" >/dev/null 2>&1; then
    ...
    "$DIR/fit_server" --port "$PORT" --start-detached \
        >>"$HOME/Library/Logs/Fit/server.log" 2>&1 || true
fi
...
function New-MacAppBundle([string] $OutDir, [string] $Client, [string] $Server,
    ...
    [System.IO.File]::WriteAllText($stubPath, ($MacLauncherScript -replace "`r`n", "`n"),
                                   (New-Object System.Text.UTF8Encoding $false))
