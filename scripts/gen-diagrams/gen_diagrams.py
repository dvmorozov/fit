...
def explanation_block(item):
    ...
    if item.get('limitations'):
        b.append('<h4>Limitations</h4><ul>%s</ul>' % ''.join(
    ...
    if item.get('references'):
        ...
        b.append('<h4>Sources</h4><ul>%s</ul>' % ''.join(refs))
    if item.get('related'):
        b.append('<p>See also: %s</p>' % ' &middot; '.join(
            '<a href="#%s">%s</a>' % (topic_anchor(t), esc(t))
            for t in item['related']))
    b.append('</section>')
    return '\n'.join(b)


def explanations_page(seams):
    s = seams['explanations']
    b = [BANNER, '<h1>Everything this build explains</h1>',
         '<p>Every curve type, rule and choice a user can meet in the application '
         'explains itself - what it is, its standing (the field\'s canonical '
         'sources, convention, or this software\'s own choice), what it does not '
         'cover and where to read more. This page is the same text the Explain '
         'pane shows, generated from the registry.</p>',
         '<p>%d explanations are registered in this build.</p>' % s['count']]
    for namespace, items in explanations_by_namespace(s['items']).items():
        b.append('<h2>%s</h2>' % esc(namespace_caption(namespace)))
        b.extend(explanation_block(i) for i in items)
    return page('Fit', FIT_TAGLINE, FIT_REPO, '\n'.join(b),
                nav(('index.html', 'Home'), ('architecture.html', 'Architecture'),
...
def gen_fit(args):
    ...
    write(out / 'how-many-curves.html',
          page('Fit', FIT_TAGLINE, FIT_REPO,
               BANNER + content('fit-curve-count.html'),
               nav(('index.html', 'Home'), ('architecture.html', 'Architecture'))))
    ...
    write(out / 'explanations.html', explanations_page(seams))
