...
{$IFDEF UseNoGUI}
  Interfaces,
{$ENDIF}
  SysUtils, fpcunit, fpcunitreport, plaintestreport,
  ...
  //  Where data can be fetched from, and what the wizard derives from that.
  testcase_data_source_registry,
  testcase_download_cache, testcase_data_source_advice,
  //  Each source over canned replies: what it asks for, and what it makes
  //  of the answer. Nothing here reaches the network.
  testcase_builtin_sources,
  //  Which steps a source needs, derived from what it declared.
  testcase_data_source_wizard,
  //  And everything that can go wrong while fetching: a service down, a
  //  withdrawn series, a cap, a cancel, a full disk, an answer that is a
  //  web page. Each is a branch no happy-path test takes.
  testcase_download_failures,
  //  What the system curl is asked and what its answer means - the rules
  //  under the one transport that reaches the internet. No process here.
  testcase_curl_client,
  //  And which downloaded files are the user's and which are litter.
  testcase_download_session,
  //  And the whole of it through the surface a user reaches: the wizard,
  //  a real download onto disk, the ordinary import, and a real server.
  testcase_data_source_e2e,
  testcase_lineshapes,
  ...
  testcase_two_branches_pseudo_voigt,
  ...
  testcase_coordinate_axis,
  testcase_duration_axis,
  testcase_run_clock,
  ...
  //  The framework's own rows of that panel, which a module's tree is
  //  matched against by the curve each row stands for.
  testcase_model_curve_rows,
  ...
  //  How much room a toolbar button gets around its icon. Left at the widget
  //  set's own metrics the icons run together in one block with no boundary,
  //  which nothing raises and no headless suite can see.
  testcase_toolbar_metrics,
  ...
  //  Where the recorded pointer is on each frame and how many frames a
  //  stretch of real time is worth - the rules of a recording that only a
  //  person watching the video would otherwise check.
  testcase_screen_recording,
  ...
  //  What makes an explanation worth showing, and the one registry every surface
  //  finds one through. The app teaches as well as fits: an explanation that
  //  repeats its title, or calls itself canonical and quotes nobody, reads as
  //  teaching and teaches nothing - and a provider answering a question it was
  //  not asked is confidently wrong.
  testcase_explanation,
  testcase_explanation_registry,
  //  What each built-in shape says about itself, in the suite that needs no
  //  Lazarus - the registry walk links the LCL (testcase_curve_type_explanations).
  testcase_curve_type_explanation_content,
  //  A child's readiness and a parent's lifeline, over a loopback socket.
  testcase_readiness_channel,
  //  fit_server says it is listening once, and then gets out of the way.
  testcase_server_readiness,
  testcase_export_conversation,
  testcase_pick_refresh,
  testcase_curve_type_selection,
  //  Which explanation the Explain pane shows as focus moves between rows,
  //  curve types and menu entries - walked over every event order, because
  //  every wrong answer is quiet: the previous thing, or a blank pane.
  testcase_explanation_focus,
  //  How an explanation is drawn. Its text comes from modules and the
  //  literature and contains '<', '&' and quotes; unescaped, a formula is broken
  //  markup and a title is an injected tag.
  testcase_explanation_html,
  //  The report a module contributes: a finding with no status must not read as
  //  a verdict, and a failure that links to no explanation says only THAT
  //  something is wrong, never why.
  testcase_module_report,
  //  How that report is drawn: its text carries measured values and limits
  //  ("x < 1"), and every verdict must say in words whether it passed and link
  //  to the rule it applies.
  testcase_report_html,
  //  Which modules get a report tab and when it shows: a build with no module
  //  that reports must have none, and a restored project must not ask for a tab
  //  that is still hidden.
  testcase_report_tabs,
  //  How large that report is drawn: the ladder the two zoom buttons walk, and
  //  the style the page carries so its headings grow with the text rather than
  //  keeping the size the HTML component gives them.
  testcase_report_zoom,
  //  What the report's own "how to read" link opens: it names the words the
  //  page uses, so a page and its description cannot drift apart unnoticed.
  testcase_report_explanations,
  //  Help > Explain Everything: how a user who does not yet know what to point
  //  at finds what the application can teach. A topic missing from it is a
  //  lesson nobody can find.
  testcase_explanation_index,
  ...
  //  How an R-factor reads. A good fit showed as a row of zeros.
  testcase_rfactor_text,
  ...
  //  A running fit's progress: what the engine records while it works, and the
  //  wire form the client draws its live loss chart from. Plain values and an
  //  injected clock, so every rule about time is tested without waiting.
  testcase_fit_progress_json,
  testcase_fit_progress_log,
  //  And the desktop's half: what the chart area shows while a fit runs, what
  //  the client asks the server for meanwhile, and what the view explains.
  testcase_fit_progress,
  testcase_client_fit_progress,
  testcase_wire_point_sets,
  testcase_fit_progress_explanations,
  //  The main window's form file: every action a control is bound to exists.
  testcase_form_file,
  //  A project save that fails half way leaves the previous file whole.
  testcase_safe_replace,
  //  An exported number reads back as the same double.
  testcase_table_text,
  ...
  testcase_dat_parser,
  //  The line scanner the DAT reader and a module's JCAMP-DX reader share.
  testcase_text_numbers,
  //  Reads the source tree rather than the product: no Pascal label or goto
  //  anywhere, because one makes its unit unassemblable on Intel macOS.
  testcase_no_pascal_labels,
  testcase_framework_boundary,
  testcase_module_preferences,
  //  Beside WinSock, TSocket names its unit: the two declarations differ, and
  //  mixing them broke every readiness wait on Windows. A source scan, because
  //  no other platform can reach the mismatch.
  testcase_winsock_types,
  ...
  //  The other half of the same search: listing what is checked out beside this
  //  repository and asking the disk which candidate is really there. It builds a
  //  tree of its own, because a failure here is what left every private build
  //  reporting the Python component as unstartable on a machine that had it.
  testcase_sidecar_location,
  ...
  //  Model > Clear Model: asked first, then every curve by the one-curve verb.
  testcase_model_clearing,
...
{$IFDEF UseNoGUI}
  ...
  //  The About box's icon, scaled with its transparency rather than stretched
  //  by the LCL, which drew light stripes through it. LCL images.
  , testcase_app_logo
  //  A chart series keeping a caption per point. TAGraph is LCL-bound.
  , testcase_chart_captions
  //  A point with no place on the chart - the logarithm of zero - is kept
  //  and left undrawn, and takes no part in the extents.
  , testcase_chart_gaps
  //  The user guide the application carries: its chapters reach Explain
  //  Everything in reading order, and every menu command is named in it by
  //  the path a user follows - a command added and left undocumented fails.
  //  Here because the Fitting chapter lists the minimisers from their
  //  registry, which reaches the fit engine and through it the LCL.
  , testcase_client_explanations
  //  The axis modes a build registers for either coordinate, and the rule
  //  that chooses among them: the model, then the data, then - only over an
  //  empty model - the selected curve type. Here because every mode's topic is
  //  resolved in the guide, which reaches the LCL the way the line above does.
  , testcase_axis_modes
  //  Every registered curve type explains itself - walked over the registry,
  //  so the next type to arrive without an explanation fails by name.
  , testcase_curve_type_explanations
  //  The seam the Explain pane is drawn through. The HTML component cannot build
  //  against the headless widget set this suite uses, so the form names only the
  //  seam - and a form with no view registered must get none, not a fault.
  , testcase_explain_view
  ...
  , testcase_rest_api
  //  What the server says its own surface is. Reaches fit_rest_api - it checks
  //  the hand-written schemas against replies a live service produces - so it
  //  lives here with its neighbours, unit test all the same.
  , testcase_rest_spec
  , testcase_http_fit_service
  , testcase_worker_process
  //  Animation Mode against the REAL server: the model redrawn while it is
  //  fitted. Every other animation test answers the client from a canned
  //  reply, which says nothing about whether a running server produces one.
  , testcase_live_animation
  , testcase_live_stop
  //  A worker fixture whose SetUp fails stops the fit_server it started - left
  //  running, it held the output pipe and the release job never ended.
  , testcase_worker_setup_failure
  //  The same document fetched over a socket from the started binary: the
  //  content type and the contents of the DEPLOYED server, neither of which is
  //  visible from inside this process.
  , testcase_rest_spec_e2e
  //  The web client's own transport over real sockets: a public https host,
  //  and the real fit_server on loopback. Integration, every case.
  , testcase_web_transport
  ...
  //  A process started inside some snap's environment giving it back. Here for
  //  the same reason as its neighbours: Common is not on the light suite's path.
  , testcase_snap_environment
  ...
  //  Clearing the model against a real engine; a user formula reaches the LCL.
  ...
  testcase_ui_menus,
  //  Ctrl means Command on a Mac: the one place that translation is stated, and
  //  the one part of it a headless binary on any platform can check.
  testcase_mac_menu,
  ...
  //  A fit in another process relaying its progress into the task, and the
  //  stateless fit publishing it. Both build real tasks, so nogui-only.
  testcase_remote_fit_progress,
  testcase_stateless_fit,
  ...
  //  The automatic decomposition end to end on Data/2.dat.
  testcase_auto_decomposition
{$ENDIF}
  ;

type
  { Names each test as it starts. The plain report is written only once the
    whole run is over, so a test that never returns leaves no word of which one
    it was - on the Windows release runner that was a job silent for half an
    hour, twice. }
  TTracingPlainResultsWriter = class(TPlainResultsWriter)
  public
    procedure StartTest(ATest: fpcunit.TTest); override;
  end;

  { The stock runner, with the tracing report when FIT_TEST_TRACE is set. Off by
    default: a developer's run reads the same as it always has. }
  TFitTestRunner = class(TTestRunner)
  protected
    function GetResultsWriter: TCustomResultsWriter; override;
  end;

procedure TTracingPlainResultsWriter.StartTest(ATest: fpcunit.TTest);
begin
  inherited StartTest(ATest);
  WriteLn('>> ', ATest.TestSuiteName, '.', ATest.TestName);
  //  Flushed, because the point is the line that is there when nothing follows.
  Flush(Output);
end;

function TFitTestRunner.GetResultsWriter: TCustomResultsWriter;
var
  Stock: TCustomResultsWriter;
begin
  Result := inherited GetResultsWriter;
  if (GetEnvironmentVariable('FIT_TEST_TRACE') = '') or
    (Result.ClassType <> TPlainResultsWriter) then
    Exit;
  Stock := Result;
  Result := TTracingPlainResultsWriter.Create(nil);
  Result.SkipTiming := Stock.SkipTiming;
  Result.Sparse := Stock.Sparse;
  Result.SkipAddressInfo := Stock.SkipAddressInfo;
  Stock.Free;
end;
