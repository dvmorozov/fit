...
uses
    Classes, SysUtils,
    int_fit_viewer, mock_support, module_view_types, module_report_types,
    mscr_specimen_list,
    named_points_set, neutron_points_set, self_copied_component,
    title_points_set, fit_progress;

type
    TMockFitViewer = class(TMockBase, IFitViewer, IFitProgressView)
    private
        FRefreshes: longint;
        FClears: longint;
        FHints: TStringList;
        ...
        FLastPanelId: string;
        FLastPanelRows: TOutline;
        FLastReport: TModuleReport;
        FReportCount: longint;
        ...
        FLastProgress: TFitProgressView;
        FProgressShown: longint;
        FProgressHidden: longint;
    ...
    public
        ...
        { The report a contributor last refreshed, and how many times. }
        property LastReport: TModuleReport read FLastReport;
        property ReportCount: longint read FReportCount;
        ...
        { What the last ShowFitProgress was given, copied - it is a record. }
        property LastProgress: TFitProgressView read FLastProgress;
        property ProgressShown: longint read FProgressShown;
        property ProgressHidden: longint read FProgressHidden;
        ...
        { The rows of the last panel shown, and whose they were - what a test
          composes the Model panel from, the way the window does. }
        property LastPanelId: string read FLastPanelId;
        property LastPanelRows: TOutline read FLastPanelRows;
...
begin
    ...
    FLastPanelId := APanelId;
    FLastPanelRows := ARows;
    FLog.Note('ShowModulePanel', APanelId);
end;

procedure TMockFitViewer.ShowModuleReport(const AReport: TModuleReport);
begin
    FLastReport := AReport;
    Inc(FReportCount);
    FLog.Note('ShowModuleReport', AReport.Module);
end;
...
procedure TMockFitViewer.ShowFitProgress(const AView: TFitProgressView);
begin
    FLastProgress := AView;
    Inc(FProgressShown);
    FLog.Note('ShowFitProgress');
end;

procedure TMockFitViewer.HideFitProgress;
begin
    Inc(FProgressHidden);
    FLog.Note('HideFitProgress');
end;
