...
type
    ...
    public
        ...
        { Fails the next WRITE only - a read before it still answers, so a test can
          reach the point where something has been read and the write is refused. }
        procedure FailNextSendWith(const AMessage: string);
...
procedure TMockHttpService.FailNextSendWith(const AMessage: string);
begin
    FFailNextSend := True;
    FFailWith := AMessage;
end;
...
begin
    ...
    FLastUrl := AUrl;
    FLastBody := ABody;
    //  A READ that happens to travel as a Send is not the write this refuses.
    if FFailNextSend and not SameText(AMethod, 'GET') then
    begin
        FFailNextSend := False;
        raise EUserException.Create(FFailWith);
    end;
