...
uses
    Classes, SysUtils, int_ui_host, module_view_types, module_report_types,
    mock_support;
...
type
    ...
    private
        ...
        FExplanations: TStringList;
        FLastReport: TModuleReport;
        FReportCount: longint;
        FLastBroughtForward: boolean;
    public
        constructor Create;
        ...
        { The report the module last showed, how many it showed, and whether it
          asked for the tab to come forward - a refresh that seizes focus and a
          command that fails to are both defects only this can see. }
        property LastReport: TModuleReport read FLastReport;
        property ReportCount: longint read FReportCount;
        property LastBroughtForward: boolean read FLastBroughtForward;
        ...
        { The topic the module last asked to have explained, or ''. }
        function LastExplanation: string;
        function ExplanationCount: longint;
...
begin
    ...
    FExplanations := TStringList.Create;
    FPanelRows := -1;
    FPicksPerGesture := -1;
...
procedure TMockUiHost.ShowModuleReport(const AReport: TModuleReport;
    ABringForward: boolean);
begin
    FLastReport := AReport;
    Inc(FReportCount);
    FLastBroughtForward := ABringForward;
    FLog.Note('ShowModuleReport', AReport.Module);
end;

procedure TMockUiHost.ShowExplanation(const ATopic: string);
begin
    FExplanations.Add(ATopic);
    FLog.Note('ShowExplanation', ATopic);
end;

{ ---- what the module did --------------------------------------------------- }

function TMockUiHost.LastExplanation: string;
begin
    Result := '';
    if FExplanations.Count > 0 then
        Result := FExplanations[FExplanations.Count - 1];
end;

function TMockUiHost.ExplanationCount: longint;
begin
    Result := FExplanations.Count;
end;
