...
type
    ...
    published
        //  What the window may rebuild while an operation runs.
        procedure AnIdleWindowMayRebuildTheModelPanel;
        procedure AWindowWaitingOnAnOperationMayNot;
        procedure AndMayAgainOnceItIsDone;
        procedure NorIsAModuleCalledBackWhileOneRuns;
        procedure ButItIsWhenNothingIsRunning;
        //  Nothing open.
        ...
        procedure ClearingTheModelNeedsACurveToClear;
        procedure ClearingTheModelNeedsSomethingOpen;
        procedure AndClearingItIsNotOfferedWhileAFitRuns;
        ...
        //  The two panes that are drawn as HTML, each zoomed by its own pair
        //  of buttons.
        procedure APaneInTheMiddleOfItsRangeOffersBothZooms;
        procedure APaneAtItsLargestOffersNoFurtherZoomIn;
        procedure APaneAtItsSmallestOffersNoFurtherZoomOut;
        procedure EachPanesButtonsFollowThatPaneAlone;
        procedure ZoomingAPaneNeedsNothingOpen;
...
begin
    ...
    //  And another mode running unticks it.
...
procedure TActionStateTest.ClearingTheModelNeedsACurveToClear;
begin
    //  NO ROW NEEDED, unlike Delete curve: the command is about the whole
    //  model, so what it follows is whether the model holds anything at all.
    FInputs.Open := OpenSuccess;
    FInputs.ModelHasCurves := False;
    Decide;
    AssertFalse('an empty model has nothing to clear', IsEnabled(ucClearModel));
    FInputs.ModelHasCurves := True;
    Decide;
    AssertTrue('a model with curves can be cleared', IsEnabled(ucClearModel));
end;

procedure TActionStateTest.ClearingTheModelNeedsSomethingOpen;
begin
    FInputs.Open := OpenFailure;
    FInputs.ModelHasCurves := True;
    Decide;
    AssertFalse('nothing open, nothing to clear', IsEnabled(ucClearModel));
end;

procedure TActionStateTest.AndClearingItIsNotOfferedWhileAFitRuns;
begin
    FInputs.Open := OpenSuccess;
    FInputs.ModelHasCurves := True;
    FInputs.Async := AsyncWorks;
    Decide;
    AssertFalse('not under a running optimiser', IsEnabled(ucClearModel));
end;
...
begin
    ...
    AssertFalse('the value''s rule', IsEnabled(ucValueUseRule));
end;
...
begin
    ...
    AssertTrue('the value''s rule', IsEnabled(ucValueUseRule));
end;

{ ---- the panes drawn as HTML ----------------------------------------------- }

procedure TActionStateTest.APaneInTheMiddleOfItsRangeOffersBothZooms;
begin
    FInputs.ReportCanZoomIn := True;
    FInputs.ReportCanZoomOut := True;
    FInputs.ExplainCanZoomIn := True;
    FInputs.ExplainCanZoomOut := True;
    Decide;
    AssertTrue('the report grows', IsEnabled(ucReportZoomIn));
    AssertTrue('and shrinks', IsEnabled(ucReportZoomOut));
    AssertTrue('the Explain pane grows', IsEnabled(ucExplainZoomIn));
    AssertTrue('and shrinks', IsEnabled(ucExplainZoomOut));
end;

procedure TActionStateTest.APaneAtItsLargestOffersNoFurtherZoomIn;
begin
    //  A button that stays live at the end of the ladder is a click that does
    //  nothing, which is indistinguishable from one that is broken.
    FInputs.ReportCanZoomIn := False;
    FInputs.ReportCanZoomOut := True;
    Decide;
    AssertFalse('nothing larger', IsEnabled(ucReportZoomIn));
    AssertTrue('but it can come back', IsEnabled(ucReportZoomOut));
end;

procedure TActionStateTest.APaneAtItsSmallestOffersNoFurtherZoomOut;
begin
    FInputs.ExplainCanZoomIn := True;
    FInputs.ExplainCanZoomOut := False;
    Decide;
    AssertTrue('it can still grow', IsEnabled(ucExplainZoomIn));
    AssertFalse('nothing smaller', IsEnabled(ucExplainZoomOut));
end;

procedure TActionStateTest.EachPanesButtonsFollowThatPaneAlone;
begin
    //  THE POINT OF TWO PAIRS. Buttons on the report that greyed out because
    //  the Explain pane had reached its limit would be a window whose controls
    //  answer for something the user is not looking at.
    FInputs.ReportCanZoomIn := True;
    FInputs.ReportCanZoomOut := True;
    FInputs.ExplainCanZoomIn := False;
    FInputs.ExplainCanZoomOut := False;
    Decide;
    AssertTrue('the report is unaffected', IsEnabled(ucReportZoomIn));
    AssertTrue('in both directions', IsEnabled(ucReportZoomOut));
    AssertFalse('the Explain pane is at its end', IsEnabled(ucExplainZoomIn));
    AssertFalse('in both directions', IsEnabled(ucExplainZoomOut));
end;

procedure TActionStateTest.ZoomingAPaneNeedsNothingOpen;
begin
    //  The Explain pane says what a command means before any data is loaded -
    //  it is how a new user finds out what this program does - so its size
    //  must not be gated on having opened something.
    FInputs := EmptyUiInputs;
    FInputs.ExplainCanZoomIn := True;
    FInputs.ExplainCanZoomOut := True;
    Decide;
    AssertTrue('larger', IsEnabled(ucExplainZoomIn));
    AssertTrue('smaller', IsEnabled(ucExplainZoomOut));
end;
...
begin
    ...
    //  Both HTML panes away from the ends of their ladder, which is where a
    //  freshly started window is.
    FInputs := EmptyUiInputs;
    FInputs.ReportCanZoomIn := True;
    FInputs.ReportCanZoomOut := True;
    FInputs.ExplainCanZoomIn := True;
...
procedure TActionStateTest.AnIdleWindowMayRebuildTheModelPanel;
begin
    AssertTrue('nothing running', ModelStructureMayBeRebuilt(AsyncStart));
end;

procedure TActionStateTest.AWindowWaitingOnAnOperationMayNot;
begin
    //  THE FREEZE THIS RULE IS MADE OF. Rebuilding the panel puts the selected
    //  row back, and that hands the row to the module owning the panel. A
    //  module that answers by asking the engine about the model waits for the
    //  running fit's lock - on the WINDOW's thread - so the window stops
    //  repainting, stops polling the fit's progress and stops answering the
    //  user until the fit ends. Reported twice as "it hangs, and there is no
    //  animation": the live progress view was working and nobody could see it.
    AssertFalse('a fit is running', ModelStructureMayBeRebuilt(AsyncWorks));
end;

procedure TActionStateTest.NorIsAModuleCalledBackWhileOneRuns;
begin
    //  THE OTHER TWO DOORS: the user selecting a row hands it to the module
    //  directly, and right-clicking one asks the module what it offers. Neither
    //  goes through a rebuild, so neither is covered by the rule above - and a
    //  module that reads the model to answer freezes the window exactly the
    //  same way, only by hand rather than on idle.
    AssertFalse('a fit is running', ModuleMayBeCalledBack(AsyncWorks));
end;

procedure TActionStateTest.ButItIsWhenNothingIsRunning;
begin
    AssertTrue('idle', ModuleMayBeCalledBack(AsyncStart));
    AssertTrue('finished', ModuleMayBeCalledBack(AsyncDone));
end;

procedure TActionStateTest.AndMayAgainOnceItIsDone;
begin
    //  The result is a new model, and the panel must describe it.
    AssertTrue('finished', ModelStructureMayBeRebuilt(AsyncDone));
end;
