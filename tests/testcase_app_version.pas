...
type
    ...
    published
        ...
        procedure TheProductIsNamedByTheBuildItIs;
        procedure AnAmpersandInACaptionIsShownRatherThanTakenAsAShortcut;
        procedure ACaptionWithoutOneIsUnchanged;
    end;
...
procedure TAppVersionTest.TheProductIsNamedByTheBuildItIs;
begin
    //  One name for the application a build makes, read by the window title,
    //  the About box and the macOS application menu alike. FIT_PRO is defined
    //  by the project that links a private module in (FitPro.lpi), and by no
    //  other - so this suite, built from Fit's own tests, is the public build.
{$IFDEF FIT_PRO}
    AssertEquals('Fit Pro', GetProductName);
{$ELSE}
    AssertEquals('Fit', GetProductName);
{$ENDIF}
end;

procedure TAppVersionTest.AnAmpersandInACaptionIsShownRatherThanTakenAsAShortcut;
begin
    //  The title read "SP500.fitproj" for S&P500.fitproj: a widget caption
    //  takes '&' as the mark of a shortcut key and drops it.
    AssertEquals('S&&P500.fitproj', LiteralCaption('S&P500.fitproj'));
    AssertEquals('every one', 'A&&B&&C', LiteralCaption('A&B&C'));
end;

procedure TAppVersionTest.ACaptionWithoutOneIsUnchanged;
begin
    AssertEquals('model.fitproj', LiteralCaption('model.fitproj'));
    AssertEquals('', LiteralCaption(''));
end;
