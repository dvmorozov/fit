...
type
    ...
    published
        ...
        //  Removing every curve.
        procedure ClearingTheModelDeletesEveryCurveByItsHandle;
        procedure AndTakesEachOffTheChart;
        procedure AndReReadsTheModelOnce;
        procedure ClearingAnEmptyModelAsksTheServiceToDeleteNothing;
        procedure ARefusalPartWayDoesNotStopTheRest;
        procedure AndIsHandedBackRatherThanRaised;
...
const
    TWO_CURVES =
        ...
        '"params":[{"name":"x0","value":2,"type":2,"error":-1}]}]}';

procedure TClientCommandsTest.ClearingTheModelDeletesEveryCurveByItsHandle;
var
    Reason: string;
begin
    StubTheProfileRoutes;
    FSvc.Reply('curves', TWO_CURVES);
    FSvc.Log.Clear;
    //  THE SAME VERB AS DELETING ONE, spent on every handle. A second removal
    //  mechanism would have to agree with the first about the picks, the
    //  identities and a module's markup - see TFitService.DeleteCurve.
    AssertEquals('both curves removed', 2, FClient.ClearModel(Reason));
    AssertEquals('the first went out: ' + FSvc.Log.AsText, 1,
        CallsTo('/points/positions/{AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA}'));
    AssertEquals('and the second', 1,
        CallsTo('/points/positions/{BBBBBBBB-BBBB-BBBB-BBBB-BBBBBBBBBBBB}'));
end;

procedure TClientCommandsTest.AndTakesEachOffTheChart;
var
    Reason: string;
begin
    StubTheProfileRoutes;
    FSvc.Reply('curves', TWO_CURVES);
    FClient.ClearModel(Reason);
    AssertTrue('the first curve''s series',
        FView.Plotted('HideSeriesOwnedBy:{AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA}'));
    AssertTrue('and the second''s',
        FView.Plotted('HideSeriesOwnedBy:{BBBBBBBB-BBBB-BBBB-BBBB-BBBBBBBBBBBB}'));
end;

procedure TClientCommandsTest.AndReReadsTheModelOnce;
var
    Reason: string;
begin
    StubTheProfileRoutes;
    ...
    FSvc.Log.Clear;
    FClient.ClearModel(Reason);
    //  ONE refresh at the end rather than one per curve: a refresh reads the
    //  computed profile once, so counting its reads counts the refreshes.
    AssertEquals('the model was re-read once: ' + FSvc.Log.AsText, 1,
        CallsTo('/calc-profile'));
end;

procedure TClientCommandsTest.ClearingAnEmptyModelAsksTheServiceToDeleteNothing;
var
    Reason: string;
begin
    StubTheProfileRoutes;
    ...
    FSvc.Log.Clear;
    AssertEquals('nothing to remove', 0, FClient.ClearModel(Reason));
    AssertEquals('and nothing was asked to go', 0,
        CallsTo('/points/positions/'));
end;

{ GRACEFUL, NOT ALL-OR-NOTHING. This used to raise on the first curve the
  server declined, which stopped the clearing half-way and left the rest of the
  model standing for no reason of its own. One curve that cannot go is a fact to
  report once at the end, not a fault that ends the command. }
procedure TClientCommandsTest.ARefusalPartWayDoesNotStopTheRest;
var
    Reason: string;
begin
    StubTheProfileRoutes;
    FSvc.Reply('curves', TWO_CURVES);
    //  The session is opened by the first request, and it is a write too - so
    //  opened here, or the refusal below would land on it instead.
    FSvc.GetCurveCount;
    FSvc.Log.Clear;
    FSvc.FailNextSendWith('a pack refuses to remove this curve');
    AssertEquals('the other curve still went', 1, FClient.ClearModel(Reason));
    AssertEquals('asked for the second after the first was declined', 1,
        CallsTo('/points/positions/{BBBBBBBB-BBBB-BBBB-BBBB-BBBBBBBBBBBB}'));
    AssertEquals('and the model was re-read once: ' + FSvc.Log.AsText, 1,
        CallsTo('/calc-profile'));
end;

procedure TClientCommandsTest.AndIsHandedBackRatherThanRaised;
var
    Reason: string;
begin
    StubTheProfileRoutes;
    ...
    FSvc.FailNextSendWith('a pack refuses to remove this curve');
    FClient.ClearModel(Reason);
    AssertEquals('the server''s own words', 'a pack refuses to remove this curve',
        Reason);
end;
