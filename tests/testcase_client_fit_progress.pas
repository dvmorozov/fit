...
type
    ...
    published
        ...
        procedure InAnimationTheProfileTheFitChangedIsDrawn;
        procedure AnUnchangedProfileIsNotDrawnAgain;
...
function WithChangedProfile(ARevision: longint): string;
begin
    Result := '{"ok":true,"busy":true,"elapsed":1.5,"nextSeq":1,' +
        '"samples":[{"seq":0,"t":0.5,"value":0.1}],' +
        '"profileRevision":' + IntToStr(ARevision) + ',' +
        '"experimentalProfile":{"title":"","x":[1,2,3],"y":[0,5,0]},' +
        '"snapshot":{"seq":0,' +
        '"profile":{"title":"","x":[1,2,3],"y":[1,2,3]},' +
        '"delta":{"title":"","x":[1,2,3],"y":[0,0,0]},' +
        '"curves":[]}}';
end;

procedure TClientFitProgressTest.InAnimationTheProfileTheFitChangedIsDrawn;
begin
    //  FIT > AUTOMATICALLY TAKES THE BACKGROUND OFF THE PROFILE, and the curves
    //  it fits are drawn against the profile without it. Until the fit was
    //  over the chart kept the profile the client had loaded, background and
    //  all, under curves that sat on zero.
    FClient.GiveProfile(5);
    FClient.AnimationMode := True;
    FClient.DoAllAutomatically;
    ReplyProgress(WithChangedProfile(1));
    FView.Log.Clear;
    FClient.PollProgress;
    AssertEquals('the profile as the fit left it: ' + FView.Log.AsText, 3,
        FView.PointsPlottedIn('PlotExpProfile'));
    AssertEquals('and it is the one the client holds', 3,
        FClient.GetProfilePoints.PointsCount);
end;

procedure TClientFitProgressTest.AnUnchangedProfileIsNotDrawnAgain;
begin
    FClient.GiveProfile(5);
    FClient.AnimationMode := True;
    FClient.DoAllAutomatically;
    ...
    FView.Log.Clear;
    FClient.PollProgress;
    AssertFalse('the same revision again: ' + FView.Log.AsText,
