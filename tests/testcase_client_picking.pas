...
type
    ...
    published
        ...
        procedure RemoveAllCurvePositionsEmptiesThemOnTheServer;
        procedure RemoveAllFitIntervalsEmptiesThemOnTheServer;
        procedure RemoveAllBackgroundPointsEmptiesThemOnTheServer;
    end;
...
{ ---- Remove All --------------------------------------------------------------
  ...
  THE REPORT: Model > Curve Positions > Remove All and Model > Fit Intervals >
  Remove All dropped the client's copy and told the engine nothing, so the next
  refresh drew the picks again and the curves built from them stayed. The
  command is the user's edit of MODEL INPUT, and model input lives on the
  server. }

function PutTo(ALog: string; const APath: string): string;
var
    Lines: TStringList;
    i: longint;
begin
    ...
    try
        ...
        for i := 0 to Lines.Count - 1 do
            if (Pos('PUT(', Lines[i]) = 1) and (Pos(APath + ' ', Lines[i]) > 0) then
                Result := Lines[i];
    finally
        Lines.Free;
    end;
...
procedure TClientPickingTest.RemoveAllCurvePositionsEmptiesThemOnTheServer;
var
    Put: string;
begin
    //  The model the read-back after it finds: none.
    FSvc.Reply('curves', '{"curves":[]}');
    FClient.RemoveAllCurvePositions;
    Put := PutTo(FSvc.Log.AsText, '/positions');
    AssertTrue('the engine was told: ' + FSvc.Log.AsText, Put <> '');
    AssertTrue('to hold none: ' + Put, Pos('"x" : []', Put) > 0);
end;

procedure TClientPickingTest.RemoveAllFitIntervalsEmptiesThemOnTheServer;
var
    Put: string;
begin
    FSvc.Reply('curves', '{"curves":[]}');
    FClient.RemoveAllRFactorBounds;
    Put := PutTo(FSvc.Log.AsText, '/rfactor-bounds');
    AssertTrue('the engine was told: ' + FSvc.Log.AsText, Put <> '');
    AssertTrue('to hold none: ' + Put, Pos('"x" : []', Put) > 0);
end;

procedure TClientPickingTest.RemoveAllBackgroundPointsEmptiesThemOnTheServer;
var
    Put: string;
begin
    FClient.RemoveAllBackgroundPoints;
    Put := PutTo(FSvc.Log.AsText, '/background');
    AssertTrue('the engine was told: ' + FSvc.Log.AsText, Put <> '');
    AssertTrue('to hold none: ' + Put, Pos('"x" : []', Put) > 0);
end;
