...
uses
    ...
    title_points_set, points_set, coordinate_axis;
...
type
    ...
    private
        FFirst: double;
        FArgumentMode, FValueMode: string;
        FDates: TAxisDates;
    public
        constructor Create(AFirstX: double);
        { What this loader says its data's coordinates are. }
        constructor CreateSaying(AFirstX: double;
            const AArgumentMode, AValueMode: string);
    ...
    published
        ...
        //  What the data says its coordinates are - which the axis rule asks.
        procedure TheDataSaysWhatItsCoordinatesAre;
        procedure ASecondFileReplacesWhatTheFirstSaid;
        procedure DataThatSaysNothingLeavesNothingBehind;
        procedure TheBarsDatesComeWithTheProfile;
        procedure AFileWithoutDatesLeavesNoneOfTheLastOnes;
...
constructor TFakeLoader.CreateSaying(AFirstX: double;
    const AArgumentMode, AValueMode: string);
begin
    Create(AFirstX);
    FArgumentMode := AArgumentMode;
    FValueMode := AValueMode;
end;

function TFakeLoader.CoordinateMode(ADimension: TAxisDimension): string;
begin
    if ADimension = adArgument then
        Result := FArgumentMode
    else
        Result := FValueMode;
end;

function TFakeLoader.ArgumentDates: TAxisDates;
begin
    Result := FDates;
end;

function DatedLoader: TFakeLoader;
begin
    Result := TFakeLoader.CreateSaying(0, 'price.date', 'price.price');
    SetLength(Result.FDates, 2);
    Result.FDates[0] := EncodeDate(2020, 9, 24);
    Result.FDates[1] := EncodeDate(2020, 9, 25);
end;
...
procedure TClientProfileTest.TheDataSaysWhatItsCoordinatesAre;
begin
    FLoaderRef := nil;
    FLoaderRef := TFakeLoader.CreateSaying(10, 'price.date', 'price.price');
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    AssertEquals('price.date', FClient.GetDataCoordinateMode(adArgument));
    AssertEquals('price.price', FClient.GetDataCoordinateMode(adValue));
    AssertEquals('and the axis rule is told', 'price.price',
        FClient.AxisPreferences(adValue).Data);
end;

procedure TClientProfileTest.ASecondFileReplacesWhatTheFirstSaid;
begin
    FLoaderRef := nil;
    FLoaderRef := TFakeLoader.CreateSaying(10, 'price.date', 'price.price');
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    FLoaderRef := nil;
    FLoaderRef := TFakeLoader.CreateSaying(20, 'diffraction.two-theta',
        'diffraction.intensity');
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    AssertEquals('diffraction.two-theta', FClient.GetDataCoordinateMode(adArgument));
    AssertEquals('diffraction.intensity', FClient.GetDataCoordinateMode(adValue));
end;

procedure TClientProfileTest.DataThatSaysNothingLeavesNothingBehind;
begin
    //  A plain two-column file after a price series is not still a price.
    FLoaderRef := nil;
    FLoaderRef := TFakeLoader.CreateSaying(10, 'price.date', 'price.price');
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    FLoaderRef := nil;
    FLoaderRef := TFakeLoader.Create(20);
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    AssertEquals('', FClient.GetDataCoordinateMode(adArgument));
    AssertEquals('', FClient.GetDataCoordinateMode(adValue));
end;

procedure TClientProfileTest.TheBarsDatesComeWithTheProfile;
begin
    FLoaderRef := nil;
    FLoaderRef := DatedLoader;
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    AssertEquals(2, Length(FClient.GetArgumentDates));
    AssertEquals(EncodeDate(2020, 9, 25), FClient.GetArgumentDates[1], 0);
end;

procedure TClientProfileTest.AFileWithoutDatesLeavesNoneOfTheLastOnes;
begin
    //  Left behind, they would name a diffraction pattern's points by the
    //  trading days of the series before it.
    FLoaderRef := nil;
    FLoaderRef := DatedLoader;
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    ...
    FClient.UseLoader(FLoaderRef);
    FClient.TakeProfileFromLoader;
    AssertEquals(0, Length(FClient.GetArgumentDates));
end;
