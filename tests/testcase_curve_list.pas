...
uses
    ...
    amplitude_curve_parameter, curve_instance_id, curve_list,
...
type
    ...
    published
        ...
        procedure TheExportHeadsItsColumnsWithTheParameterNames;
        procedure TheExportWritesEveryDigit;
        procedure AnUncertaintyIsExportedInAColumnOfItsOwn;

        //  Which curve a row stands for
        procedure ARowNamesTheCurveItHoldsByHandle;
        procedure EachRowNamesItsOwnCurve;
        procedure ACurveNeverIssuedAHandleNamesNone;
        procedure ARowOutsideTheDataNamesNoCurve;
        procedure TheRowNamingAnotherCurveSkipsTheSelectedOne;
        procedure ARowNamingNoCurveIsNeverAnotherCurve;
        procedure WithNothingSelectedTheFirstNamedRowIsAnother;

        //  What a cell accepts back
        procedure AValidNumberIsApplied;
        procedure ATypedValueIsPlannedForItsParameter;
        procedure ATypedValueIsPlannedForTheParameterOfItsName;
        procedure AComputedParameterCannotBeTyped;
        procedure ATypedNonNumberIsRefusedInWords;
        procedure AColumnTheCurveLacksPlansNothing;
...
{ ---- the export -------------------------------------------------------------
  ...
  THE REPORT: File > Export > Curve Parameters wrote the text the table shows -
  four decimals, and the uncertainty in the same cell as "value +- error", which
  no spreadsheet reads as a number. The export is the list's own: every digit,
  and an uncertainty in a column beside its value. }

procedure TCurveListTest.TheExportHeadsItsColumnsWithTheParameterNames;
var
    Rows: TTextRows;
begin
    AddCurve(['A', 'B'], [Variable, Variable], [1, 2]);
    Rows := FList.ExportRows;
    AssertEquals('a heading and one curve', 2, Length(Rows));
    AssertEquals('A', Rows[0][0]);
    AssertEquals('B', Rows[0][1]);
end;

procedure TCurveListTest.TheExportWritesEveryDigit;
var
    Rows: TTextRows;
begin
    AddCurve(['A'], [Variable], [1 / 3]);
    Rows := FList.ExportRows;
    AssertEquals(ExportedNumber(1 / 3), Rows[1][0]);
end;

procedure TCurveListTest.AnUncertaintyIsExportedInAColumnOfItsOwn;
var
    Rows: TTextRows;
begin
    AddCurve(['A', 'B'], [Variable, Variable], [1, 2]);
    ...
    Rows := FList.ExportRows;
    AssertEquals('the value', 'A', Rows[0][0]);
    AssertEquals('its uncertainty beside it', 'A error', Rows[0][1]);
    AssertEquals('then the next parameter', 'B', Rows[0][2]);
    AssertEquals('1', Rows[1][0]);
    AssertEquals('0.25', Rows[1][1]);
end;
...
{ ---- typing a value for the engine ------------------------------------------

  THE REPORT: the Curve Attributes table could not be typed into - the model
  had an inverse for a typed cell and nothing ever used it. A typed value is now
  sent to the engine, which rebuilds the model with it. What is sent - which
  parameter of which curve, and the value in the engine's own terms - is
  planned here. }

procedure TCurveListTest.ATypedValueIsPlannedForItsParameter;
var
    ParamIndex: longint;
    Value: double;
    Why: string;
begin
    AddCurve(['A', 'B'], [Variable, Variable], [1, 2]);
    AssertTrue('planned: ' + Why,
        FList.PlanParameterEdit(0, 1, '7.5', ParamIndex, Value, Why));
    AssertEquals('the parameter of the column''s name', 'B',
        Curve_parameters(FList.Items[0]).Parameters[ParamIndex].Name);
    AssertEquals('the value', 7.5, Value, 1e-12);
    AssertEquals('the table is not changed by planning', 2.0,
        Curve_parameters(FList.Items[0]).FindByName('B').Value, 1e-12);
end;

procedure TCurveListTest.ATypedValueIsPlannedForTheParameterOfItsName;
var
    ParamIndex: longint;
    Value: double;
    Why: string;
begin
    //  COLUMNS ARE NAMES, not positions: the second curve holds B first.
    AddCurve(['A', 'B'], [Variable, Variable], [1, 2]);
    AddCurve(['B', 'A'], [Variable, Variable], [3, 4]);
    AssertTrue(FList.PlanParameterEdit(1, 0, '9', ParamIndex, Value, Why));
    AssertEquals('A, wherever that curve holds it', 'A',
        Curve_parameters(FList.Items[1]).Parameters[ParamIndex].Name);
end;

procedure TCurveListTest.AComputedParameterCannotBeTyped;
var
    ParamIndex: longint;
    Value: double;
    Why: string;
begin
    AddCurve(['A', 'C'], [Variable, Calculated], [1, 2]);
    AssertFalse(FList.PlanParameterEdit(0, 1, '5', ParamIndex, Value, Why));
    AssertTrue('says why: ' + Why, Pos('computed', Why) > 0);
end;

procedure TCurveListTest.ATypedNonNumberIsRefusedInWords;
var
    ParamIndex: longint;
    Value: double;
    Why: string;
begin
    AddCurve(['A'], [Variable], [1]);
    AssertFalse(FList.PlanParameterEdit(0, 0, '7 cps', ParamIndex, Value, Why));
    AssertTrue('names what was typed: ' + Why, Pos('7 cps', Why) > 0);
end;

procedure TCurveListTest.AColumnTheCurveLacksPlansNothing;
var
    ParamIndex: longint;
    Value: double;
    Why: string;
...
procedure TCurveListTest.ARowNamesTheCurveItHoldsByHandle;
var
    Row: longint;
    Id: TCurveInstanceId;
begin
    Row := AddCurve(['A'], [Variable], [1]);
    Id := NewCurveInstanceId;
    Curve_parameters(FList.Items[Row]).FInstanceId := Id;
    //  The wire form, which is what the Model panel's rows and every curve
    //  route carry - so the table and the panel can be matched by it.
    AssertEquals(CurveInstanceIdToWire(Id), FList.CurveHandleOfRow(Row));
end;

procedure TCurveListTest.EachRowNamesItsOwnCurve;
var
...
begin
    ...
    Curve_parameters(FList.Items[0]).FInstanceId := A;
    Curve_parameters(FList.Items[1]).FInstanceId := B;
    AssertEquals('the second row', CurveInstanceIdToWire(B),
        FList.CurveHandleOfRow(1));
    AssertEquals('the first row', CurveInstanceIdToWire(A),
        FList.CurveHandleOfRow(0));
end;

procedure TCurveListTest.ACurveNeverIssuedAHandleNamesNone;
begin
    //  NOT THE ALL-ZERO IDENTIFIER. It is a well-formed string that names no
    //  curve, so a caller handed it would address nothing and say nothing.
    AddCurve(['A'], [Variable], [1]);
    AssertEquals('', FList.CurveHandleOfRow(0));
end;

procedure TCurveListTest.ARowOutsideTheDataNamesNoCurve;
begin
    AddCurve(['A'], [Variable], [1]);
    Curve_parameters(FList.Items[0]).FInstanceId := NewCurveInstanceId;
    AssertEquals('above', '', FList.CurveHandleOfRow(-1));
    AssertEquals('below', '', FList.CurveHandleOfRow(1));
end;

procedure TCurveListTest.TheRowNamingAnotherCurveSkipsTheSelectedOne;
var
    A: TCurveInstanceId;
begin
    AddCurve(['A'], [Variable], [1]);
    ...
    A := NewCurveInstanceId;
    Curve_parameters(FList.Items[0]).FInstanceId := A;
    Curve_parameters(FList.Items[1]).FInstanceId := NewCurveInstanceId;
    AssertEquals(1, FList.RowNamingAnotherCurve(CurveInstanceIdToWire(A)));
end;

procedure TCurveListTest.ARowNamingNoCurveIsNeverAnotherCurve;
var
    B: TCurveInstanceId;
begin
    //  The first row was never issued a handle, and the only named one is the
    //  selected curve: there is no other curve to choose.
    AddCurve(['A'], [Variable], [1]);
    AddCurve(['A'], [Variable], [2]);
    B := NewCurveInstanceId;
    Curve_parameters(FList.Items[1]).FInstanceId := B;
    AssertEquals(-1, FList.RowNamingAnotherCurve(CurveInstanceIdToWire(B)));
end;

procedure TCurveListTest.WithNothingSelectedTheFirstNamedRowIsAnother;
begin
    AddCurve(['A'], [Variable], [1]);
    AddCurve(['A'], [Variable], [2]);
    Curve_parameters(FList.Items[1]).FInstanceId := NewCurveInstanceId;
    AssertEquals(1, FList.RowNamingAnotherCurve(''));
end;
