...
type
    ...
    published
        ...
        procedure WithNoModeSetAPositionIsShownAsStored;
        procedure AParametersListOwnsItsCurveList;
...
begin
    ...
    FList.FArgumentMode := SinThetaOverLambdaAxisModeId;
...
begin
    ...
    FList.FArgumentMode := PositionAxisModeId;
    P := AParameter(InvariablePosition, 60);
...
procedure TCurveListAxisTest.WithNoModeSetAPositionIsShownAsStored;
var
    P: TSpecialCurveParameter;
begin
    //  What the compute server's lists are: nobody tells them an axis, and the
    //  registry is not consulted - it is not populated there.
    FList.FArgumentMode := '';
    P := AParameter(VariablePosition, 60);
    AssertEquals('shown as stored', 60.0, FList.Shown(P), 1E-12);
    FList.Typed(P, 42);
...
procedure TCurveListAxisTest.AParametersListOwnsItsCurveList;
var
    L: Parameters_list;
begin
    L := Parameters_list.Create(nil);
    try
        AssertNotNull('it has one from the start', L.Parameters);
        AssertEquals('with no axis set', '', L.Parameters.FArgumentMode);
    finally
        L.Free;
    end;
end;
...
begin
    ...
    try
        Plain.FArgumentMode := CustomAxisModeId;
        Plain.FArgumentDefinition.Name := 'Energy';
        Plain.FArgumentDefinition.UnitName := 'eV';
        Plain.FArgumentDefinition.Forward := '1239.84/x';
        Plain.FArgumentDefinition.Inverse := '1239.84/x';
        Copy := TMSCRCurveList(Plain.GetCopy);
        AssertEquals('the mode', CustomAxisModeId, Copy.FArgumentMode);
        AssertEquals('the name', 'Energy', Copy.FArgumentDefinition.Name);
        AssertEquals('the unit', 'eV', Copy.FArgumentDefinition.UnitName);
        AssertEquals('the formula', '1239.84/x', Copy.FArgumentDefinition.Forward);
        AssertEquals('and its inverse', '1239.84/x', Copy.FArgumentDefinition.Inverse);
        Copy.Free;
    finally
        Plain.Free;
    end;
...
begin
    ...
    Plain := TMSCRCurveList.Create;
    try
        Plain.FArgumentMode := SinThetaOverLambdaAxisModeId;
        Copy := TMSCRCurveList(Plain.GetCopy);
        Copy.FArgumentMode := PositionAxisModeId;
        AssertEquals('the original kept its own', SinThetaOverLambdaAxisModeId,
            Plain.FArgumentMode);
        Copy.Free;
    finally
        Plain.Free;
