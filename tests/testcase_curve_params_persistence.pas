...
type
  ...
  published
    ...
    procedure AHeldParameterStaysHeldThroughTheFile;
...
procedure TCurveParamsPersistenceTest.AHeldParameterStaysHeldThroughTheFile;
var
  FileName: string;
  Cfg: TXMLConfig;
  Saved, Loaded: Curve_parameters;
begin
  //  A user-defined curve type is kept in this file format, so a parameter held
  //  at its value in Set Curve Type Properties has to come back held - or the
  //  next session fits what the user said to keep.
  FileName := GetTempFileName('', 'cparm') + '.xml';
  Saved := Curve_parameters.Create(nil);
  try
    Saved.Parameters[0].Name := 'k';
    Saved.Parameters[0].VariationDisabled := True;
    Cfg := TXMLConfig.Create(FileName);
    try
      WriteComponentToXMLConfig(Cfg, 'Component', Saved);
      Cfg.Flush;
  ...
  try
    ...
    AssertTrue('still held', Loaded.Parameters[0].VariationDisabled);
  finally
    Loaded.Free;
    if FileExists(FileName) then
      DeleteFile(FileName);
