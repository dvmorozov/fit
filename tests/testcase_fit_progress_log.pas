...
type
    ...
    published
        ...
        //  The profile the fit changed - the background taken off it.
        procedure AProfileChangedDuringTheFitIsReportedWithTheFrames;
        procedure ButNotToAPollThatAsksForNoFrame;
        procedure EachChangeRaisesTheRevision;
        procedure StartingAgainForgetsTheChangedProfile;
...
function OnePointProfile(AY: double): TPointsData;
begin
...
procedure TFitProgressLogTest.AProfileChangedDuringTheFitIsReportedWithTheFrames;
var
    R: TFitProgressReport;
begin
    //  FIT > AUTOMATICALLY TAKES THE BACKGROUND OFF THE PROFILE before it
    //  fits, and says so through the service's ShowProfile - a callback that
    //  reaches no client across HTTP. So the animated fit drew the curves
    //  against the profile with its background still in it, until the fit was
    //  over. The changed profile travels with the frames instead.
    FLog.Start(At(0));
    FLog.StoreProfile(OnePointProfile(7));
    R := ReportAt(1, 0, True);
    AssertTrue('the profile is reported', R.HasProfile);
    AssertEquals('as it now is', 7.0, R.Profile.Y[0], 1e-12);
    AssertEquals('its revision', 1, R.ProfileRevision);
end;

procedure TFitProgressLogTest.ButNotToAPollThatAsksForNoFrame;
begin
    //  Without animation nothing is drawn mid-fit, and the result re-reads
    //  the profile when the fit is over.
    FLog.Start(At(0));
    FLog.StoreProfile(OnePointProfile(7));
    AssertFalse(ReportAt(1).HasProfile);
end;

procedure TFitProgressLogTest.EachChangeRaisesTheRevision;
begin
    //  The client redraws the profile only when it has changed, and the
    //  revision is how it knows.
    FLog.Start(At(0));
    FLog.StoreProfile(OnePointProfile(7));
    FLog.StoreProfile(OnePointProfile(3));
    AssertEquals(2, ReportAt(1, 0, True).ProfileRevision);
    AssertEquals('the latest', 3.0, ReportAt(1, 0, True).Profile.Y[0], 1e-12);
end;

procedure TFitProgressLogTest.StartingAgainForgetsTheChangedProfile;
begin
    //  A profile from an earlier operation may be older than the one the
    //  client has since loaded; drawn over it, it would undo the load.
    FLog.Start(At(0));
    FLog.StoreProfile(OnePointProfile(7));
    FLog.Start(At(2));
    AssertFalse(ReportAt(3, 0, True).HasProfile);
end;
