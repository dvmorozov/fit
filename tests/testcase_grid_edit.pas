...
type
    ...
    published
        ...
        procedure ChangingOnlyTheAmplitudeOfAPointMovesIt;
        procedure ChangingOnlyThePositionOfAPointMovesIt;
        procedure ARowLeftAsItWasAppliesNothing;
        procedure ANewRowWaitsForBothCells;
        procedure ARowWithAnUnreadableCellIsRefusedInWords;
        procedure ARowOfNumbersIsNotRefused;
...
procedure TGridEditTest.ChangingOnlyTheAmplitudeOfAPointMovesIt;
begin
    //  THE REPORT: a point's row applied only once BOTH its cells had been
    //  edited, so correcting one amplitude - the ordinary edit - did nothing,
    //  and the table went on showing the new number over the old point. A
    //  point that already exists is fully stated: the unchanged cell says the
    //  rest.
    AssertTrue(RowEditApplies('116.5', '773', '116.5', '800'));
end;

procedure TGridEditTest.ChangingOnlyThePositionOfAPointMovesIt;
begin
    AssertTrue(RowEditApplies('116.5', '773', '117', '773'));
end;

procedure TGridEditTest.ARowLeftAsItWasAppliesNothing;
begin
    AssertFalse(RowEditApplies('116.5', '773', '116.5', '773'));
end;

procedure TGridEditTest.ANewRowWaitsForBothCells;
begin
    //  What the all-cells rule was for: a row being typed from nothing is not
    //  a point until both halves are there.
    AssertFalse('position only', RowEditApplies('', '', '117', ''));
    AssertTrue('both', RowEditApplies('', '', '117', '5'));
end;

procedure TGridEditTest.ARowWithAnUnreadableCellIsRefusedInWords;
var
    Why: string;
begin
    //  THE TRAP THIS UNIT KEPT: a typo, a stray letter or a unit typed after the
    //  value read as zero, and the point moved to the origin without a word.
    //  Such a row is refused, and the refusal names the text.
    Why := RowEditRefusal('116.5', '773 cps');
    AssertTrue('refused: ' + Why, Why <> '');
    AssertTrue('naming what was typed: ' + Why, Pos('773 cps', Why) > 0);
    AssertTrue('the position too', RowEditRefusal('11b', '5') <> '');
end;

procedure TGridEditTest.ARowOfNumbersIsNotRefused;
begin
    AssertEquals('', RowEditRefusal('116.5', '-3.2e2'));
end;
