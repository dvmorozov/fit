...
type
    ...
    published
        ...
        procedure AModuleDocumentIsPassedThroughUnread;
        procedure ARefusalOnTheModuleChannelReachesTheUserAsItsOwnMessage;
        procedure AndSoDoesOneAnsweringAFetch;
...
procedure THttpServiceMarshallingTest.AModuleDocumentIsPassedThroughUnread;
var
    Json: string;
begin
    //  A module's own vocabulary, with no "ok" in it - which is what a module's
    //  overlay markup actually looks like. It must arrive exactly as sent: the
    //  refusal check below must not start reading documents it does not own.
    FSvc.Reply('overlay', '{"patterns":[{"typeName":"Pattern A (5-3-5)"}]}');
    Json := FSvc.ModuleGet('sample/overlay');
    AssertEquals('the document, unchanged',
        '{"patterns":[{"typeName":"Pattern A (5-3-5)"}]}', Json);
end;

procedure THttpServiceMarshallingTest.ARefusalOnTheModuleChannelReachesTheUserAsItsOwnMessage;
var
    Message_: string;
begin
    //  THE MESSAGE THE USER NEEDS, and it was being thrown away. The module
    //  channel does not go through CheckedResponse - a module's document is its
    //  own and the framework must not parse it - but a REFUSAL is not that
    //  document: it is the envelope every other route answers with, and it
    //  carries the one sentence that says what to do.
    //
    //  Detect on a build with no Python component is exactly this case. The
    //  server answered 503 with "This feature needs the Python component, which
    //  could not be started. Set it up in Worker/py: ...", the caller could not
    //  parse it as its own document, and the user was told only "the wave
    //  detector returned a reply this version cannot read" - which names neither
    //  the cause nor the fix.
    FSvc.Reply('decompose',
        '{"ok":false,"error":"This feature needs the Python component"}');
    Message_ := '';
    try
        FSvc.ModulePost('sample/decompose', '');
    except
        on E: EUserException do
            Message_ := E.Message;
    end;
    AssertEquals('the server''s own words',
        'This feature needs the Python component', Message_);
end;

procedure THttpServiceMarshallingTest.AndSoDoesOneAnsweringAFetch;
var
    Message_: string;
begin
    //  Both verbs, because a resource is read on every redraw and refused for
    //  the same reasons a write is.
    FSvc.Reply('overlay', '{"ok":false,"error":"nothing is marked up yet"}');
    Message_ := '';
    try
        FSvc.ModuleGet('sample/overlay');
    except
        on E: EUserException do
            Message_ := E.Message;
    end;
    AssertEquals('the server''s own words',
        'nothing is marked up yet', Message_);
end;
