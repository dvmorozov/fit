...
type
    ...
    published
        ...
        //  Whose pushed rows the panel shows.
        procedure AModulesRowsAreShownWhileItsTypeIsSelected;
        procedure AModulesRowsWaitWhileAnotherTypeIsSelected;
        procedure TheFrameworksRowsAreGatedWhereTheyAreBuilt;
        procedure RowsNoModuleClaimsAreShownAsTheyCome;
        procedure HeldRowsAreShownWhenTheirTypeIsSelected;
        procedure OnlyTheLatestPushOfAPanelIsHeld;
        procedure NothingHeldForTheSelectedTypeShowsNothing;
        procedure HeldRowsAlreadyShownAreNotShownAgain;
        procedure APushWhileItsTypeIsSelectedIsShownAndKept;
        procedure APushWhileAnotherTypeIsSelectedIsKeptNotShown;
        procedure TheFrameworksRowsAreNeverKept;
...
{ THE PANEL SHOWS ONE DESCRIPTION OF THE MODEL, and which one follows the
  selected type. A module pushes its rows whenever it redraws - after every fit,
  every refresh - and until this rule every push replaced whatever the panel held:
  a Gaussian model's rows, the selection on them, and the commands that selection
  enabled, all gone because a pack with nothing placed had redrawn. }
procedure TModelOutlineTest.AModulesRowsAreShownWhileItsTypeIsSelected;
begin
    AssertTrue(ModulePanelRowsShown('some-markup-set', 'some-markup-set',
        ['some-markup-set']));
    AssertTrue('the same name however it is cased',
        ModulePanelRowsShown('Some-Markup-Set', 'some-markup-set', ['some-markup-set']));
end;

procedure TModelOutlineTest.AModulesRowsWaitWhileAnotherTypeIsSelected;
begin
    AssertFalse('a type placed by picks',
        ModulePanelRowsShown('some-markup-set', '', ['some-markup-set']));
    AssertFalse('another module''s type',
        ModulePanelRowsShown('some-markup-set', 'other-set',
        ['some-markup-set', 'other-set']));
end;

procedure TModelOutlineTest.TheFrameworksRowsAreGatedWhereTheyAreBuilt;
begin
    //  RefreshModelStructure already asks FrameworkFillsStructure before it
    //  builds them; a second gate here would be a second place to disagree.
    AssertTrue(ModulePanelRowsShown(FrameworkStructureId, 'some-markup-set',
        ['some-markup-set']));
end;

procedure TModelOutlineTest.RowsNoModuleClaimsAreShownAsTheyCome;
begin
    //  No module's type can be selected for them, so there is nothing to wait
    //  for - the window's own self-check pushes rows like these.
    AssertTrue(ModulePanelRowsShown('check.rows', '', ['some-markup-set']));
    AssertTrue(ModulePanelRowsShown('check.rows', '', []));
end;

function OneRow(const AId: string): TOutline;
begin
    Result := nil;
    SetLength(Result, 1);
    Result[0].Id := AId;
...
{ HELD, so selecting the module's type shows what it last pushed at once -
  otherwise the panel would go on describing the old model until that module
  happened to redraw. }
procedure TModelOutlineTest.HeldRowsAreShownWhenTheirTypeIsSelected;
var
    Held: THeldPanelRows;
    Id: string;
    Rows: TOutline;
begin
    ...
    HoldPanelRows(Held, 'some-markup-set', OneRow('a'));
    HoldPanelRows(Held, 'other-set', OneRow('b'));
    AssertTrue(HeldRowsToShow(Held, 'Other-Set', FrameworkStructureId, Id, Rows));
    AssertEquals('other-set', Id);
...
begin
    ...
    AssertTrue(HeldRowsToShow(Held, 'some-markup-set', '', Id, Rows));
    AssertEquals('new', Rows[0].Id);
end;

procedure TModelOutlineTest.NothingHeldForTheSelectedTypeShowsNothing;
var
    Held: THeldPanelRows;
    Id: string;
    Rows: TOutline;
begin
    Held := Default(THeldPanelRows);
    AssertFalse('nothing held at all', HeldRowsToShow(Held, 'some-markup-set', '', Id, Rows));
    HoldPanelRows(Held, 'other-set', OneRow('b'));
    AssertFalse('held for another type',
        HeldRowsToShow(Held, 'some-markup-set', '', Id, Rows));
    AssertEquals('', Id);
    AssertEquals(0, Length(Rows));
end;

procedure TModelOutlineTest.HeldRowsAlreadyShownAreNotShownAgain;
var
    Held: THeldPanelRows;
    Id: string;
    Rows: TOutline;
begin
    //  Every refresh of a markup type passes through here; rebuilding the tree
    //  from the same rows each time would reset the user's scroll and selection
    //  for nothing.
    Held := Default(THeldPanelRows);
    HoldPanelRows(Held, 'some-markup-set', OneRow('a'));
    AssertFalse(HeldRowsToShow(Held, 'some-markup-set', 'some-markup-set', Id, Rows));
end;

{ KEPT EVEN WHEN SHOWN. Switch away from the module's type and back, and what
  comes back must be what it pushed last - not what it pushed the last time its
  rows happened to be held. }
procedure TModelOutlineTest.APushWhileItsTypeIsSelectedIsShownAndKept;
var
    Held: THeldPanelRows;
    Id: string;
    Rows: TOutline;
begin
    Held := Default(THeldPanelRows);
    AssertFalse('held while away', PushPanelRows(Held, 'some-markup-set', '',
        ['some-markup-set'], OneRow('old')));
    AssertTrue('shown while selected', PushPanelRows(Held, 'some-markup-set',
        'some-markup-set', ['some-markup-set'], OneRow('new')));
    AssertTrue(HeldRowsToShow(Held, 'some-markup-set', FrameworkStructureId, Id, Rows));
    AssertEquals('the latest push comes back', 'new', Rows[0].Id);
end;

procedure TModelOutlineTest.APushWhileAnotherTypeIsSelectedIsKeptNotShown;
var
    Held: THeldPanelRows;
    Id: string;
    Rows: TOutline;
begin
    Held := Default(THeldPanelRows);
    AssertFalse(PushPanelRows(Held, 'some-markup-set', 'other-set',
        ['some-markup-set', 'other-set'], OneRow('a')));
    AssertTrue(HeldRowsToShow(Held, 'some-markup-set', 'other-set', Id, Rows));
    AssertEquals('a', Rows[0].Id);
end;

procedure TModelOutlineTest.TheFrameworksRowsAreNeverKept;
var
    Held: THeldPanelRows;
begin
    //  They are rebuilt from the model whenever its type is selected; a kept
    //  copy could only ever be an old one.
    Held := Default(THeldPanelRows);
    AssertTrue(PushPanelRows(Held, FrameworkStructureId, '', ['some-markup-set'],
        OneRow('f')));
    AssertEquals(0, Length(Held.Ids));
end;

{ "show" in a report names a row of a model whose type is not selected: the rows
  are kept, not gone, and saying they are gone sends the user looking for a
...
begin
    Held := Default(THeldPanelRows);
    AssertEquals('nothing kept', RowGoneHint, RowLinkMissHint(Held, 'a'));
    HoldPanelRows(Held, 'some-markup-set', OneRow('a'));
    AssertEquals('kept', RowKeptHint, RowLinkMissHint(Held, 'a'));
    AssertEquals('not among the kept', RowGoneHint, RowLinkMissHint(Held, 'zzz'));
