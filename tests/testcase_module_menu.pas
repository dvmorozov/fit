...
type
    ...
    published
        ...
        procedure ADeclarationThatSaysNothingIsEnabled;
        procedure ADisabledDeclarationIsADisabledNode;
        procedure ADisabledSubmenuIsADisabledNode;
        procedure ATopicIsCarriedToTheNode;
        procedure AnEntryWithoutATopicCarriesNone;
        procedure ARowMenuNestsTwoLevelsDeepUnderItsDeclaredSubmenus;
    end;

    { WHICH MODULE IS ASKED for the context menu over a row. Only the module
      that filled the panel knows what its rows stand for; asking another would
      offer one module's commands over a different module's rows. }
    TRowMenuModuleTest = class(TTestCase)
    private
        FA, FB: TMockUiModule;
        function Both: TUiModuleArray;
    protected
        procedure SetUp; override;
        procedure TearDown; override;
    published
        procedure TheModuleWhoseRowsAreShownIsAsked;
        procedure TheFirstModuleCanBeTheOneAsked;
        procedure NoModuleIsAskedOverTheFrameworksOwnRows;
        procedure AnEmptyPanelIdAsksNoModule;
        procedure NoModulesAsksNone;
        procedure OverAModulesRowsOnlyThatModuleIsAskedForItsMenu;
        procedure OverRowsNoModuleClaimsEveryModuleIsAskedInOrder;
        procedure OverAPanelWithNoIdNobodyIsAskedForAMenu;
        procedure WithNoModulesNobodyIsAskedForAMenu;
    end;
...
procedure TModuleMenuTest.ADeclarationThatSaysNothingIsEnabled;
begin
    //  Every declaration written before Disabled existed says nothing about it.
    Declare('a', '', 'A', mkCommand);
    Build;
    AssertTrue(NodeOf('a').Enabled);
end;

procedure TModuleMenuTest.ADisabledDeclarationIsADisabledNode;
begin
    Declare('a', '', 'A', mkCommand);
    FDecls[0].Disabled := True;
    Build;
    AssertFalse(NodeOf('a').Enabled);
    AssertTrue('still shown, still an entry', NodeOf('a').Clickable);
end;

procedure TModuleMenuTest.ADisabledSubmenuIsADisabledNode;
begin
    Declare('sub', '', 'Sub', mkSubmenu);
    FDecls[0].Disabled := True;
    Declare('a', 'sub', 'A', mkCommand);
    Build;
    AssertFalse(NodeOf('sub').Enabled);
    AssertTrue('its child says nothing, so it is enabled', NodeOf('a').Enabled);
end;

procedure TModuleMenuTest.ATopicIsCarriedToTheNode;
begin
    Declare('a', '', 'A', mkCommand);
    FDecls[0].Topic := 'pack/rule/P4';
    Build;
    AssertEquals('pack/rule/P4', NodeOf('a').Topic);
end;

procedure TModuleMenuTest.AnEntryWithoutATopicCarriesNone;
begin
    Declare('a', '', 'A', mkCommand);
    Build;
    AssertEquals('', NodeOf('a').Topic);
end;

procedure TModuleMenuTest.ARowMenuNestsTwoLevelsDeepUnderItsDeclaredSubmenus;
var
    i, Decompose, Wave: longint;
begin
    Declare('decompose', '', 'Decompose', mkSubmenu);
    Declare('decompose.3', 'decompose', 'Wave 3', mkSubmenu);
    Declare('decompose.3.impulse', 'decompose.3', 'Impulse (5)', mkCommand);
    Build;
    Decompose := -1;
    Wave := -1;
    for i := 0 to High(FNodes) do
    begin
        if FNodes[i].Id = 'decompose' then
            Decompose := i;
        if FNodes[i].Id = 'decompose.3' then
            Wave := i;
    end;
    AssertEquals(-1, NodeOf('decompose').ParentIndex);
    AssertEquals(Decompose, NodeOf('decompose.3').ParentIndex);
    AssertEquals(Wave, NodeOf('decompose.3.impulse').ParentIndex);
end;

{ ---- which module a row menu is asked of ---- }

procedure TRowMenuModuleTest.SetUp;
begin
    FA := TMockUiModule.Create('alpha');
    FB := TMockUiModule.Create('beta');
end;

procedure TRowMenuModuleTest.TearDown;
begin
    FA.Free;
    FB.Free;
end;

function TRowMenuModuleTest.Both: TUiModuleArray;
begin
    SetLength(Result, 2);
    Result[0] := FA;
    Result[1] := FB;
...
function Joined(const AIndexes: TModuleIndexes): string;
var
    i: longint;
begin
    Result := '';
...
procedure TRowMenuModuleTest.OverAModulesRowsOnlyThatModuleIsAskedForItsMenu;
begin
    AssertEquals('1;', Joined(RowMenuModulesToAsk(Both, FB.PanelId)));
end;

procedure TRowMenuModuleTest.OverRowsNoModuleClaimsEveryModuleIsAskedInOrder;
begin
    //  The framework's own rows: a curve there may be one a module placed, and
    //  only that module can say what it offers over it.
    AssertEquals('0;1;', Joined(RowMenuModulesToAsk(Both, 'fit.model')));
end;

procedure TRowMenuModuleTest.OverAPanelWithNoIdNobodyIsAskedForAMenu;
begin
    AssertEquals('', Joined(RowMenuModulesToAsk(Both, '')));
end;

procedure TRowMenuModuleTest.WithNoModulesNobodyIsAskedForAMenu;
begin
    AssertEquals('', Joined(RowMenuModulesToAsk(nil, 'fit.model')));
end;

procedure TRowMenuModuleTest.TheModuleWhoseRowsAreShownIsAsked;
begin
    AssertEquals(1, RowMenuModuleIndex(Both, FB.PanelId));
end;

procedure TRowMenuModuleTest.TheFirstModuleCanBeTheOneAsked;
begin
    AssertEquals(0, RowMenuModuleIndex(Both, FA.PanelId));
end;

procedure TRowMenuModuleTest.NoModuleIsAskedOverTheFrameworksOwnRows;
begin
    AssertEquals(-1, RowMenuModuleIndex(Both, 'fit.model'));
end;

procedure TRowMenuModuleTest.AnEmptyPanelIdAsksNoModule;
begin
    AssertEquals(-1, RowMenuModuleIndex(Both, ''));
end;

procedure TRowMenuModuleTest.NoModulesAsksNone;
begin
    AssertEquals(-1, RowMenuModuleIndex(nil, FA.PanelId));
end;

initialization
    //  A unit test: declarations in, structure out. No menu and no module.
    RegisterTest('unit', TModuleMenuTest);
    RegisterTest('unit', TRowMenuModuleTest);
