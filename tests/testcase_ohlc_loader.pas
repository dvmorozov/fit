...
type
  ...
  published
    ...
    procedure TheChosenColumnAndArgumentReachANewLoader;
    procedure AChoiceOutOfRangeFallsBackToTheDefaults;
...
{ THE REPORT: the column a .csv file is read as, and what its argument is, had
  no way to be chosen - the loader could do both and nothing in the window set
  them. Data > Price Data chooses them, and the choice reaches every loader the
  import creates, the way the injector creates one: with no arguments. }
procedure TOhlcLoaderTest.TheChosenColumnAndArgumentReachANewLoader;
var
  Loader: TOHLCFileLoader;
  P: TTitlePointsSet;
begin
  ApplyPriceDataChoice(Ord(vcHigh), Ord(xmDateSerial));
  try
    Loader := TOHLCFileLoader.Create(nil);
    try
      Loader.LoadFromLines(Csv([
        'Date,Open,High,Low,Close',
        '2024-01-02,10,20,5,15',
        '2024-01-05,11,21,6,16']));
      P := Loader.GetPointsSetCopy;
      try
        AssertEquals('the high', 20.0, P.PointYCoord[0], 1e-9);
        AssertEquals('against the date: three days apart', 3.0,
          P.PointXCoord[1] - P.PointXCoord[0], 1e-9);
      finally
        P.Free;
      end;
    finally
    ...
    ApplyPriceDataChoice(0, 0);
  end;
end;

procedure TOhlcLoaderTest.AChoiceOutOfRangeFallsBackToTheDefaults;
begin
  //  A settings file edited by hand, or written by a later version with a
  //  column this one does not know.
  ApplyPriceDataChoice(99, -1);
  try
    AssertTrue('close', DefaultOhlcValueColumn = vcClose);
    AssertTrue('bar number', DefaultOhlcXMode = xmBarIndex);
  finally
    ApplyPriceDataChoice(0, 0);
  end;
end;
...
begin
  ...
  //  By the menu path that makes it, now that there is one.
  AssertTrue('offers the bar-number remedy: ' + Msg,
    Pos('Data > Price Data > Argument > Bar Number', Msg) > 0);
end;
...
begin
  ...
  AssertTrue('refused: ' + Msg, Msg <> '');
  AssertTrue('and names the way out: ' + Msg,
    Pos('Data > Price Data > Argument > Bar Number', Msg) > 0);
end;
