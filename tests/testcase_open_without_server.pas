...
begin
  ...
  //  READ AGAIN, as File > Reload Profile does once the workflow has found the
  //  project's file. Must not raise, for the same reason the first open must
  //  not: an unreachable server may not cost the user their file.
  FApp.FitClient.LoadDataSet(DataFile);
