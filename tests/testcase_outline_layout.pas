...
type
    ...
    published
        ...
        //  What the panel selects when it is filled.
        procedure ARowThatWasSelectedStaysSelected;
        procedure WithNothingSelectedTheFirstRowIs;
        procedure ARowThatStandsForNothingIsNotSelectedByDefault;
        procedure ARowThatIsGoneGivesWayToTheFirstRow;
        procedure AnEmptyOutlineSelectsNothing;
...
procedure TOutlineLayoutTest.ARowThatWasSelectedStaysSelected;
begin
    //  A refill must never move the user's choice.
    AddRow(0, 'one', 'a');
    AddRow(0, 'two', 'b');
    Build;
    AssertEquals(1, InitialOutlineIndex(FNodes, 'b'));
end;

procedure TOutlineLayoutTest.WithNothingSelectedTheFirstRowIs;
begin
    //  The panel opened for the first time: the model's first element is what
    //  the Explain pane describes, not a curve type the model may not contain.
    AddRow(0, 'one', 'a');
    AddRow(1, 'child', 'c');
    AddRow(0, 'two', 'b');
    Build;
    AssertEquals(0, InitialOutlineIndex(FNodes, ''));
end;

procedure TOutlineLayoutTest.ARowThatStandsForNothingIsNotSelectedByDefault;
begin
    //  A row with no id is a heading or a note; selecting it would select
    //  nothing while looking like a choice.
    AddRow(0, 'a heading', '');
    AddRow(1, 'the curve', 'a');
    Build;
    AssertEquals(1, InitialOutlineIndex(FNodes, ''));
end;

procedure TOutlineLayoutTest.ARowThatIsGoneGivesWayToTheFirstRow;
begin
    //  The selected curve was deleted: the panel falls back to the first element
    //  rather than to a blank selection.
    AddRow(0, 'one', 'a');
    AddRow(0, 'two', 'b');
    Build;
    AssertEquals(0, InitialOutlineIndex(FNodes, 'gone'));
end;

procedure TOutlineLayoutTest.AnEmptyOutlineSelectsNothing;
begin
    AddRow(0, 'a heading', '');
    Build;
    AssertEquals('only a row that stands for nothing', -1,
        InitialOutlineIndex(FNodes, ''));
    SetLength(FRows, 0);
    Build;
    AssertEquals('no rows at all', -1, InitialOutlineIndex(FNodes, ''));
end;
