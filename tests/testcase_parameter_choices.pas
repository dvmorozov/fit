...
type
    ...
    published
        //  The held check-list.
        procedure TheHeldListOffersTheOrdinaryParameters;
        procedure AHeldParameterIsTicked;
        procedure HoldingSwitchesOffTheFitsVariation;
        //  The abscissa combo.
...
{ ---- the held check-list ---------------------------------------------------
  ...
  THE REPORT: a user-defined curve's "fixed" parameters were shared, which the
  fit still varies; nothing could keep a parameter at the value the user gave
  it. The engine could - a parameter whose variation is disabled is not varied,
  by either engine - and nothing set it. The held list does. The position is
  not in it: the shared list's tick already holds a position where it is
  placed. }

procedure TParameterChoicesTest.TheHeldListOffersTheOrdinaryParameters;
begin
    Add('x', Argument);
    ...
    Add('k', Shared);
    Add('f', Calculated);
    AssertEquals('c k ', NamesOf(HeldChoices(FParams)));
end;

procedure TParameterChoicesTest.AHeldParameterIsTicked;
begin
    Add('c', Variable).VariationDisabled := True;
    Add('k', Variable);
    AssertTrue('held', HeldChoices(FParams)[0].Marked);
    AssertFalse('varied', HeldChoices(FParams)[1].Marked);
end;

procedure TParameterChoicesTest.HoldingSwitchesOffTheFitsVariation;
var
    P: TSpecialCurveParameter;
begin
    P := Add('c', Variable);
    HoldParameter(P, True);
    AssertTrue('held', P.VariationDisabled);
    AssertTrue('its kind unchanged', P.Type_ = Variable);
    HoldParameter(P, False);
    AssertFalse('varied again', P.VariationDisabled);
end;
