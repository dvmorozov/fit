...
type
    ...
    published
        ...
        procedure AFileWrittenBeforeAxisIdsKeepsItsDisplayMode;
        procedure TheBarsDatesSurviveARoundTrip;
        procedure AProfileWithoutDatesWritesNone;
        procedure OneUnreadableDateDropsThemAll;
        procedure TheFormerDisplayModeIsNoLongerWritten;
        procedure TheProvenanceOfTheSourceFileSurvives;
        procedure WhereFetchedDataCameFromSurvives;
        procedure AFileTheUserChoseWritesNoOrigin;
...
begin
    ...
    Result.UserCurveParams[0].HasKind := True;
    Result.UserCurveParams[0].Kind := 3;
    Result.UserCurveParams[0].Held := True;
    ...
    Result.HasUi := True;
    Result.Ui.Argument.Mode := 'diffraction.sin-theta-over-lambda';
    Result.Ui.Argument.ChosenByUser := True;
    Result.Ui.Argument.DataMode := 'price.bar';
    Result.Ui.Value.Mode := 'logarithmic';
    Result.Ui.Value.ChosenByUser := True;
    Result.Ui.Value.DataMode := 'price.price';
    Result.Ui.Value.CustomName := 'Counts';
    Result.Ui.Value.CustomInverse := 'x*1000';
    ...
    Result.Ui.Argument.CustomUnit := 'A';
    Result.Ui.Argument.CustomForward := 'x*2';
    Result.Ui.Argument.CustomInverse := 'x/2';
end;
...
begin
    ...
    AssertTrue('its role', Got.UserCurveParams[0].HasKind);
    AssertEquals('', 3, Got.UserCurveParams[0].Kind);
    AssertTrue('and that it is held', Got.UserCurveParams[0].Held);
end;
...
begin
    ...
    AssertEquals('the argument axis, by id', 'diffraction.sin-theta-over-lambda',
        Got.Ui.Argument.Mode);
    AssertTrue('and that the user chose it', Got.Ui.Argument.ChosenByUser);
    AssertEquals('the value axis', 'logarithmic', Got.Ui.Value.Mode);
    AssertTrue('chosen too', Got.Ui.Value.ChosenByUser);
    AssertEquals('what the data said of its argument', 'price.bar',
        Got.Ui.Argument.DataMode);
    AssertEquals('and of its value', 'price.price', Got.Ui.Value.DataMode);
    AssertEquals('the value''s own definition', 'Counts', Got.Ui.Value.CustomName);
    AssertEquals('', 'x*1000', Got.Ui.Value.CustomInverse);
    AssertFalse('an id, so nothing to migrate', Got.Ui.HasLegacyViewMode);
    ...
    AssertEquals('the user-defined axis', 'd', Got.Ui.Argument.CustomName);
    AssertEquals('', 'x*2', Got.Ui.Argument.CustomForward);
end;

procedure TProjectDocumentTest.AFileWrittenBeforeAxisIdsKeepsItsDisplayMode;
var
    Parts: TProjectParts;
    Got: TProjectDocument;
    Fault: string;
begin
    //  Its working context as every earlier build wrote it: an integer, the
    //  flag beside it, and the custom definition under the keys it still has.
    Parts := WithPart(ProjectToParts(AFullDocument), ProjectUiPart,
        '{"viewMode":2,"viewModeChosenByUser":true,"customAxisName":"d",' +
        '"customAxisForward":"x*2","customAxisInverse":"x/2","activeTab":1}');
    Got := EmptyProjectDocument;
    AssertTrue(Fault, ProjectFromParts(Parts, Got, Fault));
    AssertTrue('there is a former mode to migrate', Got.Ui.HasLegacyViewMode);
    AssertEquals(2, Got.Ui.LegacyViewMode);
    AssertTrue('and it was chosen', Got.Ui.Argument.ChosenByUser);
    AssertEquals('no id yet', '', Got.Ui.Argument.Mode);
    AssertEquals('the definition is where it always was', 'd',
        Got.Ui.Argument.CustomName);
    AssertEquals('x/2', Got.Ui.Argument.CustomInverse);
    AssertFalse('the value was never chosen', Got.Ui.Value.ChosenByUser);
end;

procedure TProjectDocumentTest.TheBarsDatesSurviveARoundTrip;
var
    Doc, Got: TProjectDocument;
begin
    //  SAVED for the reason the data's mode is: the project holds the points
    //  and not the file, and the bar numbers alone cannot say what day each
    //  one was.
    Doc := AFullDocument;
    SetLength(Doc.Ui.ArgumentDates, 2);
    Doc.Ui.ArgumentDates[0] := EncodeDate(2020, 9, 24);
    Doc.Ui.ArgumentDates[1] := EncodeDateTime(2020, 9, 25, 14, 30, 0, 0);
    Got := RoundTrip(Doc);
    AssertEquals(2, Length(Got.Ui.ArgumentDates));
    AssertEquals(EncodeDate(2020, 9, 24), Got.Ui.ArgumentDates[0], 0);
    AssertEquals('a time of day too, to the second',
        EncodeDateTime(2020, 9, 25, 14, 30, 0, 0), Got.Ui.ArgumentDates[1],
        1 / SecsPerDay / 2);
end;

procedure TProjectDocumentTest.AProfileWithoutDatesWritesNone;
var
    Text: string;
begin
    AssertTrue(PartContent(ProjectToParts(AFullDocument), ProjectUiPart, Text));
    AssertEquals('an ordinary profile''s file is unchanged', 0,
        Pos('argumentDates', Text));
    AssertEquals(0, Length(RoundTrip(AFullDocument).Ui.ArgumentDates));
end;

procedure TProjectDocumentTest.OneUnreadableDateDropsThemAll;
var
    Parts: TProjectParts;
    Got: TProjectDocument;
    Fault: string;
begin
    //  One per bar or none: a list with a hole in it names every later bar by
    //  its neighbour's day. The project still opens; its bars go unnamed.
    Parts := WithPart(ProjectToParts(AFullDocument), ProjectUiPart,
        '{"argumentAxis":"","argumentDates":["2020-09-24","yesterday"]}');
    Got := EmptyProjectDocument;
    AssertTrue(Fault, ProjectFromParts(Parts, Got, Fault));
    AssertEquals(0, Length(Got.Ui.ArgumentDates));
end;

procedure TProjectDocumentTest.TheFormerDisplayModeIsNoLongerWritten;
var
    Text: string;
begin
    //  A build that still reads 'viewMode' finds none, and falls back to its
    //  own default rather than onto an axis a stale integer names.
    AssertTrue(PartContent(ProjectToParts(AFullDocument), ProjectUiPart, Text));
    AssertEquals(0, Pos('"viewMode"', Text));
    AssertTrue('the id is', Pos('"argumentAxis"', Text) > 0);
end;
...
procedure TProjectDocumentTest.WhereFetchedDataCameFromSurvives;
var
    Doc, Got: TProjectDocument;
begin
    //  A DOWNLOADED FILE'S PATH SAYS NOTHING. It is a name in a cache
    //  directory - 'downloads/fred/SP500.csv' - so the service, the question
    //  asked and the day it was fetched are recorded beside it. That is what
    //  has to be quoted when a result computed from it is published.
    Doc := AFullDocument;
    Doc.Provenance.OriginSourceId := 'fred';
    Doc.Provenance.OriginSourceTitle := 'FRED (St. Louis Fed)';
    Doc.Provenance.OriginQuery := 'Series SP500, Bars Monthly';
    Doc.Provenance.OriginAddress := 'https://example.org/fredgraph.csv?id=SP500';
    Doc.Provenance.OriginRetrievedAt := '2026-09-23T20:50:00';

    Got := RoundTrip(Doc);
    AssertEquals('', 'fred', Got.Provenance.OriginSourceId);
    AssertEquals('', 'FRED (St. Louis Fed)', Got.Provenance.OriginSourceTitle);
    AssertEquals('', 'Series SP500, Bars Monthly', Got.Provenance.OriginQuery);
    AssertEquals('', 'https://example.org/fredgraph.csv?id=SP500',
        Got.Provenance.OriginAddress);
    AssertEquals('', '2026-09-23T20:50:00', Got.Provenance.OriginRetrievedAt);
    //  And what was already recorded is still there.
    AssertEquals('', 'C:\data\2.dat', Got.Provenance.SourcePath);
end;

procedure TProjectDocumentTest.AFileTheUserChoseWritesNoOrigin;
var
    Got: TProjectDocument;
begin
    //  Nothing fetched it, so there is nothing to record - and a project from
    //  before data sources existed reads back exactly as it always did.
    Got := RoundTrip(AFullDocument);
    AssertEquals('', '', Got.Provenance.OriginSourceId);
    AssertEquals('', '', Got.Provenance.OriginAddress);
end;
