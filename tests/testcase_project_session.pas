...
uses
    ...
    //  The model as the window reads it back to draw: the fingerprint must not
    //  move for that.
    self_copied_component, mscr_specimen_list;
...
type
    ...
    published
        //  A setting the file does not state.
        procedure ASettingTheFileLeavesOutKeepsTheEnginesDefault;
        ...
        //  THE FINGERPRINT, which is what tells unsaved work from none.
        procedure AProblemNothingHappenedToKeepsItsFingerprint;
        procedure DrawingTheModelDoesNotChangeTheFingerprint;
        //  The axes are the project's, and the rest of the window is not.
        procedure ChoosingAnAxisIsUnsavedWork;
        procedure WhatTheDataSaysItIsIsPartOfTheDocument;
        procedure TheTabAndTheSelectionAreNot;
        procedure MovingAPickDoesChangeIt;
        procedure TheDifferingPartsOfTwoIdenticalListsAreNone;
        procedure APartWhoseContentMovedIsNamed;
        procedure APartOnlyOneListHasIsNamedAsNewOrGone;
        ...
        procedure EachUserParameterKeepsItsRoleAndWhetherItIsHeld;
    end;
...
begin
    ...
    Ctx.HasUi := True;
    Ctx.Ui.Argument.Mode := 'diffraction.sin-theta-over-lambda';
    Ctx.Ui.ActiveTab := 1;
    ...
    AssertEquals('', 'diffraction.sin-theta-over-lambda', Doc.Ui.Argument.Mode);
...
begin
    ...
    AssertTrue('applied: ' + Fault, Applied);
    //  AN IMPLICIT CONVERSION, NOT `as`. On a CORBA interface `as` looks the
    //  interface up by name at run time, and the entry it found here answered
    //  this call with some other method: the result was a points set, which
    //  the Assigned check below was happy to accept.
    Svc := FTarget;
    CP := Svc.GetSpecialCurveParameters;
    try
...
procedure TProjectSessionTest.EachUserParameterKeepsItsRoleAndWhetherItIsHeld;
var
    Doc: TProjectDocument;
    CP: Curve_parameters;
    Svc: IFitService;
    Fault: string;
    i: longint;
    SawArgument, SawHeld: boolean;

    procedure Add(const AName: string; AType: TParameterType; AHeld: boolean);
    var
        Container: TPersistentCurveParameterContainer;
    begin
        Container := TPersistentCurveParameterContainer(CP.Params.Add);
        Container.Parameter.Name := AName;
        Container.Parameter.Value := 3;
        Container.Parameter.Type_ := AType;
        Container.Parameter.VariationDisabled := AHeld;
    end;

begin
    //  A ROLE IS WHAT THE USER CHOSE in the curve's dialog, and so is a held
    //  value: a project that forgot them would reopen with x fitted as a
    //  parameter and tau varied although the user had pinned it.
    GivenAProblem(FSource);
    Svc := FSource;
    CP := Curve_parameters.Create(nil);
    CP.Params.Clear;
    Add('x', Argument, False);
    Add('A', Variable, False);
    Add('tau', Variable, True);
    Svc.SetSpecialCurveParameters('A*exp(-x/tau)', CP);
    Doc := CaptureProject(FSource, WithAUserCurve, EmptyProjectDocument);

    AssertTrue('applied: ' + Fault, ApplyProject(FTarget, Doc, Fault));
    Svc := FTarget;
    CP := Svc.GetSpecialCurveParameters;
    try
        SawArgument := False;
        ...
        begin
            if CP[i].Name = 'x' then
                SawArgument := CP[i].Type_ = Argument;
            if CP[i].Name = 'tau' then
                SawHeld := CP[i].VariationDisabled;
            if CP[i].Name = 'A' then
                AssertFalse('A is not held', CP[i].VariationDisabled);
        end;
        AssertTrue('x is still the argument', SawArgument);
        AssertTrue('tau is still held', SawHeld);
    finally
        CP.Free;
    end;
end;
...
procedure TProjectSessionTest.ASettingTheFileLeavesOutKeepsTheEnginesDefault;
var
    Parts: TProjectParts;
    Text, Fault: string;
    Doc: TProjectDocument;
    Svc: IFitService;
begin
    //  THE REPORT: a project written without maxRFactor opened with a ceiling of
    //  0 - no fit can meet it - because the reader's fallback for an absent
    //  member was a literal zero, applied as if the file had said it. The same
    //  zero for backFactor is a divisor in the peak search, and an absent
    //  curveScaling turned scaling off although the engine's default is on.
    //  A member the file does not state leaves what the engine already has.
    GivenAProblem(FSource);
    Parts := ProjectToParts(CaptureProject(FSource, NoContext,
        EmptyProjectDocument));
    AssertTrue('the problem part is there',
        PartContent(Parts, ProjectProblemPart, Text));
    Text := StringReplace(Text, '"maxRFactor"', '"gone1"', []);
    Text := StringReplace(Text, '"backFactor"', '"gone2"', []);
    Text := StringReplace(Text, '"curveScaling"', '"gone3"', []);
    Parts := WithPart(Parts, ProjectProblemPart, Text);
    AssertTrue('it still opens: ' + Fault, ProjectFromParts(Parts, Doc, Fault));
    AssertTrue('and applies: ' + Fault, ApplyProject(FTarget, Doc, Fault));

    Svc := FTarget;
    AssertEquals('the engine''s ceiling', 0.0001, Svc.GetMaxRFactor, 1e-12);
    AssertEquals('the engine''s background factor', 30, Svc.GetBackFactor, 1e-12);
    AssertTrue('the engine''s curve scaling', Svc.GetCurveScalingEnabled);
end;

{ ---- the fingerprint ------------------------------------------------------ }

{ THE REPORT THESE ARE FOR: closing a project nobody had touched asked "Project
  has been modified. Save?". The window's own modified flags are one half of
  that answer and this is the other, so what it must never do is move on its
  own. }
procedure TProjectSessionTest.AProblemNothingHappenedToKeepsItsFingerprint;
var
    First: string;
begin
    GivenAProblem(FSource);
    First := ProjectFingerprint(FSource, NoContext);
    AssertTrue('a problem that can be asked has a fingerprint', First <> '');
    AssertEquals('asked twice, with nothing in between', First,
        ProjectFingerprint(FSource, NoContext));
end;

procedure TProjectSessionTest.DrawingTheModelDoesNotChangeTheFingerprint;
var
    Before: string;
    Svc: IFitService;
...
begin
    //  WHAT THE WINDOW DOES WHILE THE USER DOES NOTHING: it re-reads the model
    //  to draw it, asks for the statistics to put in the status bar, and asks
    //  each module for its state. None of that is a change, and a fingerprint
    //  that moved for it would be exactly the spurious question reported.
    GivenAProblem(FSource);
    Svc := FSource;
    Before := ProjectFingerprint(FSource, NoContext);

    Curves := Svc.GetCurves;
    Curves.Free;
    Attrs := Svc.GetCurveAttributes;
    Attrs.Free;
    Svc.GetCalcProfilePointsSet.Free;
    Svc.GetDeltaProfilePointsSet.Free;
    Svc.GetResultedCurvePositions.Free;
    Svc.GetStatistics;
    Svc.GetRFactorStr;
    Svc.GetModuleProjectStates;

    AssertEquals('reading is not changing', Before,
        ProjectFingerprint(FSource, NoContext));
end;

procedure TProjectSessionTest.MovingAPickDoesChangeIt;
var
    Before: string;
    Svc: IFitService;
begin
    //  AND THE OTHER HALF OF THE RULE: it must still notice real work, or the
    //  question would stop being asked when it matters.
    GivenAProblem(FSource);
    Before := ProjectFingerprint(FSource, NoContext);
    Svc := FSource;
    Svc.AddPointToBackground(5, 11);
    AssertTrue('an edited background is a different document',
        ProjectFingerprint(FSource, NoContext) <> Before);
end;

procedure TProjectSessionTest.ChoosingAnAxisIsUnsavedWork;
var
    Chosen: TProjectClientContext;
begin
    //  PROJECT SCALE: an axis is saved with the project and restored from it,
    //  so choosing one is a change to the document - and closing without
    //  saving must ask, as it asks for a moved pick.
    GivenAProblem(FSource);
    Chosen := NoContext;
    Chosen.HasUi := True;
    Chosen.Ui.Value.Mode := 'logarithmic';
    Chosen.Ui.Value.ChosenByUser := True;
    AssertTrue(ProjectFingerprint(FSource, Chosen) <>
        ProjectFingerprint(FSource, NoContext));
end;

procedure TProjectSessionTest.WhatTheDataSaysItIsIsPartOfTheDocument;
var
    Said: TProjectClientContext;
begin
    GivenAProblem(FSource);
    Said := NoContext;
    Said.HasUi := True;
    Said.Ui.Argument.DataMode := 'price.bar';
    AssertTrue(ProjectFingerprint(FSource, Said) <>
        ProjectFingerprint(FSource, NoContext));
end;

procedure TProjectSessionTest.TheTabAndTheSelectionAreNot;
var
    Looked: TProjectClientContext;
begin
    //  Looking at another tab, or selecting a curve, changes what the window
    //  shows and nothing the project is about.
    GivenAProblem(FSource);
    Looked := NoContext;
    Looked.HasUi := True;
    Looked.Ui.ActiveTab := 3;
    Looked.Ui.SelectedCurveId := 'BF4AC0C2-A363-457A-92D5-44CACB82F185';
    Looked.Ui.SelectionMode := 2;
    AssertEquals(ProjectFingerprint(FSource, NoContext),
        ProjectFingerprint(FSource, Looked));
end;

procedure TProjectSessionTest.TheDifferingPartsOfTwoIdenticalListsAreNone;
var
    Parts: TProjectParts;
begin
    GivenAProblem(FSource);
    Parts := ProjectFingerprintParts(FSource, NoContext);
    AssertTrue('a captured problem has parts', Length(Parts) > 0);
    AssertEquals('nothing differs', '', DifferingPartNames(Parts, Parts));
end;

procedure TProjectSessionTest.APartWhoseContentMovedIsNamed;
var
    Before, Now_: TProjectParts;
begin
    Before := ProjectFingerprintParts(FSource, NoContext);
    Now_ := Copy(Before, 0, Length(Before));
    Now_[0].Content := Now_[0].Content + '!';
    AssertEquals('the one that moved, by name', Before[0].Name,
        DifferingPartNames(Before, Now_));
end;

procedure TProjectSessionTest.APartOnlyOneListHasIsNamedAsNewOrGone;
var
    Before, Now_: TProjectParts;
begin
    SetLength(Before, 1);
    Before[0].Name := 'stays.json';
    Before[0].Content := 'x';
    Now_ := Copy(Before, 0, 1);
    Now_ := WithPart(Now_, 'modules/demo.json', 'y');
    AssertEquals('added', 'modules/demo.json (new)',
        DifferingPartNames(Before, Now_));
    //  A MODULE THAT DROPPED ITS STATE changes the document as surely as one
    //  that rewrote it, and comparing the new list alone cannot see that.
    AssertEquals('removed', 'modules/demo.json (gone)',
        DifferingPartNames(Now_, Before));
end;
