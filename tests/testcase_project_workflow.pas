...
uses
    ...
    gauss_points_set, coordinate_axis;
...
type
    ...
    published
        ...
        procedure NewAsksAboutUnsavedWorkAndCancelKeepsIt;
        procedure NewAfterYesSavesFirst;
        procedure NewAfterNoDiscardsWithoutSaving;
        procedure AFailedSaveStopsTheNew;
        procedure OpenAsksAboutUnsavedWorkBeforeTheDialog;
        procedure OpeningARecentProjectAsksToo;
        procedure PicksBeforeAnyModelCountAsUnsavedWork;
        procedure AFreshImportIsNotUnsavedWork;
        //  A file that was fetched rather than chosen.
        procedure ADownloadRemembersWhereItCameFrom;
        //  Whether a data file is new to the document - which is when its axes
        //  start again on Automatic.
        procedure AnyFileIsNewToADocumentThatHasNone;
        procedure TheFileTheDocumentCameFromIsNotNew;
        procedure AnotherFileIsNew;
        procedure AFileTheUserChoseRecordsNoOrigin;
        procedure TheTablesFilledByALoadAreNotUnsavedWorkEither;
        procedure NothingIsUnsavedRightAfterASave;
        procedure NothingIsUnsavedRightAfterAnOpen;
        ...
        procedure ReloadingReadsTheFileTheOpenProjectCameFrom;
        procedure AProjectThatNamesNoSourceHasNothingToReload;
        procedure ASourceThatIsGoneIsReportedRatherThanRead;
...
procedure TProjectWorkflowTest.NewAsksAboutUnsavedWorkAndCancelKeepsIt;
begin
    //  THE REPORT: New Project and Open Project replaced the work without a
    //  word, while closing the window asked. Same loss, same question.
    FFlow.Path := PathIn('a.fitproj');
    FHostObj.ScriptUnsavedWork(True);
    FHostObj.ScriptCloseAnswer(saCancel);
    FFlow.NewProject;
    AssertTrue('asked', FHostObj.Log.Saw('AskSaveBeforeClosing'));
    AssertFalse('and cancelled: nothing was cleared',
        FHostObj.Log.Saw('ClearEverything'));
    AssertEquals('the document is still the same', PathIn('a.fitproj'),
        FFlow.Path);
end;

procedure TProjectWorkflowTest.NewAfterYesSavesFirst;
begin
    GivenAProblem;
    FFlow.Path := PathIn('a.fitproj');
    FHostObj.ScriptUnsavedWork(True);
    ...
    FFlow.NewProject;
    AssertEquals('saved', 1, SaveCount);
    AssertEquals('where it was', PathIn('a.fitproj'), SavedTo);
    AssertTrue('then cleared', FHostObj.Log.Saw('ClearEverything'));
end;

procedure TProjectWorkflowTest.NewAfterNoDiscardsWithoutSaving;
begin
    FHostObj.ScriptUnsavedWork(True);
    FHostObj.ScriptCloseAnswer(saNo);
    FFlow.NewProject;
    AssertEquals('not saved', 0, SaveCount);
    AssertTrue('cleared', FHostObj.Log.Saw('ClearEverything'));
end;

procedure TProjectWorkflowTest.AFailedSaveStopsTheNew;
begin
    //  The user asked for the work to be kept; clearing after failing to keep
    //  it destroys exactly what they asked to save.
    GivenAProblem;
    FFlow.Path := PathIn('a.fitproj');
    FHostObj.ScriptUnsavedWork(True);
    FHostObj.ScriptCloseAnswer(saYes);
    SaveWorks := False;
    FFlow.NewProject;
    AssertFalse('nothing cleared', FHostObj.Log.Saw('ClearEverything'));
end;

procedure TProjectWorkflowTest.OpenAsksAboutUnsavedWorkBeforeTheDialog;
begin
    FHostObj.ScriptUnsavedWork(True);
    FHostObj.ScriptCloseAnswer(saCancel);
    FHostObj.ScriptOpen(PathIn('b.fitproj'));
    AssertFalse('nothing opened', FFlow.OpenProject);
    AssertFalse('and the Open dialog was never shown',
        FHostObj.Log.Saw('AskProjectToOpen'));
end;

procedure TProjectWorkflowTest.OpeningARecentProjectAsksToo;
begin
    FHostObj.ScriptUnsavedWork(True);
    FHostObj.ScriptCloseAnswer(saCancel);
    AssertFalse('nothing opened', FFlow.OpenRecentProject(PathIn('b.fitproj')));
    AssertEquals('nothing shown', '', FHostObj.Shown);
    FHostObj.ScriptCloseAnswer(saNo);
    AssertTrue('opened once the user says to go on',
        FFlow.OpenRecentProject(PathIn('b.fitproj')));
end;

procedure TProjectWorkflowTest.PicksBeforeAnyModelCountAsUnsavedWork;
var
    Picks: TTitlePointsSet;
begin
    //  THE REPORT: "unsaved" was only the two table-modified flags, so points
    //  picked before any model existed - which reach neither table - were not
    //  work, and New or closing threw them away without a question.
    GivenAProblem;
    FFlow.RememberSource(PathIn('one.dat'), 'dat');
    Picks := TTitlePointsSet.Create(nil);
    Picks.AddNewPoint(3, 13);
    FService.SetBackgroundPointsSet(Picks);
    FHostObj.ScriptCloseAnswer(saCancel);
    FFlow.NewProject;
    AssertTrue('asked', FHostObj.Log.Saw('AskSaveBeforeClosing'));
    AssertFalse('and kept', FHostObj.Log.Saw('ClearEverything'));
end;

{ WHY THE PATH IS NOT ENOUGH. A downloaded file's path is a name in a cache
  directory - 'downloads/doi/quartz.xy' - which says nothing about which service
  it came from, what was asked for, or when. That is exactly what has to be
  quoted when a result computed from it is published, so it is recorded beside
  the path rather than left to be guessed from it. }
procedure TProjectWorkflowTest.AnyFileIsNewToADocumentThatHasNone;
begin
    GivenAProblem;
    AssertTrue(FFlow.IsNewSource(PathIn('quartz.xy')));
end;

procedure TProjectWorkflowTest.TheFileTheDocumentCameFromIsNotNew;
begin
    //  Reload Profile: the same data, so the axes the user chose for it stay.
    GivenAProblem;
    FFlow.RememberSource(PathIn('quartz.xy'), 'xy');
    AssertFalse(FFlow.IsNewSource(PathIn('quartz.xy')));
end;

procedure TProjectWorkflowTest.AnotherFileIsNew;
begin
    GivenAProblem;
    FFlow.RememberSource(PathIn('quartz.xy'), 'xy');
    AssertTrue(FFlow.IsNewSource(PathIn('S&P500.csv')));
end;

procedure TProjectWorkflowTest.ADownloadRemembersWhereItCameFrom;
var
    Provenance: TProjectProvenance;
begin
    GivenAProblem;
    FFlow.RememberSource(PathIn('quartz.xy'), 'xy');
    FFlow.RememberOrigin('doi', 'Published data (DOI)',
        'DOI 10.5281/zenodo.1234567',
        'https://zenodo.org/api/records/1234567/files/quartz.xy/content',
        '2026-09-23T11:00:00');
    Provenance := FFlow.ContextToSave.Provenance;
    AssertEquals('the source that fetched it', 'doi', Provenance.OriginSourceId);
    AssertEquals('what was asked for', 'DOI 10.5281/zenodo.1234567',
        Provenance.OriginQuery);
    AssertTrue('where the bytes came from',
        Pos('zenodo.org', Provenance.OriginAddress) > 0);
    AssertEquals('and when', '2026-09-23T11:00:00', Provenance.OriginRetrievedAt);
    //  And the file itself is still recorded as it always was.
    AssertTrue('the path is still there',
        Pos('quartz.xy', Provenance.SourcePath) > 0);
end;

procedure TProjectWorkflowTest.AFileTheUserChoseRecordsNoOrigin;
begin
    //  Nothing fetched it, so there is nothing to record - and the project
    //  file then carries exactly the keys it has always carried.
    GivenAProblem;
    FFlow.RememberSource(PathIn('one.dat'), 'dat');
    AssertEquals('', FFlow.ContextToSave.Provenance.OriginSourceId);
end;

procedure TProjectWorkflowTest.AFreshImportIsNotUnsavedWork;
begin
    //  Reading a file is not work: the file is still there.
    GivenAProblem;
    FFlow.RememberSource(PathIn('one.dat'), 'dat');
    FFlow.NewProject;
    AssertFalse('nothing asked', FHostObj.Log.Saw('AskSaveBeforeClosing'));
end;

procedure TProjectWorkflowTest.TheTablesFilledByALoadAreNotUnsavedWorkEither;
begin
    //  THE REPORT: start Fit with /INFILE, or File > Import Profile, touch
    //  nothing, close - and it asks "Project has been modified. Save?".
    //
    //  Loading a data file FILLS THE TABLES, and filling a table is what raises
    //  the window's two modified flags (fit_viewer) - they say a table was
    //  drawn, not that anyone changed anything. Rebasing alone cannot answer
    //  them, because the flags are read first and short-circuit the comparison.
    //  So the one path that means "this came from a file that still holds it"
    //  has to say so to the window as well.
    GivenAProblem;
    //  What LoadDataSet does to the window on its way through.
    FHostObj.ScriptUnsavedWork(True);
    FFlow.RememberSource(PathIn('one.dat'), 'dat');
    AssertTrue('the window was told the tables are not unsaved work',
        FHostObj.Log.Saw('MarkSaved'));
    AssertTrue('and closing goes ahead without a question', FFlow.MayClose);
    AssertFalse('nothing asked', FHostObj.Log.Saw('AskSaveBeforeClosing'));
end;

procedure TProjectWorkflowTest.NothingIsUnsavedRightAfterASave;
var
    Picks: TTitlePointsSet;
begin
    GivenAProblem;
...
procedure TProjectWorkflowTest.NothingIsUnsavedRightAfterAnOpen;
begin
    //  What the opened project put into the engine; the fake open itself
    //  touches nothing, so the problem is built first and the open follows.
    GivenAProblem;
    AssertTrue('opened', FFlow.OpenProjectAt(PathIn('a.fitproj')));
    FFlow.NewProject;
    AssertFalse('nothing asked', FHostObj.Log.Saw('AskSaveBeforeClosing'));
end;
...
begin
    ...
    OpenedUi.HasUi := True;
    OpenedUi.Ui.Argument.ChosenByUser := True;
    OpenedUi.Ui.Argument.Mode := 'diffraction.sin-theta-over-lambda';
    OpenedUi.Ui.ActiveTab := 1;
    FHostObj.ScriptOpen(PathIn('a.fitproj'));
    AssertTrue('opened', FFlow.OpenProject);
    AssertTrue('the window was told', FHostObj.Log.Saw('ApplyWorkingContext'));
    AssertTrue('the axis', FHostObj.Applied.Axes[adArgument].Apply);
    AssertEquals('', 'diffraction.sin-theta-over-lambda',
        FHostObj.Applied.Axes[adArgument].Mode);
...
begin
    ...
    ExistingPaths := PathIn('two.dat');
    FHostObj.ScriptConfirmAnswer(False);
    FFlow.ReloadProfile;
    AssertEquals('refused, so nothing was read', '', FHostObj.Loaded);
    AssertTrue('and it named the file it would re-read: ' + FHostObj.Asked,
        Pos('two.dat', FHostObj.Asked) > 0);
end;

procedure TProjectWorkflowTest.ReloadingReadsTheFileTheOpenProjectCameFrom;
begin
    //  THE REPORT: Reload re-read whatever file had last been IMPORTED in this
    //  session. After importing a.dat and opening a project built from c.dat
    //  the question named c.dat and the command re-read a.dat; with no import
    //  at all it tripped an internal check. The file to re-read is the one the
    //  open document says it came from.
    FFlow.RememberSource(PathIn('a.dat'), 'dat');
    OpenedUi.Provenance.SourcePath := PathIn('c.dat');
    ExistingPaths := PathIn('a.dat') + ';' + PathIn('c.dat');
    FHostObj.ScriptOpen(PathIn('c.fitproj'));
    AssertTrue('opened', FFlow.OpenProject);
    FHostObj.ScriptConfirmAnswer(True);
    FFlow.ReloadProfile;
    AssertEquals('the project''s own source was read', PathIn('c.dat'),
        FHostObj.Loaded);
    AssertEquals('without a problem', '', FHostObj.Problem);
end;

procedure TProjectWorkflowTest.AProjectThatNamesNoSourceHasNothingToReload;
begin
    FHostObj.ScriptOpen(PathIn('c.fitproj'));
    AssertTrue('opened', FFlow.OpenProject);
    FFlow.ReloadProfile;
    AssertEquals('nothing read', '', FHostObj.Loaded);
    AssertTrue('and the user is told why', FHostObj.Problem <> '');
end;

procedure TProjectWorkflowTest.ASourceThatIsGoneIsReportedRatherThanRead;
begin
    OpenedUi.Provenance.SourcePath := PathIn('moved.dat');
    FHostObj.ScriptOpen(PathIn('c.fitproj'));
    AssertTrue('opened', FFlow.OpenProject);
    FHostObj.ScriptConfirmAnswer(True);
    FFlow.ReloadProfile;
    AssertEquals('nothing read', '', FHostObj.Loaded);
    AssertTrue('the message names the file: ' + FHostObj.Problem,
        Pos('moved.dat', FHostObj.Problem) > 0);
    AssertEquals('and nothing was asked first', '', FHostObj.Asked);
end;
