...
type
    ...
    published
        ...
        procedure TheProblemAsksForProgress;
        procedure ProgressIsAskedOfTheSidecarsProgressRoute;
        procedure TheFinalReplyWinsOverTheLastProgress;
    ...
    published
        ...
        procedure TheFitGoesToTheFitRoute;
        procedure TheProblemSentCarriesTheProfile;
        procedure ProgressIsAskedOfTheServersProgressRoute;
...
procedure TPythonBackendTest.TheProblemAsksForProgress;
begin
    //  Without an id the sidecar has nowhere to put what it reaches, and the fit
    //  is silent until it returns - which is what it was.
    FBackend.Reply('{"errorCode":0,"rFactor":0.1}');
    FBackend.Fit(FTask);
    AssertTrue('a progress id is sent: ' + Copy(FBackend.LastBody, 1, 200),
        Pos('"progressId"', FBackend.LastBody) > 0);
end;

procedure TPythonBackendTest.ProgressIsAskedOfTheSidecarsProgressRoute;
begin
    FBackend.Reply('{"errorCode":0,"rFactor":0.1}');
    FBackend.HoldPostUntilPolls := 1;
    FBackend.Fit(FTask);
    AssertTrue('asked while the fit was out', FBackend.Gets >= 1);
    AssertEquals('of the progress route, under this fit''s id: ' +
        FBackend.LastGetUrl, 1, Pos(URL + '/fit/progress?id=',
        FBackend.LastGetUrl));
end;

procedure TPythonBackendTest.TheFinalReplyWinsOverTheLastProgress;

    function WithSigma(ASigma: double): string;
    var
        O: TFitOutcome;
        j: longint;
    begin
    ...
    begin
        ...
        for j := 0 to High(O.Curves[0].Params) do
            if SameText(O.Curves[0].Params[j].Name, 'Sigma') then
                Result := O.Curves[0].Params[j].Value;
    end;

begin
    //  A frame is what the far side had reached; the reply is what it finished
    //  with. A relay that let the last frame stand would report a fit that
    //  stopped short of its own result.
    FBackend.ProgressReply := WithSigma(3.0);
    FBackend.Reply(WithSigma(2.0));
    FBackend.HoldPostUntilPolls := 1;
    FBackend.Fit(FTask);
    AssertEquals('the result, not the last frame', 2.0, SigmaOfTask, 1e-9);
end;
...
procedure TServerBackendTest.ProgressIsAskedOfTheServersProgressRoute;
begin
    //  The same contract as the sidecar's, so a fit routed to a remote compute
    //  server shows its progress as a local one does.
    FBackend.ReplyToPost('{"errorCode":0,"rFactor":0.1}');
    FBackend.HoldPostUntilGets := 1;
    FBackend.Fit(FTask);
    AssertTrue('a progress id is sent', Pos('"progressId"', FBackend.LastBody) > 0);
    AssertTrue('asked while the fit was out', FBackend.Gets >= 1);
    AssertEquals('of the progress route: ' + FBackend.LastGetUrl, 1,
        Pos(FBackend.Url + '/fit/progress?id=', FBackend.LastGetUrl));
end;
