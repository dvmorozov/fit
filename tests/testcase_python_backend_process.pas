...
type
  TPyCB = class
    { How often the engine was told the fit improved. A fit in another
      process relays each improvement through this same callback. }
  ...
  private
    ...
    { Twelve peaks over two thousand points, each seeded off its own: a fit
      the sidecar takes SECONDS over, which is what makes the progress it
      publishes meanwhile observable at all. }
    function ManyPeaksTask: TFitTask;
  ...
  published
    ...
    procedure ARealSidecarFitRelaysItsProgressWhileItRuns;
  end;
...
procedure TPyCB.Count; begin Inc(Improvements); end;
...
function TPythonBackendProcessTest.ManyPeaksTask: TFitTask;
var
  P: TFitProblem;
  i, n: integer;
  v, Sum: double;
begin
  ...
  begin
    ...
    Sum := 0;
    for i := 0 to 11 do
      Sum := Sum + GaussPoint(100, 1.5, 5 + i * 8.0, v);
    P.ProfileY[n] := Sum;
    Inc(n);
    v := v + 0.05;
  end;
  SetLength(P.PositionsX, 12);
  SetLength(P.PositionsY, 12);
  for i := 0 to 11 do
  begin
    //  SEEDED OFF ITS PEAK, so the solver has real work to do.
    P.PositionsX[i] := 5 + i * 8.0 + 2.5;
    P.PositionsY[i] := 20;
  end;
  P.CurveTypeId := GUIDToString(TGaussPointsSet.GetCurveTypeId);
  P.MinimizerKind := 0;
  Result := BuildTaskFromProblem(P);
  //  THE FUNNEL EVERY ENGINE REPORTS THROUGH, counted.
  Result.ServerShowCurMin := @FCB.Count;
end;

{ A FIT IN ANOTHER PROCESS, WATCHED WHILE IT RUNS - against the real sidecar.

  The desktop draws a live loss curve for every fit, whichever engine runs it.
  For the sidecar that means: the problem carries a progressId, lmfit publishes
  the best parameters it has reached under it, and this side polls
  GET /fit/progress and relays each answer into the task through the same call
  the native minimizer uses. Nothing but an end-to-end fit can say that whole
  path is connected - a mocked one proves the marshalling and nothing about the
  two processes. }
procedure TPythonBackendProcessTest.ARealSidecarFitRelaysItsProgressWhileItRuns;
var
  Url: string;
  Task: TFitTask;
  Backend: IFitBackend;
begin
  ...
  try
    ...
    Backend := TPythonFitBackend.Create(Url);
    Backend.Fit(Task);
    AssertTrue(Format('the sidecar''s progress reached the task while it was ' +
      'fitting (%d improvements relayed)', [FCB.Improvements]),
      FCB.Improvements > 0);
  finally
    Task.Free;
  end;
