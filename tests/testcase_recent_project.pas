...
type
    ...
    published
        ...
        //  What the window remembers as it retitles itself.
        procedure OpeningAProjectIsWhatGetsRemembered;
        procedure StartingANewProjectKeepsTheLastOneRemembered;
    ...
    published
        ...
        //  What File > Open Recent shows.
        procedure EachRememberedProjectIsALineThatOpensIt;
        procedure TheWholePathIsTheCaptionBecauseNamesRepeat;
        procedure AnAmpersandInAPathIsShownAndStillOpensIt;
        procedure AnEmptyListStillShowsALine;
        procedure ThatLineIsDisabledAndOpensNothing;
        procedure TheLinesAreInTheOrderTheListHoldsThem;
    end;
...
procedure TRecentProjectTest.OpeningAProjectIsWhatGetsRemembered;
begin
    AssertEquals('the project just opened is the one to reopen',
        '/p/two.fitproj', RememberedAfterShowing('/p/one.fitproj',
        '/p/two.fitproj'));
end;

procedure TRecentProjectTest.StartingANewProjectKeepsTheLastOneRemembered;
begin
    //  AN UNSAVED DOCUMENT IS NOT A PROJECT TO REOPEN, and forgetting the one
    //  that was remembered loses a convenience for nothing: the window shows
    //  an empty document with no path, and the next start would then open
    //  nothing at all.
    //
    //  THE REPORT: every import from a data source starts a new project, so
    //  fetching a series once made the application stop reopening the project
    //  the user had been working on.
    AssertEquals('/p/one.fitproj',
        RememberedAfterShowing('/p/one.fitproj', ''));
    AssertEquals('and nothing remembered stays nothing', '',
        RememberedAfterShowing('', ''));
end;
...
procedure TRecentListTest.EachRememberedProjectIsALineThatOpensIt;
var
    L: TRecentMenuLines;
begin
    L := RecentMenu('a.fitproj|b.fitproj');
    AssertEquals('one line per project', 2, Length(L));
    AssertTrue('the first can be clicked', L[0].Enabled);
    AssertTrue('and it opens a project', L[0].OpensAProject);
end;

procedure TRecentListTest.TheWholePathIsTheCaptionBecauseNamesRepeat;
var
    L: TRecentMenuLines;
begin
    //  TWO PROJECTS IN DIFFERENT FOLDERS ARE VERY OFTEN CALLED THE SAME THING,
    //  so a caption trimmed to the file name offers the user a choice between
    //  two identical lines. It is also what makes the click need no lookup: the
    //  caption IS the path handed to the project flow.
    L := RecentMenu('/work/one/model.fitproj|/work/two/model.fitproj');
    AssertEquals('the whole path', '/work/one/model.fitproj', L[0].Caption);
    AssertEquals('and the other one in full', '/work/two/model.fitproj',
        L[1].Caption);
end;

procedure TRecentListTest.AnAmpersandInAPathIsShownAndStillOpensIt;
var
    L: TRecentMenuLines;
begin
    //  "S&P500.fitproj": a menu reads '&' as the mark of a shortcut key, so the
    //  line read "SP500" and the P was underlined. The caption doubles it; the
    //  path the click opens is the file's own.
    L := RecentMenu('/work/S&P500.fitproj');
    AssertEquals('what the menu shows', '/work/S&&P500.fitproj', L[0].Caption);
    AssertEquals('what the click opens', '/work/S&P500.fitproj', L[0].Path);
end;

procedure TRecentListTest.AnEmptyListStillShowsALine;
var
    L: TRecentMenuLines;
begin
    //  A MENU THAT DISAPPEARS READS AS A BUILD WITHOUT THE FEATURE. The user
    //  who has opened no project yet must find the command and learn that it is
    //  empty, rather than go looking for where it went.
    L := RecentMenu('');
    AssertEquals('exactly one line', 1, Length(L));
end;

procedure TRecentListTest.ThatLineIsDisabledAndOpensNothing;
var
    L: TRecentMenuLines;
begin
    L := RecentMenu('');
    AssertFalse('it cannot be clicked', L[0].Enabled);
    //  AND IT IS NOT A PATH. The window has only the caption to go on, so a
    //  placeholder that claimed to open something would be opened as a file
    //  called "(none)".
    AssertFalse('and it is not a project', L[0].OpensAProject);
end;

procedure TRecentListTest.TheLinesAreInTheOrderTheListHoldsThem;
var
    L: TRecentMenuLines;
begin
    //  MOST RECENT FIRST, which is the list's own order - the menu must not
    //  re-sort it, or the entry the user reaches for by position moves.
    L := RecentMenu('newest.fitproj|older.fitproj|oldest.fitproj');
    AssertEquals('newest first', 'newest.fitproj', L[0].Caption);
    AssertEquals('then the next', 'older.fitproj', L[1].Caption);
    AssertEquals('then the last', 'oldest.fitproj', L[2].Caption);
end;
