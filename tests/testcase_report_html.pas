...
type
    ...
    published
        ...
        procedure AFailedCheckIsColouredAndStillReadsAsAWord;
        procedure AWarningIsColouredApartFromAFailure;
        procedure APassOrAnInfoIsNotColoured;
        procedure TheVerdictTakesTheColourOfItsOwnStatus;
        procedure EachStatusIsColouredOrNotButNeverBoth;
    end;
...
begin
    ...
    begin
        ...
        //  The word, in bold, immediately before what it judges - whether or
        //  not a colour is wrapped around it.
        Word := '<b>' + ReportStatusCaption(S) + '</b>';
        if StatusColour(S) <> '' then
            Word := Word + '</font>';
        AssertTrue(Html, Contains(Html, Word + ' Wave 2 retrace'));
    end;
end;
...
procedure TReportHtmlTest.AFailedCheckIsColouredAndStillReadsAsAWord;
var
    Html: string;
begin
    //  THE WORD IS NOT REPLACED BY THE COLOUR. The colour wraps the bold word
    //  and never stands in for it, so a reader who sees no colour reads the
    //  same page.
    Html := ModuleReportHtml(Sample, @Explain, '');
    AssertTrue(Html, Contains(Html, '<font color="' + FailColour +
        '"><b>Fail</b></font> Wave 2 retrace'));
end;

procedure TReportHtmlTest.AWarningIsColouredApartFromAFailure;
var
    R: TModuleReport;
    Html: string;
begin
    R := Sample;
    R.Verdict := rpWarning;
    R.Sections[0].Findings[0].Status := rpWarning;
    Html := ModuleReportHtml(R, @Explain, '');
    AssertTrue(Html, Contains(Html, '<font color="' + WarningColour +
        '"><b>Warning</b></font> Wave 2 retrace'));
    AssertFalse(Html, Contains(Html, FailColour));
end;

procedure TReportHtmlTest.APassOrAnInfoIsNotColoured;
var
    S: TReportStatus;
    R: TModuleReport;
    Html: string;
begin
    //  A page where every line is coloured is a page where nothing stands out.
    for S in [rpPass, rpInfo] do
    begin
        R := Sample;
        R.Verdict := S;
        R.Sections[0].Findings[0].Status := S;
        R.Sections[0].Findings[1].Status := S;
        Html := ModuleReportHtml(R, @Explain, '');
        AssertEquals(Html, 0, Count(Html, '<font'));
        AssertTrue(Html, Contains(Html, '<b>' + ReportStatusCaption(S) +
            '</b> Wave 2 retrace'));
    end;
end;

procedure TReportHtmlTest.TheVerdictTakesTheColourOfItsOwnStatus;
var
    S: TReportStatus;
    R: TModuleReport;
    Html, Verdict: string;
begin
    ...
    begin
        ...
        R.Verdict := S;
        Html := ModuleReportHtml(R, @Explain, '');
        Verdict := '<b>Verdict: ' + ReportStatusCaption(S) + '</b>';
        if StatusColour(S) <> '' then
            AssertTrue(Html, Contains(Html, '<font color="' + StatusColour(S) +
                '">' + Verdict + '</font>'))
        else
            AssertTrue(Html, Contains(Html, '<p>' + Verdict));
    end;
end;

procedure TReportHtmlTest.EachStatusIsColouredOrNotButNeverBoth;
var
    S: TReportStatus;
begin
    //  Two colours and no more: a third would have to be told apart from these
    //  at a glance, which is the whole point of colouring only these two.
    AssertTrue(FailColour <> WarningColour);
    for S := Low(TReportStatus) to High(TReportStatus) do
        if S in [rpFail, rpWarning] then
            AssertTrue(ReportStatusCaption(S), StatusColour(S) <> '')
        else
            AssertEquals(ReportStatusCaption(S), '', StatusColour(S));
end;
