...
uses Classes, SysUtils, Math, fpcunit, testregistry, fpjson,
  ...
  //  TFitSession: one test holds a problem's lock to pin that the two
  //  self-description routes are answered before any session is looked up.
  fit_server_session,
...
type
  ...
  published
    ...
    procedure TheServerDescribesItselfWithoutBeingGivenAProblem;
    procedure AndServesThePageThatReadsTheDescription;
    procedure AndDoesSoWithoutWaitingForAnyProblem;

    //  A running fit's progress.
    procedure AFitsProgressIsReadableBeforeAnythingRuns;
    procedure AndIsAnsweredWhileAnotherThreadHoldsTheProblem;
    procedure TheProgressOfAnUnknownProblemIs404;
  end;
  ...
  published
    ...
    //  What a fit records about itself while it runs.
    procedure AFitRecordsItsLossAsItGoes;
    procedure ItsLastSampleIsTheResultTheUserIsShown;
    procedure APollFromTheLastSampleOnGetsNothingNew;
    procedure AWatcherIsGivenTheModelAsItStoodDuringTheFit;
    procedure ButAPlainPollIsNot;
    //  Every fit, not only one verb.
    procedure DoingItAllAutomaticallyRecordsItsProgressToo;
    procedure SoDoesMinimizingTheNumberOfCurves;
    procedure FittingAgainRecordsItsOwnProgress;
    //  Read while the fit is actually running.
    procedure APollDuringARunningFitSeesItRunning;
    //  Any model.
    procedure AUserDefinedCurveReportsProgressAndIsAnimated;
    procedure AFitOverSeveralIntervalsIsOneProgressWithEveryCurve;
    procedure TheProgressNamesWhatIsBeingMinimisedAndByWhat;
    procedure AndTheNamesFollowTheProblemsOwnSettings;
    //  A computation that is not a fit is timed the same way.
    procedure AComputationAfterAFitStartsItsOwnClock;
  end;
...
{ THE TWO SELF-DESCRIPTION ROUTES, from the router's side. rest_spec's own tests
  check that the document is TRUE; what matters here is that it is REACHABLE -
  before any problem exists, on a server nothing has happened to yet, which is
  precisely the state a caller is in when it needs to be told what this API is. }
procedure TRestApiTest.TheServerDescribesItselfWithoutBeingGivenAProblem;
var
  Code: longint;
  R: TJSONObject;
begin
  //  No NewProblem call: the point is that this needs none.
  R := Call('GET', '/openapi.json', '', Code);
  try
    AssertEquals('served', 200, Code);
    AssertEquals('as an OpenAPI document', '3.0.3', R.Get('openapi', ''));
    AssertTrue('describing some paths', R.Objects['paths'].Count > 0);
  finally
    R.Free;
  end;
end;

procedure TRestApiTest.AndServesThePageThatReadsTheDescription;
var
  Code: longint;
  Body: string;
begin
  //  Not through Call: this is the one reply that is not JSON, which is the
  //  whole reason fit_server asks rest_spec what to label a response with.
  FApi.Handle('GET', '/docs', '', Code, Body);
  AssertEquals('served', 200, Code);
  AssertTrue('as a page: ' + Copy(Body, 1, 40), Pos('<html', LowerCase(Body)) > 0);
  AssertTrue('that reads this server''s own document',
    Pos('/openapi.json', Body) > 0);
end;

{ NEITHER ROUTE GOES NEAR A PROBLEM, which is what makes the documentation
  readable while the server is busy fitting one. Handle takes a problem's lock
  only through SessionOfPath, and that needs a /problems path - so a route
  answered before the session lookup cannot wait behind an operation.

  What this pins is that ORDER. The lock itself is recursive, so holding it on
  this thread would not block a route that did take it; the check that would
  fail is the one that matters - moving these two below the session block makes
  them answer 404 here. }
procedure TRestApiTest.AndDoesSoWithoutWaitingForAnyProblem;
var
  Id, Code: longint;
  Body: string;
  Session: TFitSession;
begin
  Id := NewProblem;
  Session := FApi.Sessions.Find(Id);
  AssertTrue('the problem is there', Assigned(Session));
  Session.Lock;
  try
    FApi.Handle('GET', '/openapi.json', '', Code, Body);
    AssertEquals('the document is served while a problem is held', 200, Code);
    FApi.Handle('GET', '/docs', '', Code, Body);
    AssertEquals('and so is the page', 200, Code);
  finally
    Session.Unlock;
  end;
end;
...
type
  { Makes one request from ANOTHER thread and says whether it came back.

    A critical section is re-entrant on the thread that holds it, so a poll
    made on the thread holding a problem's lock would be answered whether or
    not the route takes that lock - and prove nothing. From a second thread a
    route that takes the lock blocks, and the waiting test sees it has not
    returned rather than hanging with it. }
  TRequestFromAnotherThread = class(TThread)
  private
    FApi: TFitRestApi;
    FMethod: string;
    FPath: string;
  ...
  public
    ...
    Body: string;
    Answered: boolean;
    constructor Create(AApi: TFitRestApi; const AMethod, APath: string);
  end;

constructor TRequestFromAnotherThread.Create(AApi: TFitRestApi;
...
procedure TRequestFromAnotherThread.Execute;
begin
  try
    FApi.Handle(FMethod, FPath, '', Code, Body);
  finally
    Answered := True;
  end;
end;

procedure TRestApiTest.AFitsProgressIsReadableBeforeAnythingRuns;
var
  Id, Code: longint;
  R: TJSONObject;
begin
  //  The client starts polling the moment the user presses Fit, and its first
  //  poll may arrive before the engine has begun. That is an ordinary answer.
  Id := NewProblem;
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    AssertEquals('answered', 200, Code);
    AssertFalse('nothing is running', R.Get('busy', True));
    AssertTrue('a sample list', R.Find('samples') is TJSONArray);
    AssertEquals('with nothing in it', 0, R.Arrays['samples'].Count);
    AssertEquals('and no seq issued', 0, R.Get('nextSeq', -1));
  finally
    R.Free;
  end;
end;

procedure TRestApiTest.AndIsAnsweredWhileAnotherThreadHoldsTheProblem;
var
  Id: longint;
  Session: TFitSession;
  Request: TRequestFromAnotherThread;
  Deadline: TDateTime;
begin
  //  A FIT HOLDS ITS PROBLEM'S LOCK FOR AS LONG AS IT RUNS. Held here, by this
  //  thread, the way the fit would hold it; the poll comes from another.
  Id := NewProblem;
  Session := FApi.Sessions.Find(Id);
  AssertTrue('the problem is there', Assigned(Session));
  Session.Lock;
  Request := nil;
  try
    Request := TRequestFromAnotherThread.Create(FApi, 'GET',
      Format('/problems/%d/progress?since=0&snapshot=1', [Id]));
    Deadline := Now + 5 / SecsPerDay;
    while (not Request.Answered) and (Now < Deadline) do
      Sleep(5);
    AssertTrue('the poll was answered while the problem was held',
      Request.Answered);
    AssertEquals('successfully', 200, Request.Code);
  finally
    //  Released BEFORE waiting: a route that did take the lock is parked on
    //  it, and only this lets that thread finish so the test can report.
    Session.Unlock;
...
procedure TRestApiTest.TheProgressOfAnUnknownProblemIs404;
var
  Code: longint;
  R: TJSONObject;
begin
  R := Call('GET', '/problems/999/progress', '', Code);
  try
    AssertEquals('not found', 404, Code);
  finally
    R.Free;
...
procedure TRestApiFitTest.AFitRecordsItsLossAsItGoes;
var
  Id, Code: longint;
  R: TJSONObject;
  Samples: TJSONArray;
begin
  ...
  R := Call('GET', Format('/problems/%d/progress?since=0', [Id]), '', Code);
  try
    AssertEquals('answered', 200, Code);
    AssertFalse('the fit is over', R.Get('busy', True));
    Samples := R.Arrays['samples'];
    //  More than one: a single sample would be the final report alone, which
    //  is what a fit that recorded nothing along the way would also send.
    AssertTrue(Format('the engine recorded its way down (%d samples)',
      [Samples.Count]), Samples.Count > 1);
    AssertTrue('it took some time', R.Get('elapsed', -1.0) >= 0);
    AssertEquals('every sample was issued a seq',
      TJSONObject(Samples.Items[Samples.Count - 1]).Get('seq', -1) + 1,
      R.Get('nextSeq', -1));
  finally
    R.Free;
  end;
...
procedure TRestApiFitTest.ItsLastSampleIsTheResultTheUserIsShown;
var
  Id, Code: longint;
  R: TJSONObject;
  Samples: TJSONArray;
  Last, CurMin: double;
begin
  //  THE CHART'S LAST POINT AND THE STATUS BAR MUST AGREE. A live curve that
  //  ended somewhere other than the reported result would teach the user that
  //  one of the two numbers is not to be trusted.
  Id := FittableProblem;
  R := Call('POST', Format('/problems/%d/actions/minimize-difference', [Id]),
    '', Code);
  R.Free;
  ...
  try
    Samples := R.Arrays['samples'];
    AssertTrue('samples', Samples.Count > 0);
    Last := TJSONObject(Samples.Items[Samples.Count - 1]).Get('value', -1.0);
  finally
    R.Free;
  end;
  R := Call('GET', Format('/problems/%d/async', [Id]), '', Code);
  try
    CurMin := R.Get('curMin', -2.0);
  finally
    R.Free;
  end;
  AssertEquals('the last sample is the reported minimum', CurMin, Last, 1e-12);
  AssertTrue('and it is a real one', Last > 0);
end;

procedure TRestApiFitTest.APollFromTheLastSampleOnGetsNothingNew;
var
  Id, Code, NextSeq: longint;
  R: TJSONObject;
begin
  //  What makes polling several times a second cheap: each poll carries only what
  ...
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    NextSeq := R.Get('nextSeq', -1);
  finally
    R.Free;
  end;
  R := Call('GET', Format('/problems/%d/progress?since=%d', [Id, NextSeq]),
    '', Code);
  try
    AssertEquals('answered', 200, Code);
    AssertEquals('nothing new', 0, R.Arrays['samples'].Count);
  finally
    R.Free;
  end;
end;

procedure TRestApiFitTest.AWatcherIsGivenTheModelAsItStoodDuringTheFit;
var
  Id, Code: longint;
  R, Snapshot, Curves: TJSONObject;
  SnapCurves, ModelCurves: TJSONArray;
  D: TJSONData;
begin
  //  An animating client asks for the model as it polls. Asked here just before
  //  the fit, which is when a client that has Animation ticked first asks.
  Id := FittableProblem;
  R := Call('GET', Format('/problems/%d/progress?snapshot=1', [Id]), '', Code);
  R.Free;
  R := Call('POST', Format('/problems/%d/actions/minimize-difference', [Id]),
    '', Code);
  R.Free;
  ...
  try
    D := R.Find('snapshot');
    AssertTrue('a snapshot was taken during the fit', D is TJSONObject);
    Snapshot := TJSONObject(D);
    AssertTrue('of the computed profile',
      TJSONObject(Snapshot.Find('profile')).Arrays['x'].Count > 0);
    SnapCurves := Snapshot.Arrays['curves'];
    ModelCurves := Curves.Arrays['curves'];
    AssertEquals('of every curve', ModelCurves.Count, SnapCurves.Count);
    //  By HANDLE, so the chart redraws each curve in place rather than adding
    //  a new line per frame.
    AssertEquals('each named as the model names it',
      TJSONObject(ModelCurves.Items[0]).Get('id', 'model'),
      TJSONObject(SnapCurves.Items[0]).Get('id', 'snapshot'));
    AssertTrue('with its points',
      TJSONObject(TJSONObject(SnapCurves.Items[0]).Find('points'))
        .Arrays['y'].Count > 0);
  finally
    R.Free;
    Curves.Free;
...
procedure TRestApiFitTest.ButAPlainPollIsNot;
var
  Id, Code: longint;
  R: TJSONObject;
begin
  //  Most clients do not animate, and a model's worth of points four times a
  //  second is not something to send to one that will not draw it.
  Id := FittableProblem;
  R := Call('GET', Format('/problems/%d/progress?snapshot=1', [Id]), '', Code);
  R.Free;
  R := Call('POST', Format('/problems/%d/actions/minimize-difference', [Id]),
  ...
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    AssertTrue('no snapshot', R.Find('snapshot') = nil);
  finally
    R.Free;
  end;
end;

{ Runs AVerb on AId and answers how many samples its progress holds from ASince. }
function SamplesAfter(ATest: TRestApiFitTest; AId: longint;
  const AVerb: string; ASince: longint): longint;
var
  Code: longint;
  R: TJSONObject;
begin
  R := ATest.Call('POST', Format('/problems/%d/actions/%s', [AId, AVerb]), '',
    Code);
  try
    ATest.AssertEquals(AVerb + ': ' + R.AsJSON, 200, Code);
  finally
    R.Free;
  end;
  R := ATest.Call('GET', Format('/problems/%d/progress?since=%d', [AId, ASince]),
    '', Code);
  try
    ATest.AssertFalse(AVerb + ' is over', R.Get('busy', True));
    Result := R.Arrays['samples'].Count;
  finally
    R.Free;
  end;
end;

procedure TRestApiFitTest.DoingItAllAutomaticallyRecordsItsProgressToo;
begin
  AssertTrue('samples', SamplesAfter(Self, FittableProblem,
    'do-all-automatically', 0) > 0);
end;

procedure TRestApiFitTest.SoDoesMinimizingTheNumberOfCurves;
begin
  AssertTrue('samples', SamplesAfter(Self, FittableProblem,
    'minimize-number-of-curves', 0) > 0);
end;

procedure TRestApiFitTest.FittingAgainRecordsItsOwnProgress;
var
  Id, Code: longint;
  R: TJSONObject;
  Bounds: TPointsData;
begin
  //  The verb that continues from the model as it stands. The engine offers it
  //  only on a problem that is READY - a model and an interval, not yet fitted
  //  (see AndAFitCanBeContinuedFromWhereItStopped) - so that is what this sets up.
  Id := FittableProblem;
  Bounds := Default(TPointsData);
  Bounds.Title := 'rfactor-bounds';
  SetLength(Bounds.X, 2);
  ...
  R := Call('PUT', Format('/problems/%d/rfactor-bounds', [Id]),
    PointsToJsonString(Bounds), Code);
  R.Free;
  AssertEquals('an interval', 200, Code);
  AssertTrue('the fit recorded samples',
    SamplesAfter(Self, Id, 'minimize-difference-again', 0) > 0);
end;

procedure TRestApiFitTest.APollDuringARunningFitSeesItRunning;
var
  Id, Code: longint;
  R: TJSONObject;
  Fit: TRequestFromAnotherThread;
  SeenRunning: boolean;
begin
  //  THE WHOLE POINT, with the fit really running: one thread runs it, and this
  //  one polls. No positions are picked, so the engine seeds a curve on every
  //  sample, which keeps the fit long enough to be watched.
  Id := NewProblem;
  R := Call('PUT', Format('/problems/%d/settings', [Id]),
    Format('{"curveType":"%s"}', [GUIDToString(TGaussPointsSet.GetCurveTypeId)]),
    Code);
  R.Free;
  R := Call('PUT', Format('/problems/%d/profile', [Id]), CoarseGaussianProfileJson,
    Code);
  R.Free;

  SeenRunning := False;
  Fit := TRequestFromAnotherThread.Create(FApi, 'POST',
    Format('/problems/%d/actions/minimize-difference', [Id]));
  try
    while not Fit.Answered do
    begin
      R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
      try
        if R.Get('busy', False) and (R.Arrays['samples'].Count > 0) then
          SeenRunning := True;
      finally
        R.Free;
      end;
    ...
    Fit.WaitFor;
    AssertEquals('the fit itself succeeded: ' + Fit.Body, 200, Fit.Code);
    Fit.Free;
  end;
  AssertTrue('a poll made while the fit ran saw it running, with samples',
    SeenRunning);
end;

procedure TRestApiFitTest.AComputationAfterAFitStartsItsOwnClock;
var
  Id, Code: longint;
  R: TJSONObject;
begin
  //  The client's clock reads the progress of whatever runs; a computation
  //  that left the previous fit's samples and duration there would show the
  //  fit's, not its own.
  Id := FittableProblem;
  AssertTrue('the fit recorded samples',
    SamplesAfter(Self, Id, 'minimize-difference', 0) > 0);
  R := Call('POST', Format('/problems/%d/actions/compute-curve-bounds', [Id]),
    '', Code);
  R.Free;
  AssertEquals('bounds computed', 200, Code);
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    AssertEquals('the fit''s samples are not the computation''s', 0,
      R.Arrays['samples'].Count);
  finally
    R.Free;
  end;
end;

procedure TRestApiFitTest.AUserDefinedCurveReportsProgressAndIsAnimated;
var
  Id, Code: longint;
  R: TJSONObject;
  D: TJSONData;
begin
  //  A MODEL THE ENGINE HAS NEVER HEARD OF: the user's own formula.
  Id := FittableProblem;
  R := Call('PUT', Format('/problems/%d/special-params', [Id]),
    '{"expression":"A*exp(-sqr((x-x0)/SIGMA))"}', Code);
  R.Free;
  AssertEquals('formula', 200, Code);
  R := Call('PUT', Format('/problems/%d/settings', [Id]),
    Format('{"curveType":"%s"}', [GUIDToString(TUserPointsSet.GetCurveTypeId)]),
    Code);
  R.Free;
  ...
  R := Call('GET', Format('/problems/%d/progress?snapshot=1', [Id]), '', Code);
  try
    AssertTrue('samples', R.Arrays['samples'].Count > 0);
    D := R.Find('snapshot');
    AssertTrue('a snapshot', D is TJSONObject);
    AssertTrue('of its curve', TJSONObject(D).Arrays['curves'].Count > 0);
  finally
    R.Free;
  end;
end;

procedure TRestApiFitTest.AFitOverSeveralIntervalsIsOneProgressWithEveryCurve;
var
  Id, Code, i: longint;
  R, Curves: TJSONObject;
  Pos_: TPointsData;
  D: TJSONData;
begin
  //  Each interval is its own task; the progress is ONE loss over all of them,
  //  and a frame carries every task's curves.
  Id := NewProblem;
  R := Call('PUT', Format('/problems/%d/settings', [Id]),
    Format('{"curveType":"%s"}', [GUIDToString(TGaussPointsSet.GetCurveTypeId)]),
    Code);
  ...
  begin
    R := Call('POST', Format('/problems/%d/points/rfactor-bounds', [Id]),
      Format('{"x":%d.0,"y":0.0}', [Ord(i = 1) * 9 + Ord(i = 2) * 11 +
        Ord(i = 3) * 20]), Code);
    R.Free;
  end;
  Pos_ := Default(TPointsData);
  ...
  try
    ...
    D := R.Find('snapshot');
    AssertTrue('a snapshot', D is TJSONObject);
    AssertTrue('the model has a curve per interval',
      Curves.Arrays['curves'].Count >= 2);
    AssertEquals('and the frame carries every one of them',
      Curves.Arrays['curves'].Count, TJSONObject(D).Arrays['curves'].Count);
  finally
    R.Free;
    Curves.Free;
  end;
...
procedure TRestApiFitTest.TheProgressNamesWhatIsBeingMinimisedAndByWhat;
var
  Id, Code: longint;
  R: TJSONObject;
begin
  //  THE HEADER OVER THE CHART SAYS BOTH, and it can only say what the server
  //  tells it. The same model under another objective, or fitted by another
  //  engine, converges differently - a live chart that does not name them is a
  //  number with no unit.
  Id := FittableProblem;
  R := Call('POST', Format('/problems/%d/actions/minimize-difference', [Id]),
    '', Code);
  ...
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    AssertTrue('the objective is named: ' + R.AsJSON,
      R.Get('lossName', '') <> '');
    AssertTrue('the engine is named: ' + R.AsJSON,
      R.Get('engineName', '') <> '');
  finally
    R.Free;
  end;
...
procedure TRestApiFitTest.AndTheNamesFollowTheProblemsOwnSettings;
var
  Id, Code: longint;
  R: TJSONObject;
  Default_, Changed: string;
begin
  //  NOT A CONSTANT SOMEWHERE. Two problems fitted under different objectives
  //  must describe themselves differently, or the line is decoration.
  Id := FittableProblem;
  R := Call('POST', Format('/problems/%d/actions/minimize-difference', [Id]),
    '', Code);
  R.Free;
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    Default_ := R.Get('lossName', '');
  finally
    R.Free;
  end;

  Id := FittableProblem;
  R := Call('PUT', Format('/problems/%d/settings', [Id]), '{"lossKind":3}',
    Code);
  R.Free;
  AssertEquals('the objective was changed', 200, Code);
  R := Call('POST', Format('/problems/%d/actions/minimize-difference', [Id]),
    '', Code);
  R.Free;
  R := Call('GET', Format('/problems/%d/progress', [Id]), '', Code);
  try
    Changed := R.Get('lossName', '');
  finally
    R.Free;
  end;
  AssertTrue(Format('%s is not %s', [Changed, Default_]), Changed <> Default_);
  AssertTrue('and it is named at all', Changed <> '');
end;
