...
type
    ...
    published
        ...
        procedure SoAreTheTwoFitProgressRoutes;
...
procedure TRestPollingTest.SoAreTheTwoFitProgressRoutes;
begin
    //  The loss curve a running fit draws, read ten times a second for as long
    //  as it runs - once from the desktop to the server, and once from the
    //  server to a backend fitting remotely. A heartbeat by any measure, and
    //  the one that most needs to be answered without waiting for the fit.
    AssertTrue('a problem''s progress', IsPolledRoute('/problems/1/progress'));
    AssertTrue('with its query', IsPolledRoute('/problems/1/progress?since=4'));
    AssertTrue('a stateless fit''s progress',
        IsPolledRoute('http://127.0.0.1:8788/fit/progress?id=abc'));
end;
