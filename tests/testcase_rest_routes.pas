...
type
    ...
    published
        ...
        procedure TheSelfDescriptionRoutesAreTheirOwn;
        procedure AndAreNotConfusedWithTheirNearMisses;
        ...
        procedure AQueryParameterIsReadByName;
        procedure AnAbsentParameterGivesTheDefault;
        procedure APathWithNoQueryHasNoParameters;
        procedure AParameterWithNoValueIsEmptyNotAbsent;
        procedure ParameterNamesAreMatchedWhole;
...
procedure TRestRoutesTest.TheSelfDescriptionRoutesAreTheirOwn;
begin
    //  The two routes that describe the server rather than a problem. They
    //  hang off the root, which is the only other place anything does.
    CheckRoute('GET', '/openapi.json', rtOpenApi);
    CheckRoute('GET', '/docs', rtDocs);
    CheckRoute('GET', '/docs/', rtDocs);
end;

procedure TRestRoutesTest.AndAreNotConfusedWithTheirNearMisses;
begin
    //  Neither is a write, and neither hangs off a problem: a document is what
    //  the SERVER is, and every build serves the same one.
    CheckRoute('POST', '/docs', rtUnknown);
    CheckRoute('PUT', '/openapi.json', rtUnknown);
    CheckRoute('GET', '/openapi', rtUnknown);
    CheckRoute('GET', '/docs/index.html', rtUnknown);
    CheckRoute('GET', '/problems/12/docs', rtUnknown);
end;
...
begin
    ...
    //  And the fourth, which a running fit is watched through.
    CheckRoute('GET', '/problems/12/progress', rtProgress);
    CheckRoute('GET', '/problems/12/progress?since=40&snapshot=1', rtProgress);
    //  A read, and nothing else: there is nothing to write to a fit's progress.
    CheckRoute('POST', '/problems/12/progress', rtUnknown);
end;
...
procedure TRestRoutesTest.AQueryParameterIsReadByName;
begin
    AssertEquals('the first', '40',
        QueryParam('/problems/12/progress?since=40&snapshot=1', 'since'));
    AssertEquals('the second', '1',
        QueryParam('/problems/12/progress?since=40&snapshot=1', 'snapshot'));
end;

procedure TRestRoutesTest.AnAbsentParameterGivesTheDefault;
begin
    //  A client that never animates never sends snapshot; its absence is the
    //  ordinary case, not a malformed request.
    AssertEquals('absent', '0',
        QueryParam('/problems/12/progress?since=40', 'snapshot', '0'));
end;

procedure TRestRoutesTest.APathWithNoQueryHasNoParameters;
begin
    AssertEquals('no query at all', 'none',
        QueryParam('/problems/12/progress', 'since', 'none'));
    AssertEquals('an empty one', 'none',
        QueryParam('/problems/12/progress?', 'since', 'none'));
end;

procedure TRestRoutesTest.AParameterWithNoValueIsEmptyNotAbsent;
begin
    AssertEquals('present but empty', '',
        QueryParam('/problems/12/progress?since=&snapshot=1', 'since', 'x'));
    AssertEquals('present with no equals sign', '',
        QueryParam('/problems/12/progress?snapshot', 'snapshot', 'x'));
end;

procedure TRestRoutesTest.ParameterNamesAreMatchedWhole;
begin
    //  'since' must not be found inside 'sinceWhen', nor inside a value.
    AssertEquals('a longer name is another parameter', 'd',
        QueryParam('/p?sinceWhen=5', 'since', 'd'));
    AssertEquals('a value is not a name', 'd',
        QueryParam('/p?x=since', 'since', 'd'));
end;
