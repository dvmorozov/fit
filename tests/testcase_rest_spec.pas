...
uses
    ...
    int_app_module, module_registry, fit_rest_api, fit_progress_json;
...
type
    ...
    published
        ...
        procedure TheFitProgressSchemaSaysWhatTheServerSends;
        procedure IncludingTheFieldsOnlyARunningFitCarries;
...
var
    ...
    i, Count: integer;
begin
    //  THE STATELESS FIT, which is two endpoints now: the fit, and the progress
    //  a relaying backend reads while it runs. Both are answered in fit_server
    //  before the REST surface, and both are in stateless_fit where tests reach
    //  them. The name stays "exactly one" in meaning - one feature at the socket
    //  layer - and the count is its two paths.
    Ops := Operations;
    Count := 0;
    for i := 0 to High(Ops) do
        if Ops[i].ServedOutsideRouter then
        begin
            Inc(Count);
            AssertTrue('an endpoint answered before the router is the ' +
                'stateless fit''s: ' + Ops[i].Method + ' ' + Ops[i].Path,
                ((Ops[i].Method = 'POST') and (Ops[i].Path = '/fit')) or
                ((Ops[i].Method = 'GET') and (Ops[i].Path = '/fit/progress')));
        end;
    ...
    AssertEquals('endpoints answered outside the route table', 2, Count);
end;
...
procedure TRestSpecTest.TheFitProgressSchemaSaysWhatTheServerSends;
var
    Documented, Sent: TStringList;
    Code, Id, i: longint;
    Body: string;
...
begin
    ...
    //  A SUBSET, like the points set: the snapshot is sent only to a client that
    //  asked and only once one was taken, so an idle problem's reply lacks it.
    //  What must hold is that nothing is sent undescribed and nothing required
    //  is missing.
    Documented := PropertiesOf(SchemaNamed('FitProgress'));
    Sent := KeysOfReply('GET', Format('/problems/%d/progress', [Id]));
    try
        for i := 0 to Sent.Count - 1 do
            AssertTrue('/progress sends ' + Sent[i] +
                ', which the document does not describe',
                Documented.IndexOf(Sent[i]) >= 0);
        Required := SchemaNamed('FitProgress').Arrays['required'];
        for i := 0 to Required.Count - 1 do
            AssertTrue('/progress omits the required ' + Required.Strings[i],
                Sent.IndexOf(Required.Strings[i]) >= 0);
    finally
        Documented.Free;
        Sent.Free;
    end;
...
procedure TRestSpecTest.IncludingTheFieldsOnlyARunningFitCarries;
var
    Documented: TStringList;
    R: TFitProgressReport;
    O: TJSONObject;
    i: longint;
begin
    //  THE TEST ABOVE ASKS AN IDLE PROBLEM, and an idle problem sends the
    //  smallest reply there is: a field the server only fills once an operation
    //  has started is undescribed without anything failing. So this asks the
    //  ENCODER instead, with every field set, which is the widest reply the
    //  server can send.
    R := Default(TFitProgressReport);
    R.Busy := True;
    R.Elapsed := 1.0;
    R.LossName := 'R-factor';
    R.EngineName := 'Downhill Simplex (native)';
    Documented := PropertiesOf(SchemaNamed('FitProgress'));
    O := FitProgressToJson(R);
    try
        for i := 0 to O.Count - 1 do
            AssertTrue('/progress can send ' + O.Names[i] +
                ', which the document does not describe',
                Documented.IndexOf(O.Names[i]) >= 0);
    finally
        O.Free;
        Documented.Free;
    end;
