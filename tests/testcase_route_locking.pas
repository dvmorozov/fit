...
type
    ...
    published
        ...
        procedure SoIsTheFitProgressRoute;
        ...
        procedure ExceptStopWhichExistsToReachABusyProblem;
        procedure AndOnlyStopAmongTheActions;
        procedure StopIsUnlockedOnlyAsAnAction;
...
procedure TRouteLockingTest.SoIsTheFitProgressRoute;
begin
    //  THE ROUTE THAT EXISTS ONLY TO BE READ DURING A FIT. Taking the lock here
    //  would answer the first poll after the fit had finished, and the user
    //  would watch an empty chart for exactly as long as it did before this
    //  route existed.
    AssertTrue('progress', IsUnlockedRoute('GET', '/problems/1/progress'));
    AssertTrue('with its query',
        IsUnlockedRoute('GET', '/problems/1/progress?since=12&snapshot=1'));
end;
...
procedure TRouteLockingTest.ExceptStopWhichExistsToReachABusyProblem;
begin
    //  THE ONE MUTATING ROUTE THAT MUST NOT WAIT. Everything else in this
    //  program waits for the running fit because waiting is correct; a stop
    //  that waits for the fit is a stop that never happens. Served on the
    //  locked path it froze the window for the rest of the fit and was then
    //  answered "the calculation not started", the operation having ended while
    //  the request queued.
    AssertTrue('stop', IsUnlockedRoute('POST', '/problems/1/actions/stop'));
end;

procedure TRouteLockingTest.AndOnlyStopAmongTheActions;
begin
    //  Every other action IS engine work on an idle problem.
    AssertFalse('minimize-difference',
        IsUnlockedRoute('POST', '/problems/1/actions/minimize-difference'));
    AssertFalse('do-all-automatically',
        IsUnlockedRoute('POST', '/problems/1/actions/do-all-automatically'));
    //  And a name that merely begins the same way is not it.
    AssertFalse('stop-something-else',
        IsUnlockedRoute('POST', '/problems/1/actions/stop-and-subtract'));
end;

procedure TRouteLockingTest.StopIsUnlockedOnlyAsAnAction;
begin
    //  The word appears in other paths - a fit interval has a stop index - and
    //  none of those is this.
    AssertFalse('a deeper path',
        IsUnlockedRoute('POST', '/problems/1/actions/stop/now'));
    AssertFalse('read rather than posted',
        IsUnlockedRoute('GET', '/problems/1/actions/stop'));
    AssertFalse('not under actions',
        IsUnlockedRoute('POST', '/problems/1/stop'));
end;
