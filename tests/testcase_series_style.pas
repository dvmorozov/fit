...
type
    ...
    published
        ...
        //  The curve selected in the Model panel, told apart on the chart
        procedure TheSelectedCurvesSeriesAreDrawnWideAndOnTop;
        procedure AnotherCurvesSeriesKeepTheNormalWidth;
        procedure NothingSelectedHighlightsNothing;
        procedure AModelWideSeriesIsNeverHighlighted;
        procedure TheHighlightIsVisiblyWiderThanNormal;
        procedure OnlyAWideSeriesOnTopReadsAsHighlighted;
        procedure WhatSeriesEmphasisGivesReadsBackTheSame;
    end;
...
procedure TSeriesStyleTest.TheSelectedCurvesSeriesAreDrawnWideAndOnTop;
var
    E: TSeriesEmphasis;
begin
    //  Owned by the curve the user picked: every series of it, the framework's
    //  line and a contributor's markers alike, since both carry the handle.
    E := SeriesEmphasis('curve-7', 'curve-7');
    AssertEquals('drawn wide', HighlightedLineWidth, E.LineWidth);
    AssertTrue('and over the others, so no later series hides it', E.OnTop);
end;

procedure TSeriesStyleTest.AnotherCurvesSeriesKeepTheNormalWidth;
var
    E: TSeriesEmphasis;
begin
    E := SeriesEmphasis('curve-3', 'curve-7');
    AssertEquals('drawn as before', NormalLineWidth, E.LineWidth);
    AssertFalse('in the order it was added', E.OnTop);
end;

procedure TSeriesStyleTest.NothingSelectedHighlightsNothing;
var
    E: TSeriesEmphasis;
begin
    //  A row that names no curve selects nothing, and '' must not match the
    //  model-wide series, whose owner is also ''.
    E := SeriesEmphasis('', '');
    AssertEquals('a model-wide series stays normal', NormalLineWidth,
        E.LineWidth);
    AssertFalse('and in its place', E.OnTop);
    E := SeriesEmphasis('curve-3', '');
    AssertEquals('a curve stays normal', NormalLineWidth, E.LineWidth);
    AssertFalse('and in its place', E.OnTop);
end;

procedure TSeriesStyleTest.AModelWideSeriesIsNeverHighlighted;
var
    E: TSeriesEmphasis;
begin
    //  The profile, the residual, the bounds: nobody's curve, so selecting one
    //  must not reach them.
    E := SeriesEmphasis('', 'curve-7');
    AssertEquals('normal width', NormalLineWidth, E.LineWidth);
    AssertFalse('not on top', E.OnTop);
end;

procedure TSeriesStyleTest.TheHighlightIsVisiblyWiderThanNormal;
begin
    //  One pixel more reads as a rendering difference, not as a selection.
    AssertTrue('at least two pixels wider',
        HighlightedLineWidth >= NormalLineWidth + 2);
end;

procedure TSeriesStyleTest.OnlyAWideSeriesOnTopReadsAsHighlighted;
begin
    AssertTrue('wide and on top', SeriesDrawnHighlighted(HighlightedLineWidth, True));
    //  Wide under a later curve is still hidden; on top at one pixel does not
    //  stand out. Either alone is a highlight half applied.
    AssertFalse('wide but underneath', SeriesDrawnHighlighted(HighlightedLineWidth, False));
    AssertFalse('on top but thin', SeriesDrawnHighlighted(NormalLineWidth, True));
    AssertFalse('neither', SeriesDrawnHighlighted(NormalLineWidth, False));
end;

procedure TSeriesStyleTest.WhatSeriesEmphasisGivesReadsBackTheSame;
var
    E: TSeriesEmphasis;
begin
    //  The style applied and the style read back must agree, or the legend and
    //  the self-check would disagree with the chart they describe.
    E := SeriesEmphasis('curve-7', 'curve-7');
    AssertTrue('the selected curve reads as highlighted',
        SeriesDrawnHighlighted(E.LineWidth, E.OnTop));
    E := SeriesEmphasis('curve-3', 'curve-7');
    AssertFalse('another curve does not',
        SeriesDrawnHighlighted(E.LineWidth, E.OnTop));
end;
