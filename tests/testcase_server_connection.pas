...
type
    ...
    published
        ...
        procedure TheDefaultAddressFollowsFitPort;
        procedure WithoutFitPortTheDefaultPortIsUsed;
    end;
...
procedure TServerConnectionTest.TheDefaultAddressFollowsFitPort;
begin
    //  THE REPORT: the launchers honour FIT_PORT and start the server on it,
    //  while the window kept looking for one at 8787 - so with FIT_PORT set the
    //  installed program started a server and then could not find it.
    AssertEquals('the port the launcher uses', 'http://127.0.0.1:9123',
        DefaultServerUrlFor('9123'));
end;

procedure TServerConnectionTest.WithoutFitPortTheDefaultPortIsUsed;
begin
    AssertEquals('unset', 'http://127.0.0.1:8787', DefaultServerUrlFor(''));
    AssertEquals('not a port, as the launcher reads it',
        DefaultServerUrlFor('not a port'), DefaultServerUrlFor(''));
end;
