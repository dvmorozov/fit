...
deliberate choice - an empty SelectedCurveType says "the user never picked" -
and a changed default silently moves every existing user onto a different
model. Those are asserted by value here.
}
...
type
    ...
    published
        ...
        procedure AnimationIsOffUntilTheUserTurnsItOn;
        procedure BothPanesStartAtTheSizeTheComponentDraws;
        ...
        //  Both axes by id - what the window stores and reads back.
        //  A settings file written by a NEWER build, read by this one.
        procedure AFieldFromANewerBuildIsSkippedAndTheRestIsRead;
        //  The axes are the project's.
        procedure AnAxisIsNotASetting;
        procedure AnOlderFileThatChoseAnAxisIsReadWithoutIt;
        procedure HowAPriceFileIsReadIsNoFrameworkSetting;
...
procedure TSettingsModelTest.AnimationIsOffUntilTheUserTurnsItOn;
var
    S: Settings_v1;
begin
    //  OFF, which is also what every settings file written before it was saved
    //  says. Animating costs a model rebuild on the server twice a second, and
    //  the live loss chart is the view a fit gets unless the user asks for more.
    S := Settings_v1.Create(nil);
    try
        AssertFalse(S.AnimationMode);
    finally
        S.Free;
    end;
end;

procedure TSettingsModelTest.BothPanesStartAtTheSizeTheComponentDraws;
var
    S: Settings_v1;
begin
    //  A fresh install draws the report and the Explain pane exactly as the
    //  build before the zoom buttons did - and a settings file written before
    //  they existed says zero, which report_zoom repairs to this same size.
    S := Settings_v1.Create(nil);
    try
        AssertEquals('the report', ReportZoomDefault, S.ReportZoom);
        AssertEquals('the Explain pane', ReportZoomDefault, S.ExplainZoom);
    finally
        S.Free;
    end;
end;
...
begin
    ...
    try
        ...
        Saved.Reserved := 7;
        Saved.AnimationMode := True;
        //  TWO SIZES, NOT ONE: the two panes are zoomed by their own buttons,
        //  so a file that stored one number for both would move a pane nobody
        //  clicked on.
        Saved.ReportZoom := 140;
        Saved.ExplainZoom := 85;
        ...
        //  The user asked to watch the model move; the next session should not
        //  have to be asked again.
        AssertTrue('animation', Loaded.AnimationMode);
        AssertEquals('the report''s size', 140, Loaded.ReportZoom);
        AssertEquals('the Explain pane''s size', 85, Loaded.ExplainZoom);
    finally
        Loaded.Free;
        Saved.Free;
    end;
...
procedure TSettingsModelTest.AnAxisIsNotASetting;
const
    //  Every name an axis was ever kept under in this file.
    Former: array[0..13] of string = ('ViewMode', 'ViewModeChosenByUser',
        'ArgumentAxis', 'ValueAxis', 'ValueAxisChosenByUser',
        'CustomAxisName', 'CustomAxisUnit', 'CustomAxisForward',
        'CustomAxisInverse', 'CustomValueAxisName', 'CustomValueAxisUnit',
        'CustomValueAxisForward', 'CustomValueAxisInverse', 'Axis');
var
    i: longint;
begin
    //  PROJECT SCALE. A choice kept here followed the user from one project
    //  into the next - a price series opened on 2*Theta because a diffraction
    //  project had been on it. The project keeps its own; a new one starts on
    //  Automatic.
    for i := 0 to High(Former) do
        AssertNull(Former[i] + ' is no setting any more',
            GetPropInfo(Settings_v1, Former[i]));
end;

procedure TSettingsModelTest.HowAPriceFileIsReadIsNoFrameworkSetting;
begin
    //  THE PRICE-SERIES MODULE'S CHOICE, kept in its own preferences
    //  (module_preferences) since price data became the module's. A property
    //  here would be the framework keeping a field's choice under its own name.
    AssertNull(GetPropInfo(Settings_v1, 'PriceValueColumn'));
    AssertNull(GetPropInfo(Settings_v1, 'PriceArgument'));
end;

procedure TSettingsModelTest.AnOlderFileThatChoseAnAxisIsReadWithoutIt;
var
    Saved: Settings_v1;
    Loaded: TComponent;
    Props: TDOMNode;

    procedure Stored(const AKind, AName, AValue: string);
    var
        E: TDOMElement;
    begin
        E := FCfg.Document.CreateElement(AKind);
        E.SetAttribute('name', AName);
        E.SetAttribute('value', AValue);
        Props.InsertBefore(E, Props.FirstChild);
    end;

begin
    //  What every settings file written until now holds.
    Saved := Settings_v1.Create(nil);
    Loaded := nil;
    try
        Saved.LastProjectFile := 'projects/run7.fitproj';
        WriteComponentToXMLConfig(FCfg, 'Component', Saved);
        Props := FCfg.Document.GetElementsByTagName('properties').Item[0];
        Stored('integer', 'ViewMode', '0');
        Stored('boolean', 'ViewModeChosenByUser', 'true');
        Stored('string', 'ArgumentAxis', 'diffraction.two-theta');
        Stored('string', 'CustomAxisName', 'Energy');
        Loaded := Settings_v1.Create(nil);
        ReadComponentFromXMLConfig(FCfg, 'Component', Loaded, @FindSettings, nil);
        AssertEquals('and everything else is still read',
            'projects/run7.fitproj', Settings_v1(Loaded).LastProjectFile);
    finally
        Loaded.Free;
        Saved.Free;
    end;
...
procedure TSettingsModelTest.AFieldFromANewerBuildIsSkippedAndTheRestIsRead;
var
    Saved: Settings_v1;
    Loaded: TComponent;
    Props: TDOMNode;
    Unknown: TDOMElement;
begin
    //  THE REPORT: a settings file written by a newer build carried fields
    //  this one has never heard of. Reading raised on the first of them, and
    //  the window's fallback then threw EVERY setting away - the last project,
    //  the recent list, the server - for a field that meant nothing here.
    Saved := Settings_v1.Create(nil);
    Loaded := nil;
    try
        Saved.LastProjectFile := 'projects/run7.fitproj';
        Saved.ServerUrl := 'http://compute.example:8080';
        WriteComponentToXMLConfig(FCfg, 'Component', Saved);
        //  AHEAD of the known fields, so a reader that stops at it loses them.
        Props := FCfg.Document.GetElementsByTagName('properties').Item[0];
        AssertNotNull('the written settings have a property list', Props);
        Unknown := FCfg.Document.CreateElement('string');
        Unknown.SetAttribute('name', 'FieldFromANewerBuild');
        Unknown.SetAttribute('value', 'anything');
        Props.InsertBefore(Unknown, Props.FirstChild);

        Loaded := Settings_v1.Create(nil);
        ReadComponentFromXMLConfig(FCfg, 'Component', Loaded, @FindSettings, nil);
        AssertEquals('what this build knows is still read',
            'projects/run7.fitproj', Settings_v1(Loaded).LastProjectFile);
        AssertEquals('all of it', 'http://compute.example:8080',
