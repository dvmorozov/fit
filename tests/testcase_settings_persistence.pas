...
type
  ...
  published
    ...
    procedure PersistsThePriceDataChoice;
...
procedure TSettingsPersistenceTest.PersistsThePriceDataChoice;
var
  FileName: string;
  Cfg: TXMLConfig;
  Saved, Loaded: Settings_v1;
begin
  //  Which price column a .csv file is read as, and what its argument is. Chosen
  //  under Data > Price Data, and kept, so a user who works with highs does not
  //  re-pick the column for every file.
  FileName := GetTempFileName('', 'fitset') + '.xml';
  Saved := Settings_v1.Create(nil);
  try
    Saved.PriceValueColumn := 2;
    Saved.PriceArgument := 1;
    Cfg := TXMLConfig.Create(FileName);
    try
      WriteComponentToXMLConfig(Cfg, 'Component', Saved);
      Cfg.Flush;
  ...
  try
    ...
    AssertEquals('the value column', 2, Loaded.PriceValueColumn);
    AssertEquals('the argument', 1, Loaded.PriceArgument);
  finally
    Loaded.Free;
    if FileExists(FileName) then
      DeleteFile(FileName);
