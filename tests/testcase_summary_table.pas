...
type
    ...
    published
        ...
        procedure TheExportHasEveryDigitTheScreenRoundsAway;
        procedure TheExportRowsAreTheWholeTable;
...
procedure TSummaryTableTest.TheExportHasEveryDigitTheScreenRoundsAway;
begin
    //  THE REPORT: File > Export > Summary Table wrote what the table shows,
    //  four decimals. The table keeps both: the rounded text for the screen and
    //  the exact one for the file.
    FProfile.AddNewPoint(0, 1 / 3);
    FComputed.AddNewPoint(0, 1 / 3);
    FDelta.AddNewPoint(0, 0);
    FProfile.AddNewPoint(1, 2);
    FComputed.AddNewPoint(1, 2);
    FDelta.AddNewPoint(1, 0);
    AddInterval(0, 1);
    Build;
    AssertEquals('the screen rounds', CurveValueText(1 / 3), FTable.CellAt(1, 2));
    AssertEquals('the export does not', ExportedNumber(1 / 3),
        FTable.ExportCellAt(1, 2));
    AssertEquals('a heading is itself in both', FTable.CellAt(1, 0),
        FTable.ExportCellAt(1, 0));
end;

procedure TSummaryTableTest.TheExportRowsAreTheWholeTable;
var
    Rows: TTextRows;
begin
    //  What File > Export > Summary Table writes: every row and column of the
    //  table, from the model, with the exact numbers.
    GiveAProfile;
    AddInterval(0, 3);
    Rows := SummaryExportRows(FProfile, FCurves, FComputed, FDelta, FBounds);
    Build;
    AssertEquals('every row', FTable.RowCount, Length(Rows));
    AssertEquals('every column', FTable.ColCount, Length(Rows[0]));
    AssertEquals('the heading', FTable.CellAt(0, 0), Rows[0][0]);
    AssertEquals('a number', FTable.ExportCellAt(1, 3), Rows[3][1]);
end;
