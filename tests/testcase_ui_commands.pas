...
type
    ...
    published
        ...
        procedure ClearingTheModelIsAMenuCommandOverTheWholeModel;
        ...
        procedure ClearModelFollowsWhetherTheModelHasCurves;
...
procedure TUiCommandsTest.ClearingTheModelIsAMenuCommandOverTheWholeModel;
var
    D: TCommandDecl;
begin
    D := FTable.Item(RowOf(CmdClearModel));
    AssertEquals('it drives its own action', 'ActionClearModel', D.TargetName);
    AssertTrue('an action', D.TargetKind = tkAction);
    //  THE WHOLE MODEL, so no row: offering it only over a selected row would
    //  hide a command whose meaning does not depend on one.
    AssertTrue('not row-scoped', D.Scope = scGlobal);
    //  A destructive command one click away on the pane, beside the
    //  everyday Pick/Auto buttons, is a mis-click waiting to happen.
    AssertTrue('in the menu, not on the pane', D.Surface = csMenu);
    AssertFalse('nothing to tick', D.WithChecked);
end;
...
procedure TUiCommandsTest.ClearModelFollowsWhetherTheModelHasCurves;
var
    I: TUiInputs;
begin
    I := EmptyUiInputs;
    I.Open := OpenSuccess;
    Refresh(I);
    AssertFalse('an empty model', EnabledOf(CmdClearModel));

    I.ModelHasCurves := True;
    Refresh(I);
    AssertTrue('a model with curves', EnabledOf(CmdClearModel));

    I.Async := AsyncWorks;
    Refresh(I);
    AssertFalse('not while a fit runs', EnabledOf(CmdClearModel));
end;
