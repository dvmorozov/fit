...
type
  ...
  published
    ...
    //  A value the user held in the curve's dialog is where the fit leaves it.
    procedure AHeldParameterKeepsItsValueThroughTheFit;
  end;
...
procedure TUserCurveFitTest.AHeldParameterKeepsItsValueThroughTheFit;
var
  Task: TFitTask;
  Profile, Positions: TPointsSet;
  CB: TUcCB;
  ...
  i, Checked: longint;
begin
  //  THE DATA SAYS 5 AND THE USER SAYS 2. A fit free to move B would put it
  //  at the offset in the data; held, it must stay at the value it was given
  //  and let A and SIGMA do what they can.
  SetExceptionMask([exDenormalized, exUnderflow, exPrecision]);
  CB := TUcCB.Create;
  ...
  while x <= 20 + 1e-9 do
  begin
    Profile.AddNewPoint(x, GaussPoint(100, 1.5, 10, x) + 5);
    x := x + 0.2;
  end;
  Positions := TPointsSet.Create(nil);
  Positions.AddNewPoint(10, 0);
  Params := BuildParams;
  AddParam(Params, 'B', Variable, 2);
  Params[Params.Count - 1].VariationDisabled := True;

  Task := TFitTask.Create(nil, False, False);
  try
    Task.ServerShowCurMin := @CB.NoOp;
    Task.ServerDoneProc := @CB.NoOp;
    Task.SetSpecialCurve('A*exp(-((x-x0)/SIGMA)^2)+B', Params);
    Task.SetProfilePointsSet(Profile);
    Task.SetCurvePositions(Positions);
    Task.RecreateCurves(nil);
    Task.BegIndex := 0;
    ...
    Task.MinimizeDifference;
    Checked := 0;
    for i := 0 to Task.GetCurves.Count - 1 do
    begin
      Curve := TCurvePointsSet(Task.GetCurves[i]);
      AssertEquals('B is where the user held it', 2,
        Curve.ValuesByName['B'], 1e-12);
      Inc(Checked);
    end;
    AssertEquals('one curve was fitted', 1, Checked);
  finally
    Task.Free;
    CB.Free;
  end;
