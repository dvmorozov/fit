...
type
    ...
    published
        ...
        procedure AndRemovesTheDraftItStored;
    end;
...
procedure TUserCurveFlowTest.AndRemovesTheDraftItStored;
begin
    //  THE DECISION THE OLD TEST ASKED FOR. The type is stored when the formula
    //  parses; cancelling the roles used to leave it there, while the window
    //  told the user "User-defined curve not created" - so the message and the
    //  User menu contradicted each other, and a type with no roles assigned
    //  sat in the list. Cancelling now takes the draft away, as rejecting the
    //  roles always did.
    Script(Answers(daAccepted), Answers(daCancelled));
    Run;
    AssertEquals('added, then deleted', 'Add(my curve);Delete(my curve)',
