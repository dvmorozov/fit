...
{$mode objfpc}{$H+}
interface
uses Classes, SysUtils, fpcunit, process, http_fit_service, title_points_set, readiness_channel;

const
  { The port every worker this fixture starts listens on. }
  WorkerTestPort = 8811;

type
  TWorkerProcessTest = class(TTestCase)
  private
    { Stops and frees the worker this test started, if there is one. }
  ...
  protected
    ...
    { How long SetUp waits for the worker to say it is listening. Virtual so a
      test can make SetUp fail with the worker already running. }
    function ReadyBudgetMs: longint; virtual;
...
function TWorkerProcessTest.ReadyBudgetMs: longint;
begin
  Result := 30000;
end;
...
begin
  ...
  //  THE SERVER SAYS WHEN IT IS LISTENING (readiness_channel): one wait on that
  //  event, which ends the moment it is up or the moment it dies - never a probe
  //  repeated until something answers.
  ...
  try
    ...
    FProc.Parameters.Add('--ready-port');
    FProc.Parameters.Add(IntToStr(Listener.Port));
    FProc.Execute;
    Readiness := Listener.WaitForReady(ReadyBudgetMs);
  finally
    Listener.Free;
  end;
  //  STOPPED BEFORE FAILING. fpcunit runs SetUp outside the try that guards
  //  TearDown, so a SetUp that fails never reaches TearDown - and the worker it
  //  had already started outlived the suite. On Windows that process holds the
  //  output pipe it inherited, and the release job waited on it for half an hour.
  if Readiness <> rdReady then
  begin
    StopWorker;
    Fail('the worker this test started said it was listening');
  end;
  ...
  begin
    FreeAndNil(FSvc);
    StopWorker;
    Fail('the worker this test started is running (a stale fit_server on '
      + 'port ' + IntToStr(TEST_PORT) + ' would answer in its place)');
  end;
end;

procedure TWorkerProcessTest.StopWorker;
begin
